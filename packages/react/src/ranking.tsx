...
export function Ranking(
  props: RankingProps | (SemanticProps & { interactive?: boolean }),
) {
  return "store" in props ? (
    <SemanticRanking {...props} />
  ) : (
    <StandaloneRanking {...props} />
  );
}
    ...
    <RankingView
      ...
      onSelect={
        props.interactive === false
          ? undefined
          : (id) => select(props.store, props.node, id)
      }
...
function StandaloneRanking(props: RankingProps) {
  ...
  return (
    <RankingView
      ...
      node={{ id: "standalone-ranking", ...props }}
...
function RankingView({
  ...
  dataset?: Dataset;
  metric?: MetricField;
  node: Partial<WorkspaceNode> & { id: string };
  ...
  const records = useMemo(
    () =>
      metric
        ? [...data.records]
            .sort((a, b) => compareMetricRecords(a, b, metric, node.direction))
  ...
  const domain = useMemo(() => {
    const values = records.map((record) =>
      metric ? (metricValue(record, metric) ?? 0) : 0,
    );
    ...
    <div ref={container} data-adaptation={mode}>
      ...
      >
        {!ready(data) || !dataset || !metric ? (
          <DataState
            data={
              !metric && data.status === "ready"
                ? {
                    status: "error",
                    records: [],
                    error: "Choose a declared metric for this ranking.",
                  }
                : data
            }
          />
                  ...
                  <li key={id}>
                    ...
                    >
                      ...
                      <span className="aeliqo-record-label">
                        {String(record[node.dimension ?? dataset.labelField])}
                        {mode === "full" && (
                          <small>
                            {String(
                              record[
                                dataset.dimensions.find((field) =>
                                  /organization|provider|family/i.test(
                                    field.key,
                                  ),
                                )?.key ?? ""
                              ] ?? dataset.entity,
