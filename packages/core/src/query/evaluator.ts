import type { Catalog, Expression, Outcome, SemanticType, VersionRef } from '../contracts/types.js';
...
import { DEFAULT_LIMITS, validatePlanSemantics } from './planner.js';
import {
  type AggregateSpec,
  type LogicalPlan,
  type PlanNode,
...
function entity(catalog: Catalog, id: string): CatalogEntity | undefined {
...
function normalizeSourceRelation(
  ...
  if (expectedEntity !== undefined && source.entity !== expectedEntity)
    return failure(
      'query.source-entity',
  ...
  state.scannedRows += source.rows.length;
  const schema: QuerySchema = {
    fields: definition.fields.map((field) => ({
      id: fieldKey(definition.id, field.id),
      label: field.label,
      type:
        field.type.grain === undefined
          ? { ...field.type, grain: definition.rowGrain.map((grain) => fieldKey(definition.id, grain)) }
          : field.type,
      role: field.role,
      source: { entity: definition.id, field: field.id },
    })),
  ...
  for (let index = 0; index < source.rows.length; index += 1) {
    ...
    for (const key of Object.keys(input)) {
      ...
      if (field === undefined)
        return failure(
          'query.source-value',
          `Source row ${index} for ${source.entity} has an invalid value for ${key}.`,
        );
    ...
    for (const field of definition.fields) {
      if (
        !Object.hasOwn(input, field.id) ||
        input[field.id] === undefined ||
        (input[field.id] === null && !field.type.nullable)
      )
        return failure(
          'query.source-value',
          `Source row ${index} for ${source.entity} is missing non-nullable field ${field.id}.`,
        );
    }
    const tuple: string[] = [];
    for (const fieldId of definition.identity) {
      ...
      if (!identity.ok)
        return failure('query.source-value', `Source row ${index} for ${source.entity} has an invalid identity value.`);
    ...
    state.sourceBytes += outputBytes([output]);
...
function validateExecutionContext(context: unknown): Outcome<ValidatedExecutionContext> {
  ...
  if (
    ...
    (!isPlainDataRecord(context.cancellation) || typeof context.cancellation.aborted !== 'boolean')
  )
    return failure('query.context', 'Execution context cancellation must expose a boolean aborted flag.', [
      'cancellation',
    ]);
  if (context.clock !== undefined && typeof context.clock !== 'function')
...
function validQuerySchema(value: unknown, catalog: Catalog): value is QuerySchema {
  ...
  for (const candidate of value.fields) {
    if (
      ...
      !['identity', 'attribute', 'dimension', 'measure', 'time'].includes(String(candidate.role)) ||
    ...
    if (candidate.source !== undefined) {
      ...
      if (
        !isRecord(source) ||
        !planId(source.entity) ||
        !planId(source.field) ||
        entity(catalog, source.entity) === undefined ||
        entity(catalog, source.entity)!.fields.some((field) => field.id === source.field) === false
      )
        return false;
    }
  }
  return (
    value.identity.every((id): id is string => typeof id === 'string' && ids.has(id)) &&
    value.grain.every((id): id is string => typeof id === 'string' && ids.has(id))
  );
}

function planFields(schema: QuerySchema): Set<string> {
...
function expressionFields(
  ...
  if (expression.kind === 'field') {
    ...
    return candidates.length === 1
      ...
      : failure('query.plan', 'Plan contains an unknown or ambiguous field expression.');
  }
...
function aggregateExpressionShape(expression: Expression, registry: FunctionRegistry, depth = 0): Outcome<void> {
  ...
  if (expression.kind === 'field')
    return planId(expression.ref) && (expression.entity === undefined || planId(expression.entity))
      ? { ok: true, value: undefined }
      : failure('query.plan', 'Aggregate field reference is invalid.');
  if (expression.kind === 'literal')
    return validateSemanticType(expression.type).ok
      ? { ok: true, value: undefined }
...
function predicateFields(
  ...
  const expression =
    predicate.op === 'compare'
      ? predicate.left
      : predicate.op === 'is-null' || predicate.op === 'in'
...
function validateNode(
  ...
  if (
    ...
    !node.inputs.every(planId) ||
    !validQuerySchema(node.output, catalog) ||
    !validNodeCost(node.cost)
  )
  ...
  if (node.op === 'scan') {
    ...
    return expected !== undefined && stable(node.output) === stable(expected)
      ? { ok: true, value: undefined }
      : failure('query.plan', 'Scan node schema does not match the catalog entity.');
  }
  ...
  if (node.op === 'project' || node.op === 'derive' || node.op === 'time-bucket' || node.op === 'window') {
    ...
    for (const item of node.items as readonly unknown[]) {
      if (
        ...
        (node.op !== 'window' && !isRecord(item.expression))
      )
        return failure('query.plan', `${node.op} item is invalid.`);
      ...
      if (node.op === 'window') {
        ...
        for (const sort of item.orderBy as readonly unknown[]) {
          if (!isRecord(sort) || !isRecord(sort.expression))
            return failure('query.plan', 'Window sort item is invalid.');
    ...
    if (
      !sameIds(
        node.output.fields.map((field) => field.id),
        expectedIds,
      )
    )
  ...
  if (node.op === 'group') {
    ...
    for (const key of node.keys) {
      if (!isRecord(key) || !planId(key.id) || !isRecord(key.expression))
    ...
    if (
      !sameIds(
        node.output.fields.map((field) => field.id),
        (node.keys as readonly PlanRecord[]).map((key) => key.id as string),
      )
    )
  ...
  if (node.op === 'aggregate') {
    ...
    const expectedIds = [
      ...input.output.fields.map((field) => field.id),
      ...(node.items as readonly PlanRecord[]).map((item) => item.id as string),
    ];
    if (
      !sameIds(
        node.output.fields.map((field) => field.id),
  ...
  if (node.op === 'sort') {
    if (node.inputs.length !== 1 || !Array.isArray(node.items) || stable(node.output) !== stable(input.output))
  ...
  if (node.op === 'top-k')
    return node.inputs.length === 1 && safePositiveCount(node.limit) && stable(node.output) === stable(input.output)
      ? { ok: true, value: undefined }
      : failure('query.plan', 'Top-K node shape is invalid.');
  if (node.op === 'join' || node.op === 'semijoin') {
    ...
    if (
      ...
      relation.sourceEntity !== input.output.fields.find((field) => field.source !== undefined)?.source?.entity ||
      relation.targetEntity !== node.spec.rightEntity
    )
      return failure('query.plan', 'Join relationship is not declared or has the wrong direction.');
    const declaredKeys = relation.keys.map((key) => ({
      left: fieldKey(relation.sourceEntity, key.sourceField),
      right: fieldKey(relation.targetEntity, key.targetField),
    }));
    if (stable(node.keys) !== stable(declaredKeys))
      return failure('query.relationship-keys', 'Plan join keys must exactly match the declared relationship keys.');
    if ((relation.cardinality === 'one-to-many' || relation.cardinality === 'many-to-many') && node.op === 'join')
      return failure('query.plan', 'A regular join cannot use a fanout relationship.');
    if (node.op === 'join' && !['inner', 'left'].includes(String(node.spec.kind)))
    ...
    for (const key of node.keys) {
      if (
        ...
        !planId(key.left) ||
        !planId(key.right) ||
        !planFields(input.output).has(key.left) ||
        !planFields(second.output).has(key.right)
      )
      ...
      if (
        ...
        !compatibleSemanticTypes(leftField.type, rightField.type)
      )
        return failure(
...
function validateLogicalPlan(
  ...
  if (Object.getOwnPropertyNames(input).some((key) => Object.getOwnPropertyDescriptor(input, key)?.enumerable !== true))
  ...
  // These two strings encode the whole plan, rather than an application scalar.
  // Inspect the actual fields with scalar bounds and the complete plan with the
  // unchanged document byte bound; do not raise the user-value text limit.
  ...
  if (input.pins.catalogRevision !== catalog.revision || input.pins.functionRegistryDigest !== registry.digest)
  ...
  for (const key of ['sourceRevision', 'scopeDigest', 'policyRevision'] as const) {
    if (input.pins[key] !== undefined && !planId(input.pins[key]))
      return failure('query.plan', `Plan pin ${key} is invalid.`);
  }
  const nodes = input.nodes as PlanNode[];
  if (nodes.length > limits.maxNodes) return failure('query.budget', 'Logical plan exceeds the effective node budget.');
  const map = new Map<string, PlanNode>();
  for (const candidate of input.nodes as readonly unknown[]) {
    if (!isRecord(candidate) || !planId(candidate.id) || map.has(candidate.id))
  ...
  for (const node of nodes) {
    ...
    for (const inputId of inputs)
      if (!map.has(inputId)) return failure('query.plan', `Logical plan input ${inputId} is missing.`);
  ...
  const visit = (id: string, depth = 1): Outcome<void> => {
    if (depth > limits.maxDepth) return failure('query.budget', 'Logical plan exceeds the effective depth budget.');
    ...
    for (const inputId of node.inputs) {
      const checked = visit(inputId, depth + 1);
      if (!checked.ok) return checked;
    }
    visiting.delete(id);
    visited.add(id);
  ...
  const acyclic = visit(input.root);
  ...
  if (
    input.cost.nodes !== nodes.length ||
    stable(input.output) !== stable(rootNode.output) ||
    stable(input.cost) !== stable(rootNode.cost)
  )
  ...
  for (let index = 0; index < nodes.length; index += 1) {
    ...
    if (index > 0 && cost.operations < nodes[index - 1]!.cost.operations)
  ...
  if (input.canonical !== canonical || input.planKey !== `query-${canonical}`)
    return failure('query.plan', 'Logical plan canonical identity does not match its contents.');
  if (
    input.explain.length !== nodes.length ||
    !sameIds(
      input.explain.map((entry) => (isRecord(entry) ? String(entry.nodeId) : '')),
  ...
  for (let index = 0; index < input.explain.length; index += 1) {
    ...
    if (
      ...
      explanation.operation !== node.op ||
      !Array.isArray(explanation.inputIds) ||
      !sameIds(
        explanation.inputIds.filter((id): id is string => typeof id === 'string'),
        node.inputs,
      ) ||
      explanation.estimatedRows !== node.cost.estimatedRows ||
      explanation.estimatedBytes !== node.cost.estimatedBytes
    )
...
function decimalAdd(
  ...
  const coefficient =
    a.coefficient * 10n ** BigInt(scale - a.scale) + sign * b.coefficient * 10n ** BigInt(scale - b.scale);
...
function expressionSemanticType(
  expression: Expression,
  schema: QuerySchema,
  registry: FunctionRegistry,
  ...
  if (signature.output.kind === 'same-as' || signature.output.kind === 'nullable-same-as') {
    ...
    return source === undefined
      ? undefined
      : { ...source, nullable: signature.output.kind === 'nullable-same-as' ? true : source.nullable };
  }
  const first = arguments_.find((argument): argument is SemanticType => argument !== undefined);
  const value =
    signature.output.forceFloat === true ||
...
function evaluateExpression(
  ...
  return evaluateCall(
    ...
    signature,
    arguments_,
    expression.arguments.map((argument) => expressionSemanticType(argument, schema, state.registry)),
  );
}
...
function evaluateCall(
  ...
  if (id === 'core.equal') {
    ...
    return compared === undefined
      ? failure('query.equal-type', 'Equality received incompatible runtime values.')
      : { ok: true, value: compared === 0 };
  }
  if (id === 'core.text.includes-casefold') {
    ...
    return {
      ok: true,
      value: haystack.normalize('NFKC').toLowerCase().includes(needle.normalize('NFKC').toLowerCase()),
    };
  }
  if (id === 'core.add' || id === 'core.subtract' || id === 'core.multiply') {
    ...
    if (
      isDecimal(left) &&
      typeof right === 'number' &&
      argumentTypes[1]?.value === 'integer' &&
      Number.isSafeInteger(right)
    )
      right = { decimal: String(right) };
    if (
      isDecimal(right) &&
      typeof left === 'number' &&
      argumentTypes[0]?.value === 'integer' &&
      Number.isSafeInteger(left)
    )
      left = { decimal: String(left) };
    if (isDecimal(left) && isDecimal(right)) {
      const result =
        id === 'core.multiply'
          ? decimalMultiply(left, right)
          : decimalAdd(left, right, id === 'core.subtract' ? -1n : 1n);
      return result === undefined
        ? failure('query.numeric-overflow', 'Decimal operation exceeded the bounded exact representation.')
    ...
    if (typeof left === 'number' && typeof right === 'number') {
      ...
      if (
        argumentTypes.every((type) => type?.value === 'integer') &&
  ...
  if (id.startsWith('core.divide')) {
    ...
    if (denominator) {
      if (signature.zeroDenominator === 'error')
        return failure('query.zero-denominator', 'Division denominator is zero.');
    ...
    if (
      ...
      return failure('query.numeric-type', 'Division received incompatible runtime values.');
...
function evaluatePredicate(
  ...
  if (predicate.op === 'and' || predicate.op === 'or') {
    ...
    if (predicate.op === 'and')
      return { ok: true, value: values.includes('false') ? 'false' : values.includes('unknown') ? 'unknown' : 'true' };
  ...
  if (predicate.op === 'is-null')
    return {
      ok: true,
      value: value.value === null ? (predicate.negate ? 'false' : 'true') : predicate.negate ? 'true' : 'false',
    };
  ...
  if (predicate.op === 'compare') {
    ...
    const compared = compareValue(
      ...
      right.value,
      field?.type.value ??
        expressionValueType(predicate.left, schema, state.registry) ??
    ...
    const matched =
      predicate.comparison === 'eq'
        ? compared === 0
        : predicate.comparison === 'ne'
          ? compared !== 0
          : predicate.comparison === 'lt'
            ? compared < 0
            : predicate.comparison === 'lte'
              ? compared <= 0
              : predicate.comparison === 'gt'
                ? compared > 0
                : compared >= 0;
  ...
  const type =
    expressionValueType(predicate.expression, schema, state.registry) ??
    (typeof value.value === 'number'
      ? 'float'
      : typeof value.value === 'boolean'
        ? 'boolean'
...
function tick(state: EvalState, amount = 1): Outcome<void> {
  ...
  if (state.context.clock !== undefined && state.lastClock !== undefined) {
    ...
    if (
      ...
      state.context.maxMilliseconds !== undefined &&
      now - state.startedAt > state.context.maxMilliseconds
    )
...
function appendRow(state: EvalState, rows: QueryRow[], row: QueryRow): Outcome<void> {
...
function outputValue(value: QueryValue | undefined, field: QueryField | undefined): Outcome<QueryValue> {
  ...
  return checked.ok
    ? checked
    : failure('query.output-value', 'Computed value is incompatible with its declared output type.');
}
    ...
    let compared =
      aNull || bNull
        ...
        : (compareValue(
            a.value,
            b.value,
            field?.type.value ?? expressionValueType(spec.expression, schema, state.registry) ?? 'text',
          ) ?? 0);
    if (compared !== 0) {
  ...
  if (appendIdentity) {
    for (const identity of schema.identity) {
      const compared = compareValue(
        left[identity],
        right[identity],
        schema.fields.find((field) => field.id === identity)?.type.value ?? 'text',
      );
...
function bucket(value: QueryValue | undefined, item: TimeBucketSpec): QueryOutcome<QueryValue | undefined> {
  ...
  if (item.grain === 'year') {
    ...
    if (start === undefined)
      return failure('query.temporal-range', 'Weekly time bucket starts outside the supported four-digit date range.');
    year = start.year;
    month = start.month;
    day = start.day;
  }
...
function aggregateValues(
  ...
  const signature = trusted.value;
  if (signature.ref.id === 'core.if')
    return evaluateAggregateExpression(
      state,
      { kind: 'call', function: item.function, arguments: item.arguments },
      group,
    );
  if (
    signature.operation !== 'aggregate' &&
    ...
    return evaluateCall(
      ...
      signature,
      arguments_,
      item.arguments.map((argument) => expressionSemanticType(argument, group.schema, state.registry)),
    );
  }
  const charged = tick(state);
  if (!charged.ok) return charged;
  ...
  const id = signature.ref.id;
  if (id === 'core.aggregate.count')
    return { ok: true, value: values[0] === undefined ? 0 : values[0].filter((value) => value !== null).length };
  if (id === 'core.aggregate.count-distinct') {
    const type =
      item.arguments[0] === undefined
        ? undefined
        : expressionValueType(item.arguments[0], group.schema, state.registry);
  ...
  if (id === 'core.aggregate.sum') {
    ...
    if (signature.nullPolicy === 'propagate' && inputs.some((value) => value === null))
      return { ok: true, value: null };
    return sumValues(
      state,
      inputs.filter((value) => value !== null),
      expressionSemanticType(item.arguments[0]!, group.schema, state.registry),
    );
  }
  if (id.startsWith('core.ratio-of-sums')) {
    ...
    const denominator = sumValues(
      state,
      values[1]!,
      expressionSemanticType(item.arguments[1]!, group.schema, state.registry),
      true,
    );
    ...
    const divideId =
      signature.zeroDenominator === 'error'
        ? 'core.divide.error'
        : signature.zeroDenominator === 'unknown'
          ? 'core.divide.unknown'
          : 'core.divide.null';
  ...
  if (id === 'core.mean-of-rates') {
    if ((values[0] ?? []).some((value) => value === null) && signature.nullPolicy === 'propagate')
      return { ok: true, value: null };
  ...
  return unsupported(
    'aggregate-runtime',
    `No trusted local aggregate implementation exists for ${id}@${signature.ref.revision}.`,
  );
}
...
function evaluateAggregateExpression(
  ...
  if (expression.kind === 'literal') {
    const step = tick(state);
    return step.ok ? { ok: true, value: expression.value } : step;
  }
  ...
  return evaluateCall(
    ...
    signature,
    arguments_,
    expression.arguments.map((argument) => expressionSemanticType(argument, group.schema, state.registry)),
  );
}
...
function sumValues(
  ...
  if (type?.value === 'integer') {
    ...
    if (total > BigInt(Number.MAX_SAFE_INTEGER) || total < BigInt(Number.MIN_SAFE_INTEGER)) {
      ...
      return allowWide
        ? { ok: true, value: { decimal: String(total) } }
        : failure('query.numeric-overflow', 'Integer sum exceeded the exact safe integer representation.');
    }
...
function executeWindow(
  ...
  for (const item of items) {
    ...
    for (let index = 0; index < rows.length; index += 1) {
      ...
      const key = values
        .map((value, index) =>
          scalarKey(value, expressionValueType(item.partitionBy[index]!, input.schema, state.registry)),
        )
        .join('|');
    ...
    for (const indexes of partitions.values()) {
      ...
      if (sortError !== undefined) return sortError;
      for (let position = 0; position < indexes.length; position += 1) {
        ...
        if (item.function.id === 'core.window.rank') {
          let rank = 1;
          const rankSpecs = item.orderBy.filter(
            (spec) =>
              spec.expression.kind !== 'field' ||
              !input.schema.identity.includes(sourceField(input.schema, spec.expression)?.id ?? ''),
          );
          ...
          if (hasNull && signature.nullPolicy === 'propagate') value = null;
            ...
            const summed = sumValues(
              state,
              values,
              expressionSemanticType(item.arguments[0]!, input.schema, state.registry),
            );
        ...
        const checkedValue = outputValue(
          value,
          output.fields.find((field) => field.id === item.id),
        );
        if (!checkedValue.ok) return checkedValue;
        rows[rowIndex]![item.id] = checkedValue.value;
        const bytes = outputBytes([rows[rowIndex]!]);
        totalBytes += bytes - rowSizes[rowIndex]!;
        rowSizes[rowIndex] = bytes;
        if (totalBytes > state.context.maxBytes!)
...
export function evaluateLogicalPlan(
  ...
  if (!isPlainDataRecord(limits) || Object.keys(DEFAULT_LIMITS).some((key) => !safePositiveCount(limits[key])))
    return failure('query.budget', 'Evaluator host limits must be positive safe integers.');
  context = {
    ...context,
  ...
  try {
    verified = validateLogicalPlan(plan as unknown, catalog, registry, limits);
  } catch {
  ...
  if (
    plan.pins.scopeDigest !== undefined &&
    ((context.scopeDigest ?? source.scopeDigest) !== plan.pins.scopeDigest ||
  ...
  if (
    plan.pins.policyRevision !== undefined &&
    ((context.policyRevision ?? source.policyRevision) !== plan.pins.policyRevision ||
  ...
  const state: EvalState = {
    ...
    registry,
    context,
    ...(validContext.value.startedAt === undefined
      ? {}
      : { startedAt: validContext.value.startedAt, lastClock: validContext.value.startedAt }),
    unknown: [],
    approximate: false,
    operations: 0,
    scannedRows: 0,
  ...
  const evaluateNode = (id: string): Outcome<EvalRelation> => {
    ...
    if (node.op === 'scan') {
      ...
      result =
        relation === undefined
          ? failure('query.source', `Source relation ${node.entity} is missing.`)
          : normalizeSourceRelation(relation, catalog, node.entity, state);
      ...
      for (const row of input.rows) {
        ...
        if (!checkedPredicate.ok) {
          result = checkedPredicate;
          cache.set(id, result);
          return result;
        }
        if (checkedPredicate.value === 'true') {
      ...
      for (const row of input.rows) {
        ...
        for (const item of items) {
          const value =
            node.op === 'time-bucket'
              ? (() => {
                  const timeItem = item as TimeBucketSpec;
                  const expression = evaluateExpression(state, timeItem.expression, row, input.schema);
                  return expression.ok ? bucket(expression.value, timeItem) : expression;
                })()
          ...
          if (!checkedValue.ok) return checkedValue;
          projected[item.id] = checkedValue.value;
        }
        if (node.op === 'project') {
          const output: Record<string, QueryValue> = {};
          for (const field of node.output.fields) output[field.id] = projected[field.id] ?? null;
          const added = appendRow(state, rows, Object.freeze(output));
          if (!added.ok) return added;
        } else {
          const added = appendRow(state, rows, Object.freeze(projected));
          if (!added.ok) return added;
        }
      }
      result = { ok: true, value: { schema: node.output, rows, complete: input.complete } };
      ...
      if (!left.complete || !right.complete) {
        result = failure('query.incomplete-input', 'Join and semijoin require complete source populations.');
        cache.set(id, result);
        return result;
      }
      ...
      for (const row of rightRows) {
        ...
        const key = keys
          .map((joinKey, index) =>
            scalarKey(values[index], right.schema.fields.find((field) => field.id === joinKey.right)?.type.value),
          )
          .join('|');
        const bucketRows = rightMap.get(key) ?? [];
        bucketRows.push(row);
        rightMap.set(key, bucketRows);
      }
      const declaredRelationship = relationship(catalog, node.spec.relationship as VersionRef);
      if (
        node.op === 'join' &&
        declaredRelationship !== undefined &&
        (declaredRelationship.cardinality === 'one-to-one' || declaredRelationship.cardinality === 'many-to-one') &&
        [...rightMap.values()].some((rows) => rows.length > 1)
      ) {
        result = failure(
          'query.cardinality',
      ...
      if (node.op === 'join' && declaredRelationship?.cardinality === 'one-to-one') {
        for (const leftRow of left.rows) {
          ...
          const key = keys
            .map((joinKey, index) =>
              scalarKey(values[index], left.schema.fields.find((field) => field.id === joinKey.left)?.type.value),
            )
            .join('|');
      ...
      for (const leftRow of left.rows) {
        ...
        if (node.op === 'semijoin') {
          if (matches.length > 0) {
            const added = appendRow(state, rows, leftRow);
            if (!added.ok) return added;
          }
          continue;
        }
        if (matches.length > 1) {
          result = failure(
            'query.cardinality',
            'Declared one-to-one or many-to-one cardinality was violated by source rows.',
          );
          cache.set(id, result);
          return result;
        }
        if (matches.length === 0) {
          if (node.spec.kind === 'left') {
            const added = appendRow(state, rows, nullResult(left.schema, leftRow, right.schema.fields));
            if (!added.ok) return added;
          }
        } else {
          const added = appendRow(state, rows, Object.freeze({ ...leftRow, ...matches[0] }));
          if (!added.ok) return added;
        }
      }
      ...
      const input = inputs[0]!;
      if (!input.complete) {
        result = failure('query.incomplete-input', 'Grouping requires a complete source population.');
        cache.set(id, result);
        return result;
      }
      ...
      for (const row of input.rows) {
        ...
        const values: QueryValue[] = [];
        for (const key of node.keys) {
          const value = evaluateExpression(state, key.expression, row, input.schema);
          if (!value.ok) return value;
          const checkedValue = outputValue(
            value.value,
            node.output.fields.find((field) => field.id === key.id),
          );
          if (!checkedValue.ok) return checkedValue;
          values.push(checkedValue.value);
        }
        const key = values.map((value, index) => scalarKey(value, node.output.fields[index]?.type.value)).join('|');
        const current = groups.get(key) ?? { keys: {}, rows: [] };
        node.keys.forEach((groupKey, index) => {
          current.keys[groupKey.id] = values[index] ?? null;
        });
        current.rows.push(row);
        groups.set(key, current);
      }
      if (node.keys.length === 0 && groups.size === 0) groups.set('[]', { keys: {}, rows: [] });
      const groupValues: EvalGroup[] = [...groups.values()].map((group) => ({
      ...
      result = {
        ...
        value: {
          schema: node.output,
          rows: groupValues.map((group) => group.keyRow),
          complete: true,
          groups: groupValues,
        },
      };
      ...
      if (input.groups === undefined) {
        result = failure('query.aggregate', 'Aggregate node requires a preceding group node.');
        cache.set(id, result);
        return result;
      }
      const rows: QueryRow[] = [];
      for (const group of input.groups) {
        ...
        for (const item of node.items) {
          ...
          if (!checkedValue.ok) return checkedValue;
          output[item.id] = checkedValue.value;
        }
        const added = appendRow(state, rows, Object.freeze(output));
        if (!added.ok) return added;
      }
      ...
      const input = inputs[0]!;
      if (!input.complete) {
        result = failure('query.incomplete-input', 'Exact sorting requires a complete source population.');
        cache.set(id, result);
        return result;
      }
      ...
      rows.sort((left, right) => {
        if (sortError !== undefined) return 0;
        const compared = compareRows(left, right, input.schema, node.items, state);
        if (!compared.ok) {
          sortError = compared;
          return 0;
        }
      ...
      const input = inputs[0]!;
      if (!input.complete) {
        result = failure('query.incomplete-input', 'Top-K requires a complete source population.');
        cache.set(id, result);
        return result;
      }
      ...
      const input = inputs[0]!;
      if (!input.complete) {
        result = failure('query.incomplete-input', 'Window evaluation requires a complete source population.');
        cache.set(id, result);
        return result;
      }
    ...
    if (
      result.ok &&
      (result.value.rows.length > context.maxRows! ||
        outputBytes(result.value.rows) > context.maxBytes! ||
        ((node.op === 'join' || node.op === 'semijoin') && result.value.rows.length > limits.maxJoinRows))
    )
      result = failure('query.budget', 'Query intermediate exceeds its effective materialization budget.');
    cache.set(id, result);
    return result;
  };
  ...
  if (context.maxRows !== undefined && rows.length > context.maxRows)
    return failure('query.budget', 'Query result exceeds the row budget.');
  if (context.maxBytes !== undefined && bytes > context.maxBytes)
  ...
  return {
    ok: true,
    value: {
      schema: final.value.schema,
      rows,
      sourceRevision: source.revision,
      complete: final.value.complete,
      precision: state.approximate
        ? { kind: 'approximate', method: 'IEEE-754 arithmetic, division or mean' }
        : { kind: 'exact' },
