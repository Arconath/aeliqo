...
function validatePredicate(
  ...
  if (predicate.op === 'compare') {
    ...
    if (!sameTypeFamily(left.value.typed.type, right.value.typed.type))
...
function querySpecPredicate(
  ...
  const convert = (input: QuerySpec['where']): QueryOutcome<PredicateSpec> => {
    ...
    if (input.op !== 'compare' && input.op !== 'is-null' && input.op !== 'in')
      return failure('query.predicate', 'Predicate operator is not supported.');
    ...
    const type = semanticType(
      field.type,
      selectedDefinition.rowGrain.map((grain) => fieldKey(selectedEntity, grain)),
    );
    if (input.op === 'compare')
      return {
        ok: true,
...
function aggregateFromMeaning(
  meaning: MeaningDefinition,
  _definitions: readonly MeaningDefinition[],
  _catalog: Catalog,
...
function lowerQuerySpec(
  ...
  type RelationUsage = {
    readonly relation: VersionRef;
    readonly kind: 'inner' | 'left' | 'semi';
  ...
  if (query.relations.length > 0 && (relationUsage === undefined || relationUsage.length !== query.relations.length))
    return unsupported(
  ...
  if (query.relations.length === 0 && relationUsage !== undefined && relationUsage.length > 0)
    return failure(
      'query.relation-usage',
  ...
  for (let index = 0; index < query.relations.length; index += 1) {
    ...
    if (usage === undefined || seenRelations.has(relationKey(reference)))
    ...
    if (declared === undefined)
      return failure('query.relationship', `Relationship ${relationKey(reference)} is not declared.`, [
        'relations',
        index,
      ]);
  ...
  if (query.population.kind !== 'all-authorized')
    return unsupported(
  ...
  if (entityDefinition === undefined)
    return failure('query.entity', `Entity ${query.entity} is not declared.`, ['entity']);
  let searchPredicate: PredicateSpec | undefined;
  if (query.search !== undefined) {
    const uniqueFields = new Set(query.search.fields);
    if (uniqueFields.size !== query.search.fields.length)
    ...
    for (let index = 0; index < query.search.fields.length; index += 1) {
      ...
      if (field === undefined)
        return failure('query.field', `Search field ${fieldId} is not declared on ${query.entity}.`, [
          'search',
          'fields',
          index,
        ]);
      ...
      const call: Expression = {
        ...
        arguments: [
          fieldExpression(query.entity, fieldId),
          literalExpression(query.search.text, { value: 'text', nullable: false }),
        ],
      };
      predicates.push({
        op: 'compare',
  ...
  if (query.timeBucket !== undefined) {
    ...
    if ((query.timeBucket.calendar === undefined) !== (query.timeBucket.timezone === undefined))
    ...
    if (!['day', 'week', 'month', 'quarter', 'year'].includes(query.timeBucket.grain))
      return unsupported(
        'temporal-grain',
    ...
    if (calendar !== 'gregorian' && calendar !== 'iso8601')
      return unsupported(
        'temporal-policy',
    ...
    if (query.timeBucket.grain === 'week' && query.timeBucket.weekStartsOn === undefined)
      return unsupported(
        'temporal-week-start',
    ...
    if (entityDefinition.fields.some((field) => field.id === bucketId))
    ...
    timeBuckets = [
      {
        id: bucketId,
        expression: fieldExpression(query.entity, query.timeBucket.field),
        grain: query.timeBucket.grain as TimeBucketSpec['grain'],
        calendar,
        timezone,
        ...(query.timeBucket.weekStartsOn === undefined ? {} : { weekStartsOn: query.timeBucket.weekStartsOn }),
  ...
  const expressionForField = (field: string): Expression =>
    bucketId !== undefined && query.timeBucket !== undefined && field === query.timeBucket.field
      ? query.groupBy.includes(field)
        ? { kind: 'field', ref: field }
        : { kind: 'field', ref: bucketId }
      : measureIds.has(field) || windowIds.has(field)
        ? { kind: 'field', ref: field }
        : fieldExpression(query.entity, field);
  const expressionForGroupField = (field: string): Expression =>
    bucketId !== undefined && query.timeBucket !== undefined && field === query.timeBucket.field
      ? { kind: 'field', ref: bucketId }
      : fieldExpression(query.entity, field);
  const rewriteWindowExpression = (expression: Expression): Expression => {
    ...
    return {
      ...
      arguments: expression.arguments.map(rewriteWindowExpression),
    };
  };
  ...
  const groupBy: GroupKeySpec[] = query.groupBy.map((field) => ({
    id: field,
    expression: expressionForGroupField(field),
  }));
  ...
  for (let index = 0; index < query.measures.length; index += 1) {
    ...
    if (meaning === undefined)
      return failure(
        'query.meaning',
        `Meaning ${query.measures[index]!.id}@${query.measures[index]!.revision} is not authorized or declared.`,
        ['measures', index],
      );
    const aggregate = aggregateFromMeaning(meaning, definitions, catalog);
    if (!aggregate.ok) return aggregate;
    aggregates.push(aggregate.value);
    if (!select.some((item) => item.id === aggregate.value.id))
      select.push({ id: aggregate.value.id, expression: { kind: 'field', ref: aggregate.value.id } });
  }
  for (const field of query.fields) {
    if (groupBy.length > 0 && !groupBy.some((group) => group.id === field))
  ...
  if (query.period !== undefined && (query.period.calendar !== 'gregorian' || query.period.timezone !== 'UTC'))
    return unsupported(
      'temporal-policy',
  ...
  if (searchPredicate !== undefined)
    filter = filter === undefined ? searchPredicate : { op: 'and', predicates: [filter, searchPredicate] };
  if (query.period !== undefined && query.timeBucket !== undefined) {
    const temporalField = entityDefinition.fields.find((candidate) => candidate.id === query.timeBucket!.field);
    if (temporalField === undefined)
      return failure('query.field', `Field ${query.timeBucket.field} is not declared on ${query.entity}.`, [
    ...
    if (temporalField.type.value !== 'instant')
      return unsupported(
        'query.period-type',
    ...
    const type = semanticType(
      temporalField.type,
      entityDefinition.rowGrain.map((grain) => fieldKey(query.entity, grain)),
    );
    const range: PredicateSpec = {
      op: 'and',
      predicates: [
        {
          op: 'compare',
          left: fieldExpression(query.entity, temporalField.id),
          comparison: 'gte',
  ...
  const orderBy: SortSpec[] = query.order.map((entry) => ({
    expression:
      bucketId !== undefined && query.timeBucket !== undefined && entry.field === query.timeBucket.field
        ? { kind: 'field', ref: query.groupBy.includes(entry.field) ? entry.field : bucketId }
        : measureIds.has(entry.field)
          ? { kind: 'field', ref: entry.field }
          : fieldExpression(query.entity, entry.field),
...
function nextCost(
  ...
  return {
    ...
    joinRows: joinRows + (operation === 'join' || operation === 'semijoin' ? rows : 0),
    requiresComplete:
      requiresComplete ||
      operation === 'join' ||
      operation === 'semijoin' ||
...
function nodeDetail(operation: PlanOperation): string {
  return operation === 'semijoin'
    ? 'membership join preserves left identity and row count'
    : operation === 'join'
      ? 'declared cardinality join'
      : `${operation} over bounded typed relation`;
}
...
function validateRelationshipKeyTypes(
  catalog: Catalog,
...
function timeBucketSchema(
  ...
  for (const item of items) {
    ...
    const expression = resolveExpression(item.expression, input, registry);
    if (!expression.ok) return expression;
    if (expression.value.typed.type.value !== 'date' && expression.value.typed.type.value !== 'instant')
...
function groupSchema(
  ...
  return {
    ok: true,
    value: { fields: fields.map((field) => ({ ...field, type: { ...field.type, grain } })), identity: grain, grain },
  };
}
    ...
    const containsAggregate = (expression: Expression): boolean => {
      ...
      return (
        ...
        (signature.operation === 'aggregate' ||
          ...
          expression.arguments.some(containsAggregate))
      );
    };
    if (
      checked.value.typed.operation !== 'aggregate' &&
      checked.value.typed.operation !== 'ratio-of-sums' &&
      checked.value.typed.operation !== 'mean-of-rates' &&
      !containsAggregate(call)
    )
      return unsupported(
        'aggregate-function',
...
function buildPlan(
  ...
  const state: PlannerState = {
    nodes: [],
    current: '',
    schema: scanSchema(entity),
    depth: 0,
    cost: {
      estimatedRows: limits.maxRows,
      estimatedBytes: estimateBytes(scanSchema(entity), limits.maxRows),
      nodes: 0,
      joinRows: 0,
  ...
  for (let index = 0; index < (input.semiJoins ?? []).length; index += 1) {
    ...
    if (relationship === undefined)
      return failure('query.relationship', `Relationship ${relationKey(spec.relationship)} is not declared.`, [
        'semiJoins',
        index,
      ]);
    if (relationship.sourceEntity !== input.root || relationship.targetEntity !== spec.rightEntity)
      return failure(
        'query.relationship-direction',
        'A semijoin must follow the declared relationship direction from the root entity.',
        ['semiJoins', index],
      );
    if (relationship.joinPolicy !== 'validated')
      return unsupported(
        'query.relationship-policy',
        'The declared relationship is not approved for local semijoin evaluation.',
    ...
    const keys = relationship.keys.map((key) => ({
      left: fieldKey(relationship.sourceEntity, key.sourceField),
      right: fieldKey(relationship.targetEntity, key.targetField),
    }));
    const output = leftNode.output;
    const semijoin = addNode(
      state,
      'semijoin',
      output,
      { spec, keys },
  ...
  for (let index = 0; index < (input.joins ?? []).length; index += 1) {
    ...
    if (relationship.sourceEntity !== input.root || relationship.targetEntity !== spec.rightEntity)
      return failure(
        'query.relationship-direction',
        'A join must follow the declared relationship direction from the root entity.',
        ['joins', index],
      );
    if (relationship.joinPolicy !== 'validated')
      return unsupported(
        'query.relationship-policy',
        'The declared relationship is not approved for local join evaluation.',
        ['Use a validated relationship or a host capability.'],
        ['joins', index],
      );
  ...
  if ((input.timeBuckets ?? []).length > 0) {
    ...
    const bucket = addNode(
      state,
      'time-bucket',
      output.value,
      { items: input.timeBuckets! },
      limits,
      [state.current],
      state.cost.estimatedRows,
    );
  ...
  if (input.topK !== undefined) {
    ...
    const top = addNode(
      ...
      limits,
      [state.current],
      Math.min(state.cost.estimatedRows, input.topK),
    );
...
export function createQueryPlanner(options: QueryPlannerOptions): QueryOutcome<import('./types.js').QueryPlanner> {
  ...
  try {
    registryOutcome = createFunctionRegistry({
