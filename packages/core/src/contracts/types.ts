...
export type Wire<T> = [z.JSONType] extends [T]
  ? [T] extends [z.JSONType]
    ? ReadonlyJsonValue
    : WireMember<T>
  : WireMember<T>;
...
export type Contract<K extends ContractKind> = K extends 'plot-spec'
  ? PlotSpec
  : K extends 'visualization-spec'
    ? VisualizationSpec
    : Wire<z.infer<(typeof contractSchemas)[K]>>;
