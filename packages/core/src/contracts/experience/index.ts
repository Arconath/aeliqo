...
export function resolveExperienceConstraints(
  ...
  restrictions.data.forEach((restriction, index) => {
    if (applied.has(restriction.id))
      fail('experience.restriction-duplicate', 'Restriction IDs must be unique.', ['restrictions', index, 'id']);
    applied.add(restriction.id);
    if (restriction.allowedRepresentations !== undefined)
      representations = intersect(representations, restriction.allowedRepresentations, (value) => value);
    if (restriction.allowedPatterns !== undefined)
      patterns = intersect(patterns, restriction.allowedPatterns, (value) => value);
    if (restriction.extensionAllowlist !== undefined)
      extensions = intersect(extensions, restriction.extensionAllowlist, refKey);
    if (restriction.allowedOperations !== undefined)
      operations =
        operations === undefined
          ? restriction.allowedOperations
          : intersect(operations, restriction.allowedOperations, refKey);
  ...
  if (task.viewPreference?.strength === 'explicit')
    representations = representations.filter((id) => id === task.viewPreference!.representation);
  if (!representations.length)
    fail('experience.representation-conflict', 'No representation satisfies the profile and explicit restrictions.', [
  ...
  if (operations !== undefined) {
    ...
    experience.requiredOperations.forEach((id, index) => {
      if (!permittedIds.has(id))
        fail('experience.operation-conflict', 'A required profile operation is excluded by the restrictions.', [
          'requiredOperations',
          index,
        ]);
    });
  ...
  return {
    ...
    value: {
      ...
      mode: modes[mode]!,
      agentAllowed,
      allowedRepresentations: representations.sort(),
      allowedPatterns: patterns.sort(),
      extensionAllowlist: distinctRefs(extensions),
      allowedOperations: operations === undefined ? undefined : distinctRefs(operations),
      ...
      maxNodes,
      maxExpansions,
      representationReplacementAllowed: mode > 0,
      compositionChangeAllowed: mode > 1,
      transitionPolicy,
