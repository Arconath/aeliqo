  ...
  const visit = (node: PlotNode): Outcome<readonly BoundPlotUnit[]> => {
    ...
    if (node.kind === 'unit') {
      ...
      for (const [channel, encoding] of Object.entries(enc)) {
        ...
        if (encoding.zero && encoding.scale !== 'linear')
          return fail('zero', 'Zero is supported only on linear scales.');
      ...
      for (const [first, second] of [
        ...
        if (
          second !== undefined &&
          signature(first, result.fields.find((f) => f.id === first.field)!.type) !==
            signature(second, result.fields.find((f) => f.id === second.field)!.type)
        )
          return fail('endpoint', 'Endpoint scales and semantic types must be compatible.');
      }
      const unit = { node, result };
      units.push(unit);
    ...
    if (node.kind === 'facet') {
      const types = descendants.map((u) => {
        ...
        return JSON.stringify([
          field.type.value,
          field.type.nullable,
          signature({ field: node.field, scale: 'ordinal' }, field.type),
        ]);
      });
