...
const key = (ref: ResultRef): string =>
...
const temporal = (type: SemanticType): boolean =>
  (type.value === 'date' || type.value === 'instant') &&
  type.temporal !== undefined &&
  (type.value !== 'instant' || type.temporal.timezone !== undefined);
const signature = (type: SemanticType, nullable = true): string =>
  JSON.stringify([
    type.value,
...
export function bindVisualizationSpec(
  ...
  for (const inputResult of context.results) {
    ...
    if (
      ...
      new Set(result.identity).size !== result.identity.length ||
      new Set(result.rowGrain).size !== result.rowGrain.length ||
      result.rowGrain.length === 0 ||
  ...
  if (context.catalog !== undefined) {
    const checked = parseCatalog(context.catalog);
    if (!checked.ok) return checked;
    catalog = checked.value;
    if (
      new Set(catalog.entities.map((entity) => entity.id)).size !== catalog.entities.length ||
      new Set(catalog.relationships.map(refKey)).size !== catalog.relationships.length ||
      new Set(catalog.meanings.map(refKey)).size !== catalog.meanings.length
    )
      return fail('catalog', 'Catalog identities must be unambiguous.');
  }
  ...
  const additive = (ref: VersionRef, field: FieldDefinition): Outcome<true> => {
    ...
    if (
      ...
      found.authority === 'hypothesis' ||
      found.functionRegistryDigest !== catalog?.functionRegistryDigest ||
      found.aggregation !== 'additive' ||
      field.derivation === undefined ||
      refKey(field.derivation) !== refKey(found) ||
      signature(found.output) !== signature(field.type) ||
  ...
  if ('plot' in spec) {
    ...
    for (const unit of bound.value.units) {
      ...
      const x = fields.get(encoding.x.field)!;
      const y = fields.get(encoding.y.field)!;
      const mark = unit.node.mark;
      if (spec.view === 'trend' || spec.view === 'area') {
        if (
          mark !== (spec.view === 'trend' ? 'line' : 'area') ||
          encoding.x.scale !== 'temporal' ||
          !temporal(x.type) ||
          x.type.temporal?.grain === undefined ||
          !numeric(y.type) ||
          !unit.result.rowGrain.includes(x.id)
        )
          return fail('trend', 'Temporal views require temporal grain and quantitative observations.');
        if (spec.view === 'area') {
          const checked = additive(spec.meaning, y);
          if (!checked.ok) return checked;
          if (spec.stack === 'zero' && encoding.series === undefined)
            return fail('stack', 'Stacked area requires an explicit series dimension.');
        }
      } else if (spec.view === 'bar') {
        if (mark !== 'bar' || encoding.x.scale !== 'ordinal' || !numeric(y.type))
        ...
        if (
          ...
          !numeric(x.type) ||
          !numeric(y.type) ||
          !['linear', 'log'].includes(encoding.x.scale) ||
          !['linear', 'log'].includes(encoding.y.scale)
        )
          return fail('scatter', 'Scatter requires two quantitative axes.');
      } else if (spec.view === 'histogram') {
        const inspected = parseWireValue(context.histograms ?? []);
        if (!inspected.ok) return inspected;
        const declarations = z.safeParse(histogramBindingsSchema, inspected.value);
        if (
          !declarations.success ||
          declarations.data.filter(
            (binding) =>
              key(binding.result) === key(unit.result.ref) &&
        ...
        if (
          ...
          signature(x.type) !== signature(end.type) ||
        ...
        if (
          mark !== 'cell' ||
          encoding.x.scale !== 'ordinal' ||
          encoding.y.scale !== 'ordinal' ||
          color === undefined ||
          !numeric(color.type) ||
          x.id === y.id ||
          color.id === x.id ||
          color.id === y.id ||
          encoding.color?.scale !== 'linear' ||
          !unit.result.rowGrain.includes(x.id) ||
          !unit.result.rowGrain.includes(y.id)
        )
    ...
    return {
      ok: true,
      value: own({ spec, results: used, plot: bound.value, ...(meaning === undefined ? {} : { meaning }) }),
    };
  }
  const result = results.get(key(spec.result));
  ...
  if (spec.view === 'matrix') required.push(...spec.columns);
    ...
    required.push(
      ...
      ...(spec.view === 'treemap' ? [spec.value] : []),
    );
  else if (spec.view === 'relationship')
    required.push(...spec.source, ...spec.target, ...(spec.label ? [spec.label] : []));
  else if (spec.view === 'timeline')
    required.push(spec.start, ...(spec.end ? [spec.end] : []), ...(spec.label ? [spec.label] : []));
  else required.push(spec.date, ...(spec.value ? [spec.value] : []), ...(spec.label ? [spec.label] : []));
  if (required.some((id) => !fields.has(id)))
  ...
  if (spec.view === 'matrix') {
    ...
    if (
      new Set(spec.node).size !== spec.node.length ||
      !sameSet(spec.node, result.identity) ||
      !sameSet(spec.node, result.rowGrain) ||
      spec.node.length !== spec.parent.length ||
      new Set(spec.parent).size !== spec.parent.length ||
      spec.node.some((id) => spec.parent.includes(id)) ||
      spec.node.some(
        (id, index) =>
          signature(fields.get(id)!.type, false) !== signature(fields.get(spec.parent[index]!)!.type, false),
      )
    )
      return fail('hierarchy', 'Hierarchy requires one row per stable node and compatible explicit parent keys.');
    if (spec.view === 'treemap') {
      const checked = additive(spec.meaning, fields.get(spec.value)!);
      if (!checked.ok) return checked;
    }
  } else if (spec.view === 'relationship') {
    ...
    if (
      ...
      source === undefined ||
      target === undefined ||
      spec.source.length !== source.identity.length ||
      spec.target.length !== target.identity.length ||
      new Set(spec.source).size !== spec.source.length ||
      new Set(spec.target).size !== spec.target.length
    )
      return fail(
        'relationship',
        'Relationship view requires a declared versioned relation and stable endpoint identities.',
      );
    const mappings = parseWireValue(context.relationships ?? []);
    ...
    for (const [entity, projection] of [
      ...
      for (const [index, id] of entity.identity.entries()) {
        ...
        if (
          field === undefined ||
          field.type.nullable ||
          signature(field.type) !== signature(fields.get(projection[index]!)!.type)
        )
