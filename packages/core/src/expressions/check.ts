...
export function checkExpression(input: unknown, context: ExpressionCheckContext): Outcome<TypedExpression> {
  ...
  while (stack.length > 0) {
    ...
    if (frame.node.kind === 'literal') {
      ...
      const checkedField: TypedExpression = {
      ...
      if (meaning === undefined)
        return semanticFailure('semantic.unknown-definition', `Meaning ${refKey(frame.node.ref)} is not registered.`, [
          ...frame.path,
          'ref',
        ]);
      const typeCheck = validateSemanticType(meaning.output, [...frame.path, 'ref']);
      if (!typeCheck.ok) return typeCheck;
      const checkedDefinition: TypedExpression = {
        expression: frame.node,
        type: meaning.output,
        context: context.evaluationContext ?? 'row',
      };
      ...
      for (let index = frame.node.arguments.length - 1; index >= 0; index -= 1) {
        stack.push({
          kind: 'enter',
          node: frame.node.arguments[index]!,
...
function checkLiteral(
  ...
  if (value === null) {
    ...
    return semanticFailure('semantic.literal-type', 'Integer literals must be safe exact integers.', [
      ...path,
      'value',
    ]);
    ...
    (node.type.value === 'text' || node.type.value === 'date' || node.type.value === 'instant') &&
    typeof value !== 'string'
  ) {
    ...
    (node.type.value === 'date' || node.type.value === 'instant') &&
    node.type.temporal !== undefined &&
    node.type.temporal.calendar !== 'gregorian' &&
    ...
    return semanticFailure(
      ...
      'Decimal literals must use the exact decimal object representation.',
      [...path, 'value'],
    );
  }
...
function checkCall(
  ...
  if (args.length < required || args.length > maximum)
    return semanticFailure(
      'semantic.arity',
      `Function ${refKey(signature.ref)} received ${args.length} arguments; expected ${required}${Number.isFinite(maximum) ? `-${maximum}` : '+'}.`,
      [...path, 'arguments'],
    );
  ...
  if (!signature.contexts.includes(context))
    return semanticFailure(
      ...
      `Function ${refKey(signature.ref)} is not registered for ${context} evaluation.`,
      [...path, 'function'],
    );
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (signature.nullPolicy === 'reject' && argument.type.nullable)
      return semanticFailure('semantic.nullability', `Function ${refKey(signature.ref)} rejects nullable arguments.`, [
        ...path,
        'arguments',
        index,
      ]);
    if (!matchesConstraint(argument.type, parameter.constraint, args, index))
      return semanticFailure(
        'semantic.type-mismatch',
        `Argument ${index + 1} is incompatible with function ${refKey(signature.ref)}.`,
  ...
  if (signature.operation === 'arithmetic' && args.length >= 2) {
    if (!sameOrBroadcastGrain(args[0]!, args[1]!))
      return semanticFailure(
        ...
        `Function ${refKey(signature.ref)} received incompatible grains.`,
        [...path, 'arguments', 1],
      );
    ...
    if (unitRule === 'same' && !sameUnit(args[0]!.type, args[1]!.type))
      return semanticFailure(
        ...
        `Function ${refKey(signature.ref)} received incompatible units.`,
        [...path, 'arguments', 1],
      );
  ...
  if (signature.operation === 'comparison' && args.length >= 2) {
    for (let index = 1; index < args.length; index += 1) {
      ...
      if (!sameComparisonValueKind(left.type, right.type))
        return semanticFailure(
          'semantic.type-mismatch',
          'Comparison requires values of the same semantic type family.',
          [...path, 'arguments', index],
        );
  ...
  if (signature.operation === 'conditional') {
    ...
    for (let index = 0; index < args.length; index += 1)
      for (let other = index + 1; other < args.length; other += 1)
        if (
          !sameOrBroadcastGrain(args[index]!, args[other]!) &&
  ...
  const entityIds = [
    ...new Set(
      args.map((argument) => argument.entityId).filter((entityId): entityId is string => entityId !== undefined),
    ),
  ];
  ...
  const aggregation =
    signature.operation === 'ratio-of-sums'
      ...
      : signature.operation === 'mean-of-rates'
        ? 'non-additive'
        : signature.aggregation.kind;
  const inheritedGrain = args.find((argument) => grainOf(argument.type).length > 0)?.type.grain;
  const resultType =
    signature.operation === 'conditional' || signature.operation === 'comparison'
      ? {
          ...output.value,
          ...(inheritedGrain === undefined ? {} : { grain: inheritedGrain }),
          ...(signature.operation === 'conditional'
            ? {
                nullable:
                  signature.nullResult === 'non-null'
                    ? false
                    : signature.nullResult === 'preserve'
                      ? output.value.nullable
...
function matchesConstraint(
  ...
  if (constraint.kind === 'numeric')
    return isNumericType(actual) && (constraint.allowNull !== false || !actual.nullable);
  if (constraint.kind === 'boolean')
    return actual.value === 'boolean' && (constraint.allowNull !== false || !actual.nullable);
  if (constraint.kind === 'text')
    return actual.value === 'text' && (constraint.allowNull !== false || !actual.nullable);
...
function outputType(
  ...
  const forceFloat =
    ...
    operation === 'divide' ||
    operation === 'mean-of-rates' ||
    operation === 'ratio-of-sums';
  const options = {
