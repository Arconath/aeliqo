...
export const deltaConfigSchema = z
  .object({
    ...
    baselineLabel: z.string().trim().min(1).max(160),
    mode: z.enum(["absolute", "relative"]).optional(),
  })
  .strict();
export const timeInvestigationConfigSchema = z
  .object({
    baseline: z.number().finite().nullable(),
    baselineLabel: z.string().trim().min(1).max(160),
    mode: z.enum(["absolute", "relative"]).optional(),
    eventDatasetId: z.string().min(1).max(80).optional(),
    eventTimeField: z.string().min(1).max(80).optional(),
    eventLabelField: z.string().min(1).max(80).optional(),
  })
  .strict()
  .refine(value => Boolean(value.eventDatasetId) === Boolean(value.eventTimeField), "Event dataset and time field must be supplied together");
...
export function createComponentRegistry(
  ...
  for (const definition of definitions) {
    ...
    if (
      !/^[A-Za-z][A-Za-z0-9_.:-]{0,99}$/.test(capability.component) ||
      entries.has(capability.component)
    )
      throw new Error("Component ids must be valid and unique");
    entries.set(capability.component, Object.freeze({ ...definition }));
    manifests.set(
      capability.component,
      Object.freeze({
        ...capability,
        accepts: Object.freeze([...capability.accepts]),
        interactions: Object.freeze([...capability.interactions]),
  ...
  return Object.freeze({
    ...
    validate(node: WorkspaceNode, dataPort: DataPort) {
      ...
      if (definition.configSchema)
        definition.configSchema.parse(node.config ?? {});
      else if (node.config && Object.keys(node.config).length)
        throw new Error("Component does not accept configuration");
