...
export function createCatalogIndex(catalog: Catalog): Outcome<CatalogIndex> {
    ...
    for (const identity of entity.identity) {
      if (!entityFields.has(identity))
        return semanticFailure(
          ...
          `Identity field ${identity} is not declared on ${entity.id}.`,
          ['entities', entityIndex, 'identity'],
        );
    }
  ...
  for (let index = 0; index < canonical.capabilities.length; index += 1) {
    ...
    if (capabilities.has(capabilityKey))
      return semanticFailure(
        'semantic.duplicate-capability',
        `Capability ${capabilityKey} is declared more than once.`,
        ['capabilities', index],
      );
  ...
  const index: CatalogIndex = {
    ...
    resolveField(entityId, fieldId) {
      if (entityId !== undefined) {
        ...
        return field
          ? { ok: true, value: field }
          : semanticFailure('semantic.unknown-field', `Field ${fieldId} is not available on entity ${entityId}.`, [
              'ref',
            ]);
      }
      ...
      if (fields.length > 1)
        return semanticFailure('semantic.ambiguous-field', `Field ${fieldId} belongs to more than one entity.`, [
          'ref',
        ]);
