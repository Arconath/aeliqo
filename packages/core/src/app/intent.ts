...
function compileStandard(intent: Exclude<Intent, { kind: 'custom' }>, context: IntentCompilerContext): Outcome<Task> {
  ...
  if (intent.kind === 'browse') {
    ...
    return {
      ok: true,
      value: queryTask(intent, resource, context.regionId, context.taskRevision, query, 'data.read', requested.value),
    };
  }
...
export function compileIntent(input: unknown, options: CompileIntentOptions): Outcome<Task> {
  ...
  if (intent.preferredView !== undefined && !options.resource.presentation.allowedViews.includes(intent.preferredView))
    return failure('intent.unknown-view', `View ${intent.preferredView} is not allowed for ${options.resource.id}.`, [
      'preferredView',
    ]);
  ...
  if (!inputParsed.success) {
    const diagnostics = inputParsed.error.issues.slice(0, 16).map(
      (issue) =>
        ({
  ...
  try {
    outcome = definition.compile(inputParsed.data, context);
  } catch {
  ...
  if (
    ...
    task.value.catalogRevision !== options.resource.catalog.revision ||
    task.value.functionRegistryDigest !== options.resource.catalog.functionRegistryDigest
  )
    return failure(
