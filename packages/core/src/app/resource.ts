...
function unwrap(schema: ZodRuntimeSchema): {readonly schema: ZodRuntimeSchema; readonly nullable: boolean; readonly optional: boolean} {
  let current = schema;
  let nullable = false;
  let optional = false;
...
function inferType(schema: z.ZodType, path: Path): Outcome<SemanticType> {
  ...
  if (unwrapped.optional) {
    return {ok: false, diagnostics: [diagnostic('resource.optional-field', 'Optional record fields cannot be represented as nullable values.', path,
  ...
  if (runtime.type === 'string') value = runtime.format === 'date' ? 'date' : runtime.format === 'datetime' ? 'instant' : 'text';
    ...
    value = typeof literal === 'boolean' ? 'boolean' : typeof literal === 'number' ? (Number.isInteger(literal) ? 'integer' : 'float') : typeof literal === 'string' ? 'text' : undefined;
  }
  if (value === undefined) {
    return {ok: false, diagnostics: [diagnostic('resource.unsupported-schema', 'This field schema does not map to an Aeliqo scalar.', path,
...
function inspectResource<Schema extends z.ZodObject>(input: ResourceInput<Schema>): Outcome<ResourceDefinition<Schema>> {
  if (!validIdentifier(input.id)) return {ok: false, diagnostics: [diagnostic('resource.id', 'Resource ID must be a bounded identifier without whitespace.', ['id'])]};
  if (input.label.trim().length === 0) return {ok: false, diagnostics: [diagnostic('resource.label', 'Resource label must not be empty.', ['label'])]};
  const shapeKeys = Object.keys(input.schema.shape);
  for (const metadataKey of Object.keys(input.fields ?? {})) {
    if (!shapeKeys.includes(metadataKey)) return {ok: false, diagnostics: [diagnostic('resource.unknown-field', `Field metadata references unknown schema field ${metadataKey}.`, ['fields', metadataKey])]};
  }
  ...
  if (entity === undefined) return {ok: false, diagnostics: [diagnostic('resource.entity', `Catalog entity ${entityId} is not declared.`, ['entity'])]};
  const schemaFields = new Set(shapeKeys);
  for (const field of entity.fields) {
    if (!schemaFields.has(field.id)) return {ok: false, diagnostics: [diagnostic('resource.schema-field', `Catalog field ${field.id} is missing from the runtime schema.`, ['schema', field.id])]};
  }
  for (const key of schemaFields) {
    if (!entity.fields.some((field) => field.id === key)) return {ok: false, diagnostics: [diagnostic('resource.catalog-field', `Runtime schema field ${key} is missing from the Catalog entity.`, ['schema', key])]};
  }
  const defaultIntents = STANDARD_INTENTS.filter((intent) => intent !== 'create' && intent !== 'edit' || input.forms?.[intent] !== undefined);
  const intents = Object.freeze([...(input.intents ?? defaultIntents)]);
  if (new Set(intents).size !== intents.length) return {ok: false, diagnostics: [diagnostic('resource.duplicate-intent', 'Resource intent declarations must be unique.', ['intents'])]};
  for (const intent of ['create', 'edit'] as const) {
    if (intents.includes(intent) && input.forms?.[intent] === undefined)
      return {ok: false, diagnostics: [diagnostic('resource.form-intent', `${intent} requires a registered form schema and action.`, ['intents'])]};
  }
  const allowedViews = Object.freeze([...input.presentation.allowedViews]);
  if (allowedViews.length === 0 || new Set(allowedViews).size !== allowedViews.length || allowedViews.some((view) => !validIdentifier(view)))
    return {ok: false, diagnostics: [diagnostic('resource.views', 'A resource needs unique bounded allowed view IDs.', ['presentation', 'allowedViews'])]};
  ...
  const definition: ResourceDefinition<Schema> = {
    ...
    catalog: catalogOutcome.value, entity, fieldMetadata, intents,
    presentation: Object.freeze({allowedViews, ...(input.presentation.preferred === undefined ? {} : {preferred: Object.freeze({...input.presentation.preferred})})}),
    ...(input.forms === undefined ? {} : {forms: Object.freeze({...input.forms})}),
    parseRecord(value) {
      const parsed = input.schema.safeParse(value);
      if (parsed.success) return {ok: true, value: parsed.data};
      const issues = parsed.error.issues.slice(0, 16).map((issue) => diagnostic(`resource.record.${issue.code}`, issue.message,
        issue.path.filter((part): part is string | number => typeof part !== 'symbol')));
      const firstIssue = issues[0];
      return firstIssue === undefined
        ? {ok: false, diagnostics: [diagnostic('resource.record.invalid', 'The record does not match its runtime schema.', [])]}
...
export function validateResource<Schema extends z.ZodObject>(input: ResourceInput<Schema>): Outcome<ResourceDefinition<Schema>> {
