...
export function serializeWorkspace(store: WorkspaceStore): string {
  ...
  const operations = [
    ...
    ...Object.values(state.bindings).map((binding) => ({
      type: "connect",
      binding,
    })),
    ...Object.entries(state.selections)
      .filter(([, recordId]) => recordId !== null)
      .map(([id, recordId]) => ({ type: "select", id, recordId })),
    ...Object.entries(state.interactions)
      .filter(([, payload]) => payload !== null)
      .map(([id, payload]) => ({ type: "interact", id, payload })),
  ];
...
export function restoreWorkspace(
  serialized: string,
  options: { dataPort: DataPort; registry?: ComponentRegistry },
): WorkspaceStore {
  if (serialized.length > 1_000_000)
    throw new Error("Workspace document exceeds size limit");
  ...
  for (let offset = 0; offset < document.operations.length; offset += 30) {
    const request = patchSchema.parse({
      version: 1,
      baseRevision: temporary.getState().revision,
      operations: document.operations.slice(offset, offset + 30),
    });
    if (
      request.operations.some(
        (operation) =>
          !["mount", "connect", "select", "interact"].includes(operation.type),
      )
    )
  ...
  return createWorkspace({
    ...options,
    nodes: snapshot.order.map((id) => snapshot.nodes[id]!),
    bindings: Object.values(snapshot.bindings),
