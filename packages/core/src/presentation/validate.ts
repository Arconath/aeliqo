...
function ownedEmptyConfigOutcome(raw: unknown, values: PresentationValues): ResolvedPresentationConfig | undefined {
  try {
    ...
    if (
      ...
      !descriptors.fields?.enumerable ||
      !descriptors.ports?.enumerable ||
      !descriptors.operations?.enumerable
    )
      return undefined;
...
function preparePresentationTree(
  ...
  for (const entry of cache.treeEntries) {
    ...
    for (let index = 0; index < nodeIds.length; index++) {
      ...
      if (
        entry.nodeIds[index] !== nodeIds[index] ||
        (cachedChildren !== candidateChildren &&
          (cachedChildren.length !== candidateChildren.length ||
  ...
  for (let i = 0; i < visit.length; i++) {
    ...
    visited.add(id);
    visit.push(...childrenById.get(id)!);
  }
...
function prepareCoverage(
  ...
  for (const entry of plan.coverage) {
    if (
      coverage.has(entry.needId) ||
      new Set(entry.nodeIds).size !== entry.nodeIds.length ||
      new Set(entry.operations.map(versionKey)).size !== entry.operations.length
    )
    ...
    for (const id of entry.nodeIds) {
      ...
      if (
        !entry.operations.every((op) =>
          (node.config.operations ?? []).some((supported) => versionKey(supported) === versionKey(op)),
        )
      )
      ...
      if (
        ...
        !entry.operations.every((op) => cache.allowedOperations!.has(versionKey(op)))
      )
  ...
  if (
    [...cache.taskNeeds.values()].some((need) => need.required && !coverage.has(need.id)) ||
    [...cache.requiredOperations].some((op) => !operations.has(op))
  )
  ...
  const patterns =
    requiredPattern === undefined
      ...
      : (registry.patterns ?? []).filter(
          (pattern) => samePattern(pattern, requiredPattern) && patternIsAllowed(pattern, prepared.constraints),
        );
...
export function validatePreparedPresentationPlan(
  ...
  if (
    c.task.revision !== current.taskRevision ||
    c.task.catalogRevision !== current.catalogRevision ||
    c.task.functionRegistryDigest !== current.functionRegistryDigest ||
    c.experience.revision !== current.experienceRevision
  )
  ...
  const requiredResults = [
    ...prepared.value.taskStructure.resultReferences,
    ...plan.nodes.flatMap((n) => (n.result === undefined ? [] : [n.result])),
  ];
  ...
  if (ownedPlan && validationCache !== undefined && Object.isFrozen(plan) && Object.isFrozen(plan.preconditions)) {
    ...
    readSet = outcomes.find(
      (entry) =>
        entry.references.length === requiredResults.length &&
        entry.references.every((reference, index) => reference === requiredResults[index]),
    )?.outcome;
  ...
  for (const node of plan.nodes) {
    ...
    if (!allowedRepresentations.has(m.ref.id) || (m.extension && !extensions.has(key)))
    ...
    if (
      ...
      (m.result === 'required' && result === undefined) ||
      (m.result === 'none' && result !== undefined)
    )
    ...
    if (result !== undefined) {
      ...
      if (task.kind === 'data') {
        ...
        if (
          output === undefined ||
          (output.kind === 'reuse' ? refKey(output.result) !== refKey(result.ref) : result.taskId !== task.id)
        )
    ...
    const cachedConfig =
      output !== null && typeof output === 'object' ? preparedCache.value.resolvedConfigs.get(output) : undefined;
    ...
    if (
      ...
      enabled.some((op) => !m.operations.some((declared: VersionRef) => versionKey(op) === versionKey(declared)))
    )
    ...
    if (config.fields.some((field: string) => result === undefined || !resultFields.get(result)!.has(field)))
    ...
    const resolvedConfig = freezePresentationContainer({
      values,
      fields: freezePresentation(config.fields),
      ports: freezePresentation(portGraph.value.nodes[0]!.ports),
    ...
    const resolvedNode = freezePresentationContainer({
      node,
      manifest: m.ref,
  ...
  let preparedCoverage = coverageVariants?.find(
    (entry) =>
      entry.nodes.length === coverageRefs.length && entry.nodes.every((node, index) => node === coverageRefs[index]),
  )?.outcome;
  ...
  const hasPotentialComparison =
    [...coverage.values()].some((entry) => entry.nodeIds.length > 1) ||
  ...
  if (context.incumbent !== undefined) {
    ...
    const oldReadSet = validateCommitReadSet(
    ...
    if (!stateCapabilities.ok) return stateCapabilities;
    const nodeById = new Map(plan.nodes.map((node) => [node.id, node]));
    const topology = (p: typeof plan) =>
      JSON.stringify(
        p.nodes
          .map((n) => [n.id, n.role, n.children])
          .sort((a, b) => (String(a[0]) < String(b[0]) ? -1 : String(a[0]) > String(b[0]) ? 1 : 0)),
      );
    const variants = (p: typeof plan) =>
      JSON.stringify(p.nodes.map((n) => [n.id, versionKey(n.representation)]).sort());
    ...
    const semantics = (p: typeof plan) =>
      JSON.stringify([p.links, p.coverage, p.nodes.map((n) => [n.id, n.result]).sort()]);
    ...
    if (
      (structural || replacing || configurationChanged || semanticChanged) &&
      (context.transitionBlocked === true ||
        (c.transitionPolicy === 'explicit-only' && context.explicitTransition !== true))
    )
      return fail(
        'transition',
    ...
    for (const transfer of plan.stateTransfer) {
      ...
      const identity =
        ...
        transfer.mapping.id === 'aeliqo.state.identity' &&
        transfer.mapping.revision === '1';
  ...
  if (plan.links.length === 0) {
    ...
    if (value === undefined) {
      value = freezePresentation({
        nodes: resolved.map((n) => ({ id: n.node.id, ports: n.config.ports })),
        links: [],
        mappings: [],
      });
  ...
  const unchangedOwnedPlan =
    ownedPlan &&
    Object.isFrozen(plan) &&
    Object.isFrozen(plan.nodes) &&
    resolved.every((node, index) => node.node === plan.nodes[index]);
  const finalPlan = unchangedOwnedPlan
    ? (plan as PresentationPlan)
    : (freezePresentationContainer({
        ...plan,
        nodes: freezePresentationContainer(resolved.map((n) => n.node)),
      }) as PresentationPlan);
  if (
    options.requiredPattern !== undefined &&
    (!patternIsAllowed(options.requiredPattern, c) ||
      !matchesPattern(finalPlan, prepared.value, registry, options.requiredPattern))
  )
