...
function materializationOutcome(value: unknown): InteractionOutcome<InteractionMaterialization> {
  ...
  if (record.ok === false) {
    ...
    return checked.ok
      ? failure('runtime.interaction-denied', 'The host materializer returned an invalid failure.')
      : (checked as InteractionOutcome<InteractionMaterialization>);
  }
  if (
    record.ok !== true ||
    Object.keys(record).length !== 2 ||
  ...
  if (
    Object.keys(candidate).some((key) => key !== 'state' && key !== 'resultHandles') ||
  ...
  if (
    ...
    (!Array.isArray(candidate.resultHandles) || candidate.resultHandles.length > WIRE_LIMITS.array)
  )
  ...
  const resultHandles =
    candidate.resultHandles === undefined ? undefined : Object.freeze([...candidate.resultHandles] as ResultHandle[]);
  return {
    ok: true,
    value: Object.freeze({
...
class InteractionControllerImpl implements InteractionController {
  ...
  constructor(options: InteractionControllerOptions) {
    ...
    if (
      !Number.isSafeInteger(this.maxEventMilliseconds) ||
      this.maxEventMilliseconds < 1 ||
      this.maxEventMilliseconds > 86_400_000
    )
    ...
    if (
      !Number.isSafeInteger(this.maxQueuedEvents) ||
      this.maxQueuedEvents < 1 ||
      this.maxQueuedEvents > WIRE_LIMITS.array
    )
  ...
  dispatch(input: unknown, options: InteractionDispatchOptions = {}): Promise<InteractionOutcome<InteractionReceipt>> {
    ...
    if (this.pending.has(event.eventId))
      return Promise.resolve(
        failure('runtime.interaction-stale', 'An interaction with this event ID is already pending.'),
      );
  ...
  private readHost(snapshot: RegionSnapshot): InteractionOutcome<InteractionHostContext> {
    ...
    if (
      ...
      typeof host !== 'object' ||
      Array.isArray(host) ||
      !validText(host.principalKey) ||
      !validText(host.draftDomain) ||
      host.actor === null ||
      typeof host.actor !== 'object' ||
      Array.isArray(host.actor) ||
      !validText(host.actor.id) ||
      !['user', 'service', 'system'].includes(host.actor.kind) ||
      !Array.isArray(host.grants) ||
      host.grants.length > WIRE_LIMITS.array ||
      host.grants.some((grant) => !validText(grant)) ||
      ...
      !Array.isArray(host.results) ||
      host.results.length > WIRE_LIMITS.array ||
      host.results.some((ref) => ref.scopeDigest !== host.scopeDigest)
    )
    ...
    if (
      ...
      host.catalogRevision !== snapshot.readSet.catalogRevision ||
      host.experienceRevision !== snapshot.readSet.experienceRevision ||
      host.functionRegistryDigest !== snapshot.readSet.functionRegistryDigest ||
      host.scopeDigest !== snapshot.readSet.scopeDigest ||
  ...
  private async callHost<T>(
    ...
    if (deadline !== undefined && this.deadlineExpired(deadline, controller))
    ...
    try {
      ...
      if (result === 'timeout' || timedOut) {
        controller.abort();
        return failure('runtime.interaction-budget', 'The host interaction callback exceeded its bounded time budget.');
      }
      if (result === 'aborted')
  ...
  private async callMaterialize(
    ...
    if (this.options.materialize === undefined)
      return failure(
    ...
    try {
      ...
      if (result === 'timeout' || timedOut) {
        controller.abort();
        return failure('runtime.interaction-budget', 'The host materializer exceeded its bounded time budget.');
      }
      if (result === 'aborted')
        return deadline?.expired
          ? failure('runtime.interaction-budget', 'The interaction exceeded its bounded event time budget.')
  ...
  private async applyPayload(
    ...
    if (controller.signal.aborted) return failure('runtime.interaction-cancelled', 'The interaction was cancelled.');
    if (payload.kind === 'selection') {
      if (!hasGrant(host, 'experience.commit'))
      ...
      if (payload.selection.mode === 'ids') {
        if (!hasGrant(host, 'result.inspect'))
        ...
        if (refKey(checked.value.ref) !== refKey(payload.selection.result))
          return failure('runtime.interaction-stale', 'The selected result is not the current authorized result.');
        result = checked.value;
      }
      const checked = await this.callHost(
        this.options.validateSelection,
        payload.selection,
        context,
        controller,
        deadline,
    ...
    if (payload.kind === 'draft') {
      if (!hasGrant(host, 'draft.edit') || !hasGrant(host, 'experience.commit'))
      ...
      if (!checked.ok) return checked;
      const nextDraft = freeze({
        domain: host.draftDomain,
    ...
    if (payload.kind === 'navigate') {
      if (!hasGrant(host, 'navigation.propose'))
      ...
      if (!current.ok) return current;
      const checked = await this.callHost(
        this.options.validateNavigation,
        payload,
        this.resolutionContext(event, current.value, snapshot, controller.signal),
        controller,
        deadline,
      );
      if (!checked.ok) return checked;
    ...
    if (payload.kind === 'action-request') {
      if (!hasGrant(host, 'action.propose'))
  ...
  private addHandle(handles: ResultHandle[], handle: ResultHandle): void {
    ...
    try {
      ...
      /* malformed handles are rejected by region.stage */
    }
    if (
      !handles.some((candidate) => {
  ...
  private async materialize(
    ...
    const result = await this.callMaterialize(
      payloads,
      this.resolutionContext(event, host, snapshot, controller.signal),
      next,
      controller,
      deadline,
    );
    if (!result.ok) return result;
    const checked =
      result.value.state.interaction === undefined ? undefined : parseInteractionState(result.value.state.interaction);
  ...
  private async commitState(
    ...
    if (this.region.snapshot().regionRevision !== before.regionRevision)
    ...
    try {
      const staged = await this.region.stage({
        requestId: event.eventId,
        expected,
        state,
        ...(resultHandles.length === 0 ? {} : { resultHandles }),
      });
      ...
      if (!committed.ok) {
        if (deadline.expired || this.deadlineExpired(deadline, controller))
          return failure('runtime.interaction-budget', committed.diagnostics[0]!.message);
  ...
  private async processEvent(
    ...
    if (before.status !== 'active')
      return failure(
        before.status === 'revoked' ? 'runtime.interaction-revoked' : 'runtime.interaction-disposed',
    ...
    if (event.regionId !== before.id || event.regionRevision !== before.regionRevision)
      return failure(
        'runtime.interaction-stale',
        'The interaction event is stale against the current region revision.',
      );
    ...
    if (this.deadlineExpired(deadline, controller))
      return failure('runtime.interaction-budget', 'The interaction exceeded its bounded event time budget.');
    const routedItems: InteractionRoutedPayload[] = [
      { route: source.value, payload: event.payload, causationId: event.causationId },
      ...routed.value,
    ];
    ...
    for (const item of routedItems) {
      ...
      applied.add(key);
      const next = await this.applyPayload(
        item.payload,
        item.route,
        event,
        host.value,
      ...
      if (
        item.payload.kind === 'filter' ||
        item.payload.kind === 'range' ||
        item.payload.kind === 'group' ||
        item.payload.kind === 'page'
      )
        queryPayloads.push(item.payload);
    }
    ...
    } else if (this.deadlineExpired(deadline, controller))
      return failure('runtime.interaction-budget', 'The interaction exceeded its bounded event time budget.');
    ...
    for (const navigate of navigations) {
      ...
      if (!current.ok) return current;
      const callback = await this.callHost(
        this.options.onNavigate,
        navigate,
        this.resolutionContext(event, current.value, region, controller.signal),
        controller,
        deadline,
      );
