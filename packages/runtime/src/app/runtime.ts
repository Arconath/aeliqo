...
function statusFor(diagnostics: readonly Diagnostic[]): Exclude<RuntimeRenderStatus, 'committed'> {
  ...
  if (code.includes('cancel') || code.includes('abort') || code.includes('disposed') || code.includes('stale'))
    return 'cancelled';
...
export function createAeliqoRuntime(options: AeliqoRuntimeOptions): AeliqoRuntime {
  ...
  for (const binding of options.resources) {
    if (resources.has(binding.resource.id))
  ...
  const resultStore: ResultStore = {
    ...
    get(input) {
      return underlyingStore.get(input);
    },
    revoke(input) {
  ...
  const authorityForResource = (
    slot: MountedRegion,
    resourceId: string,
    effect: 'render' | 'commit' | 'context',
    signal?: AbortSignal,
    ...
    try {
      outcome = options.authority.read({
        resourceId,
  ...
  const authorityFor = (
    ...
    effect: 'render' | 'commit' | 'context',
    signal?: AbortSignal,
  ): Outcome<AppAuthorityContext> => authorityForResource(slot, slot.resourceId, effect, signal);
  const describeResource = (resourceId: string, current: AppAuthorityContext): RuntimeResourceContext => {
    ...
    return Object.freeze({
      ...
      fields: Object.freeze(
        resource.entity.fields.flatMap((field) =>
          resource.fieldMetadata[field.id]?.hidden === true
            ...
            : [
                Object.freeze({
                  id: field.id,
                  label: field.label,
                  ...(resource.fieldMetadata[field.id]?.description === undefined
                    ? {}
                    : { description: resource.fieldMetadata[field.id]!.description }),
                  role: field.role,
                  type: field.type,
                  ...(resource.fieldMetadata[field.id]?.values === undefined
                    ? {}
                    : { values: resource.fieldMetadata[field.id]!.values }),
                }),
              ],
        ),
      ),
      meanings: Object.freeze(
        resource.catalog.meanings.map((meaning) =>
          Object.freeze({
            id: meaning.id,
  ...
  const regionAuthority = (regionId: string): RegionOutcome<RegionAuthority> => {
    ...
    if (slot === undefined)
      return { ok: false, diagnostics: [diagnostic('runtime.region-denied', 'The region is not mounted.')] };
    ...
    if (slot.principalKey !== undefined && current.value.principalKey !== slot.principalKey)
      return {
        ok: false,
        diagnostics: [diagnostic('runtime.region-denied', 'The authenticated principal changed before commit.')],
      };
    return {
      ok: true,
      value: {
  ...
  const regions = createRegionStore({
    ...
    authorizeCommit: ({ regionId, authority, signal }) => {
      ...
      if (!current.ok) return current;
      const same =
        current.value.principalKey === authority.principalKey &&
        current.value.scopeDigest === authority.scopeDigest &&
        current.value.policyRevision === authority.policyRevision &&
        current.value.experienceRevision === authority.experienceRevision;
      return same
        ? { ok: true, value: undefined }
        : { ok: false, diagnostics: [diagnostic('runtime.region-stale', 'Authority changed during commit.')] };
    },
    ...(options.maxRegions === undefined ? {} : { maxRegions: options.maxRegions }),
  });
  const evaluator = createTaskEvaluator({
    host: {
      readContext: ({ task, signal }) => {
        const slot = mounted.get(task.regionId);
        ...
        const context: TrustedEvaluationContext = {
          ...
          catalogRevision: binding.resource.catalog.revision,
          functionRegistryDigest: binding.resource.catalog.functionRegistryDigest,
          grants: current.value.grants,
          catalog: binding.resource.catalog,
          data: binding.data,
          resultStore,
          readContext: current.value.readContext,
          resolveResult,
          now: current.value.now ?? (() => Date.now()),
          ...(current.value.budget === undefined ? {} : { budget: current.value.budget }),
        };
  ...
  const failReceipt = (
    ...
    setState(slot, {
      ...
      results: slot.refs,
      diagnostics,
      ...(regions.get(slot.regionId) === undefined ? {} : { region: regions.get(slot.regionId)!.snapshot() }),
    });
  ...
  const mount = (input: RuntimeMountInput) => {
    ...
    if (!validId(input.regionId) || !resources.has(input.resourceId))
      return failure<RuntimeRegionState>(
        'runtime.mount-invalid',
        'Mount requires a bounded region ID and registered resource.',
        ['regionId'],
      );
    if (mounted.has(input.regionId))
      return failure<RuntimeRegionState>('runtime.mount-duplicate', `Region ${input.regionId} is already mounted.`, [
        'regionId',
      ]);
    const state: RuntimeRegionState = Object.freeze({
  ...
  const render = async (input: RuntimeRenderInput): Promise<RuntimeRenderReceipt> => {
    ...
    try {
      ...
      const binding = resources.get(slot.resourceId)!;
      const compiled = compileIntent(parsedIntent.value, {
      ...
      return failReceipt(slot, sequence, requestId, [
  ...
  const runtime: AeliqoRuntime = {
    ...
    contexts(regionId) {
      ...
      if (!active.value.grants.includes('catalog.read'))
        return failure('runtime.context-denied', 'Catalog discovery is not permitted for this session.');
      ...
      for (const resourceId of resources.keys()) {
        ...
        if (
          current.value.principalKey !== active.value.principalKey ||
          current.value.scopeDigest !== active.value.scopeDigest
        )
          continue;
    ...
    async commitPresentation(input: RuntimePresentationInput) {
      ...
      if (
        current.readSet === undefined ||
        current.state?.task.id !== input.task.id ||
        current.state.task.revision !== input.task.revision
      )
      ...
      if (committed.ok)
        setState(slot, { ...slot.state, task: committed.value.state?.task ?? input.task, region: committed.value });
      return committed;
    },
    snapshot(regionId) {
      return mounted.get(regionId)?.state;
    },
    ...
    dispose() {
      if (disposed) return;
      disposed = true;
      for (const slot of mounted.values()) {
        slot.active?.abort();
        slot.listeners.clear();
      }
      mounted.clear();
