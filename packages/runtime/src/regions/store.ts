...
const failure = <T>(code: RegionFailure['code'], message: string): RegionOutcome<T> => ({
  ok: false,
  diagnostics: [{ code, message, retryable: false }],
});

function normalizeHostOutcome<T>(
  value: unknown,
  ...
  try {
    if (value !== null && typeof value === 'object' && !Array.isArray(value)) {
      ...
      if (record.ok === true && keys.length === 2 && keys.includes('ok') && keys.includes('value'))
      ...
      if (
        ...
        for (const candidate of record.diagnostics) {
          ...
          if (
            Object.keys(diagnostic).some((key) => !allowed.includes(key)) ||
          ...
          if (
            ...
            (!Array.isArray(diagnostic.path) ||
              diagnostic.path.length > WIRE_LIMITS.array ||
              diagnostic.path.some(
                (part) => !(typeof part === 'string' ? validId(part) : Number.isSafeInteger(part) && part >= 0),
              ))
          )
            return failure(code, FAILURE.denied);
          if (
            ...
            (!Array.isArray(diagnostic.remedies) ||
              diagnostic.remedies.length > WIRE_LIMITS.array ||
              diagnostic.remedies.some((remedy) => !validText(remedy)))
          )
            return failure(code, FAILURE.denied);
          diagnostics.push(
            frozen({
              ...
              retryable: diagnostic.retryable as boolean,
              ...(diagnostic.path === undefined
                ? {}
                : { path: Object.freeze([...(diagnostic.path as readonly (string | number)[])]) }),
              ...(diagnostic.remedies === undefined
                ? {}
                : { remedies: Object.freeze([...(diagnostic.remedies as readonly string[])]) }),
            }),
          );
        }
...
function validateState(value: unknown, regionId: string): RegionOutcome<RegionContent> {
  ...
  const presentation =
    record.presentation === undefined ? undefined : parseContract('presentation-plan', record.presentation);
  if (presentation !== undefined && !presentation.ok)
  ...
  if (interaction !== undefined && !interaction.ok)
...
function validateAuthority(value: unknown): RegionOutcome<RegionAuthority> {
  ...
  if (
    ...
    record.principalKey.length === 0 ||
    record.principalKey.length > WIRE_LIMITS.id * 4 ||
    /[\u0000-\u001f\u007f]/u.test(record.principalKey)
  )
    return failure('runtime.region-denied', FAILURE.denied);
  ...
  private notify(
    ...
    for (const observer of observers) {
      ...
      try {
        ...
        if (result !== undefined && typeof (result as unknown as PromiseLike<void>)?.then === 'function')
  ...
  async stage(input: RegionStageInput): Promise<RegionOutcome<RegionCommitToken>> {
    ...
    try {
      ...
      const checked = validateCommitReadSet(
        stripData({ ...expected.value, results: capturedResults.value }),
        stripData(actual),
        required,
      );
      ...
      const bytes =
        new TextEncoder().encode(canonical(checkedState.value)).byteLength +
  ...
  private authorizeWithDeadline(
    ...
    if (callerSignal?.aborted)
      return Promise.resolve(
        failure('runtime.region-cancelled', 'The region commit was cancelled before publication.'),
      );
    ...
    const onAbort = (): void => {
      if (!this.live(expectedEpoch)) finish(this.closedOutcome());
        ...
        finish(failure('runtime.region-cancelled', 'The region commit was cancelled before publication.'));
    ...
    pending.then(
      (authorized) => {
        try {
          ...
          if (!this.live(expectedEpoch)) {
            finish(this.closedOutcome());
            return;
          }
  ...
  async commit(token: RegionCommitToken, options: RegionCommitOptions = {}): Promise<RegionOutcome<RegionSnapshot>> {
    ...
    const queued: Promise<RegionOutcome<RegionSnapshot>> = this.enqueue<RegionSnapshot>(async () => {
      ...
      try {
        ...
        if (!this.live(queuedEpoch)) return this.closedOutcome();
        if (
          beforeAuthority.value.principalKey !== this.principalKey ||
          beforeAuthority.value.principalKey !== record.principalKey
        )
          return failure('runtime.region-stale', FAILURE.stale);
        const current = this.snapshotValue();
        if (!this.authorizationConfigured) return failure('runtime.region-denied', FAILURE.denied);
        const authorizationInput: Omit<RegionCommitAuthorizationInput, 'signal'> = {
          regionId: this.id,
          token,
          state: record.state,
          current,
          authority: beforeAuthority.value,
        };
        ...
        if (
          afterAuthority.value.principalKey !== this.principalKey ||
          afterAuthority.value.principalKey !== record.principalKey ||
          record.capturedReadSet.dataRevision !== fresh.dataRevision ||
          !validateCommitReadSet(stripData(record.capturedReadSet), stripData(fresh), record.requiredResults).ok
        )
          return failure('runtime.region-stale', FAILURE.stale);
        ...
        const nextReadSet = frozen({
          ...authorityReadSet(afterAuthority.value, nextTaskRevision, nextRegionRevision, this.dataRevision),
    ...
    return this.enqueue(async () => {
      ...
      try {
        ...
        if (
          ...
          Object.keys(publication).some((key) => !['resultHandles', 'results', 'reason'].includes(key))
        )
          return failure('runtime.region-invalid', FAILURE.invalid);
        ...
        if (publication?.resultHandles !== undefined) {
          if (!Array.isArray(publication.resultHandles) || publication.resultHandles.length > WIRE_LIMITS.array)
            return failure('runtime.region-budget', FAILURE.budget);
        ...
        if (
          ...
          authority.value.scopeDigest !== currentPins.scopeDigest ||
          authority.value.policyRevision !== currentPins.policyRevision ||
          authority.value.catalogRevision !== currentPins.catalogRevision ||
          authority.value.experienceRevision !== currentPins.experienceRevision ||
        ...
        if (explicit && normalized.value.length === 0) {
          ...
          if (
            authority.value.results.length !== currentPins.results.length ||
            authority.value.results.some((ref, index) => refKey(ref) !== refKey(currentPins.results[index]!))
          )
            return failure('runtime.region-stale', FAILURE.stale);
        ...
        if (normalized.value.some((ref) => !authorityRefs.has(refKey(ref))))
          return failure('runtime.region-stale', FAILURE.stale);
        ...
        const effectiveChangedResults = [...changedByLogical.values()]
        ...
        this.entries = [
          ...this.entries,
          this.makeHistory('data', effectiveChangedResults, undefined, publication?.reason, at),
        ];
        this.trimHistory();
        ...
        const update: RegionUpdate = frozen({
          kind: 'data',
          snapshot,
          ...(effectiveChangedResults.length === 0 ? {} : { changedResults: effectiveChangedResults }),
          ...(publication?.reason === undefined ? {} : { reason: publication.reason }),
        });
        this.notify(
          update,
          explicit
            ? effectiveChangedResults
            : effectiveChangedResults.length === 0
              ? undefined
              : effectiveChangedResults,
        );
        return { ok: true, value: snapshot };
  ...
  observe(listener: (update: RegionUpdate) => void, options: RegionObserverOptions = {}): RegionObserverFunction {
    ...
    if (options.results !== undefined) {
      ...
      for (const candidate of options.results) {
        const ref =
          typeof (candidate as ResultHandle)?.snapshot === 'function'
            ? resultRefFromHandle(candidate as ResultHandle)
            : validateResultRef(candidate)
              ? ({ ok: true, value: candidate } as const)
              : failure<ResultRef>('runtime.region-invalid', FAILURE.invalid);
...
export class RegionStoreImpl implements RegionStore {
  ...
  constructor(options: RegionStoreOptions) {
    ...
    this.maxCommitAuthorizationMilliseconds =
      options?.maxCommitAuthorizationMilliseconds ?? DEFAULT_COMMIT_AUTHORIZATION_MILLISECONDS;
    ...
    this.readAuthority = this.authorityConfigured
      ? options.readAuthority
      : () => failure('runtime.region-denied', FAILURE.denied);
    this.authorizeCommit = this.authorizationConfigured
      ? options.authorizeCommit
      : () => failure('runtime.region-denied', FAILURE.denied);
    if (!Number.isSafeInteger(this.maxRegions) || this.maxRegions < 1 || this.maxRegions > 10_000)
      throw new TypeError('maxRegions must be a bounded positive safe integer.');
    if (!Number.isSafeInteger(this.maxHistory) || this.maxHistory < 1 || this.maxHistory > WIRE_LIMITS.array)
      throw new TypeError('maxHistory must be a bounded positive safe integer.');
    if (!Number.isSafeInteger(this.maxQueuedCommands) || this.maxQueuedCommands < 1 || this.maxQueuedCommands > 10_000)
      throw new TypeError('maxQueuedCommands must be a bounded positive safe integer.');
    if (!Number.isSafeInteger(this.maxStagedCommits) || this.maxStagedCommits < 1 || this.maxStagedCommits > 10_000)
      throw new TypeError('maxStagedCommits must be a bounded positive safe integer.');
    if (
      !Number.isSafeInteger(this.maxStagedBytes) ||
      this.maxStagedBytes < 1 ||
      this.maxStagedBytes > WIRE_LIMITS.bytes
    )
      throw new TypeError('maxStagedBytes must be a bounded positive safe integer.');
    if (
      !Number.isSafeInteger(this.maxCommitAuthorizationMilliseconds) ||
      this.maxCommitAuthorizationMilliseconds < 1 ||
      this.maxCommitAuthorizationMilliseconds > MAX_COMMIT_AUTHORIZATION_MILLISECONDS
    )
      throw new TypeError('maxCommitAuthorizationMilliseconds must be a bounded positive duration.');
    if (
      !Number.isSafeInteger(this.maxRestoreMilliseconds) ||
      this.maxRestoreMilliseconds < 1 ||
      this.maxRestoreMilliseconds > MAX_COMMIT_AUTHORIZATION_MILLISECONDS
    )
  ...
  private restoreWithDeadline(
    ...
    if (expectedEpoch !== this.epoch || this.disposed)
      return Promise.resolve(failure('runtime.region-disposed', FAILURE.disposed));
    const controller = new AbortController();
    ...
    const onAbort = (): void => {
      finish(
        timedOut
          ? failure('runtime.region-budget', 'The host restore/requery exceeded its bounded time budget.')
          : failure('runtime.region-disposed', FAILURE.disposed),
      );
    };
    ...
    timer = setTimeout(
      ...
      Math.min(this.maxRestoreMilliseconds, 2_147_483_647),
    );
    let pending: Promise<RegionOutcome<RegionRestoreMaterialization>>;
    try {
      pending = Promise.resolve(
        restoreRegion({ regionId: document.id, document, authority, signal: controller.signal }),
      );
    } catch {
      finish(failure('runtime.region-denied', FAILURE.denied));
      return result;
  ...
  private createInternal(
    ...
    try {
      ...
      const handle = new RegionHandleImpl(this, input, options, {
        authority,
      ...
      if (duplicate || this.regions.size + otherPendingRestoresAfterConstruction >= this.maxRegions) {
        handle.dispose();
        return failure(
          duplicate ? 'runtime.region-invalid' : 'runtime.region-budget',
          duplicate ? 'A region with this stable ID already exists.' : FAILURE.budget,
        );
      }
  ...
  create(input: RegionCreateInput): RegionOutcome<RegionHandle> {
    ...
    if (
      state.value.task.catalogRevision !== authority.value.catalogRevision ||
      state.value.task.functionRegistryDigest !== authority.value.functionRegistryDigest
    )
    ...
    const required = validateCommitReadSet(
      stripData(initialReadSet),
      stripData(initialReadSet),
      requiredResultReferences(state.value),
    );
  ...
  async restore(documentInput: unknown): Promise<RegionOutcome<RegionHandle>> {
    ...
    try {
      ...
      if (!authority.ok) return authority as RegionOutcome<RegionHandle>;
      if (
        document.value.task.catalogRevision !== authority.value.catalogRevision ||
        document.value.task.functionRegistryDigest !== authority.value.functionRegistryDigest
      )
        return failure(
      ...
      if (checked.value.task.id !== document.value.task.id)
      ...
      if (
        freshAuthority.value.principalKey !== authority.value.principalKey ||
        freshAuthority.value.scopeDigest !== authority.value.scopeDigest ||
        freshAuthority.value.policyRevision !== authority.value.policyRevision ||
        freshAuthority.value.catalogRevision !== authority.value.catalogRevision ||
        freshAuthority.value.experienceRevision !== authority.value.experienceRevision ||
        freshAuthority.value.functionRegistryDigest !== authority.value.functionRegistryDigest
      )
        return failure('runtime.region-stale', 'The host authorization changed while restoring the region.');
      if (
        checked.value.task.catalogRevision !== freshAuthority.value.catalogRevision ||
        checked.value.task.functionRegistryDigest !== freshAuthority.value.functionRegistryDigest
      )
        return failure(
          'runtime.region-stale',
          'The restored Task is bound to a different catalog or function registry.',
        );
      const incarnation = newRegionRevision();
      ...
      for (const handle of handles) {
        ...
        if (!ref.ok) {
          ...
          return ref as RegionOutcome<RegionHandle>;
        }
        if (restoreEpoch !== this.epoch || this.disposed) {
          releaseLeases(leases);
          return failure('runtime.region-disposed', FAILURE.disposed);
        }
        const bindingForHandle = bindResultHandleToAuthority(handle, freshAuthority.value);
        if (!bindingForHandle.ok) {
          releaseLeases(leases);
          return bindingForHandle as RegionOutcome<RegionHandle>;
        }
        const lease = retainResultHandle(handle);
        if (!lease.ok) {
          releaseLeases(leases);
          return lease as RegionOutcome<RegionHandle>;
        }
        leases.push({ ref: ref.value, lease: lease.value });
        required.push(ref.value);
      }
      ...
      if (!readCheck.ok) {
        releaseLeases(leases);
        return failure('runtime.region-stale', readCheck.diagnostics[0]!.message);
      }
      ...
      const created = this.createInternal(
        { id: document.value.id, state: finalState.value },
        freshAuthority.value,
        {
