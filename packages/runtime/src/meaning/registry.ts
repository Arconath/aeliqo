...
import {
  ...
  meaningDigest,
  meaningRefKey,
  snapshotMeaningAuthoringOptions,
} from './authoring.js';
import type {
  MeaningEntry,
...
function activationCandidate(
  ...
  return context.policy.allowlistedDefinitions.find(
    (candidate) => candidate !== null && typeof candidate === 'object' && sameRef(candidate, draft),
  );
}
...
function contextFailure(
...
): Outcome<MeaningActivationReceipt> | undefined {
  if (
    context === null ||
  ...
  if (
    context.readSet !== undefined &&
    (context.readSet === null ||
      typeof context.readSet !== 'object' ||
      context.readSet.scopeDigest !== context.scopeDigest ||
      context.readSet.policyRevision !== context.policyRevision ||
      context.readSet.catalogRevision !== context.catalogRevision ||
      context.readSet.functionRegistryDigest !== context.functionRegistryDigest)
  )
  ...
  if (
    ...
    (!Array.isArray(context.allowedScopes) || !context.allowedScopes.includes(meaning.scope))
  )
...
export function createMeaningRegistry(options: MeaningRegistryOptions): Outcome<MeaningRegistry> {
  ...
  try {
    checked = validateMeaningBundle(bundle, {
      catalog: options.catalog,
      registry: options.registry,
      definitions: inheritedDefinitions,
      ...(options.policy === undefined ? {} : { policy: options.policy }),
    });
  } catch {
  ...
  const validationOptions = (definitions = availableDefinitions()): MeaningRegistryOptions => ({
    ...
    catalog: {
      ...options.catalog,
      meanings: options.catalog.meanings.filter((meaning) => !entries.get(meaningRefKey(meaning))?.revoked),
    },
    definitions,
  });
    ...
    if (prior !== undefined) {
      ...
      return {
        ...
        value: freezeMeaningValue({
          ...
          meaning: { id: canonicalDraft.meaning.id, revision: canonicalDraft.meaning.revision },
          digest: prior.draft.digest,
          catalogRevision: options.catalog.revision,
          idempotent: true,
          active: prior.active,
        }),
  ...
  const registerBundle = (
    ...
    if (
      retainedBytes +
        drafts
          .filter((draft) => !entries.has(meaningRefKey(draft.meaning)))
  ...
  const list = (
    filter: { readonly includeRevoked?: boolean; readonly activeOnly?: boolean } = {},
  ...
  const definitions = (filter: { readonly activeOnly?: boolean } = {}): readonly MeaningDefinition[] => {
      ...
      availableDefinitions()
        .filter((meaning) => {
          ...
          return (
            (entry === undefined ||
              entry.active ||
              inheritedDefinitions.some((inherited) => sameRef(inherited, meaning))) &&
            (filter?.activeOnly !== true || meaning.lifecycle === 'active')
          );
        })
    ...
    while (changed) {
      ...
      for (const [key, meaning] of output)
        if (meaning.dependencies.some((ref) => !output.has(meaningRefKey(ref)))) {
          output.delete(key);
          changed = true;
        }
    }
  ...
  const activate = async (
    ref: VersionRef,
    activationOptions: MeaningActivationOptions = {},
    ...
    if (activationOptions.expectedAuthority !== undefined) {
      ...
      expected = freezeMeaningValue(parsed.value) as unknown as NonNullable<
        MeaningActivationOptions['expectedAuthority']
      >;
      if (!validId(expected?.principalKey))
    ...
    try {
      before = snapshotContext(
        await host.readContext({
          ...
          signal: activationOptions.signal ?? new AbortController().signal,
        }),
      );
    } catch {
    ...
    const beforeFailure = contextFailure(
      before.value,
      entry.draft.meaning,
    ...
    if (
      expected !== undefined &&
      (expected.principalKey !== before.value.principalKey ||
        (expected.readSet !== undefined &&
          canonicalMeaning(expected.readSet) !== canonicalMeaning(before.value.readSet)))
    )
      return failure('runtime.meaning-stale', 'The activation registry does not match the trusted calling authority.');
    if (revokedScopes.has(before.value.scopeDigest))
      return failure('runtime.meaning-revoked', 'The activation scope has been revoked.');
    if (
      activatedOwner !== undefined &&
      (activatedOwner.principalKey !== before.value.principalKey ||
        activatedOwner.scopeDigest !== before.value.scopeDigest)
    )
      return failure(
    ...
    try {
      candidateCheck = validateMeaningBundle(
        {
          catalogRevision: options.catalog.revision,
          functionRegistryDigest: options.registry.digest,
          meanings: [approved],
    ...
    if (
      !sameActivationContext(before.value, after.value) ||
      generation !== beforeGeneration ||
      get(ref)?.revoked === true
    )
  ...
  const revoke = (ref: VersionRef): Outcome<MeaningRevocationReceipt> => {
    ...
    if (prior.revoked)
      return {
        ...
        value: freezeMeaningValue({
          ...
          meaning: { id: prior.draft.meaning.id, revision: prior.draft.meaning.revision },
          catalogRevision: options.catalog.revision,
        }),
      };
    ...
    while (changed) {
      changed = false;
      for (const [dependentKey, entry] of entries)
        if (
          !revoked.has(dependentKey) &&
          entry.draft.meaning.dependencies.some((dependency) => revoked.has(meaningRefKey(dependency)))
        ) {
          revoked.add(dependentKey);
          changed = true;
        }
    }
  ...
  const revokeScope = (scopeDigest: string): Outcome<readonly MeaningRevocationReceipt[]> => {
    ...
    for (const [key, entry] of entries)
      if (activeScopes.get(key) === scopeDigest && entry.active) {
        const result = revoke({ id: entry.draft.meaning.id, revision: entry.draft.meaning.revision });
        if (!result.ok) return result;
      }
    const receipts: MeaningRevocationReceipt[] = [...entries]
  ...
  const registry: MeaningRegistry = Object.freeze({
    catalog: options.catalog,
    registry: options.registry,
    register,
    registerBundle,
