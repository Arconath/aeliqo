...
import type { Diagnostic, Outcome, QuerySpec, ResultRef } from '@aeliqo/core';
import type { DataService, PlanRequest, QueryBudget, ReadContext } from '../data/types.js';
    ...
    async evaluate(input) {
      ...
      const clearOwned = (): void => {
        for (const handle of owned) {
          try {
            ...
            /* Best-effort cleanup after revoked authority. */
          }
        }
        owned.clear();
        for (const lease of leases.splice(0)) lease.release();
      ...
      try {
        ...
        const seeds =
          explicit === undefined
            ? requested.outputs
                .filter((output) => output.kind === 'reuse' || output.delivery === 'eager')
        ...
        const requiresInspect = [...selected].some((id) => {
          const output = outputById.get(id);
          return (
            output?.kind === 'reuse' || (output?.kind === 'query' && output.query.population.kind !== 'all-authorized')
          );
        });
        if (requiresInspect && !context.grants.includes('result.inspect'))
          return failure(
        ...
        const resolver =
          ...
          options.cohortResolver ??
          localResolver(context.data) ??
          createResultCohortResolver();
        const resolveMaterialized = (ref: ResultRef): ResultHandle | undefined =>
          materialized.get(ref.outputId)?.ref && sameRef(materialized.get(ref.outputId)!.ref, ref)
            ? materialized.get(ref.outputId)!.handle
            : context.resolveResult(ref);
        const cohortCapability: NonNullable<ReadContext['cohort']> = {
          principalKey: context.principalKey,
          resolver,
          resultStore: context.resultStore,
          resolveResult: resolveMaterialized,
        };
        const contextForResolver = (): CohortResolverContext => ({
          readContext: contextRead(context, outer.signal),
          principalKey: context.principalKey,
        ...
        const bindLivePopulation = async (query: QuerySpec): Promise<Outcome<QuerySpec>> => {
          ...
          const cohortRequest: CohortRequest = {
            source: upstream.ref,
        ...
        for (const outputId of structure.value.outputOrder) {
          ...
          if (output.kind === 'reuse') {
            ...
            try {
              handle = context.resolveResult(output.result);
            } catch {
            ...
            try {
              ...
              return failure('runtime.evaluation-denied', 'The reused output could not be retained.', [
                'outputs',
                outputId,
              ]);
            }
            if (lease.released)
              return failure('runtime.evaluation-denied', 'The reused output is no longer available.', [
                'outputs',
                outputId,
              ]);
            leases.push(lease);
            ...
            materialized.set(outputId, {
              outputId,
              kind: 'reuse',
              ref: snapshot.descriptor.ref,
              descriptor: snapshot.descriptor,
              handle,
              lineage: snapshot.descriptor.lineage.flatMap((entry) => entry.inputs),
            });
            continue;
          }
          ...
          const planRequest: PlanRequest = {
            ...
            requestId,
            catalogRevision: context.catalogRevision,
            target: { taskId: requested.id, outputId: output.id },
            query: bound.value,
            budget,
          };
          ...
          if (
            ...
            accepted.target.taskId !== requested.id ||
          ...
          const key = {
            ...
            outputId: output.id,
            taskId: accepted.target.taskId ?? accepted.requestId,
            requestId,
          };
          let handle: ResultHandle;
          try {
            handle = context.resultStore.begin(key);
          } catch {
          ...
          const subscription = handle.subscribe(
            context.data.execute(accepted, contextRead(context, outer.signal, cohortCapability)),
            { signal: outer.signal },
          );
          try {
            for await (const update of subscription) {
              ...
              if (
                ...
                update.snapshot.status === 'denied' ||
                update.snapshot.status === 'unsupported' ||
                update.snapshot.status === 'stale' ||
                ...
                return failure(
                  ...
                  diagnostic?.message ?? 'The task output failed.',
                );
              }
            }
          } catch {
        ...
        if (
          ...
          (requiresInspect && !finalHost.value.grants.includes('result.inspect'))
        ) {
          clearOwned();
          return failure(
            'runtime.evaluation-stale',
            'Task authority changed before the evaluation could be published.',
          );
        }
        const outputs = structure.value.outputOrder
