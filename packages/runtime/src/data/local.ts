...
function validInstant(value: string): boolean {
  return instantParts(value) !== undefined;
}
...
function normalizeSourceLimits(input: LocalDataServiceOptions['sourceLimits'] | undefined): SourceLimits {
  ...
  if (
    ...
    typeof input !== 'object' ||
    Array.isArray(input) ||
    !isSafePositive(input.rows) ||
    !isSafePositive(input.bytes)
  )
...
function normalizeSnapshot(snapshot: LocalSnapshot, sourceLimits: SourceLimits): StoredSnapshot {
  ...
  for (const [entityId, rows] of Object.entries(snapshot.records)) {
    ...
    if (rows.length > sourceLimits.rows - totalRows)
    ...
    records[entityId] = Object.freeze(
      Array.from({ length: rows.length }, (_, rowIndex) => {
        const row = rows[rowIndex];
        if (row === null || typeof row !== 'object' || Array.isArray(row))
          throw new TypeError(`Row for ${entityId} must be a plain object.`);
        if (Object.getPrototypeOf(row) !== Object.prototype && Object.getPrototypeOf(row) !== null)
          throw new TypeError(`Row for ${entityId} must be a plain object.`);
        const keys = Object.keys(row);
        if (keys.length > WIRE_LIMITS.properties)
          throw new TypeError(`Row for ${entityId} exceeds the bounded field limit.`);
        ...
        for (const field of entity.fields) {
          if (
            !field.type.nullable &&
            (!Object.hasOwn(row, field.id) || row[field.id] === undefined || row[field.id] === null)
          )
            throw new TypeError(`Row for ${entityId} is missing non-nullable field ${field.id}.`);
        }
        for (const identity of entity.identity)
          if (!Object.hasOwn(row, identity) || row[identity] === undefined || row[identity] === null)
            throw new TypeError(`Row for ${entityId} has an invalid identity field ${identity}.`);
        const identityKey = canonical(
          entity.identity.map((identity) => identityPart(row[identity]!, fieldsById.get(identity)!.type.value)),
        );
        if (identityKeys.has(identityKey))
          throw new TypeError(`Rows for ${entityId} contain a duplicate identity tuple.`);
  ...
  return Object.freeze({
    catalog: freezeCatalog(parsed.value),
...
function encodeCursor(value: CursorValue): string {
  return base64Encode(canonical(value));
}
...
function minBudget(requested: QueryBudget, host: QueryBudget, grant?: Partial<QueryBudget>): QueryBudget {
  ...
  return Object.freeze({
    ...
    maxMilliseconds: cap('maxMilliseconds'),
    maxColumns: cap('maxColumns'),
  });
}
...
function validateReadGrant(grant: ReadGrant): Outcome<ReadGrant> {
  try {
    assertSafeId(grant.scopeDigest, 'scopeDigest');
  } catch {
    return failure('data.authorization', 'The ADC authorization returned an invalid scope digest.');
  }
  if (grant.policyRevision !== undefined) {
    try {
      assertSafeId(grant.policyRevision, 'policyRevision');
    } catch {
      return failure('data.authorization', 'The ADC authorization returned an invalid policy revision.');
    }
  }
  if (grant.entities !== undefined) {
    if (
      !Array.isArray(grant.entities) ||
      grant.entities.length > WIRE_LIMITS.array ||
      new Set(grant.entities).size !== grant.entities.length
    )
    ...
    for (const entity of grant.entities) {
      try {
        assertSafeId(entity, 'entity scope');
      } catch {
        return failure('data.authorization', 'The ADC authorization returned an invalid entity scope.');
      }
    }
  ...
  if (grant.fields !== undefined) {
    ...
    for (const [entity, fields] of Object.entries(grant.fields)) {
      try {
        assertSafeId(entity, 'field scope entity');
      } catch {
        return failure('data.authorization', 'The ADC authorization returned an invalid field scope.');
      }
      ...
      for (const field of fields) {
        try {
          assertSafeId(field, 'field scope');
        } catch {
          return failure('data.authorization', 'The ADC authorization returned an invalid field scope.');
        }
      }
  ...
  if (grant.maxBudget !== undefined) {
    ...
    for (const [key, value] of Object.entries(grant.maxBudget)) {
      if (
        !['maxRows', 'maxBytes', 'maxMessages', 'maxMilliseconds', 'maxColumns'].includes(key) ||
        !isSafePositive(value as number)
      )
        return failure('data.authorization', 'The ADC authorization returned an invalid budget.');
    }
  }
  ...
  if (authorize === undefined)
    return Promise.resolve(
      context.signal?.aborted
        ? failure('data.aborted', 'The ADC authorization was cancelled.')
        : { ok: true, value: { scopeDigest: DEFAULT_SCOPE } },
    );
  let pending: Promise<Outcome<ReadGrant>>;
  try {
    pending = Promise.resolve(
      authorize({ operation, requestId, target, context, ...(query === undefined ? {} : { query }) }),
    );
  } catch {
...
async function authorizeWithDeadline(
  ...
  if (milliseconds <= 0)
    return context.signal?.aborted
      ? failure('data.aborted', 'The ADC authorization was cancelled.')
      : failure('data.budget', 'Authorization exceeded the effective time budget.');
...
async function digestWithDeadline(
  value: unknown,
  prefix: string,
  context: ReadContext,
  milliseconds: number,
  ...
  if (milliseconds <= 0)
    return context.signal?.aborted
      ? failure('data.aborted', 'The ADC operation was cancelled.')
      : failure('data.budget', 'The ADC operation exceeded the effective time budget.');
  const deadline = makeDeadline(context, milliseconds);
  try {
...
function resolveValueWithAbort<T>(
  value: Promise<T> | T,
...
function mergeCatalogPage(
  ...
  let entities = catalog.entities.filter(
    (entity) =>
      allowedEntity(grant, entity.id) &&
      allowedIdentity(grant, entity) &&
      entity.rowGrain.every((field) => allowedField(grant, entity.id, field)),
  );
  ...
  const capabilities = catalog.capabilities.filter((capability) => {
    ...
    return (
      capability.fields.every((field) => allowedField(grant, capability.entity, field)) &&
  ...
  const pageEntities = page.map((entity) => {
    const fields = restrictedFields
      ? entity.fields.filter((field) => allowedField(grant, entity.id, field.id))
      : entity.fields;
...
function validateQuery(query: QuerySpec, catalog: Catalog, grant: ReadGrant, allowFixedCohort = false): Outcome<void> {
  ...
  if (!allowedEntity(grant, query.entity))
  ...
  if (entity === undefined)
    return failure('data.invalid-entity', `Entity ${query.entity} is not available.`, ['query', 'entity']);
  ...
  if (!isGroupedOrDerived) {
    for (const identity of entity.identity) {
      if (!query.fields.includes(identity))
        return unsupported(
          {
            kind: 'source',
            id: 'identity-projection',
            reason: `Identity field ${identity} must be projected for stable result lineage.`,
    ...
    for (const index of query.order.keys()) {
      ...
      if (!query.fields.includes(entry.field))
        return unsupported(
          {
            kind: 'operator',
            id: 'order-projection',
...
function plannerFailure<T>(outcome: Outcome<T>): Outcome<T> {
  ...
  if (
    first.code === 'query.stale-catalog' ||
    first.code === 'query.stale-source' ||
    first.code === 'query.stale-registry'
  )
...
function dependenciesForPlan(plan: LogicalPlan, catalog: Catalog): PlanDependencies {
  ...
  const addEntity = (entity: string): void => {
    entities.add(entity);
    if (!fields.has(entity)) fields.set(entity, new Set());
  };
  const addField = (entity: string, field: string): void => {
    addEntity(entity);
    fields.get(entity)!.add(field);
  };
  const nodes = new Map(plan.nodes.map((node) => [node.id, node] as const));
  const inputSchema = (node: PlanNode, index = 0): QuerySchema =>
    nodes.get(node.inputs[index] ?? '')?.output ?? node.output;
  const collectExpression = (
    expression: Expression,
    schema: QuerySchema,
    seenDefinitions: Set<string> = new Set(),
  ): void => {
    ...
    const meaning = catalog.meanings.find(
      (candidate) => candidate.id === expression.ref.id && candidate.revision === expression.ref.revision,
    );
    if (meaning?.implementation.kind === 'expression')
      collectExpression(meaning.implementation.expression, schema, seenDefinitions);
  };
  const collectPredicate = (predicate: PredicateSpec, schema: QuerySchema): void => {
    if (predicate.op === 'and' || predicate.op === 'or') {
      for (const child of predicate.predicates) collectPredicate(child, schema);
      return;
    }
    if (predicate.op === 'not') {
      collectPredicate(predicate.predicate, schema);
      return;
    }
  ...
  for (const node of plan.nodes) {
    if (node.op === 'scan') {
      ...
      if (definition !== undefined)
        for (const field of [...definition.identity, ...definition.rowGrain]) addField(node.entity, field);
      continue;
    }
    if (node.op === 'filter') collectPredicate(node.predicate, inputSchema(node));
    else if (node.op === 'project' || node.op === 'derive')
      for (const item of node.items) collectExpression(item.expression, inputSchema(node));
    else if (node.op === 'time-bucket')
      for (const item of node.items) collectExpression(item.expression, inputSchema(node));
    ...
    const relationship = catalog.relationships.find(
      (candidate) =>
        candidate.id === node.spec.relationship.id && candidate.revision === node.spec.relationship.revision,
    );
...
function checkPlanDependencies(dependencies: PlanDependencies, grant: ReadGrant): Outcome<void> {
  for (const entity of dependencies.entities) {
    if (!allowedEntity(grant, entity))
      return failure(
        'data.denied',
        'The requested query relation is not available in the current authorization scope.',
        ['query'],
      );
    for (const field of dependencies.fields.get(entity) ?? []) {
      if (!allowedField(grant, entity, field))
        return failure(
          'data.denied',
          'The requested query field is not available in the current authorization scope.',
          ['query'],
        );
    }
  }
...
function scanEntityIds(plan: LogicalPlan): readonly string[] {
  return Object.freeze([
    ...new Set(
      plan.nodes
        .filter((node): node is Extract<PlanNode, { readonly op: 'scan' }> => node.op === 'scan')
...
function resultWarnings(result: QueryResult): readonly Diagnostic[] {
  ...
  return Object.freeze(
    warnings.length <= WIRE_LIMITS.diagnostics
      ? warnings
      : [
...
function resultPrecision(result: QueryResult):
  | { readonly kind: 'exact' }
  | {
...
function resultEvidence(result: QueryResult, plan: LogicalPlan, query: QuerySpec, queryDigest: string): ResultEvidence {
  const computed = plan.nodes.some(
    (node) =>
      node.op === 'derive' ||
      node.op === 'time-bucket' ||
      node.op === 'window' ||
  ...
  for (const entityId of plan.scanEntities) {
    ...
    for (const row of input) {
      ...
      if (Date.now() - startedAt > budget.maxMilliseconds)
        return failure('data.budget', 'Execution exceeded the effective time budget while authorizing rows.');
      if (grant.rowPolicy !== undefined) {
        ...
        if (!current())
          return failure(
            'data.stale-plan',
            'The catalog or source changed while row authorization was being resolved.',
          );
...
export function createLocalDataService(options: LocalDataServiceOptions): LocalDataService {
  ...
  const reapPlans = (now: number) => {
    ...
    while (plans.size >= maxPlans) {
      const oldest = [...plans.entries()].sort(
        (left, right) => left[1].accepted.expiresAt - right[1].accepted.expiresAt,
      )[0];
  ...
  const resolvePopulation = async (
    ...
    const request: CohortRequest = {
      ...
      ...(currentCatalog.entities.find((entity) => entity.id === query.entity) === undefined
        ? {}
        : { targetGrain: currentCatalog.entities.find((entity) => entity.id === query.entity)!.rowGrain }),
      scopeDigest: grant.scopeDigest,
      ...(grant.policyRevision === undefined ? {} : { policyRevision: grant.policyRevision }),
      catalogRevision: currentCatalog.revision,
    ...
    try {
      resolved = await resolver.resolve(request, resolverContext);
    } catch {
  ...
  const service: LocalDataService = {
    ...
    async describe(request, context = {}) {
      ...
      if (input.catalogRevision !== null && input.catalogRevision !== currentCatalog.revision)
        return failure('data.stale-catalog', 'The requested catalog revision is no longer current.', [
          'catalogRevision',
        ]);
      const grant = await authorizeWithDeadline(
        ...
        Math.min(input.budget.maxMilliseconds, options.hostBudget?.maxMilliseconds ?? DEFAULT_BUDGET.maxMilliseconds),
      );
      ...
      if (!allowedTarget(input.target, currentCatalog, grant.value))
        return failure(
          'data.denied',
          'The requested catalog target is not available in the current authorization scope.',
          ['target'],
        );
      const effectiveBudget = minBudget(input.budget, options.hostBudget ?? DEFAULT_BUDGET, grant.value.maxBudget);
      if (Date.now() - startedAt > effectiveBudget.maxMilliseconds)
      ...
      if (input.cursor !== undefined) {
        ...
        if (
          cursor?.kind !== 'catalog' ||
          cursor.catalogRevision !== currentCatalog.revision ||
          cursor.scopeDigest !== grant.value.scopeDigest ||
      ...
      if (columnCount > effectiveBudget.maxColumns)
    ...
    async plan(request, context = {}) {
      ...
      if (grant.value.rowPolicy !== undefined && grant.value.policyRevision === undefined)
        return failure(
          'data.authorization',
          'A row policy must declare a policy revision before a plan can be accepted.',
        );
      const checked = validateQuery(
        input.query,
        currentCatalog,
        grant.value,
        context.cohort !== undefined || (options.cohortResolver !== undefined && options.cohortContext !== undefined),
      );
      ...
      if (logical.output.fields.length > boundedBudget.maxColumns)
        return failure('data.budget', 'The query output exceeds the effective column budget.', [
          'budget',
          'maxColumns',
        ]);
      if (currentCatalog !== initialCatalog || snapshot !== initialSnapshot)
      ...
      const queryDigestOutcome = await digestWithDeadline(
        querySerialized.value,
        'query',
        context,
      ...
      if (input.query.page?.cursor !== undefined) {
        ...
        if (
          ...
          cursor.sourceRevision !== snapshot.sourceRevision ||
          cursor.catalogRevision !== currentCatalog.revision ||
          cursor.target !== input.target.outputId
        )
          return failure(
            'data.stale-cursor',
      ...
      const populationDigestOutcome =
        resolvedPopulation.value.membership === undefined
          ? await digestWithDeadline(
              {
                queryDigest,
                planKey: logical.planKey,
                scopeDigest: grant.value.scopeDigest,
                sourceRevision: snapshot.sourceRevision,
                catalogRevision: currentCatalog.revision,
                ...(grant.value.policyRevision === undefined ? {} : { policyRevision: grant.value.policyRevision }),
      ...
      const acceptedBase: AcceptedQuery = {
        ...
        planDigest: '',
        expiresAt,
        functionRegistryDigest: planner.registry.digest,
        ...(grant.value.policyRevision === undefined ? {} : { policyRevision: grant.value.policyRevision }),
        query: input.query,
        effectiveBudget: boundedBudget,
      };
      const planDigestOutcome = await digestWithDeadline(
        { accepted: acceptedBase, planKey: logical.planKey },
        'plan',
        context,
        boundedBudget.maxMilliseconds - (Date.now() - startedAt),
      );
      ...
      const accepted: PlanAcceptance = {
        ...
        kind: 'accepted',
        planDigest,
        supported: supportedOperations(logical, input.query),
      };
      ...
      plans.set(planDigest, {
        ...
        lineage:
          resolvedPopulation.value.membership === undefined
            ? []
            : [{ output: input.target.outputId, inputs: resolvedPopulation.value.lineage }],
      });
    ...
    async *execute(request, context = {}) {
      ...
      if (stored === undefined || Date.now() >= stored.accepted.expiresAt || !sameAccepted(stored.accepted, input)) {
        yield resultError(
          input.requestId,
          stored !== undefined && Date.now() >= stored.accepted.expiresAt ? 'data.expired-plan' : 'data.stale-plan',
          stored !== undefined && Date.now() >= stored.accepted.expiresAt
            ? 'The accepted plan handle has expired.'
            : 'The accepted plan handle is unknown, changed or expired.',
        );
        return;
      }
      if (
        stored.accepted.functionRegistryDigest !== currentCatalog.functionRegistryDigest ||
        stored.logical.pins.functionRegistryDigest !== currentCatalog.functionRegistryDigest
      ) {
        yield resultError(
          input.requestId,
          'data.stale-plan',
          'The accepted plan is pinned to a different function registry digest.',
        );
        return;
      }
      if (
        snapshot.sourceRevision !== stored.accepted.sourceRevision ||
        currentCatalog.revision !== stored.accepted.catalogRevision
      ) {
        yield resultError(
          input.requestId,
          'data.stale-plan',
          'The accepted plan is stale for the current catalog or source revision.',
        );
        return;
      }
      ...
      if (currentCatalog !== initialCatalog || snapshot !== initialSnapshot) {
        yield resultError(
          input.requestId,
          'data.stale-plan',
          'The catalog or source changed while authorization was being resolved.',
        );
        return;
      }
      if (!grant.ok) {
        yield resultError(
          input.requestId,
          grant.diagnostics[0]?.code ?? 'data.denied',
          grant.diagnostics[0]?.message ?? 'The execution was denied.',
        );
        return;
      }
      ...
      if (!checked.ok) {
        yield resultError(
          input.requestId,
          checked.diagnostics[0]?.code ?? 'data.invalid-query',
          checked.diagnostics[0]?.message ?? 'The execution query is no longer authorized.',
        );
        return;
      }
      ...
      if (
        !dependencyCheck.ok ||
        grant.value.scopeDigest !== stored.accepted.scopeDigest ||
        grant.value.policyRevision !== stored.accepted.policyRevision
      ) {
        yield resultError(
          input.requestId,
          dependencyCheck.ok ? 'data.denied' : (dependencyCheck.diagnostics[0]?.code ?? 'data.denied'),
          dependencyCheck.ok
            ? 'The execution authorization scope or policy revision changed.'
            : (dependencyCheck.diagnostics[0]?.message ?? 'The execution query is no longer authorized.'),
        );
        return;
      }
      const executionBudget = minBudget(
        stored.accepted.effectiveBudget,
        options.hostBudget ?? DEFAULT_BUDGET,
      ...
      if (Date.now() - startedAt > executionBudget.maxMilliseconds) {
        yield resultError(
          input.requestId,
          'data.budget',
          'Execution exceeded the effective time budget before reading rows.',
        );
        return;
      }
      ...
      if (!resolvedPopulation.ok) {
        ...
        yield resultError(
          input.requestId,
          first?.code ?? 'data.unsupported',
          first?.message ?? 'The fixed population could not be resolved.',
        );
        return;
      }
      if (
        resolvedPopulation.value.membership !== undefined &&
        resolvedPopulation.value.membership.tupleDigest !== stored.accepted.populationDigest
      ) {
      ...
      if (!plannerOutcome.ok) {
        yield resultError(
          input.requestId,
          plannerOutcome.diagnostics[0]?.code ?? 'data.unsupported',
          plannerOutcome.diagnostics[0]?.message ?? 'The query planner is unavailable.',
        );
        return;
      }
      const planner = plannerOutcome.value;
      const resultIdOutcome = await digestWithDeadline(
        { planDigest: stored.accepted.planDigest, requestId: input.requestId },
        'result',
        context,
        executionBudget.maxMilliseconds - (Date.now() - startedAt),
      );
      if (!resultIdOutcome.ok) {
        yield resultError(
          input.requestId,
          resultIdOutcome.diagnostics[0]?.code ?? 'data.crypto',
          resultIdOutcome.diagnostics[0]?.message ?? 'The result identity could not be computed.',
        );
        return;
      }
      if (currentCatalog !== initialCatalog || snapshot !== initialSnapshot) {
        yield resultError(
          input.requestId,
          'data.stale-plan',
          'The catalog or source changed while the result identity was being computed.',
        );
        return;
      }
      const ref = resultReference(stored.accepted, resultIdOutcome.value);
      const sourceOutcome = await authorizedSource(
        stored,
        snapshot,
        grant.value,
        input.query,
        ...
        startedAt,
        executionBudget,
        () => currentCatalog === initialCatalog && snapshot === initialSnapshot,
      );
      if (!sourceOutcome.ok) {
        yield resultError(
          input.requestId,
          sourceOutcome.diagnostics[0]?.code ?? 'data.denied',
          sourceOutcome.diagnostics[0]?.message ?? 'The authorized source could not be prepared.',
        );
        return;
      }
      ...
      if (!evalResult.ok) {
        ...
        yield resultError(
          input.requestId,
          first?.code === 'query.budget' ? 'data.budget' : 'data.unsupported',
          first?.message ?? 'The query could not be evaluated.',
        );
        return;
      }
      const result = evalResult.value;
      const requestedOffset =
        input.query.page?.cursor === undefined ? 0 : (decodeCursor(input.query.page.cursor)?.offset ?? 0);
      const pageLimit = input.query.page?.size ?? Number.MAX_SAFE_INTEGER;
      const candidates = result.rows.slice(requestedOffset);
      let outputRows: readonly DataRecord[] = candidates.slice(
        0,
        Math.min(pageLimit, executionBudget.maxRows),
      ) as readonly DataRecord[];
      let partialReason: string | undefined;
      if (requestedOffset > 0 || requestedOffset + outputRows.length < result.rows.length) {
        partialReason =
          outputRows.length >= executionBudget.maxRows && executionBudget.maxRows <= pageLimit ? 'row budget' : 'page';
      }
      ...
      const sourceRevisions: Record<string, string> = Object.fromEntries(
        stored.scanEntities.map((entity) => [entity, stored.accepted.sourceRevision]),
      );
      const descriptorBase = {
        version: '1' as const,
        ref,
        taskId: input.target.taskId ?? input.requestId,
        fields: refFields,
      ...
      const population = result.complete
        ? { kind: 'exact' as const, value: result.rows.length, populationDigest: stored.accepted.populationDigest }
      ...
      const makeDescriptor = (): DataResultEvent => ({
        kind: 'descriptor',
        descriptor: {
          ...descriptorBase,
          counts: { loaded: outputRows.length, population },
          coverage: makeCoverage(),
        },
      });
      const nextOffset = () => requestedOffset + outputRows.length;
      const makeComplete = (): DataResultEvent => ({
        kind: 'complete',
        result: ref,
        finalCoverage: makeCoverage(),
        ...(partialReason === undefined || nextOffset() >= result.rows.length
          ? {}
          : { cursor: pageCursor(stored.accepted, nextOffset()) }),
      });
      const includeProgress = executionBudget.maxMessages >= 4;
      let descriptor = makeDescriptor();
      let batch: DataResultEvent | undefined =
        outputRows.length > 0 ? { kind: 'batch', result: ref, sequence: 0, rows: outputRows } : undefined;
      ...
      const fits = (): boolean => {
        ...
        return (
          eventBytes(descriptor) +
            (batch === undefined ? 0 : eventBytes(batch)) +
            (progress === undefined ? 0 : eventBytes(progress)) +
            eventBytes(complete) <=
          executionBudget.maxBytes
        );
      };
      ...
      if (fitFailure === 'budget') {
        yield resultError(
          input.requestId,
          'data.budget',
          'Execution exceeded the effective time budget while bounding the response.',
        );
        return;
      }
      if (!responseFits) {
        yield resultError(
          input.requestId,
          'data.budget',
          'The bounded result cannot fit the effective response byte budget.',
        );
        return;
      }
      const yieldFailure = (): DataResultEvent | undefined => {
        if (context.signal?.aborted)
          return resultError(input.requestId, 'data.aborted', 'The result execution was cancelled.');
        if (Date.now() - startedAt > executionBudget.maxMilliseconds)
          return resultError(
            input.requestId,
            'data.budget',
    ...
    replaceSnapshot(next) {
      ...
      try {
        normalized = normalizeSnapshot(next, sourceLimits);
      } catch {
        return failure('data.source-shape', 'The replacement source snapshot is not a bounded canonical source.');
      }
      if (
        normalized.sourceRevision === snapshot.sourceRevision &&
        canonical(normalized.records) !== canonical(snapshot.records)
      )
        return failure(
          'data.source-revision-conflict',
          'Source records changed without a new immutable source revision.',
          ['sourceRevision'],
        );
      snapshot = normalized;
    ...
    registerMeaningBundle(bundle) {
      ...
      if (existingBundle !== undefined) {
        ...
        const registration: MeaningRegistration = freezeDeep({
          catalogRevision: currentCatalog.revision,
          meanings: replayed.value.meanings,
          receipts,
          idempotent: true,
        });
      ...
      for (const meaning of validated.value.meanings) {
        const prior = currentCatalog.meanings.find(
          (candidate) => candidate.id === meaning.id && candidate.revision === meaning.revision,
        );
        if (prior !== undefined) {
      ...
      if (added.length > 0) {
        ...
        const nextCatalogInput = {
          ...currentCatalog,
          revision: nextRevision,
          meanings: [...currentCatalog.meanings, ...added],
        };
...
function allowedTarget(target: CatalogTarget, catalog: Catalog, grant: ReadGrant): boolean {
  ...
  return (
    entity !== undefined &&
    allowedEntity(grant, target.entity) &&
    allowedIdentity(grant, entity) &&
    entity.rowGrain.every((field) => allowedField(grant, entity.id, field))
  );
}
