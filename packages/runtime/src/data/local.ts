...
function validateQuery(query: QuerySpec, catalog: Catalog, grant: ReadGrant): Outcome<void> {
  ...
  if (query.relations.length > 0) return unsupported({kind: 'relation', id: 'relations', reason: 'Joins and relation expansion require an explicit host plan.', alternatives: ['Use an analytical host capability.']}, ['query', 'relations']);
