...
const failure = <T>(code: string, message: string, path: readonly (string | number)[] = []): ActionOutcome<T> => ({
  ok: false,
...
function cloneRef(value: VersionRef): VersionRef {
...
function request(value: unknown): ActionOutcome<ParsedRequest> {
  ...
  if (inspected.value === null || typeof inspected.value !== 'object' || Array.isArray(inspected.value))
    return failure('action.invalid', 'An action request must be a bounded object.');
  const record = inspected.value as Record<string, unknown>;
  const allowed = ['requestId', 'action', 'input', 'entity', 'idempotencyKey'];
  if (
    Object.keys(record).some((key) => !allowed.includes(key)) ||
    !validId(record.requestId) ||
    !validRef(record.action)
  )
...
function grantsInclude(context: TrustedActionContext, grant: ActionGrant): boolean {
  return context.grants.includes(grant);
}

function context(value: unknown): ActionOutcome<TrustedActionContext> {
  if (value === null || typeof value !== 'object' || Array.isArray(value))
    return failure('action.denied', 'The host action context is unavailable.');
  const record = value as Record<string, unknown>;
  const allowed = [
    'principalKey',
    'actorKey',
  ...
  if (
    ...
    record.grants.length > WIRE_LIMITS.array ||
  ...
  if (record.entityRevisions !== undefined) {
    if (
      ...
      return failure('action.denied', 'The host entity revision map is malformed.');
    const map = record.entityRevisions as Record<string, unknown>;
    if (
      Object.keys(map).length > WIRE_LIMITS.properties ||
      Object.keys(map).some((key) => !validKey(key) || !validId(map[key]))
    )
      return failure('action.denied', 'The host entity revision map is malformed.');
    entityRevisions = frozen(
      Object.fromEntries(Object.entries(map).map(([key, revision]) => [key, revision as string])),
    );
  }
  return {
    ok: true,
    value: frozen({
      ...
      confirmationEpoch: record.confirmationEpoch,
      grants,
      ...(entityRevisions === undefined ? {} : { entityRevisions }),
    }),
  };
}
...
class ActionPortImpl implements ActionPort {
  ...
  private outcome<T>(code: string, message: string, path: readonly (string | number)[] = []): ActionOutcome<T> {
    return failure(code, message, path);
  }
  private lifecycle<T>(): ActionOutcome<T> {
  ...
  async preview(raw: unknown, options: { readonly signal?: AbortSignal } = {}): Promise<ActionOutcome<ActionPreview>> {
    ...
    try {
      ...
      if (!grantsInclude(hostContext.value, 'action.propose'))
      ...
      const identity =
        parsed.value.idempotencyKey === undefined
          ? undefined
          : this.idempotencyIdentity(registration, input.value, parsed.value.entity, parsed.value.idempotencyKey);
      ...
      const preview = {
        ...
        ...(parsed.value.entity === undefined ? {} : { entity: parsed.value.entity }),
        ...(parsed.value.idempotencyKey === undefined ? {} : { idempotencyKey: parsed.value.idempotencyKey }),
      ...
      const record = {
        ...
        requestId: parsed.value.requestId,
        registration,
        preview: null as unknown as ActionPreview,
        input: input.value,
      ...
      this.addHistoryAt(
        { state: 'preview', action: cloneRef(registration.descriptor.ref), previewId: id },
        at.value,
        partition(hostContext.value),
      );
  ...
  async confirm(
    ...
    if (!grantsInclude(current.value, 'action.propose') || !grantsInclude(current.value, 'action.execute'))
      return retry(
        this.outcome('action.denied', 'The host did not grant both proposal and execution for confirmation.'),
      );
    if (
      !sameContext(matched.context, current.value) ||
      resolveRegisteredAction(this.registry, matched.registration.descriptor.ref) !== matched.registration
    ) {
      this.consumePreview(matched);
      this.tryHistory(
        {
    ...
    if (matched.registration.descriptor.confirmation === 'required') {
      if (this.host.issueConfirmation === undefined)
        return retry(this.outcome('action.confirmation', 'This action requires a trusted confirmation callback.'));
      const issued = await this.callHost(
        (signal) => this.host.issueConfirmation!({ preview: matched.preview, context: current.value, signal }),
        options.signal,
      );
      if (issued.state !== 'completed') {
        if (issued.state === 'lifecycle') return retry(this.lifecycle());
        if (issued.state === 'cancelled')
          return retry(this.outcome('action.cancelled', 'The action confirmation was cancelled.'));
      ...
      if (outcome === undefined || !outcome.ok)
        return retry(this.outcome('action.confirmation', 'The trusted host did not confirm this action.'));
      if (!this.live()) return retry(this.lifecycle());
    }
    ...
    if (this.receipts.size >= this.maxPending)
      return retry(this.outcome('action.budget', 'The pending action receipt budget is full.'));
    const historyAt = this.clock();
    if (!historyAt.ok) return retry(this.outcome('action.callback', 'The action clock callback failed.'));
    if (!this.live()) return retry(this.lifecycle());
    if (options.signal?.aborted)
      return retry(this.outcome('action.cancelled', 'The action confirmation was cancelled before receipt issuance.'));
    const input = matched.input;
    if (input === undefined)
      return retry(this.outcome('action.replay', 'The action preview input is no longer available.'));
    this.consumePreview(matched);
  ...
  async execute(
    ...
    if (
      !grantsInclude(current.value, 'action.execute') ||
      !sameContext(matched.context, current.value) ||
      resolveRegisteredAction(this.registry, matched.registration.descriptor.ref) !== matched.registration ||
      !this.entityCurrent(matched, current.value)
    ) {
      this.consumeReceipt(matched);
      this.tryHistory(
        {
    ...
    if (key !== undefined) {
      ...
      const metadataBytes = this.ledgerMetadataBytes(
        ledgerKey!,
        key,
        matched.registration.descriptor.ref,
        matched.id,
        at.value,
        partition(current.value),
      );
      ...
      this.idempotency.set(ledgerKey!, {
        key,
        identity,
        partition: partition(current.value),
        identityBytes,
        metadataBytes,
        action: cloneRef(matched.registration.descriptor.ref),
        receiptId: matched.id,
    ...
    const input = matched.input;
    const dispatchCall = await this.callHost(
      (signal) =>
        matched.registration.dispatch({
          descriptor: matched.registration.descriptor,
          input: input!,
          ...(matched.entity === undefined ? {} : { entity: matched.entity }),
          ...(matched.idempotencyKey === undefined ? {} : { idempotencyKey: matched.idempotencyKey }),
    ...
    if (dispatchCall.state !== 'completed') {
      ...
      const reason =
        dispatchCall.state === 'timeout'
          ? 'The action callback exceeded its bounded time budget.'
          : dispatchCall.state === 'cancelled'
            ? 'The action callback was cancelled after dispatch.'
            : 'The action port was revoked or disposed during dispatch.';
      return this.ambiguousResult(matched, ledgerKey, reason);
    }
    const dispatch = normalizeDispatch(dispatchCall.value);
    if (options.signal?.aborted)
      return this.ambiguousResult(matched, ledgerKey, 'The action execution was cancelled after dispatch.');
    ...
    try {
      parsedOutput = matched.registration.outputSchema.parse(dispatch.output);
    } catch {
    ...
    if (
      ...
      (ledgerKey !== undefined && this.ledgerBytes + outputBytes > this.maxLedgerBytes)
    ) {
    ...
    if (!completionAt.ok)
      return this.ambiguousResult(matched, ledgerKey, 'The action clock callback failed after dispatch.', false);
    if (!this.live())
      return this.ambiguousResult(
        matched,
        ledgerKey,
    ...
    this.consumeReceipt(matched);
    this.addHistoryAt(
      { state: 'executed', action: cloneRef(matched.registration.descriptor.ref), receiptId: matched.id },
      completionAt.value,
      partition(current.value),
    );
    return {
