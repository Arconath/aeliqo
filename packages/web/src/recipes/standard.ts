import type {Diagnostic, Outcome, PresentationPlan, PresentationStateMappingManifest, PresentationValues, ReadonlyJsonValue, Result, VersionRef} from '@aeliqo/core';
...
const aliases: Readonly<Record<string, ViewChoice>> = Object.freeze({
  table: {ref: AELIQO_PRESENTATION_REFS.table, schema: AELIQO_CONFIG_SCHEMAS.table, role: 'table',
    operations: [AELIQO_OPERATION_REFS.read, AELIQO_OPERATION_REFS.compare, AELIQO_OPERATION_REFS.analyze]},
  cards: {ref: AELIQO_DATA_REFS.cardCollection, schema: AELIQO_DATA_CONFIG_SCHEMAS.cardCollection, role: 'cardCollection',
    operations: [AELIQO_OPERATION_REFS.read]},
  list: {ref: AELIQO_DATA_REFS.recordList, schema: AELIQO_DATA_CONFIG_SCHEMAS.recordList, role: 'recordList',
    operations: [AELIQO_OPERATION_REFS.read]},
  detail: {ref: AELIQO_DATA_REFS.detail, schema: AELIQO_DATA_CONFIG_SCHEMAS.detail, role: 'detail',
    operations: [AELIQO_OPERATION_REFS.read]},
  trend: {ref: AELIQO_PRESENTATION_REFS.trend, schema: AELIQO_CONFIG_SCHEMAS.trend, role: 'trend',
    operations: [AELIQO_OPERATION_REFS.read, AELIQO_OPERATION_REFS.compare, AELIQO_OPERATION_REFS.analyze]},
});
...
function sameRef(left: VersionRef, right: VersionRef): boolean { return left.id === right.id && left.revision === right.revision; }
...
function custom(context: RecipeContext, preferred: string): {readonly ref: VersionRef; readonly schema: VersionRef; readonly role: string; readonly values: PresentationValues} | undefined {
  ...
  if (definition === undefined || operation === undefined || !supportsOperation(definition.manifest.operations, operation)) return undefined;
  ...
  return {ref: definition.ref, schema: definition.manifest.configSchema, role: definition.manifest.roles[0]!, values: suggested?.value ?? {}};
}
...
function dataColumns(context: RecipeContext, typed: boolean): readonly Readonly<Record<string, ReadonlyJsonValue>>[] {
  ...
  return context.result.fields.filter((field) => requested.has(field.id)).map((field) => ({
    key: field.id,
    label: field.label,
...
function valuesFor(view: ViewChoice, context: RecipeContext): PresentationValues | undefined {
  ...
  if (sameRef(view.ref, aliases.cards!.ref)) {
    const headingKey = context.result.fields.find((field) => context.task.needs[0]?.fields.includes(field.id) === true && field.role === 'attribute' && field.type.value === 'text')?.id;
...
function selectedView(context: RecipeContext): {readonly ref: VersionRef; readonly schema: VersionRef; readonly role: string; readonly values: PresentationValues} {
  ...
  if (preferred !== undefined) {
    ...
    if (known !== undefined && values !== undefined && operation !== undefined && supportsOperation(known.operations, operation)) return {...known, values};
  ...
  if (narrow) {
    const headingKey = context.result.fields.find((field) => field.role === 'attribute' && field.type.value === 'text')?.id;
    return {...aliases.cards!, values: {...valuesFor(aliases.cards!, context)!, ...(headingKey === undefined ? {} : {headingKey})}};
  }
  return {...aliases.table!, values: valuesFor(aliases.table!, context)!};
}
function build(context: RecipeContext): Outcome<PresentationPlan> {
  if (context.task.kind !== 'data' || context.result === undefined) return failure('web.recipe.unsupported', 'Standard data recipes require a materialized data Task.');
  ...
  if (context.incumbent !== undefined) {
    for (const node of context.incumbent.nodes) {
      const mapping = sameRef(node.representation, view.ref) && node.role === view.role
        ? {id: 'aeliqo.state.identity', revision: '1'}
        : stateMapping(node.representation, view.ref)?.ref;
      if (mapping === undefined) return failure('web.recipe.transition', 'The current view cannot transfer its interaction state to the requested view.');
      stateTransfer.push({fromNode: node.id, toNode: 'primary', mapping});
    }
  }
  const plan: PresentationPlan = {
    id: `presentation-${context.task.id}`.slice(0, 160), revision: context.task.revision, rootId: 'primary', preconditions: context.current,
    nodes: [{id: 'primary', role: view.role, representation: view.ref, result: context.result.ref,
      config: {schema: view.schema, values: view.values}, children: []}], links: [],
...
function buildForm(context: RecipeContext): Outcome<PresentationPlan> {
  ...
  if (form === undefined || form.action === undefined || form.action.action.id !== context.task.action.id || form.action.action.revision !== context.task.action.revision)
  ...
  const nodes: PresentationPlan['nodes'] = [
    {id: 'form', role: 'structure', representation: AELIQO_INPUT_REFS.form,
      config: {schema: {id: 'input.form.config', revision: '1'}, values: {bindingRef: form.id, bindingRevision: context.inputBindings.revision}},
      children: fields.map((field) => field.id)},
    ...fields.map((field) => ({id: field.id, role: field.ref.id === AELIQO_INPUT_REFS.fieldGroup.id ? 'structure' : 'input', representation: field.ref,
      config: {schema: {id: `${field.ref.id}.config`, revision: '1'}, values: {bindingRef: field.id, bindingRevision: context.inputBindings!.revision}}, children: []})),
  ];
  return {ok: true, value: {
    id: `presentation-${context.task.id}`.slice(0, 160), revision: context.task.revision, rootId: 'form', preconditions: context.current,
    nodes, links: [], coverage: [],
    stateTransfer: context.incumbent === undefined ? [] : context.incumbent.nodes.map((node) => ({fromNode: node.id, toNode: node.id, mapping: {id: 'aeliqo.state.identity', revision: '1'}})),
    diagnostics: [],
  }};
}
...
export const STANDARD_STATE_MAPPINGS: readonly PresentationStateMappingManifest[] = Object.freeze([
  {ref: {id: 'aeliqo.web.table-to-cards', revision: '1'}, from: AELIQO_PRESENTATION_REFS.table, to: AELIQO_DATA_REFS.cardCollection,
    fromRole: 'table', toRole: 'cardCollection', kind: 'transfer'},
  {ref: {id: 'aeliqo.web.cards-to-table', revision: '1'}, from: AELIQO_DATA_REFS.cardCollection, to: AELIQO_PRESENTATION_REFS.table,
    fromRole: 'cardCollection', toRole: 'table', kind: 'transfer'},
]);
