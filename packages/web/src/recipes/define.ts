...
export function defineRecipe(input: RecipeInput): RecipeDefinition {
  if (!validNamespacedId(input.ref.id) || input.ref.revision.length === 0 || input.intents.length === 0 || new Set(input.intents).size !== input.intents.length)
...
export function defineView(input: ViewInput): AeliqoViewDefinition {
  if (!validNamespacedId(input.ref.id) || input.ref.id !== input.manifest.ref.id || input.ref.revision !== input.manifest.ref.revision)
