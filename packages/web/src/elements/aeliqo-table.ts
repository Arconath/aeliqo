...
export class AeliqoTableElement extends LitElement {
                ...
                <table part="table" data-virtualized=${this.virtualized ? 'true' : 'false'}>
                  ...
                  <tbody>
                    ${
                      visible.length === 0
                        ? html`<tr>
                            <td colspan=${Math.max(this.columns.length + (selectable ? 1 : 0), 1)}>
                              ${status === 'empty' ? this.emptyLabel : ''}
                            </td>
                          </tr>`
                        : visible.map((entry) => this.renderTableRow(entry.row, entry.index, selectable))
                    }
                  </tbody>
                </table>
              `
  ...
  private renderScope(rowCount: number | undefined, renderedCount: number) {
    ...
    if (this.virtualized && renderedCount !== loaded) {
      const totalText = description
        ? `; ${description}`
        : total !== undefined && total !== loaded
          ? `; ${total.toLocaleString()} matching ${this.entity}s`
          : '';
      return html`<p part="scope">
        Showing ${renderedCount.toLocaleString()} rendered of ${loaded.toLocaleString()} loaded
        ${this.entity}s${totalText}.
      </p>`;
    }
  ...
  private resolveGridFocus(
    ...
    const keyed =
      this.gridFocus?.rowKey === undefined
        ? undefined
  ...
  private readonly handleGridCellKeyDown = (event: KeyboardEvent): void => {
    ...
    if (event.key === 'ArrowRight') {
      if (nextColumn < columns - 1) nextColumn++;
        ...
        nextRow = rowPosition + 1 < rows.length ? rows[rowPosition + 1]! : Math.min(rowCount - 1, lastRow + 1);
        nextColumn = 0;
      }
    } else if (event.key === 'ArrowLeft') {
      if (nextColumn > 0) nextColumn--;
      else {
        nextRow = rowPosition > 0 ? rows[rowPosition - 1]! : Math.max(0, firstRow - 1);
    ...
    css`
      ...
      :is(button, input):focus-visible {
        outline: var(--aeliqo-focus-width, 0.1875rem) solid
          var(--aeliqo-table-focus, var(--aeliqo-color-focus, #0b63ce));
