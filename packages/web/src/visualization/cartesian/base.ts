...
const message = (diagnostics: readonly Diagnostic[]): string =>
  diagnostics[0]?.message ?? 'The visualization is unavailable.';
...
export abstract class AeliqoCartesianElement extends AeliqoFoundationElement {
  ...
  protected override willUpdate(changed: Map<string, unknown>): void {
    ...
    if (bound.value.spec.view !== this.expectedView || !('plot' in bound.value.spec)) {
      this.state = {
        kind: 'error',
        diagnostics: fail('view', `This surface only accepts the ${this.expectedView} visualization family.`)
          .diagnostics,
      };
      this.clearSelection();
      return;
  ...
  protected override render() {
    if (this.state.kind === 'empty')
      return html`<p role="status">No ${this.expectedView} visualization is available.</p>`;
  ...
  private renderNode(node: CompiledPlotNode, label: string, graphic = true, data = true): unknown {
    ...
    if (node.kind === 'concat')
      return html`<section part=${`concat-${node.direction}`} aria-label=${label}>
        ${node.children.map((child, index) => this.renderNode(child, `${label}, panel ${index + 1}`, graphic, data))}
      </section>`;
    return html`<section aria-label=${label}>
      ${data ? html`<h3>${label}</h3>` : nothing}
      <div part="layer">
        ${node.children.map((child, index) => html`<div>${this.renderNode(child, `${label}, layer ${index + 1}`, true, false)}</div>`)}
      </div>
      ${data ? node.children.map((child, index) => this.renderNode(child, `${label}, layer ${index + 1}`, false, true)) : nothing}
    </section>`;
  }
  ...
  private renderGeometry(
    ...
    const displayed =
      displayedSet === undefined ? geometry.rows : geometry.rows.filter((row) => displayedSet.has(row.identity));
    ...
    const abbreviatedTicks =
      axes !== undefined && [...axes.x.ticks, ...axes.y.ticks].some((tick) => tick.label.length > 12);
    ...
    const histogramMeasure =
      this.expectedView === 'histogram' && this.state.kind === 'ready' && this.state.bound.spec.view === 'histogram'
        ? this.state.bound.spec.bins.measure
        : undefined;
    const tableLabel =
      histogramMeasure === undefined ? valueLabel : `${valueLabel}; executor-produced ${histogramMeasure} bins`;
    const axisLeft = geometry.axisLeft ?? 64;
    // Responsive SVG text keeps rem-based zoom behavior, so reserve enough
    // user-space for the taller glyph boxes produced by some browser engines.
    ...
    return html`<figure part="figure">
      ${
        data
          ? html`<figcaption>${label}</figcaption>
              <p part="scope">
                ${scope} ${geometry.rows.length} loaded
                rows.${displayed.length !== geometry.rows.length ? ` ${displayed.length} rows in this display partition.` : nothing}
              </p>`
          : nothing
      }
      ...
      ${
        geometry.result.warnings.length > 0 && data
          ? html`<ul part="warnings" aria-label="Result warnings">
              ${geometry.result.warnings.map((warning) => html`<li>${warning.message}</li>`)}
            </ul>`
          : nothing
      }
      ...
      ${
        graphic && axes && geometry.state === 'plot'
          ? html`<div
              part="viewport"
              role="region"
              tabindex="0"
              aria-label=${`${label} ${this.expectedView} chart. Scroll to view the full graphic.`}
              style=${`inline-size: ${this.width}px`}
            >
              <svg
              ...
              >
                ...
                <path
                  d=${`M${axisLeft},24V${geometry.height - 48}H${geometry.width - 24}`}
                  fill="none"
                  stroke="currentColor"
                ></path>
                ${axes.x.ticks.map((tick, index) => svg`<text class="axis-x-tick" x=${tick.position} y=${xTickStart + (index % 2) * xTickRowGap} text-anchor=${this.xTickAnchor(index, axes.x.ticks.length)}>${this.tickText(tick.label)}</text>`)}
                ${axes.y.ticks.map((tick) => svg`<text class="axis-y-tick" x=${axisLeft - 4} y=${tick.position} text-anchor="end">${this.tickText(tick.label)}</text>`)}
                <text class="axis-title" x=${geometry.width / 2} y=${xTitleBaseline} text-anchor="middle">
                  ${axes.xLabel}
                </text>
                <text class="axis-title" x=${axisLeft + 8} y="32">${axes.yLabel}</text>
              </svg>
            </div>`
          : nothing
      }
      ${
        data
          ? html`${this.renderLegend(geometry)}${this.renderColorKey(geometry)}
              <div part="data">
                <table>
                  <caption>
                    ${label}: ${tableLabel}
                  ...
                  <thead>
                    <tr>
                      ...
                      ${geometry.result.fields.map((field) => html`<th scope="col">${field.label}${field.type.unit ? ` (${field.type.unit.symbol})` : nothing}</th>`)}
                    </tr>
                  </thead>
                  <tbody>
                    ${repeat(
                        ...
                        html`<tr>
                          <td data-label="Select">
                            <button
                              ...
                              aria-pressed=${this.isSelected(row.identity, geometry.result.ref) ? 'true' : 'false'}
                              aria-label=${`Select ${this.identityLabel(row, geometry.result)}`}
                              @click=${() => this.select(row.identity, geometry.result.ref)}
                            >
                              Select
                            </button>
                          </td>
                          ${geometry.result.fields.map((field) => html`<td data-label=${field.label}>${exactLabel(row.values[field.id]!)}</td>`)}
                        </tr>`,
                    )}
                  </tbody>
                </table>
              ...
              ${
                displayed.length > 25
                  ? html`<nav part="pagination" aria-label="${label} data pages">
                      <button
                        type="button"
                        ?disabled=${page === 0}
                        @click=${() => {
                      ...
                      >
                        Previous</button
                      ><span
                        >Rows ${page * 25 + 1}–${Math.min((page + 1) * 25, displayed.length)} of
  ...
  private renderLegend(geometry: PlotGeometry): unknown {
    ...
    return html`<ul part="legend" aria-label="Series">
      ${geometry.legend.map((entry) => html`<li><span part="legend-marker" aria-hidden="true" style=${`background:${seriesColor(geometry, entry.id)}`}></span>${entry.label || entry.id}</li>`)}
    </ul>`;
  }
    ...
    if (this.expectedView === 'histogram') {
      const coverage =
        result.coverage.kind === 'complete'
          ? 'Complete bin delivery'
          : result.coverage.kind === 'partial'
            ? `Partial bin delivery: ${result.coverage.reason}`
            : result.coverage.kind === 'sample'
              ? `Sampled bin delivery: ${result.coverage.method}`
              : `Bin scope unknown: ${result.coverage.reason}`;
      return `${coverage}. Source observation coverage and missing/outside-bin counts are unknown.`;
    }
    return result.coverage.kind === 'complete'
      ? 'Complete result.'
      : result.coverage.kind === 'partial'
        ? `Partial result: ${result.coverage.reason}.`
        : result.coverage.kind === 'sample'
          ? `Sample: ${result.coverage.method}.`
          : `Scope unknown: ${result.coverage.reason}.`;
  }

  private tickText(text: string): string {
  ...
  private identityLabel(row: { readonly values: Readonly<Record<string, Scalar>> }, result: Result): string {
  ...
  private isSelected(identity: string, result: ResultRef): boolean {
    return (
      ...
      (this.selectedResult === undefined ? this.singleResult : resultKey(result) === resultKey(this.selectedResult))
    );
  }
