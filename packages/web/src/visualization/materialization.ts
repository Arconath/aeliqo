...
export function materializeVisualizationRows(
  ...
  const inspectedRef = parseWireValue(resultRef);
  if (!inspectedRef.ok) return inspectedRef;
  ...
  if (
    ...
    ['id', 'revision', 'outputId', 'queryDigest', 'scopeDigest'].some(
      (name) =>
        !Object.hasOwn(ref, name) ||
        typeof (ref as Record<string, unknown>)[name] !== 'string' ||
        !(ref as Record<string, unknown>)[name],
    )
  )
  ...
  for (const item of inspected.value) {
    if (
      ...
      typeof item !== 'object' ||
      Array.isArray(item) ||
      Object.keys(item).some((name) => name !== 'result' && name !== 'rows') ||
      item.result === null ||
      typeof item.result !== 'object' ||
      Array.isArray(item.result)
    )
      return fail('dataset', 'A materialization requires an exact result reference and rows.');
    ...
    const refId = key(ref);
    if (seenRefs.has(refId)) return fail('datasets', 'An exact materialization reference is repeated.');
    seenRefs.add(refId);
  ...
  for (const row of rows) {
    ...
    for (const id of result.identity) {
      ...
      if (field === undefined || values[id] === null)
        return fail('identity', 'A stable identity value is unavailable.');
      const checked = scalarIdentity(values[id], field.type);
      if (!checked.ok) return checked;
      parts.push(checked.value);
    }
    const identity = JSON.stringify(parts);
    if (identities.has(identity)) return fail('identity', 'Row identities must be unique.');
