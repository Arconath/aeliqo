...
export abstract class AeliqoHierarchyElementBase extends AeliqoFoundationElement {
  ...
  private renderHierarchyTable(geometry: HierarchyGeometry) {
    ...
    return html`<div part="data" tabindex="0" role="region" aria-label=${`${this.label}: scrollable data`}><table><caption>${label}</caption><thead><tr><th scope="col">Select</th>${result.fields.map(field => html`<th scope="col">${field.label}${field.type.unit ? ` (${field.type.unit.symbol})` : ''}</th>`)}</tr></thead><tbody>${repeat(geometry.rows.slice(this.page*25,this.page*25+25),row=>row.identity,row => html`<tr aria-selected=${this.selectedIdentity === row.identity ? 'true' : 'false'}><td><button type="button" aria-pressed=${this.selectedIdentity === row.identity ? 'true' : 'false'} aria-label=${`Select row ${row.identity}`} @click=${() => this.select(row.identity, result.ref)} @keydown=${(event: KeyboardEvent) => this.keySelect(event, row.identity, result.ref)}>Select</button></td>${result.fields.map(field => html`<td>${scalarLabel(row.values[field.id]!)}</td>`)}</tr>`)}</tbody></table></div>${this.pagination(geometry.rows.length)}`;
  }
  ...
  private renderRelationshipTable(geometry: RelationshipGeometry) {
    ...
    return html`<div part="data" tabindex="0" role="region" aria-label=${`${this.label}: scrollable data`}><table><caption>${result.precision.kind === 'exact' ? 'Exact' : 'Approximate'} loaded relationship data (${geometry.cardinality}; ${source} → ${target})</caption><thead><tr><th scope="col">Select</th>${result.fields.map(field => html`<th scope="col">${field.label}${field.type.unit ? ` (${field.type.unit.symbol})` : ''}</th>`)}</tr></thead><tbody>${repeat(geometry.rows.slice(this.page*25,this.page*25+25),row=>row.identity,row => html`<tr aria-selected=${this.selectedIdentity === row.identity ? 'true' : 'false'}><td><button type="button" aria-pressed=${this.selectedIdentity === row.identity ? 'true' : 'false'} aria-label=${`Select relationship ${row.identity}`} @click=${() => this.select(row.identity, result.ref)} @keydown=${(event: KeyboardEvent) => this.keySelect(event, row.identity, result.ref)}>Select</button></td>${result.fields.map(field => html`<td>${scalarLabel(row.values[field.id]!)}</td>`)}</tr>`)}</tbody></table></div>${this.pagination(geometry.rows.length)}`;
  }
}
