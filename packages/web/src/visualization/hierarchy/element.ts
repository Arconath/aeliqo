...
const scalarLabel = (value: Scalar): string =>
  value === null ? 'Missing' : typeof value === 'object' ? value.decimal : String(value);
...
export abstract class AeliqoHierarchyElementBase extends AeliqoFoundationElement {
  ...
  protected override willUpdate(changed: Map<string, unknown>): void {
    ...
    this.focused = active instanceof (globalThis.Element ?? Object) ? (active as HTMLElement | SVGElement) : undefined;
    if (['visualization', 'context', 'datasets', 'width', 'height', 'maxMarks'].some((name) => changed.has(name))) {
      this.geometry =
        this.visualization?.view === this.expectedView
          ? compileHierarchyVisualization(this.inputs())
          : {
              ok: false,
              diagnostics: [
                {
      ...
      this.scope = scope;
      this.page = this.geometry.ok
        ? Math.min(this.page, Math.max(0, Math.ceil(this.geometry.value.rows.length / 25) - 1))
        : 0;
      if (
        this.geometry.ok &&
        this.selectedIdentity !== '' &&
        !this.geometry.value.rows.some((row) => row.identity === this.selectedIdentity)
      )
        this.selectedIdentity = '';
    }
  }
  ...
  private renderHierarchyGraphic(geometry: HierarchyGeometry) {
    ...
    return html`<div part="viewport" tabindex="0" role="region" aria-label=${`${this.label}: scrollable graphic`}>
      <svg
        ...
        role="group"
        aria-label=${`${this.label}. ${geometry.kind === 'tree' ? 'Hierarchy' : 'Treemap'}. Loaded values are available in the data table.`}
      >
        ${
          geometry.kind === 'tree'
                ...
                node.parentIdentity === undefined
                  ...
                  : (() => {
                      ...
                      return parent === undefined
                        ? ''
                        : svg`<line x1=${parent.x + parent.width / 2} y1=${parent.y + parent.height} x2=${node.x + node.width / 2} y2=${node.y} stroke="currentColor" stroke-opacity=".45"></line>`;
                    })(),
              )
            : ''
        }
