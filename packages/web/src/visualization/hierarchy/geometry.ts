...
function optionsFor(
  ...
  const maxDepth =
    Number.isSafeInteger(options.maxDepth) && options.maxDepth !== undefined && options.maxDepth > 0
      ? Math.min(MAX_DEPTH, options.maxDepth)
      : MAX_DEPTH;
...
function bindForView(
  ...
  const result = bound.results.find(
    (candidate) =>
      refKey(candidate.ref) ===
      refKey(
        inputs.visualization && 'result' in inputs.visualization ? inputs.visualization.result : bound.results[0]!.ref,
      ),
  );
...
function keyFor(
  ...
  if (nullCount !== 0) {
    if (!allowAllNull || nullCount !== fields.length)
...
function prepareHierarchy(
  ...
  view: 'tree' | 'treemap',
  options: HierarchyGeometryOptions,
): Outcome<{ prepared: PreparedHierarchy; dimensions: Required<HierarchyGeometryOptions> }> {
  ...
  const labelField =
    spec.label === undefined ? undefined : checked.value.result.fields.find((field) => field.id === spec.label);
...
function checkedHierarchyNodes(
  ...
  for (const row of prepared.rows) {
    ...
    if (!identity.ok || identity.value === undefined)
      return identity.ok ? fail('identity', 'A node identity is unavailable.') : identity;
...
function treeLayout(nodes: Map<string, CheckedNode>, width: number, height: number): readonly HierarchyNodeGeometry[] {
  ...
  for (const node of [...nodes.values()].sort(
    (left, right) => left.depth - right.depth || compareIdentity(left.identity, right.identity),
  )) {
  ...
  for (const [depth, list] of levels) {
    const gap = 2;
    const cellWidth = Math.max(1, (width - pad * 2 - gap * Math.max(0, list.length - 1)) / Math.max(1, list.length));
    list.forEach((node, index) =>
      output.push({
        ...
        x: pad + index * (cellWidth + gap),
        y: pad + depth * levelHeight,
        width: cellWidth,
        height: Math.max(1, levelHeight - 2),
...
function treemapLayout(
  ...
  const place = (children: readonly string[], x: number, y: number, w: number, h: number, depth: number): void => {
    ...
    const total = children.reduce(
      (sum, child) => addWeights(sum, subtreeWeights.get(child) ?? ZERO_WEIGHT),
      ZERO_WEIGHT,
    );
...
export function compileTreeGeometry(
  ...
  const checked = checkedHierarchyNodes(prepared.value.prepared, 'tree', prepared.value.dimensions.maxDepth);
  if (!checked.ok) return checked;
  const nodes = treeLayout(checked.value.nodes, prepared.value.dimensions.width, prepared.value.dimensions.height);
  if (
    nodes.length > prepared.value.dimensions.maxMarks ||
    nodes.some(
      (node) =>
        node.width < 24 ||
        node.height < 24 ||
        node.x + node.width > prepared.value.dimensions.width ||
        node.y + node.height > prepared.value.dimensions.height,
    )
  )
    return {
      ok: true,
      value: Object.freeze({
        ...
        bound: prepared.value.prepared.bound,
        result: prepared.value.prepared.result,
        rows: prepared.value.prepared.rows,
        nodes: Object.freeze([]),
        width: prepared.value.dimensions.width,
        height: prepared.value.dimensions.height,
...
export function compileTreemapGeometry(
  ...
  if (!prepared.ok) return prepared;
  const checked = checkedHierarchyNodes(prepared.value.prepared, 'treemap', prepared.value.dimensions.maxDepth);
  if (!checked.ok) return checked;
  ...
  if (
    layout.state === 'data-only' ||
    layout.nodes.length > prepared.value.dimensions.maxMarks ||
    layout.nodes.some((node) => node.width > 0 && node.height > 0 && (node.width < 24 || node.height < 24))
  )
    return {
      ok: true,
      value: Object.freeze({
...
function endpointKey(
  ...
  return key.value === undefined
    ? fail('relationship-endpoint', 'Relationship endpoint identities must be non-null.')
...
export function compileRelationshipGeometry(
  ...
  const edges: RelationshipEdgeGeometry[] = [];
  const sourceNodes = new Map<string, RelationshipNodeGeometry>();
  const targetNodes = new Map<string, RelationshipNodeGeometry>();
  const labelField =
    spec.label === undefined ? undefined : checked.value.result.fields.find((field) => field.id === spec.label);
  for (const row of checked.value.rows) {
    const source = endpointKey(row.values, sourceFields.value);
    if (!source.ok) return source;
    const target = endpointKey(row.values, targetFields.value);
    if (!target.ok) return target;
    const sourceId = `source:${source.value}`;
    const targetId = `target:${target.value}`;
    const sourceText = endpointLabel(row.values, sourceFields.value);
    const targetText = endpointLabel(row.values, targetFields.value);
    if (!sourceNodes.has(sourceId))
      sourceNodes.set(sourceId, { identity: sourceId, namespace: 'source', label: sourceText, x: 0, y: 0 });
    if (!targetNodes.has(targetId))
      targetNodes.set(targetId, { identity: targetId, namespace: 'target', label: targetText, x: 0, y: 0 });
    const pair = `${source.value}\u0000${target.value}`;
    if (relation.cardinality !== 'many-to-many' && edgePairs.has(pair))
      return fail('cardinality', 'The materialized relationship repeats an edge under a unique endpoint cardinality.');
    edgePairs.add(pair);
    const sourceTargets = sourceCounts.get(source.value) ?? new Set<string>();
    sourceTargets.add(target.value);
    sourceCounts.set(source.value, sourceTargets);
    const targetSources = targetCounts.get(target.value) ?? new Set<string>();
    targetSources.add(source.value);
    targetCounts.set(target.value, targetSources);
    edges.push({
      ...
      ...(labelField === undefined
        ? {}
        : { label: labelFor(row.values, { id: labelField.id, type: labelField.type }, '') }),
    });
  }
  if (
    relation.cardinality === 'one-to-one' &&
    ([...sourceCounts.values()].some((targets) => targets.size > 1) ||
      [...targetCounts.values()].some((sources) => sources.size > 1))
  )
    return fail('cardinality', 'The materialized relationship violates one-to-one cardinality.');
  if (relation.cardinality === 'many-to-one' && [...sourceCounts.values()].some((targets) => targets.size > 1))
    return fail('cardinality', 'The materialized relationship violates many-to-one cardinality.');
  if (relation.cardinality === 'one-to-many' && [...targetCounts.values()].some((sources) => sources.size > 1))
    return fail('cardinality', 'The materialized relationship violates one-to-many cardinality.');
  if (sourceNodes.size > MAX_GROUPS || targetNodes.size > MAX_GROUPS)
    return {
      ok: true,
      value: Object.freeze({
        kind: 'relationship',
        state: 'data-only',
        reason: `Relationship endpoint groups exceed the ${MAX_GROUPS} group bound. Exact values remain available in the data table.`,
        bound: checked.value.bound,
        result: checked.value.result,
        rows: checked.value.rows,
        nodes: Object.freeze([]),
  ...
  const arrange = (values: readonly RelationshipNodeGeometry[], x: number): readonly RelationshipNodeGeometry[] => {
    const gap = values.length > 1 ? (dimensions.height - 88) / (values.length - 1) : 0;
  ...
  const nodes = Object.freeze([
    ...arrange(sourceList, dimensions.width * 0.25),
    ...arrange(targetList, dimensions.width * 0.75),
  ]);
  if (
    edges.length + nodes.length > dimensions.maxMarks ||
    Math.max(sourceList.length, targetList.length) > Math.floor((dimensions.height - 88) / 24) + 1
  )
    return {
      ok: true,
      value: Object.freeze({
