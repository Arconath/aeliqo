...
class AeliqoTemporalElement extends AeliqoFoundationElement implements VisualizationInputs {
 ...
 protected override render(){
  ...
  return html`<figure><figcaption>${this.label}</figcaption><p part="scope">${scope}. ${g.rows.length} loaded rows.</p>
   ...
   <div part="data" tabindex="0" role="region" aria-label=${`${this.label}: scrollable data`}><table><caption>${this.label}: ${result.precision.kind==='exact'?'exact':'approximate'} loaded values</caption><thead><tr><th scope="col">Select</th>${fields.map(f=>html`<th scope="col">${f.label}${f.type.unit?` (${f.type.unit.symbol})`:''}</th>`)}</tr></thead><tbody>${repeat(g.rows.slice(page*25,page*25+25),row=>row.identity,row=>html`<tr><td><button type="button" ?disabled=${!this.selectionEnabled} aria-pressed=${this.selectedIdentity===row.identity?"true":"false"} aria-label=${`Select ${rowLabel(row)}`} @click=${()=>select(row)}>Select</button></td>${fields.map(f=>html`<td>${exactLabel(row.values[f.id]!)}</td>`)}</tr>`)}</tbody></table></div>
