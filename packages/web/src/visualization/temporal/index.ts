...
class AeliqoTemporalElement extends AeliqoFoundationElement implements VisualizationInputs {
  ...
  protected override render() {
    ...
    const g = compiled.value;
    const result = g.result;
    const refKey = JSON.stringify(result.ref);
    if (this.scope !== refKey) {
      if (this.scope !== '') this.selectedIdentity = '';
      this.scope = refKey;
      this.page = 0;
    }
    if (!g.rows.some((row) => row.identity === this.selectedIdentity)) this.selectedIdentity = '';
    const rowsByIdentity = new Map(g.rows.map((row) => [row.identity, row]));
    const temporalField = result.fields.find(
      (f) =>
        f.id ===
        (this.visualization?.view === 'timeline'
          ? this.visualization.start
          : this.visualization?.view === 'calendar-grid'
            ? this.visualization.date
            : ''),
    );
    const page = Math.min(this.page, Math.max(0, Math.ceil(g.rows.length / 25) - 1));
    const fields = g.columns.map((id) => result.fields.find((f) => f.id === id)!);
      ...
      coverage.kind === 'complete'
        ...
        : coverage.kind === 'sample'
          ...
          : coverage.kind === 'partial'
            ? `Partial result: ${coverage.reason}`
            : `Scope unknown: ${coverage.reason}`;
    const select = (row: VisualizationRow) => {
      if (!this.selectionEnabled) return;
      if (
        this.dispatchEvent(
          new CustomEvent('aeliqo-visualization-select', {
            bubbles: true,
            composed: true,
            cancelable: true,
            detail: Object.freeze({ source: 'user', identity: row.identity, result: result.ref }),
          }),
        )
      )
    ...
    const displayLabel = (row: VisualizationRow) =>
      spec.view !== 'matrix' && 'label' in spec && spec.label ? exactLabel(row.values[spec.label]!) : rowLabel(row);
    const rowLabel = (row: VisualizationRow) => result.identity.map((id) => exactLabel(row.values[id]!)).join(', ');
    return html`<figure class=${this.view}>
      <figcaption>${this.label}</figcaption>
      <p part="scope">${scope}. ${g.rows.length} loaded rows.</p>
      ...
      ${
        g.timeline
          ? html`<p>
                Intervals use separate lanes when they overlap. ${temporalField?.type.temporal?.calendar}; timezone:
                ${temporalField?.type.temporal?.timezone ?? 'civil dates'}. Missing endpoints appear only in the data
                table. Visual labels are limited to 40 characters. Exact endpoints, labels and selection are available
                below.
              </p>
              <div part="viewport" tabindex="0" role="region" aria-label=${`${this.label}: scrollable graphic`}>
                <svg
                  width=${g.width}
                  height=${g.height}
                  viewBox=${`0 0 ${g.width} ${g.height}`}
                  role="img"
                  aria-label=${`${this.label}: intervals in chronological order. ${scope}. Exact values below.`}
                >
                  ${g.timeline.map((mark, index, list) => svg`<line x1=${mark.x} x2=${Math.max(mark.x + 2, mark.end)} y1=${24 + mark.lane * 28} y2=${24 + mark.lane * 28} stroke="currentColor" stroke-width="12"></line><text class="timeline-label" x=${mark.x} y=${16 + mark.lane * 28} text-anchor=${index === 0 ? 'start' : index === list.length - 1 ? 'end' : 'middle'} font-size="11" fill="currentColor">${displayLabel(rowsByIdentity.get(mark.identity)!).slice(0, 40)}</text>`)}${g.ticks?.filter((_, index, list) => index === 0 || index === list.length - 1).map((tick, index) => svg`<text class="timeline-tick" x=${tick.position} y=${g.height - 16} text-anchor=${index === 0 ? 'start' : 'end'} font-size="11" fill="currentColor">${tick.label.length > 24 ? `${tick.label.slice(0, 23)}…` : tick.label}</text>`)}
                </svg>
              </div>`
          : nothing
      }
      ${
        g.days
          ? html`<p>
                Gregorian calendar; timezone: ${temporalField?.type.temporal?.timezone ?? 'civil dates'}. Weeks start on
                ${['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'][g.weekStartsOn!]}.
                Missing dates appear only in the data table. Counts below are loaded rows per day.
              </p>
              <div part="viewport" tabindex="0" role="region" aria-label=${`${this.label}: scrollable graphic`}>
                <div class="days">
                  ${g.days.map((day, index) => html`<section class="day" style=${index === 0 ? `grid-column-start:${((day.weekday - g.weekStartsOn! + 7) % 7) + 1}` : ''}><strong>${day.date}</strong><span>${day.rows.length} loaded rows</span>${day.rows.slice(0, 3).map((row) => html`<span>${displayLabel(row)}${spec.view === 'calendar-grid' && spec.value ? `: ${exactLabel(row.values[spec.value]!)}` : ''}</span>`)}${day.rows.length > 3 ? html`<span>${day.rows.length - 3} additional rows in the data table.</span>` : nothing}${day.rows.length === 1 ? html`<button type="button" ?disabled=${!this.selectionEnabled} aria-pressed=${this.selectedIdentity === day.rows[0]!.identity ? 'true' : 'false'} aria-label=${`Select ${rowLabel(day.rows[0]!)}`} @click=${() => select(day.rows[0]!)}>Select</button>` : nothing}</section>`)}
                </div>
              </div>`
          : nothing
      }
      ${this.view === 'matrix' ? html`<p part="scroll-hint">Scroll the table horizontally to compare every declared column.</p>` : nothing}
      <div part="data" tabindex="0" role="region" aria-label=${`${this.label}: scrollable data`}>
        <table>
          <caption>
            ${this.label}: ${result.precision.kind === 'exact' ? 'exact' : 'approximate'} loaded values
          </caption>
          <thead>
            <tr>
              ${
                this.view === 'matrix'
                  ? html`${fields.map((f) => html`<th scope="col">${f.label}${f.type.unit ? ` (${f.type.unit.symbol})` : ''}</th>`)}
                      <th scope="col">Select</th>`
                  : html`<th scope="col">Select</th>
                      ${fields.map((f) => html`<th scope="col">${f.label}${f.type.unit ? ` (${f.type.unit.symbol})` : ''}</th>`)}`
              }
            </tr>
          </thead>
          <tbody>
            ${repeat(
              g.rows.slice(page * 25, page * 25 + 25),
              (row) => row.identity,
              (row) =>
                this.view === 'matrix'
                  ? html`<tr>
                      ${fields.map((f) => html`<td data-label=${f.label}>${exactLabel(row.values[f.id]!)}</td>`)}
      ...
      ${
        g.rows.length > 25
          ? html`<nav aria-label="Visualization data pages">
              ...
              >
                Previous</button
              ><span>Rows ${page * 25 + 1}–${Math.min((page + 1) * 25, g.rows.length)} of ${g.rows.length}</span
              ><button
                type="button"
                ?disabled=${(page + 1) * 25 >= g.rows.length}
                @click=${() => {
                  this.page = page + 1;
                }}
              >
