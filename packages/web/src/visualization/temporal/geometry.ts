...
export function compileTemporalVisualization(input: VisualizationInputs): Outcome<TemporalGeometry> {
  ...
  const result = bound.value.results[0]!;
  const rows = materialized.value;
  if (
    ...
    return fail('Visualization dimensions or mark budget are outside supported limits.');
  const base: TemporalGeometry = {
    view: spec.view,
    result,
    rows,
    columns: spec.view === 'matrix' ? spec.columns : result.fields.map((f) => f.id),
    width: input.width,
    height: input.height,
    state: 'ready',
  };
  ...
  const dataOnly = (reason: string): Outcome<TemporalGeometry> => ({
    ok: true,
    value: { ...base, state: 'data-only', reason },
  });
  if (!['gregory', 'gregorian', 'iso8601'].includes(field.type.temporal!.calendar))
    return dataOnly('This calendar is not supported visually; the declared dates remain in the data table.');
  if (spec.view === 'timeline') {
    const intervals: { row: VisualizationRow; start: Scalar; end: Scalar }[] = [];
    for (const row of rows) {
      const start = row.values[spec.start]!;
      const end = spec.end ? row.values[spec.end]! : start;
    ...
    if (intervals.length > Math.min(input.maxMarks, 1000))
      return dataOnly('Timeline mark budget exceeded; all loaded intervals remain in the data table.');
    const compare = (a: Scalar, b: Scalar): number => {
      const c = compareScalars(a, b, field.type);
      if (!c.ok || c.value === null) throw Error('Unordered timeline');
      return c.value;
    };
    intervals.sort(
      (a, b) =>
    ...
    try {
      ...
      for (const interval of intervals) {
        ...
        for (let index = 0; index < ends.length; index++) {
          if (++work > 100_000)
            return dataOnly(
              'Timeline lane calculation exceeds the work budget; all intervals remain in the data table.',
            );
          if (compare(ends[index]!, interval.start) < 0) {
            lane = index;
            break;
          }
        }
        ...
        if ((lane + 1) * 28 > input.height - 40)
          return dataOnly(
            'Overlapping intervals exceed the visible lane budget; all intervals remain in the data table.',
          );
        marks.push({
  ...
  try {
    ...
    const weekStartsOn = spec.weekStartsOn ?? 1;
    if (!first || !last) return { ok: true, value: { ...base, days: [], weekStartsOn } };
    const start = Date.parse(`${first}T00:00:00Z`);
    const end = Date.parse(`${last}T00:00:00Z`);
    const count = Math.round((end - start) / 86_400_000) + 1;
    if (!Number.isFinite(count) || count < 1 || count > Math.min(input.maxMarks, 366))
      return dataOnly('Calendar span exceeds the day budget; all loaded dates remain in the data table.');
    const days: CalendarDay[] = [];
    for (let n = 0; n < count; n++) {
      const date = new Date(start + n * 86_400_000);
      const key = date.toISOString().slice(0, 10);
      days.push({ date: key, weekday: date.getUTCDay(), rows: grouped.get(key) ?? [] });
    }
    return { ok: true, value: { ...base, days, weekStartsOn } };
  } catch {
    return dataOnly('The declared timezone or date is not supported visually; exact dates remain in the data table.');
  }
}
