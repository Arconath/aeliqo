...
export class AeliqoQualityPanelElement extends AeliqoCompoundElement {
  ...
  protected override render() { const source = this.state?.source ?? this.source; const freshness = this.state?.freshness ?? this.freshness; const completeness = this.state?.completeness ?? this.completeness; const provenance = this.state?.provenance ?? this.provenance; const unsupported = this.state?.unsupportedClaims ?? this.unsupportedClaims; const current = status(this.status); return html`<section part="root" data-status=${current} aria-label=${this.title}><div part="header"><h2>${this.title}</h2><span part="meta">${current === "ready" ? "Reported metadata" : this.statusText(current, this.message)}</span></div><dl><dt>Source</dt><dd>${source || "Not supplied"}</dd><dt>Freshness</dt><dd>${freshness || "Not supplied"}</dd><dt>Completeness</dt><dd>${completeness || "Not supplied"}</dd><dt>Provenance</dt><dd>${provenance.length ? html`<ul>${provenance.map(item => html`<li>${item}</li>`)}</ul>` : "Not supplied"}</dd></dl>${current !== "ready" ? html`<p part="status" role="status">${this.statusText(current, this.message)}</p>` : nothing}${unsupported.length ? html`<p part="caution">Unsupported claims are shown explicitly and are not presented as facts.</p><ul part="unsupported">${unsupported.map(item => html`<li>${item}</li>`)}</ul>` : nothing}</section>`; }
}
