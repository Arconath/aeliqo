...
const event = <T>(type: string, detail: T, cancelable = true): CustomEvent<T> =>
...
function compoundControls(host: HTMLElement): readonly CompoundControl[] {
  return Array.from(
    host.querySelectorAll<HTMLElement>(
      'input, select, textarea, button, aeliqo-input, aeliqo-text-field, aeliqo-text-area, aeliqo-number-field, aeliqo-date-field, aeliqo-date-range, aeliqo-search-field, aeliqo-checkbox, aeliqo-switch, aeliqo-radio-group, aeliqo-select, aeliqo-combobox, aeliqo-slider, aeliqo-file-input',
    ),
...
function collectCompoundValues(host: HTMLElement): Readonly<Record<string, string | readonly string[]>> {
  ...
  for (const control of compoundControls(host)) {
    ...
    if (
      ...
      (control instanceof HTMLInputElement && ['submit', 'reset', 'button', 'image'].includes(control.type))
    )
      continue;
  ...
  protected override render() {
    ...
    return html`
      <section part="root" data-status=${current} aria-label=${this.title}>
        ...
        ${
          stale
            ? html`<p part="status" role="status">Results are out of date for this query. Refresh to view them.</p>`
            : html`
                <p part="scope">
                  ${total === undefined ? 'Matching count unavailable' : `${total.toLocaleString()} matching results`}
                </p>
  ...
  private moveTo(target: string, direction: 'next' | 'back'): void {
    ...
    if (direction === 'next') {
      for (let index = fromIndex; index < targetIndex; index += 1)
        if (!this.stepValid(this.steps[index]!.id, true)) return;
    }
...
export class AeliqoQualityPanelElement extends AeliqoCompoundElement {
  ...
  protected override render() {
    const source = this.state?.source ?? this.source;
    const freshness = this.state?.freshness ?? this.freshness;
    const completeness = this.state?.completeness ?? this.completeness;
    const provenance = this.state?.provenance ?? this.provenance;
    const unsupported = this.state?.unsupportedClaims ?? this.unsupportedClaims;
    const current = status(this.status);
    return html`<section part="root" data-status=${current} aria-label=${this.title}>
      <div part="header">
        <h2>${this.title}</h2>
        <span part="meta">${current === 'ready' ? 'Reported metadata' : this.statusText(current, this.message)}</span>
      </div>
      <dl>
        <dt>Source</dt>
        <dd><bdi>${source || 'Not supplied'}</bdi></dd>
        <dt>Freshness</dt>
        <dd><bdi>${freshness || 'Not supplied'}</bdi></dd>
        <dt>Completeness</dt>
        <dd><bdi>${completeness || 'Not supplied'}</bdi></dd>
        <dt>Provenance</dt>
        <dd>
          ${
            provenance.length
