import {
  ...
  type AeliqoAuthorizedVisualizationBindings,
...
} from './navigation-feedback-registry.js';
...
function columns(
  ...
  for (const item of raw) {
    ...
    if (
      candidate === undefined ||
      Object.keys(candidate).some((key) => key !== 'key' && key !== 'label') ||
    ...
    if (candidate.label !== undefined) {
      ...
      if (label.value !== descriptor.label)
        return fail('field', `Column ${key.value} must use its registered descriptor label.`);
    }
    seen.add(key.value);
    output.push({ key: key.value, label: descriptor.label });
  }
...
function stackConfig(values: PresentationValues): Outcome<ResolvedPresentationConfig> {
  ...
  if (
    input.gap !== undefined &&
    (!Number.isSafeInteger(input.gap) || (input.gap as number) < 0 || (input.gap as number) > 64)
  )
    return fail('config', 'gap must be a bounded nonnegative integer.');
...
function tableConfig(
  ...
  if (selected !== 'none' && selected !== 'single' && selected !== 'multiple')
    return fail('config', 'selection must be none, single or multiple.');
  ...
  const identityFields =
    input.identity === undefined ? { ok: true as const, value: undefined } : identity(input, result);
  if (!identityFields.ok) return identityFields;
  const port = selectionPort(input, result, 'selection', resolveEntity);
  if (!port.ok) return port;
  const output: Record<string, unknown> = {
    ...(columnList === undefined ? {} : { columns: columnList.value }),
    selection: selected,
    ...(identityFields.value === undefined ? {} : { identity: identityFields.value }),
  };
  const operations =
    selected === 'none'
  ...
  const fields =
    columnList === undefined ? result.fields.map((field) => field.id) : columnList.value.map((column) => column.key);
  if (isDefault && hasFrozenFieldIds(result)) {
    const outcome: Outcome<ResolvedPresentationConfig> = Object.freeze({
      ok: true,
      value: Object.freeze({
        ...
        operations: Object.freeze([
          Object.freeze({ ...AELIQO_OPERATION_REFS.read }),
          Object.freeze({ ...AELIQO_OPERATION_REFS.compare }),
...
function trendConfig(values: PresentationValues, result: Result | undefined): Outcome<ResolvedPresentationConfig> {
  ...
  const labelField = text(input.labelField, 'labelField');
  if (!labelField.ok) return labelField;
  const fields = fieldMap(result);
  const temporal = fields.get(labelField.value);
  ...
  if (!Array.isArray(input.series) || input.series.length === 0 || input.series.length > 8)
  ...
  for (const item of input.series) {
    const candidate = record(item);
    if (candidate === undefined || Object.keys(candidate).some((key) => !['field', 'label', 'unit'].includes(key)))
    ...
    if (candidate.label !== undefined) {
      ...
      if (!label.ok) return label;
      if (label.value !== descriptor.label)
        return fail('field', `Trend series ${field.value} must use its registered descriptor label.`);
    }
    ...
    if (unit.value !== undefined && unit.value !== descriptorUnit)
      return fail('field', `Trend series ${field.value} must use its registered descriptor unit.`);
    const descriptorUnitKey = unitKey(descriptor);
    if (firstUnitKey === undefined) firstUnitKey = descriptorUnitKey;
    else if (descriptorUnitKey !== firstUnitKey)
  ...
  if (
    ...
    (!Array.isArray(grouping) ||
      grouping.length > MAX_ITEMS ||
      grouping.some((value) => typeof value !== 'string' || value.length === 0))
  )
    return fail('config', 'seriesBy must contain bounded result field IDs.');
  const seriesBy = grouping === undefined ? [] : [...(grouping as string[])];
  if (
    new Set(seriesBy).size !== seriesBy.length ||
    seriesBy.some(
      (field) => !fields.has(field) || field === labelField.value || series.some((entry) => entry.field === field),
    )
  )
    return fail('field', 'seriesBy fields must be unique grouping fields distinct from temporal and measure fields.');
  const grain = trendGrain(seriesBy, labelField.value, result);
  ...
  return {
    ...
    value: {
      ...
      fields: [labelField.value, ...seriesBy, ...series.map((entry) => entry.field)],
      ports: [],
...
function filterConfig(
  ...
  if (Object.keys(input).some((key) => !['field', 'outputId'].includes(key)))
  ...
  const output: Record<string, unknown> = {
    ...
    ...(owner.value === undefined ? {} : { entity: owner.value }),
  };
  return {
    ok: true,
    value: {
...
function suggestTrend(
  ...
  const label =
    result.fields.find((field) => temporalField(field) && requested.has(field.id)) ??
    result.fields.find((field) => temporalField(field));
  const series =
    result.fields.find((field) => numericField(field) && requested.has(field.id) && field.id !== label?.id) ??
...
function buildManifests(options: AeliqoPresentationRegistryOptions): readonly PresentationManifest[] {
  ...
  return Object.freeze([
    ...
    },
    {
      ...
      visibility: 'leaf',
      extension: false,
      resolveConfig: (values, result) => trendConfig(values, result),
      assess: assessTrend,
...
export function createAeliqoPresentationRegistry(
  ...
  const options: AeliqoPresentationRegistryOptions = isMappings
    ? {}
    : (optionsOrMappings as AeliqoPresentationRegistryOptions);
  const registeredMappings: readonly InteractionMappingManifest[] = isMappings
    ? (optionsOrMappings as readonly InteractionMappingManifest[])
    : mappings;
  ...
  const data = createAeliqoDataPresentationManifests(
    options.data ?? [],
    options.resolveEntity === undefined ? {} : { resolveEntity: options.resolveEntity },
  );
  if (!data.ok) return data;
  const visualizations = createAeliqoVisualizationPresentationManifests(
    options.visualizations ?? [],
    options.resolveEntity === undefined ? {} : { resolveEntity: options.resolveEntity },
  );
  if (!visualizations.ok) return visualizations;
  return createPresentationRegistry(
    [
      ...buildManifests(options),
      ...foundation.value,
      ...inputs.value,
