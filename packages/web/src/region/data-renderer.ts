import type { InteractionPayload, InteractionState, ResultRef, Scalar } from '@aeliqo/core';
...
export type AeliqoDataCanonicalPayload = Extract<InteractionPayload, { readonly kind: 'selection' | 'filter' }>;
...
function selectionPort(node: AeliqoDataResolvedNode, output = false) {
  try {
...
function port(node: AeliqoDataResolvedNode, id: string, payload: InteractionPayload['kind']): boolean {
  try {
...
function hostOutput(context: AeliqoDataRenderContext): AeliqoDataHostRequestHandler | undefined {
  try {
...
function validScopeDetail(value: unknown, node: AeliqoDataResolvedNode): boolean {
  ...
  for (const key of ['loaded', 'filteredTotal', 'populationTotal'] as const) {
    if (Object.hasOwn(scope, key) && (!Number.isSafeInteger(scope[key]) || (scope[key] as number) < 0)) return false;
  }
  if (
    Object.hasOwn(scope, 'populationDigest') &&
    (typeof scope.populationDigest !== 'string' || scope.populationDigest !== node.scope.populationDigest)
  )
    return false;
  if (
    Object.hasOwn(scope, 'kind') &&
    !['loaded', 'filtered', 'population', 'sample', 'unknown'].includes(String(scope.kind))
  )
    return false;
  return !Object.hasOwn(scope, 'label') || typeof scope.label === 'string';
}

function selectedKeys(node: AeliqoDataResolvedNode, interaction: InteractionState | undefined): readonly string[] {
...
function validatedSelection(
  ...
  if (mode === 'clear') {
    return Object.keys(selection).length === 1 && Object.hasOwn(selection, 'mode') ? { mode: 'clear' } : undefined;
  }
  ...
  if (mode === 'ids') {
    ...
    if (
      Object.keys(selection).some((key) => !['mode', 'entity', 'keys', 'result'].includes(key)) ||
  ...
  if (mode === 'predicate') {
    ...
    if (
      Object.keys(selection).some(
        (key) => !['mode', 'entity', 'predicate', 'queryDigest', 'populationDigest'].includes(key),
      ) ||
...
function dispatchSelection(node: AeliqoDataResolvedNode, event: Event, context: AeliqoDataRenderContext): void {
  ...
  if (detail.mode === 'clear') {
    ...
    if (Object.hasOwn(detail, 'result') && !sameRef(detail.result as ResultRef, node.result.ref)) return;
    hostOutput(context)?.({
      kind: 'selection',
      nodeId: node.id,
      portId: selection.id,
...
function validatePredicate(value: unknown, node: AeliqoDataResolvedNode): boolean {
  try {
    ...
    const check = (item: unknown, depth = 0): boolean => {
      ...
      if (predicate.op === 'and' || predicate.op === 'or') {
        if (
          Object.keys(predicate).some((key) => !['op', 'predicates'].includes(key)) ||
      ...
      const field = typeof predicate.field === 'string' ? fields.get(predicate.field) : undefined;
      if (
        field === undefined ||
      ...
      if (predicate.op === 'is-null') {
        return (
          Object.keys(predicate).every((key) => ['op', 'field', 'negate'].includes(key)) &&
          typeof predicate.negate === 'boolean'
        );
      }
      if (predicate.op === 'compare') {
        const scalar = scalarValueForField(predicate.value);
        const checked = scalar === undefined ? undefined : validateScalar(scalar, field.type);
        return (
          Object.keys(predicate).every((key) => ['op', 'field', 'comparison', 'value'].includes(key)) &&
          ['eq', 'ne', 'lt', 'lte', 'gt', 'gte'].includes(String(predicate.comparison)) &&
          checked?.ok === true
        );
      }
      if (predicate.op === 'in') {
        return (
          Object.keys(predicate).every((key) => ['op', 'field', 'values'].includes(key)) &&
...
function dispatchFilter(node: AeliqoDataResolvedNode, event: Event, context: AeliqoDataRenderContext): void {
  if (eventType(event) !== 'aeliqo-filter-change') return;
  if (!port(node, 'filter', 'filter')) return;
  const detail = exactRecord(eventDetail(event), ['predicate', 'inherited', 'scopeLabel', 'applied'], ['applied']) as
    AeliqoFilterChangeDetail | undefined;
  if (
    detail === undefined ||
    detail.applied !== true ||
    (Object.hasOwn(detail, 'scopeLabel') && detail.scopeLabel !== undefined && typeof detail.scopeLabel !== 'string')
  )
    return;
  ...
  const predicates: Extract<InteractionPayload, { readonly kind: 'filter' }>['predicates'] =
    effective === undefined ? [] : [effective as AeliqoFilterPredicate];
  hostOutput(context)?.({
    kind: 'filter',
    nodeId: node.id,
    portId: 'filter',
...
function dispatchPage(node: AeliqoDataResolvedNode, event: Event, context: AeliqoDataRenderContext): void {
  ...
  if (
    ...
    return;
  if (request.result !== undefined && !sameRef(request.result, node.result.ref)) return;
  hostOutput(context)?.({
    kind: 'page',
    nodeId: node.id,
    portId: 'page',
...
function dispatchSort(node: AeliqoDataResolvedNode, event: Event, context: AeliqoDataRenderContext): void {
  ...
  const sort = sortValue === undefined ? undefined : record(sortValue);
  if (
    sortValue !== undefined &&
    (sort === undefined ||
      ...
      !node.columns.some((column) => column.key === sort.field && column.sortable === true))
  )
    return;
  hostOutput(context)?.({
    kind: 'sort',
    ...
    portId: 'sort',
    request:
      sort === undefined ? undefined : { field: sort.field as string, direction: sort.direction as 'asc' | 'desc' },
  });
}

function dispatchWindow(node: AeliqoDataResolvedNode, event: Event, context: AeliqoDataRenderContext): void {
  if (
...
function dispatchLoadMore(node: AeliqoDataResolvedNode, event: Event, context: AeliqoDataRenderContext): void {
  if (eventType(event) !== 'aeliqo-data-load-more' || node.component !== 'cardCollection') return;
