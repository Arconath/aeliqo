...
function semanticType(value: unknown): value is SemanticType {
  ...
  if (
    ...
    (!Array.isArray(candidate.grain) ||
      candidate.grain.some((item) => !bounded(item, 160)) ||
...
function fileDetail(event: Event): AeliqoFileChangeDetail | undefined {
  ...
  for (const item of detail.files) {
    ...
    if (
      file === undefined ||
      !exactKeys(file, ['name', 'size', 'type', 'lastModified']) ||
      !bounded(file.name, 512) ||
      !bounded(file.type, 256, false) ||
...
function sameSemanticType(left: SemanticType, right: SemanticType): boolean {
  ...
  if (leftUnit === undefined || rightUnit === undefined) {
    ...
    leftUnit.symbol !== rightUnit.symbol ||
    leftUnit.currency !== rightUnit.currency
  )
    return false;
  const leftGrain = left.grain ?? [];
  const rightGrain = right.grain ?? [];
  if (leftGrain.length !== rightGrain.length || leftGrain.some((item, index) => item !== rightGrain[index]))
    return false;
  ...
  return (
    leftTemporal.calendar === rightTemporal.calendar &&
    leftTemporal.timezone === rightTemporal.timezone &&
    leftTemporal.grain === rightTemporal.grain
  );
}
...
function boundedStep(value: unknown, minimum: unknown, maximum: unknown, step: unknown): boolean {
  ...
  if (
    parsed === undefined ||
    (minimum !== undefined && minimum !== '' && min === undefined) ||
    (maximum !== undefined && maximum !== '' && max === undefined) ||
    (step !== undefined && step !== '' && increment === undefined)
  )
    return false;
...
function interactionForDraft(node: Node, event: Event): readonly AeliqoInputInteraction[] {
  ...
  if (ref === 'input.number-field') {
    ...
    if (
      candidate === undefined ||
      !exactKeys(candidate, ['text', 'value', 'valid']) ||
    ...
    if (
      candidate === undefined ||
      !exactKeys(candidate, ['value', 'unit']) ||
    ...
    if (
      candidate.unit !== unit ||
      (registeredUnit !== undefined && registeredUnit !== candidate.unit) ||
      (registeredUnit === undefined && candidate.unit !== '')
    )
      return [];
...
function interactionForRange(node: Node, event: Event): readonly AeliqoInputInteraction[] {
  ...
  if (
    candidate === undefined ||
    !exactKeys(candidate, ['start', 'end', 'boundary', 'timezone', 'calendar', 'valid']) ||
    candidate.valid !== true ||
  ...
  switch (node.manifest.id) {
    case 'input.text-field':
      return html`<aeliqo-text-field
        ...
        .autocomplete=${text(v.autocomplete)}
        .inputType=${text(v.inputType, 'text')}
        @aeliqo-input-commit=${handle}
      ></aeliqo-text-field>`;
    case 'input.text-area':
      return html`<aeliqo-text-area
        data-aeliqo-node-id=${id}
        data-aeliqo-theme="inherit"
        .label=${label}
        .description=${description}
        ...
        .value=${text(v.value)}
        .defaultValue=${text(v.defaultValue)}
        .rows=${typeof v.rows === 'number' ? v.rows : 4}
        @aeliqo-input-commit=${handle}
      ></aeliqo-text-area>`;
    case 'input.number-field':
      return html`<aeliqo-number-field
        .locale=${locale}
        data-aeliqo-node-id=${id}
        data-aeliqo-theme="inherit"
        .label=${label}
    ...
    case 'input.checkbox':
      return html`<aeliqo-checkbox
        ...
        .value=${text(v.value, 'on')}
        @aeliqo-input-change=${handle}
      ></aeliqo-checkbox>`;
    case 'input.radio-group':
      return html`<aeliqo-radio-group
        data-aeliqo-node-id=${id}
        data-aeliqo-theme="inherit"
        .label=${label}
        .description=${description}
    ...
    case 'input.select':
      return html`<aeliqo-select
        ...
        .value=${text(v.value)}
        .defaultValue=${text(v.defaultValue)}
        .emptyLabel=${text(v.emptyLabel, 'Select an option')}
        @aeliqo-input-change=${handle}
      ></aeliqo-select>`;
    case 'input.combobox':
      return html`<aeliqo-combobox
        data-aeliqo-node-id=${id}
        data-aeliqo-theme="inherit"
        .label=${label}
        .description=${description}
        ...
        .defaultValue=${text(v.defaultValue)}
        .query=${text(v.query)}
        .minQueryLength=${typeof v.minQueryLength === 'number' ? v.minQueryLength : 0}
        .placeholder=${text(v.placeholder)}
        @aeliqo-input-change=${handle}
      ></aeliqo-combobox>`;
    case 'input.date-field':
      return html`<aeliqo-date-field
        data-aeliqo-node-id=${id}
        data-aeliqo-theme="inherit"
        .label=${label}
        .description=${description}
        ...
        .min=${text(v.min)}
        .max=${text(v.max)}
        .calendar=${v.calendar === 'gregory' ? 'gregory' : 'gregory'}
        @aeliqo-input-commit=${handle}
      ></aeliqo-date-field>`;
    case 'input.date-range':
      return html`<aeliqo-date-range
        data-aeliqo-node-id=${id}
        data-aeliqo-theme="inherit"
        .label=${label}
        .description=${description}
        ...
        .start=${text(v.start)}
        .end=${text(v.end)}
        .defaultStart=${text(v.defaultStart)}
        .defaultEnd=${text(v.defaultEnd)}
        .boundary=${v.boundary === 'exclusive' ? 'exclusive' : 'inclusive'}
        .timezone="calendar"
        .calendar="gregory"
        @aeliqo-input-commit=${handle}
      ></aeliqo-date-range>`;
    case 'input.slider':
      return html`<aeliqo-slider
        data-aeliqo-node-id=${id}
        data-aeliqo-theme="inherit"
        .label=${label}
        .description=${description}
    ...
    case 'input.search-field':
      return html`<aeliqo-search-field
        data-aeliqo-node-id=${id}
        data-aeliqo-theme="inherit"
        .label=${label}
        .description=${description}
        ...
        .placeholder=${text(v.placeholder)}
        .autocomplete=${text(v.autocomplete)}
        .queryOnInput=${bool(v.queryOnInput)}
        .debounceMs=${typeof v.debounceMs === 'number' ? v.debounceMs : 250}
        @aeliqo-input-commit=${handle}
        @aeliqo-search=${handle}
      ></aeliqo-search-field>`;
    case 'input.file-input':
      return html`<aeliqo-file-input
        data-aeliqo-node-id=${id}
        data-aeliqo-theme="inherit"
        .label=${label}
        .description=${description}
        ...
        .capture=${text(v.capture)}
        .maxFiles=${typeof v.maxFiles === 'number' ? v.maxFiles : 0}
        .maxBytes=${typeof v.maxBytes === 'number' ? v.maxBytes : 0}
        @aeliqo-file-change=${handle}
      ></aeliqo-file-input>`;
    case 'input.field-group':
      return html`<aeliqo-field-group
        data-aeliqo-node-id=${id}
        data-aeliqo-theme="inherit"
        .legend=${text(v.legend)}
        .description=${description}
        .error=${text(v.error)}
        .disabled=${disabled}
        >${children()}</aeliqo-field-group
      >`;
    case 'input.form':
      return html`<aeliqo-form
        data-aeliqo-node-id=${id}
        data-aeliqo-theme="inherit"
        .label=${text(v.label)}
        .noValidate=${bool(v.noValidate)}
        @aeliqo-form-submit=${handle}
        >${children()}${text(v.submitLabel).length > 0 ? html`<button part="submit" type="submit">${text(v.submitLabel)}</button>` : nothing}</aeliqo-form
      >`;
    default:
      return undefined;
  }
