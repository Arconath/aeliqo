...
function validateInputBinding(binding: AeliqoInputBinding): Outcome<AeliqoInputBinding> {
  ...
  if (ref !== "input.date-field" && ref !== "input.date-range" && draftValue !== undefined && ref !== "input.number-field" && ref !== "input.slider" && ref !== "input.checkbox" && draftValue.type.value !== "text") return fail("binding", `${ref} requires a text semantic type.`);
