...
const semanticType = (value: unknown): value is SemanticType => {
  ...
  if (
    candidate.grain !== undefined &&
    (!Array.isArray(candidate.grain) ||
      candidate.grain.some((item) => !boundedId(item)) ||
      new Set(candidate.grain).size !== candidate.grain.length)
  )
    return false;
  if (candidate.temporal !== undefined) {
    ...
    if (
      ...
      !['date', 'instant'].includes(String(candidate.value))
    )
      return false;
  }
  return true;
...
const jsonRecord = (value: unknown): value is Readonly<Record<string, ReadonlyJsonValue>> => {
  ...
  return (
    candidate !== undefined &&
    Object.keys(candidate).length <= 128 &&
    Object.keys(candidate).every((key) => boundedId(key)) &&
    parseWireValue(candidate).ok
  );
};
...
function numericText(value: unknown): boolean {
  return (
    value === undefined ||
    (typeof value === 'string' && value.length <= 512 && /^(?:[+-]?(?:0|[1-9][0-9]*)(?:\.[0-9]+)?)?$/u.test(value))
  );
}
...
function validateInputBinding(binding: AeliqoInputBinding): Outcome<AeliqoInputBinding> {
  ...
  if (draftRefs.has(ref) && draftValue === undefined)
    return fail('binding', `${ref} requires a registered draft target.`);
  if (ref === 'input.date-range' && (startValue === undefined || endValue === undefined))
  ...
  if (ref === 'input.date-range' && (startValue!.type.value !== 'date' || endValue!.type.value !== 'date'))
    return fail('binding', 'Date range mappings must use the date semantic type.');
  if (
    ref === 'input.date-range' &&
    (startValue!.entity !== endValue!.entity ||
      startValue!.key !== endValue!.key ||
      startValue!.entityRevision !== endValue!.entityRevision)
  )
    return fail('binding', 'Date range start and end mappings must target the same registered entity revision.');
  if (
    ref !== 'input.date-range' &&
    draftValue !== undefined &&
    ['input.number-field', 'input.slider'].includes(ref) &&
    !['integer', 'float', 'decimal'].includes(draftValue.type.value)
  )
    return fail('binding', `${ref} requires a numeric semantic type.`);
  if (
    ref !== 'input.date-field' &&
    ref !== 'input.date-range' &&
    draftValue !== undefined &&
    ref !== 'input.number-field' &&
    ref !== 'input.slider' &&
    ref !== 'input.checkbox' &&
    ref !== 'input.switch' &&
    draftValue.type.value !== 'text' &&
    !(ref === 'input.text-field' && draftValue.type.value === 'instant')
  )
    return fail('binding', `${ref} requires a text semantic type.`);
  if (
    ['input.checkbox', 'input.switch'].includes(ref) &&
    draftValue !== undefined &&
    draftValue.type.value !== 'boolean'
  )
    return fail('binding', `${ref} requires a boolean semantic type.`);
  if (ref === 'input.date-field' && draftValue !== undefined && draftValue.type.value !== 'date')
  ...
  const enumField = (key: string, allowed: readonly string[]): boolean =>
    !has(key) || (typeof config[key] === 'string' && allowed.includes(config[key]));
  const textDraft = draftValue;
  const valueCheck = (key: string): Outcome<Scalar | undefined> =>
    typeValue(textDraft?.type ?? { value: 'text', nullable: false }, config[key]);
  const numberCheck = (key: string): Outcome<Scalar | undefined> =>
    numberValue(textDraft?.type ?? { value: 'float', nullable: false }, config[key]);
  ...
  if (!commonConfig(config, allowedByRef[ref], ref !== 'input.field-group', !structure))
    return fail('binding', `${ref} contains an unknown or malformed common configuration field.`);
  if (
    ref === 'input.text-field' &&
    (!stringField('value', 16_384) ||
      !stringField('defaultValue', 16_384) ||
      !stringField('placeholder', 1_024) ||
      !stringField('autocomplete', 128) ||
      !enumField('inputType', [
        'text',
        'search',
        'url',
    ...
    return fail('binding', 'Text field configuration is invalid.');
  if (
    ref === 'input.text-area' &&
    (!stringField('value', 65_536) || !stringField('defaultValue', 65_536) || !finiteNumber(config.rows, 1, 100))
  )
    return fail('binding', 'Text area configuration is invalid.');
  if (
    ref === 'input.number-field' &&
    (!numericText(config.value) ||
      ...
      !stringField('unit', 128))
  )
    return fail('binding', 'Number field configuration is invalid.');
  if (
    ['input.checkbox', 'input.switch'].includes(ref) &&
    (!boolField('checked') || !boolField('defaultChecked') || !boolField('indeterminate') || !stringField('value', 256))
  )
    return fail('binding', 'Boolean control configuration is invalid.');
  if (
    ['input.radio-group', 'input.select', 'input.combobox'].includes(ref) &&
    (!stringField('value', 256) || !stringField('defaultValue', 256) || !optionList(config.options))
  )
    return fail('binding', `${ref} requires a bounded registered option list.`);
  if (ref === 'input.radio-group' && !enumField('orientation', ['horizontal', 'vertical']))
    return fail('binding', 'Radio orientation is invalid.');
  if (ref === 'input.select' && !stringField('emptyLabel', 256))
    return fail('binding', 'Select emptyLabel is invalid.');
  if (
    ref === 'input.combobox' &&
    (!stringField('query', 1_024) || !finiteNumber(config.minQueryLength, 0, 256) || !stringField('placeholder', 1_024))
  )
    return fail('binding', 'Combobox configuration is invalid.');
  if (
    ref === 'input.date-field' &&
    (!stringField('value', 10) ||
      !stringField('defaultValue', 10) ||
      !dateValue(config.value) ||
      !dateValue(config.defaultValue) ||
      !dateValue(config.min) ||
      !dateValue(config.max) ||
      !enumField('calendar', ['gregory']))
  )
    return fail('binding', 'Date field configuration is invalid.');
  if (
    ref === 'input.date-range' &&
    (!dateValue(config.start) ||
      ...
      !enumField('boundary', ['inclusive', 'exclusive']) ||
      !enumField('timezone', ['calendar']) ||
      !enumField('calendar', ['gregory']))
  )
    return fail('binding', 'Date range configuration is invalid.');
  if (
    ref === 'input.slider' &&
    ...
    return fail('binding', 'Slider configuration is invalid.');
  if (
    ref === 'input.search-field' &&
    (!stringField('value', 16_384) ||
      !stringField('defaultValue', 16_384) ||
      !boolField('queryOnInput') ||
      !finiteNumber(config.debounceMs, 0, 10_000) ||
      !stringField('placeholder', 1_024) ||
      !stringField('autocomplete', 128))
  )
    return fail('binding', 'Search field configuration is invalid.');
  if (
    ref === 'input.file-input' &&
    (!stringField('accept', 1_024) ||
      !boolField('multiple') ||
      !stringField('capture', 64) ||
      !finiteNumber(config.maxFiles, 0, 500) ||
      !finiteNumber(config.maxBytes, 0, 1_000_000_000))
  )
    return fail('binding', 'File input configuration is invalid.');
  if (
    ref === 'input.field-group' &&
    (!stringField('legend', 512) ||
      !stringField('description', 2_048) ||
      !stringField('error', 2_048) ||
      !boolField('disabled'))
  )
    return fail('binding', 'Field group configuration is invalid.');
  if (
    ref === 'input.form' &&
    (!stringField('label', 512) || !boolField('noValidate') || !stringField('submitLabel', 256))
  )
    return fail('binding', 'Form configuration is invalid.');
  if (textDraft !== undefined) {
    for (const key of ['value', 'defaultValue']) {
      if (ref === 'input.number-field') {
        const checked = numberCheck(key);
  ...
  if (ref === 'input.slider' && draftValue !== undefined) {
    ...
    if (unit === undefined ? config.unit !== undefined && config.unit !== '' : config.unit !== unit)
    ...
    for (const key of ['value', 'defaultValue', 'min', 'max', 'step']) {
      ...
      if (draftValue.type.value === 'integer' && !Number.isSafeInteger(config[key]))
        return fail('binding', 'An integer slider requires safe integer values.');
      if (!validateScalar(config[key], draftValue.type).ok)
  ...
  if (ref === 'input.number-field' && draftValue !== undefined) {
    ...
    for (const key of ['value', 'defaultValue', 'min', 'max', 'step']) {
      ...
      if (!numberValue(draftValue.type, config[key]).ok)
        return fail('binding', `Number field ${key} does not satisfy its registered semantic type.`);
    }
  }
  if (binding.action !== undefined) {
    if (!versionRef(binding.action.action) || !jsonRecord(binding.action.input))
  ...
  const copied = copyJson({
    ...
    ...(range === undefined ? {} : { range: { start: start!.value, end: end!.value } }),
...
function manifestFor(ref: AeliqoInputRef, bindings: AeliqoInputBindings): PresentationManifest {
  ...
  const operations =
    ref.id === 'input.form'
      ? entries.flatMap((entry) => (entry.action === undefined ? [] : [entry.action.action]))
      : [];
  ...
  return {
    ...
    resolveConfig(values) {
      ...
      return {
        ok: true,
        value: {
          values: output as PresentationValues,
          fields: [],
...
export function createInputPresentationManifests(
  ...
  return {
    ok: true,
    value: Object.freeze(Object.values(AELIQO_INPUT_REFS).map((ref) => Object.freeze(manifestFor(ref, copied.value)))),
  };
}
