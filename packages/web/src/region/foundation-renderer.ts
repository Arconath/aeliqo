...
export function renderFoundationNode(
  node: Node,
  ...
  const action = (event: AeliqoActionEvent) => {
    ...
    emit(node, 'action', {
      kind: 'action-request',
      action: v.action as Extract<InteractionPayload, { kind: 'action-request' }>['action'],
      input: v.actionInput as Extract<InteractionPayload, { kind: 'action-request' }>['input'],
    });
  };
  const navigate = (event: AeliqoLinkEvent) => {
    ...
    emit(node, 'navigate', {
      kind: 'navigate',
      route: v.route as Extract<InteractionPayload, { kind: 'navigate' }>['route'],
      params: v.params as Extract<InteractionPayload, { kind: 'navigate' }>['params'],
    });
  };
  switch (node.manifest.id as AeliqoFoundationId) {
    case 'foundation.button':
      return html`<aeliqo-button
        data-aeliqo-node-id=${id}
        data-aeliqo-theme="inherit"
        .label=${text(v.text)}
        .variant=${v.variant ?? 'solid'}
        .size=${v.size ?? 'medium'}
        type="button"
        @aeliqo-action=${action}
      ></aeliqo-button>`;
    case 'foundation.icon-button':
    ...
    case 'foundation.link':
      return html`<aeliqo-link
        ...
        @aeliqo-link=${navigate}
      ></aeliqo-link>`;
    case 'foundation.text':
    ...
    case 'foundation.avatar':
      return html`<aeliqo-avatar
        ...
        .alt=${text(v.alt)}
        .size=${v.size ?? 'medium'}
        .decorative=${bool(v.decorative)}
      ></aeliqo-avatar>`;
    case 'foundation.separator':
    ...
    case 'foundation.stack':
      return html`<aeliqo-stack
        ...
        .gap=${v.gap ?? 16}
        .align=${v.align ?? 'stretch'}
        .justify=${v.justify ?? 'start'}
        .wrap=${bool(v.wrap)}
        >${children()}</aeliqo-stack
      >`;
    case 'foundation.grid':
      return html`<aeliqo-grid
        ...
        .columns=${v.columns ?? 2}
        .gap=${v.gap ?? 16}
        .minItem=${v.minItem ?? 'medium'}
        >${children()}</aeliqo-grid
      >`;
    case 'foundation.scroll-area':
      return html`<aeliqo-scroll-area
        data-aeliqo-node-id=${id}
        data-aeliqo-theme="inherit"
        .axis=${v.axis ?? 'y'}
        .label=${text(v.label)}
        >${children()}</aeliqo-scroll-area
      >`;
    case 'foundation.split-pane':
      return html`<aeliqo-split-pane
        data-aeliqo-node-id=${id}
        data-aeliqo-theme="inherit"
        .orientation=${v.orientation ?? 'horizontal'}
        .defaultPosition=${v.position ?? 50}
        .min=${v.min ?? 20}
        .max=${v.max ?? 80}
        .step=${v.step ?? 5}
        ><div slot="start">${node.node.children[0] === undefined ? nothing : child(node.node.children[0])}</div>
        <div slot="end">
          ${node.node.children[1] === undefined ? nothing : child(node.node.children[1])}
        </div></aeliqo-split-pane
      >`;
    default:
      return undefined;
  }
