...
const EMPTY_STATE_KINDS: readonly AeliqoEmptyStateKind[] = [
  'no-records',
  'no-matches',
  'forbidden',
  'loading',
...
function normalizeBreadcrumbs(
  ...
  if (!Array.isArray(value) || value.length > MAX_ITEMS)
    return fail('bindings', 'breadcrumbs must be a bounded array.');
  const seen = new Set<string>();
  ...
  for (const item of value) {
    ...
    if (
      candidate === undefined ||
      !exactKeys(candidate, ['id', 'labelRef', 'items']) ||
      !bounded(candidate.id) ||
      seen.has(candidate.id) ||
      !requireContent(contents, candidate.labelRef) ||
    ...
    for (const rawItem of candidate.items) {
      ...
      if (
        itemValue === undefined ||
        !exactKeys(itemValue, ['id', 'labelRef', 'routeRef', 'current']) ||
        !bounded(itemValue.id) ||
        itemIds.has(itemValue.id) ||
        !requireContent(contents, itemValue.labelRef) ||
      ...
      items.push({
        ...
        ...(itemValue.current === undefined ? {} : { current: itemValue.current as boolean }),
      });
    }
    seen.add(candidate.id);
...
function normalizeMenus(
  ...
  for (const item of value) {
    ...
    for (const rawItem of candidate.items) {
      ...
      if (
        itemValue === undefined ||
        !exactKeys(itemValue, ['id', 'labelRef', 'actionRef', 'routeRef', 'disabled']) ||
        !bounded(itemValue.id) ||
        itemIds.has(itemValue.id) ||
        !requireContent(contents, itemValue.labelRef) ||
        hasAction === hasRoute ||
        (hasAction && !requireAction(actions, itemValue.actionRef)) ||
      ...
      items.push({
        id,
        labelRef,
        ...(hasAction && actionRef !== undefined ? { actionRef } : {}),
        ...(hasRoute && routeRef !== undefined ? { routeRef } : {}),
        ...(itemValue.disabled === undefined ? {} : { disabled: itemValue.disabled as boolean }),
      });
    }
    seen.add(candidate.id);
    result.push({ id: candidate.id as string, labelRef: candidate.labelRef as string, items });
...
function normalizePagination(
  ...
  for (const item of value) {
    ...
    if (
      ...
      seen.has(candidate.id) ||
      !requireContent(contents, candidate.labelRef) ||
      !bounded(candidate.outputId) ||
      !bounded(candidate.queryDigest) ||
      !Number.isSafeInteger(candidate.page) ||
      (candidate.page as number) < 1 ||
      (candidate.page as number) > MAX_PAGE ||
      (candidate.pageCount !== undefined &&
        (!Number.isSafeInteger(candidate.pageCount) ||
          (candidate.pageCount as number) < (candidate.page as number) ||
          (candidate.pageCount as number) > MAX_PAGE)) ||
      ...
      candidate.cursors.length > MAX_ITEMS
    )
    ...
    for (const rawCursor of candidate.cursors) {
      ...
      if (
        cursor === undefined ||
        !exactKeys(cursor, ['page', 'cursor']) ||
        !Number.isSafeInteger(cursor.page) ||
        (cursor.page as number) < 1 ||
        (cursor.page as number) > MAX_PAGE ||
        cursorPages.has(cursor.page as number) ||
        !bounded(cursor.cursor, MAX_TEXT)
      )
    ...
    if (
      (candidate.hasNext && !cursorPages.has(nextPage)) ||
      (candidate.hasPrevious && previousPage > 0 && !cursorPages.has(previousPage))
    )
      return fail('bindings', 'Pagination movement requires a host cursor for every enabled adjacent page.');
    seen.add(candidate.id);
    result.push({
      id: candidate.id as string,
      labelRef: candidate.labelRef as string,
      outputId: candidate.outputId as string,
      queryDigest: candidate.queryDigest as string,
      page,
      ...(pageCount === undefined ? {} : { pageCount }),
      hasPrevious: candidate.hasPrevious as boolean,
      hasNext: candidate.hasNext as boolean,
      ...(candidate.pending === undefined ? {} : { pending: candidate.pending as boolean }),
      cursors,
    });
  }
  return { ok: true, value: result };
...
function normalizeTabs(
  ...
  for (const item of value) {
    ...
    for (const rawItem of candidate.items) {
      ...
      if (
        tab === undefined ||
        !exactKeys(tab, ['id', 'labelRef', 'contentRef', 'disabled']) ||
        !bounded(tab.id) ||
        itemIds.has(tab.id) ||
        !requireContent(contents, tab.labelRef) ||
        (tab.contentRef !== undefined && !requireContent(contents, tab.contentRef)) ||
        (tab.disabled !== undefined && typeof tab.disabled !== 'boolean')
      )
    ...
    if (!items.some((tab) => tab.disabled !== true))
...
function normalizeTrees(
  ...
  for (const item of value) {
    ...
    if (
      candidate === undefined ||
      !exactKeys(candidate, ['id', 'labelRef', 'nodes']) ||
      !bounded(candidate.id) ||
      seenTrees.has(candidate.id) ||
      !requireContent(contents, candidate.labelRef) ||
    ...
    const walk = (rawNodes: readonly unknown[]): Outcome<readonly AeliqoTreeBindingNode[]> => {
      ...
      for (const rawNode of rawNodes) {
        ...
        if (
          node === undefined ||
          !exactKeys(node, ['id', 'labelRef', 'children', 'disabled', 'actionRef', 'routeRef']) ||
          !bounded(node.id) ||
          nodeIds.has(node.id) ||
          !requireContent(contents, node.labelRef) ||
        ...
        if (hasAction && !requireAction(actions, node.actionRef))
          return fail('bindings', 'Tree nodes may reference only registered actions.');
        if (hasRoute && !requireRoute(routes, node.routeRef))
          return fail('bindings', 'Tree nodes may reference only registered routes.');
        ...
        nodes.push({
          ...
          ...(children === undefined ? {} : { children: children.value }),
          ...(node.disabled === undefined ? {} : { disabled: node.disabled as boolean }),
          ...(hasAction && typeof node.actionRef === 'string' ? { actionRef: node.actionRef } : {}),
          ...(hasRoute && typeof node.routeRef === 'string' ? { routeRef: node.routeRef } : {}),
        });
      }
...
function normalizeFeedback(
  ...
  for (const item of value) {
    ...
    if (
      ...
      return fail('bindings', 'Feedback bindings require unique bounded IDs.');
    for (const key of ['labelRef', 'contentRef', 'headingRef', 'messageRef', 'actionLabelRef'] as const) {
      if (candidate[key] !== undefined && !requireContent(contents, candidate[key]))
        return fail('bindings', 'Feedback text must use registered content references.');
    }
    if (candidate.actionRef !== undefined && !requireAction(actions, candidate.actionRef))
      return fail('bindings', 'Feedback actions must use registered action references.');
    if (
      candidate.kind !== undefined &&
      (typeof candidate.kind !== 'string' || !EMPTY_STATE_KINDS.includes(candidate.kind as AeliqoEmptyStateKind))
    )
      return fail('bindings', 'Empty-state kind is not supported.');
    if (
      candidate.progressValue !== undefined &&
      (typeof candidate.progressValue !== 'number' ||
        !Number.isFinite(candidate.progressValue) ||
        candidate.progressValue < 0)
    )
      return fail('bindings', 'Progress values must be finite and nonnegative.');
    if (
      candidate.progressMax !== undefined &&
      (typeof candidate.progressMax !== 'number' ||
        !Number.isFinite(candidate.progressMax) ||
        candidate.progressMax <= 0)
    )
      return fail('bindings', 'Progress max must be finite and positive.');
    if (
      candidate.progressValue !== undefined &&
      candidate.progressMax !== undefined &&
      candidate.progressValue > candidate.progressMax
    )
      return fail('bindings', 'Progress value cannot exceed its host max.');
    seen.add(candidate.id);
    result.push({
      id: candidate.id as string,
      ...(candidate.labelRef === undefined ? {} : { labelRef: candidate.labelRef as string }),
      ...(candidate.contentRef === undefined ? {} : { contentRef: candidate.contentRef as string }),
      ...(candidate.headingRef === undefined ? {} : { headingRef: candidate.headingRef as string }),
      ...(candidate.messageRef === undefined ? {} : { messageRef: candidate.messageRef as string }),
      ...(candidate.actionLabelRef === undefined ? {} : { actionLabelRef: candidate.actionLabelRef as string }),
      ...(candidate.actionRef === undefined ? {} : { actionRef: candidate.actionRef as string }),
      ...(candidate.kind === undefined ? {} : { kind: candidate.kind as AeliqoEmptyStateKind }),
      ...(candidate.progressValue === undefined ? {} : { progressValue: candidate.progressValue as number }),
      ...(candidate.progressMax === undefined ? {} : { progressMax: candidate.progressMax as number }),
    });
  }
  return { ok: true, value: result };
}
...
function copyBindings(input: AeliqoNavigationFeedbackBindings | undefined): Outcome<AeliqoNavigationFeedbackBindings> {
  ...
  let value: unknown;
  try {
    value = JSON.parse(JSON.stringify(parsed.value));
  } catch {
  ...
  if (candidate === undefined || !exactKeys(candidate, allowed) || !bounded(candidate.revision))
  ...
  const breadcrumbs = normalizeBreadcrumbs(candidate.breadcrumbs, contentIndex, routeIndex);
  if (!breadcrumbs.ok) return breadcrumbs;
  const menus = normalizeMenus(candidate.menus, contentIndex, routeIndex, actionIndex);
  if (!menus.ok) return menus;
  const pagination = normalizePagination(candidate.pagination, contentIndex);
  if (!pagination.ok) return pagination;
  const tabs = normalizeTabs(candidate.tabs, contentIndex);
  if (!tabs.ok) return tabs;
  const trees = normalizeTrees(candidate.trees, contentIndex, routeIndex, actionIndex);
  if (!trees.ok) return trees;
  const feedback = normalizeFeedback(candidate.feedback, contentIndex, actionIndex);
...
function breadcrumbConfig(
  values: PresentationValues,
  bindings: AeliqoNavigationFeedbackBindings,
): Outcome<ReturnType<typeof resolved> extends Outcome<infer T> ? T : never> {
  ...
  const routes = routeMap(bindings);
  ...
  if (!label.ok) return label;
  return resolved(
    { ...baseValues(bindings, entry.id), label: label.value!, items },
    [],
    operations.length === 0 ? [] : [navPort('navigate', 'navigate')],
...
function menuConfig(
  ...
  const contents = contentMap(bindings);
  const routes = routeMap(bindings);
  const actions = actionMap(bindings);
  const items: RecordValue[] = [];
  const operations: VersionRef[] = [];
  let hasRoute = false;
  let hasAction = false;
  for (const item of entry.items) {
    ...
    if (item.actionRef !== undefined && action === undefined)
      return fail('binding', 'Menu actionRef is not registered.');
    if (item.disabled !== true) {
      if (route !== undefined) {
        operations.push(route.route);
        hasRoute = true;
      }
      if (action !== undefined) {
        operations.push(action.action);
        hasAction = true;
      }
    }
    items.push({
      id: item.id,
      label: label.value!,
      ...(route === undefined || item.disabled === true ? {} : routeValue(route)),
      ...(action === undefined || item.disabled === true ? {} : actionValue(action)),
...
function tabsConfig(
  ...
  if (
    node !== undefined &&
    (node.children.length > entry.items.length ||
      node.children.some((_, index) => entry.items[index]?.disabled === true))
  )
  ...
  for (const key of ['value', 'defaultValue'] as const)
    if (
      input[key] !== undefined &&
      (!bounded(input[key]) || !entry.items.some((item) => item.id === input[key] && item.disabled !== true))
    )
      return fail('config', `${key} must identify an enabled item in the registered tab set.`);
  if (input.activation !== undefined && input.activation !== 'automatic' && input.activation !== 'manual')
    return fail('config', 'Tabs activation must be automatic or manual.');
  if (input.orientation !== undefined && input.orientation !== 'horizontal' && input.orientation !== 'vertical')
    return fail('config', 'Tabs orientation must be horizontal or vertical.');
  const contents = contentMap(bindings);
  const items: RecordValue[] = [];
  for (const item of entry.items) {
  ...
  if (out.idPrefix !== undefined && !bounded(out.idPrefix))
...
function treeConfig(
  ...
  let hasRoute = false;
  let hasAction = false;
  const renderNodes = (source: readonly AeliqoTreeBindingNode[], reachable = true): RecordValue[] =>
    source.map((item) => {
      const available = reachable && item.disabled !== true;
      const childrenReachable =
        reachable &&
        (item.disabled !== true || (Array.isArray(input.expandedIds) && input.expandedIds.includes(item.id)));
      ids.add(item.id);
      const route = item.routeRef === undefined ? undefined : routes.get(item.routeRef);
      const action = item.actionRef === undefined ? undefined : actions.get(item.actionRef);
      if (available) {
      ...
      return {
        id: item.id,
        label,
        ...(item.children === undefined ? {} : { children: renderNodes(item.children, childrenReachable) }),
        ...(item.disabled === undefined ? {} : { disabled: item.disabled }),
  ...
  if (
    input.expandedIds !== undefined &&
    (!Array.isArray(input.expandedIds) ||
      input.expandedIds.length > MAX_TREE_NODES ||
      input.expandedIds.some((id) => typeof id !== 'string' || !ids.has(id)) ||
      new Set(input.expandedIds).size !== input.expandedIds.length)
  )
    return fail('config', 'expandedIds must name unique registered tree nodes.');
  if (input.selectedId !== undefined && (typeof input.selectedId !== 'string' || !ids.has(input.selectedId)))
    return fail('config', 'selectedId must name a registered tree node.');
  const label = content(contents, entry.labelRef, 'tree label');
  if (!label.ok) return label;
  return resolved(
    {
      ...baseValues(bindings, entry.id),
      label: label.value!,
      nodes,
      ...(input.expandedIds === undefined ? {} : { expandedIds: input.expandedIds }),
...
function feedbackConfig(
  values: PresentationValues,
  kind: keyof typeof AELIQO_NAVIGATION_FEEDBACK_REFS,
  bindings: AeliqoNavigationFeedbackBindings,
  node?: PresentationNode,
): Outcome<ReturnType<typeof resolved> extends Outcome<infer T> ? T : never> {
  ...
  switch (kind) {
    case 'tooltip': {
      ...
      if (checked.value.input.open !== undefined && typeof checked.value.input.open !== 'boolean')
        return fail('config', 'Tooltip open must be boolean.');
      return resolved({
        ...baseValues(bindings, checked.value.entry.id),
        label: label.value!,
        content: body.value!,
        open: checked.value.input.open ?? false,
      });
    }
    case 'popover': {
      const checked = feedbackBase(
        values,
        ['bindingRevision', 'bindingRef', 'open', 'modal', 'closeOnOutside'],
        bindings,
      );
      if (!checked.ok) return checked;
      ...
      if (!body.ok) return body;
      for (const key of ['open', 'modal', 'closeOnOutside'] as const)
        if (checked.value.input[key] !== undefined && typeof checked.value.input[key] !== 'boolean')
          return fail('config', `Popover ${key} must be boolean.`);
      return resolved({
        ...baseValues(bindings, checked.value.entry.id),
        label: label.value!,
        content: body.value!,
        open: checked.value.input.open ?? false,
        modal: checked.value.input.modal ?? false,
        closeOnOutside: checked.value.input.closeOnOutside ?? true,
      });
    }
    case 'dialog': {
      const checked = feedbackBase(
        values,
        ['bindingRevision', 'bindingRef', 'open', 'modal', 'closeOnEscape'],
        bindings,
      );
      if (!checked.ok) return checked;
      const heading = content(checked.value.contents, checked.value.entry.headingRef, 'dialog heading');
      if (!heading.ok) return heading;
      for (const key of ['open', 'modal', 'closeOnEscape'] as const)
        if (checked.value.input[key] !== undefined && typeof checked.value.input[key] !== 'boolean')
          return fail('config', `Dialog ${key} must be boolean.`);
      const open = checked.value.input.open ?? false;
      if (node !== undefined && node.children.length > 0 && !open)
        return fail('children', 'A closed dialog cannot claim child content before its host opens it.');
      return resolved({
        ...baseValues(bindings, checked.value.entry.id),
        heading: heading.value!,
        open,
        modal: checked.value.input.modal ?? true,
        closeOnEscape: checked.value.input.closeOnEscape ?? true,
      });
    }
    case 'drawer': {
      ...
      if (!heading.ok) return heading;
      if (checked.value.input.open !== undefined && typeof checked.value.input.open !== 'boolean')
        return fail('config', 'Drawer open must be boolean.');
      if (
        checked.value.input.mode !== undefined &&
        checked.value.input.mode !== 'inline' &&
        checked.value.input.mode !== 'modal'
      )
        return fail('config', 'Drawer mode must be inline or modal.');
      if (
        checked.value.input.side !== undefined &&
        checked.value.input.side !== 'start' &&
        checked.value.input.side !== 'end'
      )
        return fail('config', 'Drawer side must be start or end.');
      const open = checked.value.input.open ?? false;
      if (node !== undefined && node.children.length > 0 && !open)
    ...
    case 'toast': {
      ...
      if (checked.value.input.open !== undefined && typeof checked.value.input.open !== 'boolean')
        return fail('config', 'Toast open must be boolean.');
      if (
        checked.value.input.tone !== undefined &&
        (typeof checked.value.input.tone !== 'string' ||
          !FEEDBACK_TONES.includes(checked.value.input.tone as (typeof FEEDBACK_TONES)[number]))
      )
        return fail('config', 'Toast tone is unsupported.');
      if (
        checked.value.input.duration !== undefined &&
        (typeof checked.value.input.duration !== 'number' ||
          !Number.isSafeInteger(checked.value.input.duration) ||
          checked.value.input.duration < 0 ||
          checked.value.input.duration > 60_000)
      )
        return fail('config', 'Toast duration must be a bounded nonnegative integer.');
      if (checked.value.input.dismissible !== undefined && typeof checked.value.input.dismissible !== 'boolean')
        return fail('config', 'Toast dismissible must be boolean.');
      return resolved({
        ...baseValues(bindings, checked.value.entry.id),
        ...
        dismissible: checked.value.input.dismissible ?? true,
      });
    }
    case 'alert': {
      const checked = feedbackBase(values, ['bindingRevision', 'bindingRef', 'open', 'tone', 'dismissible'], bindings);
      if (!checked.ok) return checked;
      const heading = content(checked.value.contents, checked.value.entry.headingRef, 'alert heading', false);
      if (!heading.ok) return heading;
      const message = content(checked.value.contents, checked.value.entry.messageRef, 'alert message');
      if (!message.ok) return message;
      const actionLabel = content(
        checked.value.contents,
        checked.value.entry.actionLabelRef,
        'alert action label',
        false,
      );
      if (!actionLabel.ok) return actionLabel;
      if (checked.value.entry.actionLabelRef !== undefined && checked.value.entry.actionRef === undefined)
        return fail('binding', 'An alert action label requires a registered action.');
      if (checked.value.entry.actionRef !== undefined && checked.value.entry.actionLabelRef === undefined)
        return fail('binding', 'An alert action requires a host action label.');
      if (checked.value.input.open !== undefined && typeof checked.value.input.open !== 'boolean')
        return fail('config', 'Alert open must be boolean.');
      if (
        checked.value.input.tone !== undefined &&
        (typeof checked.value.input.tone !== 'string' ||
          !FEEDBACK_TONES.includes(checked.value.input.tone as (typeof FEEDBACK_TONES)[number]))
      )
        return fail('config', 'Alert tone is unsupported.');
      if (checked.value.input.dismissible !== undefined && typeof checked.value.input.dismissible !== 'boolean')
        return fail('config', 'Alert dismissible must be boolean.');
      const action =
        checked.value.entry.actionRef === undefined
          ? undefined
          : checked.value.actions.get(checked.value.entry.actionRef);
      if (checked.value.entry.actionRef !== undefined && action === undefined)
        return fail('binding', 'Alert actionRef is not registered.');
      ...
      return resolved(
        {
          ...
          message: message.value!,
          ...(actionLabel.value === undefined || !actionReachable ? {} : { actionLabel: actionLabel.value }),
          ...(actionReachable ? actionValue(action) : {}),
          open,
          tone: checked.value.input.tone ?? 'info',
          dismissible: checked.value.input.dismissible ?? false,
        },
        [],
        actionReachable ? [navPort('action', 'action-request')] : [],
        actionReachable ? [action!.action] : [],
      );
    }
    case 'progress': {
      ...
      return resolved({
        ...baseValues(bindings, checked.value.entry.id),
        label: label.value!,
        ...(checked.value.entry.progressValue === undefined
          ? {}
          : { progressValue: checked.value.entry.progressValue }),
    ...
    case 'skeleton': {
      ...
      if (lines !== undefined && (!Number.isSafeInteger(lines) || (lines as number) < 1 || (lines as number) > 12))
        return fail('config', 'Skeleton lines must be between one and twelve.');
      if (
        checked.value.input.variant !== undefined &&
        !['text', 'rect', 'circle'].includes(checked.value.input.variant as string)
      )
        return fail('config', 'Skeleton variant is unsupported.');
      if (checked.value.input.animated !== undefined && typeof checked.value.input.animated !== 'boolean')
        return fail('config', 'Skeleton animated must be boolean.');
      return resolved({
        ...baseValues(bindings, checked.value.entry.id),
        label: label.value!,
        lines: (lines as number | undefined) ?? 3,
    ...
    case 'emptyState': {
      const checked = feedbackBase(values, ['bindingRevision', 'bindingRef'], bindings);
      if (!checked.ok) return checked;
      const heading = content(checked.value.contents, checked.value.entry.headingRef, 'empty-state heading');
      if (!heading.ok) return heading;
      const message = content(checked.value.contents, checked.value.entry.messageRef, 'empty-state message');
      if (!message.ok) return message;
      const actionLabel = content(
        checked.value.contents,
        checked.value.entry.actionLabelRef,
      ...
      if (!actionLabel.ok) return actionLabel;
      if (checked.value.entry.actionLabelRef !== undefined && checked.value.entry.actionRef === undefined)
        return fail('binding', 'An empty-state action label requires a registered action.');
      if (checked.value.entry.actionRef !== undefined && checked.value.entry.actionLabelRef === undefined)
        return fail('binding', 'An empty-state action requires a host action label.');
      if (checked.value.entry.kind === undefined || !EMPTY_STATE_KINDS.includes(checked.value.entry.kind))
        return fail('binding', 'Empty-state bindings require a truthful kind.');
      const action =
        checked.value.entry.actionRef === undefined
          ? undefined
          : checked.value.actions.get(checked.value.entry.actionRef);
      if (checked.value.entry.actionRef !== undefined && action === undefined)
        return fail('binding', 'Empty-state actionRef is not registered.');
      return resolved(
        {
          ...baseValues(bindings, checked.value.entry.id),
          kind: checked.value.entry.kind,
          heading: heading.value!,
          message: message.value!,
        ...
        [],
        action === undefined ? [] : [navPort('action', 'action-request')],
        action === undefined ? [] : [action.action],
      );
    }
    default:
      return fail('config', `Unsupported feedback manifest ${String(refs[kind]?.id ?? kind)}.`);
  }
}
...
export function createNavigationFeedbackPresentationManifests(
  ...
  const routeRefs = routes.map((route) => route.route);
  const actionRefs = actions.map((action) => action.action);
  ...
  const manifests: readonly PresentationManifest[] = [
    ...
    manifest(
      AELIQO_NAVIGATION_FEEDBACK_REFS.menu,
      'navigation',
      { min: 0, max: 0 },
      'leaf',
      dynamicNavigation,
      (values) => menuConfig(values, bindings),
    ),
    manifest(
