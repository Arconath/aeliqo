...
export function compilePlotUnit(
  ...
  const inspected = parseWireValue(inputRows);
  ...
  if (rows.length * descriptor.fields.length > 100_000)
    return fail('cell-budget', 'The loaded data exceeds the bounded cell budget.');
  if (rows.length !== descriptor.counts.loaded)
  ...
  for (const row of rows) {
    ...
    for (const fieldId of descriptor.identity) {
      ...
      const identity = scalarIdentity(values[fieldId], field.type);
      if (!identity.ok) return identity;
      identityParts.push(identity.value);
    }
    const identity = JSON.stringify(identityParts);
    if (keys.has(identity)) return fail('identity', 'Plot row identities must be unique.');
    keys.add(identity);
    data.push({ identity, values });
  }
  if (
    projection.identities &&
    (projection.identities.length > maxRows || projection.identities.some((id) => !keys.has(id)))
  )
    return fail('projection', 'The display projection references unavailable rows.');
  if (projection.domains && (projection.domains.x.length > 10_000 || projection.domains.y.length > 10_000))
    return fail('domain-budget', 'The display scale domain exceeds its value budget.');
  ...
  const dataOnly = (reason: string): Outcome<PlotGeometry> => ({
    ok: true,
    value: {
      state: 'data-only',
      reason,
  ...
  const label = (id: string) => {
    ...
    return `${f.label}${f.type.unit ? ` (${f.type.unit.symbol})` : ''}`;
  };
  try {
    const heatmapAxisLeft = 216;
    // A heatmap needs room for its categorical y labels plus a nonzero plot.
    // Preserve the exact table instead of emitting geometry outside the SVG.
    if (family === 'heatmap' && width < heatmapAxisLeft + 24 + 64)
      return dataOnly('Heatmap graphics require at least 304 CSS pixels; exact data is available below.');
    const axisLeft = family === 'heatmap' ? heatmapAxisLeft : 64;
    const x = makePlotScale(
      e.x,
      fields.get(e.x.field)!.type,
      projection.domains?.x ??
        displayed.flatMap((d) => [d.values[e.x.field]!, ...(e.x2 ? [d.values[e.x2.field]!] : [])]),
      [axisLeft, width - 24],
    );
    if (e.size && normalizedUnit.mark !== 'point' && normalizedUnit.mark !== 'line')
      return dataOnly('Size is supported only for point marks.');
    if ((e.x2 || e.y2) && normalizedUnit.mark !== 'rect' && normalizedUnit.mark !== 'link')
      return dataOnly('Second endpoints require rect or link marks.');
    const colorScale =
      e.color && e.color.scale !== 'ordinal'
        ? makePlotScale(
            e.color,
            fields.get(e.color.field)!.type,
            displayed.map((d) => d.values[e.color!.field]!),
            [0, 1],
          )
        : undefined;
    if (e.color && e.color.scale !== 'ordinal' && ['line', 'area'].includes(normalizedUnit.mark))
      return dataOnly(
        'Quantitative color is not representable on a single line or area path. Use an ordinal series or the exact data table.',
      );
    if (e.color && e.color.scale !== 'ordinal' && displayed.some((d) => d.values[e.color!.field] === null))
      return dataOnly('Quantitative color requires a value for every displayed mark.');
    const colorAt = (value: Scalar): string | undefined => {
      if (colorScale === undefined || value === null) return undefined;
      const position = colorScale.at(value);
      if (position === undefined) return undefined;
      const first = colorScale.ticks[0]?.position ?? 0;
      const last = colorScale.ticks[colorScale.ticks.length - 1]?.position ?? 1;
    ...
    for (const d of displayed) {
      const parts: string[] = [];
      for (const encoding of [e.series, ...(e.color?.scale === 'ordinal' ? [e.color] : [])])
        if (encoding) {
      ...
      legend.set(
        id,
        [e.series, ...(e.color?.scale === 'ordinal' ? [e.color] : [])]
          .filter((v) => v !== undefined)
          .map((v) => `${fields.get(v.field)!.label}: ${exactLabel(d.values[v.field]!)}`)
          .join(', '),
      );
    ...
    if (family !== undefined) {
      const requiredFields = [
        ...
        ...(e.x2 ? [e.x2.field] : []),
        ...(e.y2 ? [e.y2.field] : []),
        ...(family === 'heatmap' && e.color ? [e.color.field] : []),
      ];
      for (const datum of data) {
        const missing = requiredFields.some(
          (field) => datum.values[field] === null || datum.values[field] === undefined,
        );
        const allowedGap =
          (family === 'trend' || family === 'area') &&
          datum.values[e.x.field] !== null &&
          datum.values[e.x.field] !== undefined &&
          datum.values[e.y.field] === null;
        if (missing && !allowedGap)
          return dataOnly('A Cartesian mark requires non-missing encoded dimensions and measures.');
      }
    }
    ...
    const addNumeric = (left: Scalar, right: Scalar): Scalar => {
      if (yType.value === 'decimal') {
        const a = decimalParts((left as { decimal: string }).decimal),
          b = decimalParts((right as { decimal: string }).decimal);
      ...
      if (!Number.isFinite(sum) || (yType.value === 'integer' && !Number.isSafeInteger(sum)))
      ...
      for (const group of series.values()) {
        ...
        for (const datum of group) {
          ...
          if (seenX.has(xIdentity.value))
            return dataOnly('Stacked area series must contain one value per temporal grain.');
          seenX.add(xIdentity.value);
          if (value === null || value === undefined) continue;
          const existing = offsets.get(xIdentity.value) ?? { positive: zero, negative: zero };
          const sign = compareScalars(value, zero, yType);
          if (!sign.ok || sign.value === null)
            return dataOnly('Stacked area magnitudes must be ordered numeric values.');
          ...
          offsets.set(
            xIdentity.value,
            negative ? { positive: existing.positive, negative: top } : { positive: top, negative: existing.negative },
          );
          stackExtras.push(top);
        }
        if (cohort === undefined) cohort = seenX;
        else if (cohort.size !== seenX.size || [...cohort].some((key) => !seenX.has(key)))
          return dataOnly('Stacked area series must share the same temporal cohorts.');
      }
    }
    if (family === 'histogram') {
      ...
      for (const datum of data) {
        const start = datum.values[e.x.field]!,
          end = datum.values[e.x2!.field]!,
          value = datum.values[e.y.field]!,
          baseline = datum.values[e.y2!.field]!;
        if (start === null || end === null || value === null || baseline === null)
          return dataOnly('Histogram bins require non-missing endpoints, values and a zero baseline.');
        const positive = compareScalars(start, end, startType);
        if (!positive.ok || positive.value === null || positive.value >= 0)
          return dataOnly('Histogram bins must have positive width.');
        const zeroBaseline = compareScalars(baseline, zero, baselineType);
        if (!zeroBaseline.ok || zeroBaseline.value !== 0)
          return dataOnly('Histogram bins must use an explicit zero baseline.');
        const nonnegative = compareScalars(value, zero, valueType);
        if (!nonnegative.ok || nonnegative.value === null || nonnegative.value < 0)
          return dataOnly('Histogram counts and densities cannot be negative.');
        if (previousStart !== undefined) {
          const ordered = compareScalars(previousStart, start, startType);
          if (!ordered.ok || ordered.value === null || ordered.value >= 0)
            return dataOnly('Histogram bins must be ordered without duplicate starts.');
        }
        if (previousEnd !== undefined) {
          const disjoint = compareScalars(previousEnd, start, startType);
          if (!disjoint.ok || disjoint.value === null || disjoint.value > 0)
            return dataOnly('Histogram bins must not overlap.');
        }
        previousStart = start;
        previousEnd = end;
      }
    ...
    const needsZero =
      ['bar', 'area', 'histogram'].includes(family ?? '') ||
      normalizedUnit.mark === 'bar' ||
      normalizedUnit.mark === 'area';
    const zeroY = needsZero ? y.at(zero) : undefined;
    if (needsZero && zeroY === undefined)
      return dataOnly('The quantitative scale cannot represent its required zero baseline.');
    const baselineY = zeroY ?? 0;
    // Quantitative color and size require explicit legend geometry; never silently ignore them.
    const size = e.size
      ? makePlotScale(
          e.size,
          fields.get(e.size.field)!.type,
          displayed.map((d) => d.values[e.size!.field]!),
          [3, 12],
        )
      : undefined;
    ...
    const actualXPositions = [
      ...new Set(
        displayed
          .map((d) => x.at(d.values[e.x.field]!))
          .filter((position): position is number => position !== undefined),
      ),
    ];
    const actualYPositions = [
      ...new Set(
        displayed
          .map((d) => y.at(d.values[e.y.field]!))
          .filter((position): position is number => position !== undefined),
      ),
    ];
    ...
    const cellWidth = Math.max(
      1,
      Math.min(160, minStep(actualXPositions, plotWidth / Math.max(1, actualXPositions.length)) * 0.9),
    );
    const cellHeight = Math.max(
      1,
      Math.min(96, minStep(actualYPositions, (height - 72) / Math.max(1, actualYPositions.length)) * 0.9),
    );
    ...
    if (normalizedUnit.mark === 'bar') {
      ...
      for (const [groupId, group] of series)
        for (const datum of group) {
          ...
          if (entries.some((entry) => entry.group === groupId))
            return dataOnly('Bar rows must contain one value per x and series grain.');
          entries.push({ group: groupId, datum });
          byX.set(identity.value, entries);
        }
      const xStep = minStep(
        [...byX.values()]
          .map((entries) => x.at(entries[0]!.datum.values[e.x.field]!)!)
          .filter((position): position is number => position !== undefined),
        0,
      );
      if (byX.size > 1 && xStep < 1)
        return dataOnly(
          'Bar categories are too dense for distinct marks at this width; exact data is available below.',
        );
      for (const entries of byX.values()) {
        ...
        if (!Number.isFinite(barWidth) || barWidth < 1)
          return dataOnly('Bar groups are too dense for distinct marks at this width; exact data is available below.');
        ...
        entries.forEach((entry, index) =>
          barLayout.set(entry.datum.identity, { center: start + index * barWidth, width: barWidth }),
        );
      }
    }
    ...
    const xPosition = (d: PlotDatum): number => {
      const position = px(d);
      if (position === undefined) throw Error('Missing x position');
      return position;
    };
    const yPosition = (d: PlotDatum): number => {
      const position = py(d);
      ...
      if (normalizedUnit.mark === 'line' || normalizedUnit.mark === 'area') {
        if (e.x.scale !== 'ordinal') {
          ...
          for (const d of group) {
            ...
            const current = d.values[e.x.field]!;
            if (previous !== null) {
              const order = compareScalars(previous, current, fields.get(e.x.field)!.type);
              if (!order.ok || order.value === null || order.value > 0)
                return dataOnly('Line and area observations must be ordered by x; missing values remain gaps.');
            }
            previous = current;
          }
        }
        const path =
          normalizedUnit.mark === 'line'
            ? line<PlotDatum>()
                .defined(defined)
                .x((d) => px(d)!)
                .y(yPosition)(group)
            : stack === 'zero'
              ? area<PlotDatum>()
                  .defined((d) => defined(d) && stackCoordinates.has(d.identity))
                  .x((d) => px(d)!)
        ...
        if (path)
          marks.push({
            kind: 'path',
            path,
            filled: normalizedUnit.mark === 'area',
            series: groupId,
            identities: group.filter(defined).map((d) => d.identity),
          });
      }
      for (const d of group) {
        const x0 = px(d),
        ...
        if (normalizedUnit.mark === 'point' || normalizedUnit.mark === 'line') {
          ...
          marks.push({
            ...
            series: groupId,
            identity: d.identity,
            ...(markColor === undefined ? {} : { color: markColor }),
          });
          ...
          const barWidth =
            layout?.width ?? Math.max(1, Math.min(28, (plotWidth / Math.max(1, displayed.length)) * 0.7));
          ...
          if (normalizedUnit.mark === 'cell') {
            ...
            if (!xIdentity.ok) return xIdentity;
            if (!yIdentity.ok) return yIdentity;
            ...
            marks.push({
              kind: 'rect',
              x: markX - barWidth / 2,
              y: Math.min(y0, baseline),
              width: barWidth,
              height: Math.max(1, Math.abs(baseline - y0)),
              series: groupId,
              identity: d.identity,
              ...(markColor === undefined ? {} : { color: markColor }),
            });
          ...
          const x1 = x.at(d.values[e.x2!.field]!),
            y1 = y.at(d.values[e.y2!.field]!);
          if (x1 === undefined || y1 === undefined) continue;
          if (normalizedUnit.mark === 'rect')
            marks.push({
              kind: 'rect',
              x: Math.min(x0, x1),
              y: Math.min(y0, y1),
    ...
    if (displayed.length === 0 || marks.length === 0)
      return dataOnly(
        displayed.length === 0
          ? 'No displayed observations are available for the requested plot partition.'
          : 'No plottable observations are available; exact values are shown below.',
      );
    ...
    const colorPresentation =
      e.color && e.color.scale !== 'ordinal' && colorScale !== undefined
    ...
    return {
      ...
      value: {
        ...
        marks,
        rows: data,
        axes: { x, y, xLabel: label(e.x.field), yLabel: label(e.y.field) },
        ...colorPresentation,
        series: [...series.keys()],
        legend: [...legend].map(([id, label]) => ({ id, label })),
        result: descriptor,
      },
    };
  } catch {
    return dataOnly('The scale cannot safely represent these observations. Loaded values are available below.');
  }
}
