...
export function compilePlotComposition(
  ...
  for (const value of inspected.value) {
    ...
    if (d.result === null || typeof d.result !== 'object' || !Array.isArray(d.rows))
      return fail('A dataset requires a result reference and row array.');
    const id = key(d.result);
    if (byRef.has(id)) return fail('Dataset result references must be unique.');
    byRef.set(id, d.rows);
  }
  const prepared = new Map<PlotNode, Prepared>();
  let work = 0;
  for (const u of bound.value.units) {
    const raw = byRef.get(key(u.result.ref));
    if (raw === undefined) return fail('An exact result dataset is unavailable.');
    work += u.result.counts.loaded;
    if (work > 50_000) return fail('Composition exceeds its row-work budget.');
    const geometry = compilePlotUnit(u.node, u.result, raw, options);
    if (!geometry.ok) return geometry;
    prepared.set(u.node, { unit: u.node, result: u.result, rows: geometry.value.rows, raw });
  }
  const descendants = (node: PlotNode): Prepared[] =>
    node.kind === 'unit'
      ? [prepared.get(node)!]
      : node.kind === 'facet'
        ? descendants(node.child)
        : node.children.flatMap(descendants);
  const matches = (p: Prepared, row: PlotDatum, filters: readonly Filter[]) =>
    filters.every((filter) => {
      const field = p.result.fields.find((f) => f.id === filter.field);
      if (!field) return false;
      const id = scalarIdentity(row.values[filter.field], field.type);
      return id.ok && id.value === filter.key;
    });
  const domains = (node: PlotNode, filters: readonly Filter[]): PlotProjection['domains'] => {
    ...
    for (const p of descendants(node))
      for (const row of p.rows)
        if (matches(p, row, filters)) {
          const e = p.unit.encoding;
          x.push(row.values[e.x.field]!);
          y.push(row.values[e.y.field]!);
          if (e.x2) x.push(row.values[e.x2.field]!);
          if (e.y2) y.push(row.values[e.y2.field]!);
        }
    return { x, y };
  };
  let nodes = 0,
  ...
  const visit = (
    ...
    if (node.kind === 'unit') {
      ...
      if (pixels > 16_000_000) return fail('Composition exceeds its total display-pixel budget.');
      const p = prepared.get(node)!;
      const identities = p.rows.filter((row) => matches(p, row, filters)).map((row) => row.identity);
      work += p.rows.length;
      if (work > 100_000) return fail('Expanded plot composition exceeds its row-work budget.');
      const g = compilePlotUnit(node, p.result, p.raw, options, {
        identities,
        ...(inherited ? { domains: inherited } : {}),
      });
      if (g.ok) {
        marks += g.value.marks.reduce((n, m) => n + (m.kind === 'path' ? m.identities.length : 1), 0);
        if (marks > 100_000) return fail('Composition exceeds its total geometry budget.');
      }
      return g.ok ? { ok: true, value: { kind: 'unit', geometry: g.value, displayedIdentities: identities } } : g;
    }
    const shared =
      inherited ?? (node.kind !== 'concat' && node.scales === 'shared-compatible' ? domains(node, filters) : undefined);
    if (shared && (shared.x.length > 10_000 || shared.y.length > 10_000))
      return fail('Shared scale domains exceed their value budget.');
    if (node.kind === 'facet') {
      const groups = new Map<string, string>();
      for (const p of descendants(node.child))
        for (const row of p.rows)
          if (matches(p, row, filters)) {
            const field = p.result.fields.find((f) => f.id === node.field)!;
            const identity = scalarIdentity(row.values[node.field], field.type);
      ...
      for (const [group, label] of groups) {
        const child = visit(node.child, [...filters, { field: node.field, key: group }], shared);
        if (!child.ok) return child;
        children.push({ key: group, label, node: child.value });
      }
      return { ok: true, value: { kind: 'facet', field: node.field, scales: node.scales, children } };
    }
    const children: CompiledPlotNode[] = [];
    for (const childNode of node.children) {
      const child = visit(childNode, filters, shared);
      if (!child.ok) return child;
      children.push(child.value);
    }
    return node.kind === 'concat'
      ? { ok: true, value: { kind: 'concat', direction: node.direction, children } }
      : { ok: true, value: { kind: 'layer', scales: node.scales, children } };
  };
