import {contractJsonSchema, parseContract, parseIntent, parseWireValue, type Diagnostic, type OperationGrant, type Outcome} from '@aeliqo/core';
...
const record = (value: unknown): value is Readonly<Record<string, AgentJsonValue>> => value !== null && typeof value === 'object' && !Array.isArray(value);
const bounded = (value: unknown): value is string => typeof value === 'string' && value.length > 0 && value.length <= 160 && !/[\s\u0000-\u001f\u007f]/u.test(value);
...
function toolSchema(value: unknown): Readonly<Record<string, AgentJsonValue>> {
  const parsed = parseWireValue(value);
  if (!parsed.ok || !record(parsed.value)) throw new TypeError('A standard agent tool schema must be a bounded JSON object.');
  return parsed.value;
}

function renderValue(receipt: RuntimeRenderReceipt | {readonly status: 'renderer-ready' | 'unsupported' | 'failed' | 'cancelled' | 'needs-input'; readonly requestId: string; readonly regionId: string; readonly runtime: Extract<RuntimeRenderReceipt, {readonly status: 'committed'}>; readonly diagnostics: readonly Diagnostic[]}): AgentJsonValue {
  const committed = 'runtime' in receipt ? receipt.runtime : receipt.status === 'committed' ? receipt : undefined;
  return {
    status: receipt.status, requestId: receipt.requestId, regionId: receipt.regionId,
    ...(committed === undefined ? {} : {
      ...
      results: committed.outputs.map((output) => {
        ...
        return descriptor === undefined ? {outputId: output.outputId, status: 'unavailable'} : {
          outputId: output.outputId, status: output.handle.snapshot().status, fields: descriptor.fields.map((field) => field.id),
          loaded: descriptor.counts.loaded, precision: descriptor.precision.kind, coverage: descriptor.coverage.kind,
        };
      }),
    }),
    diagnostics: receipt.diagnostics.map((item) => ({code: item.code, message: item.message, retryable: item.retryable})),
  };
}

interface ParsedActionPreview {readonly mode: 'preview'; readonly action: ActionRequest['action']; readonly input: ActionRequest['input']; readonly entity?: ActionRequest['entity']; readonly idempotencyKey?: string}
interface ParsedActionExecute {readonly mode: 'execute'; readonly previewId: string}
...
function parseAction(input: unknown): Outcome<ParsedAction> {
  ...
  if (!record(value) || (value.mode !== 'preview' && value.mode !== 'execute')) return failure('agent.app.action-input', 'Action input must request preview or execute.');
  if (value.mode === 'execute') {
    if (Object.keys(value).some((key) => key !== 'mode' && key !== 'previewId') || !bounded(value.previewId)) return failure('agent.app.action-input', 'Execute requires one bounded previewId.');
    return {ok: true, value: {mode: 'execute', previewId: value.previewId}};
  }
  if (Object.keys(value).some((key) => !['mode', 'action', 'input', 'entity', 'idempotencyKey'].includes(key))) return failure('agent.app.action-input', 'Preview contains an unknown field.');
  const action = value.action;
  const payload = value.input;
  if (!record(action) || !bounded(action.id) || !bounded(action.revision) || Object.keys(action).length !== 2 || !record(payload))
  ...
  if (value.idempotencyKey !== undefined && !bounded(value.idempotencyKey)) return failure('agent.app.action-input', 'Idempotency key is malformed.');
  return {ok: true, value: {mode: 'preview', action: {id: action.id, revision: action.revision}, input: payload,
...
const actionSchema = toolSchema({
  type: 'object',
  oneOf: [
    {required: ['mode', 'action', 'input'], properties: {mode: {const: 'preview'}, action: {type: 'object', required: ['id', 'revision']}, input: {type: 'object'}, entity: {type: 'object'}, idempotencyKey: {type: 'string'}}},
...
export function createAppToolEndpoint(options: AppToolEndpointOptions): Outcome<AeliqoAppToolEndpoint> {
  ...
  const renderCapability: AgentCapabilityManifest<ReturnType<typeof parseIntent> extends Outcome<infer T> ? T : never, AgentJsonValue> = {
    ref: refs.render, operation: 'task.evaluate', label: 'Render a registered Aeliqo intent',
    description: 'Validates and renders an intent through the same compiler, data, authority, adaptive policy, and Region lifecycle used by application code.',
    parse: parseIntent,
    async invoke(intent, context): Promise<AgentCapabilityHandlerResult<AgentJsonValue>> {
      if (!context.authority.grants.includes('task.propose') || !context.authority.grants.includes('experience.commit'))
        return {state: 'denied', diagnostics: [{code: 'agent.app.denied', message: 'Intent compilation and presentation commit are not both permitted.', retryable: false}]};
      ...
      if (receipt.status === 'renderer-ready') return {state: 'renderer-ready', value, regionRevision: receipt.runtime.region.regionRevision};
      if (receipt.status === 'committed') return {state: 'plan-committed', value, regionRevision: receipt.region.regionRevision};
  ...
  const actionCapability: AgentCapabilityManifest<ParsedAction, AgentJsonValue> = {
    ref: refs.action, operation: 'action.propose', label: 'Preview or execute a registered Aeliqo action',
    description: 'Previews a registered action. Execution requires a separate execute grant and any required confirmation must come from the host UI.',
    parse: parseAction,
    async invoke(input, context): Promise<AgentCapabilityHandlerResult<AgentJsonValue>> {
      if (actionPort === undefined) return {state: 'unsupported', diagnostics: [{code: 'agent.app.actions-unavailable', message: 'This application did not register an action boundary.', retryable: false}]};
      if (input.mode === 'preview') {
        const preview = await actionPort.preview({requestId: context.requestId, action: input.action, input: input.input,
          ...(input.entity === undefined ? {} : {entity: input.entity}), ...(input.idempotencyKey === undefined ? {} : {idempotencyKey: input.idempotencyKey})}, {signal: context.signal});
        if (!preview.ok) return {state: 'failed', diagnostics: preview.diagnostics};
        previews.set(preview.value.id, preview.value);
        return {state: preview.value.confirmation === 'required' ? 'needs-choice' : 'accepted', value: {
          state: 'preview', previewId: preview.value.id, action: preview.value.action, sideEffect: preview.value.sideEffect,
          confirmation: preview.value.confirmation,
        }};
      }
      if (!context.authority.grants.includes('action.execute')) return {state: 'denied', diagnostics: [{code: 'agent.app.denied', message: 'Action execution is not permitted.', retryable: false}]};
      const preview = previews.get(input.previewId);
      if (preview === undefined) return {state: 'stale', diagnostics: [{code: 'agent.app.preview-stale', message: 'The action preview is missing, expired, or already consumed.', retryable: false}]};
      let receipt = confirmations.get(input.previewId);
      if (receipt === undefined && preview.confirmation === 'required') return {state: 'needs-choice', value: {state: 'confirmation-required', previewId: input.previewId}};
      ...
      previews.delete(input.previewId); confirmations.delete(input.previewId);
      if (!execution.ok) return {state: execution.diagnostics[0]?.code === 'action.ambiguous' ? 'partial' : 'failed', diagnostics: execution.diagnostics};
      return execution.value.state === 'ambiguous'
        ? {state: 'partial', reason: execution.value.reason, value: {state: 'ambiguous', receiptId: execution.value.receiptId, action: execution.value.action}}
        : {state: 'accepted', value: {state: 'executed', receiptId: execution.value.receiptId, action: execution.value.action, output: execution.value.output}};
    },
  };
  ...
  const tools: readonly AgentToolBinding[] = [
    ...
    {name: 'aeliqo_render', capability: refs.render, operation: 'task.evaluate',
  ...
  const endpoint = createAgentToolEndpoint({transport: options.transport, targetRegionId: options.regionId, goalEpoch: options.goalEpoch,
    principalKey: initial.value.authority.principalKey, expiresAt: options.expiresAt,
    ...
    host: {readContext: () => {
      ...
      return {ok: true, value: {principalKey: current.value.authority.principalKey, regionId: options.regionId, goalEpoch: options.goalEpoch,
        grants: grants(current.value.authority.grants)}};
    }},
    ...(options.now === undefined ? {} : {now: options.now}), ...(options.maxPending === undefined ? {} : {maxPending: options.maxPending}),
    ...(options.maxMilliseconds === undefined ? {} : {maxMilliseconds: options.maxMilliseconds}),
    ...(options.maxInputBytes === undefined ? {} : {maxInputBytes: options.maxInputBytes}), ...(options.maxOutputBytes === undefined ? {} : {maxOutputBytes: options.maxOutputBytes}),
  });
  if (!endpoint.ok) return endpoint;
  const inner = endpoint.value;
  return {ok: true, value: Object.freeze({...inner,
    async confirmAction(previewId: string, input: {readonly signal?: AbortSignal} = {}) {
      const preview = previews.get(previewId);
      if (preview === undefined || actionPort === undefined) return failure<ActionReceipt>('agent.app.preview-stale', 'The action preview is missing, expired, or already consumed.');
      const confirmed = await actionPort.confirm(preview, input);
      if (confirmed.ok) confirmations.set(previewId, confirmed.value);
      else { previews.delete(previewId); confirmations.delete(previewId); }
      return confirmed;
    },
    close() {
