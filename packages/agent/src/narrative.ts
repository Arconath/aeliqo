...
export type NarrativeReceipt =
  | {
      ...
      readonly claim: Extract<NarrativeClaim, { kind: 'value' | 'comparison' }>;
...
function authorized(context: NarrativeAuthority): boolean {
  return (
    [context.principalKey, context.scopeDigest, context.catalogRevision, context.functionRegistryDigest].every(
      (value) => typeof value === 'string' && value.length > 0,
    ) && context.grants.includes('result.inspect')
  );
}
  ...
  return {
    verify(input: unknown): Outcome<NarrativeReceipt> {
      try {
        ...
        if (claim.kind !== 'value' && claim.kind !== 'comparison')
          return { ok: true, value: { state: 'unverified', reason: 'interpretation' } };
        ...
        function cellValue(cell: NarrativeCell): Outcome<Scalar> {
          ...
          if (
            ...
            !same([...descriptor.identity].sort(), Object.keys(cell.identity).sort())
          )
            return fail('identity', 'The claim must identify one complete stable row identity.');
          const identityFields = descriptor.identity.map((id) =>
            descriptor.fields.find((candidate) => candidate.id === id),
          );
          if (identityFields.some((field) => field === undefined))
            return fail('identity', 'The result identity is unavailable.');
          ...
          if (matches !== 1 || value === undefined)
            return fail('identity', 'A unique matching result row is required.');
        ...
        if (claim.kind === 'value') {
          const actual = cellValue(claim.cell);
          ...
          const left = cellValue(claim.left);
          if (!left.ok) return left;
          const right = cellValue(claim.right);
          ...
          truth =
            order !== null &&
            { eq: order === 0, ne: order !== 0, lt: order < 0, lte: order <= 0, gt: order > 0, gte: order >= 0 }[
              claim.relation
            ];
        }
        const current = options.readContext();
        if (!current.ok || !authorized(current.value) || pins(current.value) !== initialPins)
        ...
        if (!final.ok || !authorized(final.value) || pins(final.value) !== initialPins)
          return fail('denied', 'Result authority changed during verification.');
        for (const entry of used) {
        ...
        if (!truth)
          return { ok: true, value: { state: 'unverified', reason: nullComparison ? 'null-comparison' : 'false' } };
        return {
          ok: true,
          value: {
            state: 'verified',
            claim,
            evidence: used.map(({ ref, handle }) => ({ result: ref, generation: handle.generation })),
          },
        };
      } catch {
