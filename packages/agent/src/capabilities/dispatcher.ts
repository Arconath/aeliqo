...
function normalizeRef(input: unknown): Outcome<VersionRef> {
  ...
  if (!validId(object.id) || !validId(object.revision))
    return failure('agent.capability.invalid', 'The capability reference is malformed.', ['capability']);
...
function normalizeTransport(input: unknown): AgentCapabilityTransport {
  return input === 'manual' || input === 'mcp' || input === 'webmcp' || input === 'byok' || input === 'direct'
    ? input
    : 'direct';
}
...
export function normalizeAgentCapabilityRequest(input: unknown): Outcome<AgentCapabilityRequest> {
  ...
  if (wire.value === null || typeof wire.value !== 'object' || Array.isArray(wire.value))
    return failure('agent.capability.invalid', 'A capability request must be an object.');
  ...
  if (
    object.version !== '1' ||
    !validId(object.requestId) ||
    !validId(object.targetRegionId) ||
    !validId(object.goalEpoch)
  )
  ...
  if (object.transport !== undefined && object.transport !== transport)
  ...
  return {
    ...
    value: Object.freeze({
      ...
      targetRegionId: object.targetRegionId,
      goalEpoch: object.goalEpoch,
      capability: ref.value,
      operation: operation.value,
      input: payload.value,
      ...(metadata === undefined ? {} : { metadata }),
      ...(object.transport === undefined ? {} : { transport }),
    }),
  };
}
...
function normalizeHostContext(
  input: unknown,
  request: Pick<AgentCapabilityRequest, 'targetRegionId' | 'goalEpoch'>,
  ...
  if (wire.value === null || typeof wire.value !== 'object' || Array.isArray(wire.value))
    return failure('agent.capability.denied', 'The host authority context is unavailable.');
  ...
  if (!Array.isArray(value.grants) || value.grants.length > WIRE_LIMITS.array)
...
function cleanDiagnostics(input: readonly Diagnostic[] | undefined): readonly Diagnostic[] {
  ...
  for (const item of input) {
    ...
    if (
      path !== undefined &&
      (!Array.isArray(path) ||
        path.length > WIRE_LIMITS.depth ||
        path.some((part) => !(typeof part === 'string' ? validId(part) : Number.isSafeInteger(part) && part >= 0)))
    )
      continue;
    ...
    if (
      ...
      (!Array.isArray(remedies) ||
        remedies.length > WIRE_LIMITS.array ||
        remedies.some((remedy) => !validText(remedy, WIRE_LIMITS.label)))
    )
      continue;
    output.push(
      Object.freeze({
        ...
        ...(remedies === undefined ? {} : { remedies: Object.freeze([...remedies]) }),
      }),
    );
  }
...
function hasDataOutput(
  _operation: OperationGrant,
...
function outputScopeMatches(
  ...
  if (
    refs !== undefined &&
    refs.length > 0 &&
    (authority.current === undefined || !validateCommitReadSet(authority.current, authority.current, refs).ok)
  )
    return false;
  let marked = false;
  const visit = (candidate: AgentJsonValue): boolean => {
    ...
    if (['id', 'revision', 'outputId', 'queryDigest', 'scopeDigest'].every((key) => typeof object[key] === 'string')) {
      ...
      if (authority.current === undefined || !validateCommitReadSet(authority.current, authority.current, [ref]).ok)
        return false;
    }
...
function allowedState(operation: OperationGrant, state: AgentCapabilityState): boolean {
  ...
  if (operation === 'experience.commit')
    return state === 'accepted' || state === 'plan-committed' || state === 'renderer-ready';
  if (operation === 'task.evaluate')
    return state === 'accepted' || state === 'data-ready' || state === 'plan-committed' || state === 'renderer-ready';
  if (operation === 'catalog.read' || operation === 'result.inspect')
...
function changedAuthority(
  ...
  const committed =
    (operation === 'experience.commit' || operation === 'task.evaluate') &&
    (result === 'plan-committed' || result === 'renderer-ready');
  if (left === undefined || right === undefined)
    return committed && left === undefined && right !== undefined ? 'none' : left === right ? 'none' : 'changed';
  const stablePins = [
    'scopeDigest',
    'policyRevision',
    'catalogRevision',
...
function normalizeHandlerResult<T>(
  ...
  if (Object.hasOwn(raw, 'ok') && typeof (raw as { ok?: unknown }).ok === 'boolean') {
    ...
    if (!outcome.ok)
      return failure(
        ...
        outcome.diagnostics[0]?.message ?? 'The capability handler reported failure.',
      );
    candidate = outcome.value;
  } else candidate = raw as AgentCapabilityHandlerResult<T>;
  if (candidate === undefined || candidate === null || typeof candidate !== 'object' || !validText(candidate.state, 32))
    return failure('agent.capability.handler', 'The capability handler returned an invalid stage.');
  const allowed: readonly AgentCapabilityState[] = [
    'accepted',
    'bound',
    'data-ready',
    'plan-committed',
  ...
  if (!allowed.includes(candidate.state))
    return failure('agent.capability.handler', 'The capability handler returned an unknown stage.');
  const value = candidate.value === undefined ? undefined : parseWireValue(candidate.value);
  if (value !== undefined && !value.ok)
    return failure('agent.capability.output', 'The capability handler returned non-wire output.');
  const diagnostics = cleanDiagnostics(candidate.diagnostics);
  if (candidate.diagnostics !== undefined && candidate.diagnostics.length > 0 && diagnostics.length === 0)
    return failure('agent.capability.output', 'The capability handler returned malformed diagnostics.');
  const failureStates: readonly AgentCapabilityState[] = [
    'failed',
    'denied',
    'stale',
    'unsupported',
  ...
  const effectiveDiagnostics =
    failureStates.includes(candidate.state) && diagnostics.length === 0
      ? [
          diagnostic(
            `agent.capability.${candidate.state}`,
            `The capability returned ${candidate.state} without a diagnostic.`,
          ),
        ]
      : diagnostics;
  const refs = cleanRefs(candidate.affectedResults);
  if (!refs.ok) return refs;
  if (candidate.regionRevision !== undefined && !validId(candidate.regionRevision))
    return failure('agent.capability.output', 'The capability handler returned a malformed region revision.');
  if (candidate.reason !== undefined && !validText(candidate.reason, WIRE_LIMITS.label))
    return failure('agent.capability.output', 'The capability handler returned an unbounded reason.');
  return {
    ok: true,
    value: Object.freeze({
      state: candidate.state,
      ...(value === undefined ? {} : { value: value.value as T }),
      ...(effectiveDiagnostics.length === 0 ? {} : { diagnostics: effectiveDiagnostics }),
      ...(refs.value === undefined ? {} : { affectedResults: refs.value }),
...
export function createAgentCapabilityDispatcher(options: AgentCapabilityDispatcherOptions): AgentCapabilityDispatcher {
  if (
    ...
    throw new TypeError('A capability host and registry are required.');
  const maxPending =
    typeof options.maxPending === 'number' && Number.isSafeInteger(options.maxPending) && options.maxPending > 0
      ? Math.min(64, options.maxPending)
      : DEFAULT_MAX_PENDING;
  const maxMilliseconds =
    typeof options.maxMilliseconds === 'number' &&
    Number.isSafeInteger(options.maxMilliseconds) &&
    options.maxMilliseconds > 0
      ? Math.min(300_000, options.maxMilliseconds)
      : DEFAULT_MAX_MILLISECONDS;
  const maxInputBytes =
    typeof options.maxInputBytes === 'number' &&
    Number.isSafeInteger(options.maxInputBytes) &&
    options.maxInputBytes > 0
      ? Math.min(WIRE_LIMITS.bytes, options.maxInputBytes)
      : WIRE_LIMITS.bytes;
  const maxOutputBytes =
    typeof options.maxOutputBytes === 'number' &&
    Number.isSafeInteger(options.maxOutputBytes) &&
    options.maxOutputBytes > 0
      ? Math.min(WIRE_LIMITS.bytes, options.maxOutputBytes)
      : WIRE_LIMITS.bytes;
  let pending = 0;
  ...
  const dispatch = async (
    ...
    if (normalizeTransport(transport) !== transport)
      return failure('agent.capability.transport', 'The trusted dispatch transport is invalid.');
    ...
    if (!sameRef(manifest.ref, request.value.capability) || manifest.operation !== request.value.operation)
      return failureReceipt(
        request.value,
        transport,
        'invalid',
    ...
    if (pending >= maxPending)
      return failureReceipt(
        ...
        'The capability dispatch queue is full.',
      );
    pending++;
    const start = performance.now();
    try {
      const hostBoundary = await awaitBoundary(
        (signal) =>
          options.host.readContext({
            requestId: request.value.requestId,
            targetRegionId: request.value.targetRegionId,
            goalEpoch: request.value.goalEpoch,
            signal,
          }),
        dispatchOptions.signal,
        maxMilliseconds,
      );
      if (hostBoundary.kind === 'deadline')
        return failureReceipt(
          request.value,
          transport,
          'failed',
      ...
      if (!hostResult.ok)
        return failureReceipt(
          ...
          transport,
          'denied',
          EXTERNAL_TRANSPORTS.has(transport)
            ? 'agent.capability.denied'
            : (hostResult.diagnostics[0]?.code ?? 'agent.capability.denied'),
          EXTERNAL_TRANSPORTS.has(transport)
            ? 'The host did not authorize capability inspection.'
            : (hostResult.diagnostics[0]?.message ?? 'The host did not authorize capability inspection.'),
        );
      ...
      if (!authority.value.grants.includes(request.value.operation))
        return failureReceipt(
          request.value,
          transport,
          'denied',
      ...
      const parsed = (() => {
        try {
          return manifest.parse(request.value.input);
        } catch {
          return failure('agent.capability.input', 'The capability input could not be parsed safely.');
        }
      })();
      if (!parsed.ok) {
        ...
        return failureReceipt(
          ...
          transport,
          'invalid',
          EXTERNAL_TRANSPORTS.has(transport) ? 'agent.capability.input' : (first?.code ?? 'agent.capability.input'),
          EXTERNAL_TRANSPORTS.has(transport)
            ? 'The capability input is invalid.'
            : (first?.message ?? 'The capability input is invalid.'),
        );
      }
      ...
      const invocation = await awaitBoundary(
        (signal) => manifest.invoke(parsed.value, { ...context, signal }),
        dispatchOptions.signal,
        remaining,
      );
      if (invocation.kind === 'deadline')
        return failureReceipt(
          request.value,
          transport,
          'failed',
      ...
      if (!normalized.ok)
        return failureReceipt(
          ...
          EXTERNAL_TRANSPORTS.has(transport)
            ...
            : (normalized.diagnostics[0]?.code ?? 'agent.capability.output'),
          EXTERNAL_TRANSPORTS.has(transport)
            ? 'The capability output was invalid.'
            : (normalized.diagnostics[0]?.message ?? 'The capability output was invalid.'),
        );
      const result = normalized.value;
      if (!allowedState(request.value.operation, result.state))
        return failureReceipt(
          request.value,
          transport,
          'invalid',
      ...
      if (
        (result.state === 'renderer-ready' || result.state === 'plan-committed') &&
        result.regionRevision === undefined
      )
        return failureReceipt(
          request.value,
          transport,
      ...
      const finalHost = await awaitBoundary(
        (signal) =>
          options.host.readContext({
            requestId: request.value.requestId,
            targetRegionId: request.value.targetRegionId,
        ...
        dispatchOptions.signal,
        Math.max(1, maxMilliseconds - (performance.now() - start)),
      );
      if (finalHost.kind === 'deadline')
        return failureReceipt(
          request.value,
          transport,
          'partial',
      ...
      if (finalHost.kind === 'failed' || !finalHost.value.ok)
        return failureReceipt(
          request.value,
          transport,
          'partial',
          'agent.capability.recovery',
      ...
      const authorityState = changedAuthority(
        authority.value,
        finalAuthority.value,
      ...
      if (EXTERNAL_TRANSPORTS.has(transport) && !finalAuthority.value.grants.includes('model.egress')) {
        return {
          ok: true,
          value: receipt(request.value, transport, result.state, {
            diagnostics:
              result.diagnostics?.length || result.reason !== undefined
                ? [
                    diagnostic(
                      `agent.capability.${result.state}`,
                      `The capability returned ${result.state}; details require model egress.`,
                    ),
                  ]
                : [],
          }),
      ...
      return {
        ok: true,
        value: receipt(request.value, transport, result.state, {
          ...(output === undefined ? {} : { value: output.value as AgentJsonValue }),
  ...
  const port = (transport: AgentCapabilityTransport): AgentCapabilityPort =>
    Object.freeze({
      transport,
      invoke(input: AgentCapabilityRequest | unknown, invokeOptions: { readonly signal?: AbortSignal } = {}) {
        return dispatch(input, { ...invokeOptions, transport });
      },
    });
