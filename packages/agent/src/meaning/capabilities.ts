...
export function parseMeaningProposalInput(input: unknown): Outcome<MeaningProposalInput> {
  ...
  const candidate =
    asRecord(object.meaning) === undefined && 'implementation' in object ? object : asRecord(object.meaning);
  ...
  if (
    ...
    (!Array.isArray(assumptions) ||
      assumptions.length > WIRE_LIMITS.diagnostics ||
  ...
  if (base !== undefined) {
    ...
    if (baseRecord === undefined || !validId(baseRecord.id) || !validId(baseRecord.revision))
    ...
    async invoke(
      ...
      try {
        ...
        if (!activated.ok) {
          ...
          const state = codes.some((code) => code.includes('cancelled'))
            ? 'cancelled'
            : codes.some((code) => code.includes('stale') || code.includes('revoked'))
              ? 'stale'
              : codes.some((code) => code.includes('unsupported'))
                ? 'unsupported'
                : codes.some(
                      (code) =>
