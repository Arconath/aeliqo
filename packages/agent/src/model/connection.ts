import type { ToolModelPort, ToolModelRequest, ToolModelResponse, ToolModelUsage } from './types.js';
...
function normalizeBaseURL(value: string, policy: ToolModelConnectionPolicy): string {
  ...
  if (policy.allowedOrigins !== undefined) {
    if (policy.allowedOrigins.length === 0 || !policy.allowedOrigins.includes(parsed.origin))
      configuration('The model endpoint origin is not allowlisted.');
    for (const origin of policy.allowedOrigins) {
      try {
        if (new URL(origin).origin !== origin) configuration('The model origin allowlist contains an invalid origin.');
      } catch {
        configuration('The model origin allowlist contains an invalid origin.');
      }
    }
  }
...
function normalizeOptions(options: ToolModelConnectionOptions): Readonly<{
  ...
  if (
    !options ||
    !options.adapter ||
    !text(options.adapter.id, 128) ||
    !text(options.adapter.endpointPath, 256) ||
    typeof options.adapter.encodeRequest !== 'function' ||
    typeof options.adapter.estimateInputTokens !== 'function' ||
    typeof options.adapter.decodeResponse !== 'function'
  )
    configuration('The model connection requires a complete protocol adapter.');
  if (options.policy?.allowExternalEgress !== true)
    configuration('The model connection requires an explicit egress grant.');
  ...
  if (
    options.auth.scheme === 'header' &&
    (!text(options.auth.headerName, 128) ||
      ['authorization', 'cookie', 'proxy-authorization'].includes(options.auth.headerName.toLowerCase()))
  )
  ...
  for (const capability of adapterCapabilities)
    if (!['tool-calls', 'usage', 'request-cancellation', 'input-token-estimate', 'request-retry'].includes(capability))
      configuration('The model adapter declares an unknown capability.');
  if (
    !Array.isArray(options.capabilities) ||
    options.capabilities.length === 0 ||
    !options.capabilities.includes('tool-calls')
  )
  ...
  if (
    ...
    !integer(maxAttempts, 1, 4) ||
    !integer(baseDelayMs, 0, 5_000) ||
    !integer(maxDelayMs, baseDelayMs, 30_000) ||
    !integer(maxRequestBytes, 1, 8 * 1024 * 1024) ||
    !integer(maxResponseBytes, 1, 8 * 1024 * 1024)
  )
    configuration('The model connection limits are outside the supported bounds.');
  if (
    options.cost !== undefined &&
    ((options.cost.currency !== undefined && options.cost.currency !== 'USD') ||
      ...
      (options.cost.source !== undefined && !text(options.cost.source, 512)))
  )
    configuration('The model cost policy is invalid.');
  const fetch =
...
export function createToolModelConnection(options: ToolModelConnectionOptions): ToolModelPort {
  ...
  const complete = async (
    ...
    if (!integer(estimatedInputTokens, 1, 1_000_000))
    ...
    for (let attempt = 0; attempt < config.maxAttempts; attempt++) {
      ...
      try {
        ...
        let decoded: unknown;
        try {
          decoded = JSON.parse(await boundedText(response, config.maxResponseBytes)) as unknown;
        } catch (error) {
        ...
        try {
          result = config.adapter.decodeResponse(decoded, { model: config.model, estimatedInputTokens });
        } catch {
        ...
        const normalized =
          usage === result.usage
            ...
            : {
                ...result,
                usage,
                ...(result.provider === undefined ? {} : { provider: { ...result.provider, usage } }),
              };
          ...
          try {
            config.onResponse(
              Object.freeze({
                ...
                ...(normalized.provider?.responseId === undefined
                  ? {}
                  : { responseId: normalized.provider.responseId }),
    ...
    throw (
      last ?? new ToolModelProviderError('network', 'The model endpoint could not be reached.', { retryable: true })
    );
  };
  return Object.freeze({
    estimateInputTokens: (request: ToolModelRequest) => {
      const value = config.adapter.estimateInputTokens(request);
      if (!integer(value, 1, 1_000_000))
        throw new ToolModelProviderError('malformed-response', 'The model adapter returned an invalid token estimate.');
      return value;
    },
    complete,
