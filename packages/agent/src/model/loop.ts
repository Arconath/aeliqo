...
const fail = (code: string, message: string): ToolModelLoopOutcome => ({
  ok: false,
...
function validBudget(b: ToolModelBudget): boolean {
  return (
    b !== null &&
    typeof b === 'object' &&
    integer(b.maxTurns, 1, 32) &&
    integer(b.maxModelRequests, 1, 64) &&
    integer(b.maxToolCalls, 1, 64) &&
    integer(b.maxMilliseconds, 1, 300_000) &&
    integer(b.maxInputTokens, 1, 1_000_000) &&
    integer(b.maxOutputTokens, 1, 100_000) &&
    integer(b.maxTotalTokens, 1, 2_000_000) &&
    integer(b.maxInputBytes, 1, WIRE_LIMITS.bytes) &&
    integer(b.maxOutputBytes, 1, WIRE_LIMITS.bytes) &&
    integer(b.maxRepeatedCalls, 1, 4)
  );
}
function validPolicy(policy: ToolModelLoopOptions['policy']): boolean {
  ...
  return (
    ...
    policy.requiredOperationSequence.every(
      (step) =>
        step !== null &&
        typeof step === 'object' &&
        typeof step.operation === 'string' &&
        operations.has(step.operation) &&
        Array.isArray(step.acceptedStates) &&
        step.acceptedStates.length > 0 &&
        step.acceptedStates.length <= positiveStates.size &&
        new Set(step.acceptedStates).size === step.acceptedStates.length &&
        step.acceptedStates.every(
          (state: unknown) => typeof state === 'string' && positiveStates.has(state as AgentCapabilityState),
        ),
    )
  );
}
...
function response(input: unknown, limit: number): ToolModelResponse | undefined {
  ...
  if (
    !checked.ok ||
    bytes(checked.value) + (continuation?.bytes ?? 0) > limit ||
    checked.value === null ||
  ...
  if (value.text !== undefined && (typeof value.text !== 'string' || value.text.length > WIRE_LIMITS.text))
    return undefined;
  if (
    !Array.isArray(value.calls) ||
    value.calls.length > 8 ||
    !value.usage ||
    !integer(value.usage.inputTokens, 0, 1_000_000) ||
    !integer(value.usage.outputTokens, 0, 100_000)
  )
    return undefined;
...
export async function runToolModel(options: ToolModelLoopOptions): Promise<ToolModelLoopOutcome> {
  if (
    ...
    !['chat', 'experience'].includes(options.goal) ||
    typeof options.prompt !== 'string' ||
    options.prompt.length === 0 ||
    options.prompt.length > WIRE_LIMITS.text ||
    (options.instructions !== undefined &&
      (typeof options.instructions !== 'string' ||
        options.instructions.length === 0 ||
        options.instructions.length > WIRE_LIMITS.text)) ||
    !validPolicy(options.policy) ||
    !validBudget(options.budget) ||
    options.endpoint?.transport !== 'byok' ||
    typeof options.endpoint.authorizeModel !== 'function' ||
    (typeof options.model?.countInputTokens !== 'function' &&
      typeof options.model?.estimateInputTokens !== 'function') ||
    typeof options.model.complete !== 'function'
  )
    return fail(
      'agent.model.invalid',
  ...
  const remainingRequired = () =>
    policy?.requiredOperationSequence.slice(requiredIndex(policy, receipts)).map((step) => step.operation) ?? [];
  ...
  const admit = async (transition?: AgentCapabilityReceipt): Promise<ToolModelStop | undefined> => {
    ...
    if (expectedScope !== undefined && capabilityCanonical(expectedScope) !== capabilityCanonical(scope.value)) {
      const before = expectedScope.current,
        after = scope.value.current;
      if (
        transition === undefined ||
        expectedScope.principalKey !== scope.value.principalKey ||
        before === undefined ||
        after === undefined
      )
        return 'stale';
      const commit =
        transition.operation === 'experience.commit' && ['plan-committed', 'renderer-ready'].includes(transition.state);
      const stable = [
        'scopeDigest',
        'policyRevision',
        'catalogRevision',
      ...
      if (commit ? after.regionRevision !== transition.regionRevision : !validateCommitReadSet(before, after).ok)
        return 'stale';
    }
  ...
  try {
    for (let turn = 0; turn < budget.maxTurns; turn++) {
      ...
      const tools =
        required === undefined
          ? discovery.value
          : discovery.value.filter((tool) => tool.operation === required.operation);
      if (required !== undefined && tools.length === 0) return finish('failed');
      const request: ToolModelRequest = snapshot({
        messages,
        tools,
        toolChoice: required === undefined ? 'auto' : (policy?.providerToolChoice ?? 'auto'),
        maxOutputTokens: budget.maxOutputTokens,
      });
      const countIsRemote = count !== undefined;
      const continuationBytes = request.messages.reduce(
        (total, message) => total + (message.role === 'assistant' ? (message.continuation?.bytes ?? 0) : 0),
        0,
      );
      if (
        bytes(request) + continuationBytes > budget.maxInputBytes ||
        modelRequests + (countIsRemote ? 2 : 1) > budget.maxModelRequests
      )
      ...
      if (
        counted > budget.maxInputTokens ||
        inputTokens + outputTokens + counted + budget.maxOutputTokens > budget.maxTotalTokens
      )
        return finish('budget');
      ...
      if (
        candidate.usage.inputTokens > budget.maxInputTokens ||
        candidate.usage.outputTokens > budget.maxOutputTokens ||
      ...
      if (candidate.calls.length === 0) {
        if (required === undefined)
          return finish(goal === 'chat' && candidate.text ? 'text-ready' : 'no-commit', candidate.text);
        ...
        for (const call of candidate.calls)
          messages.push({
            ...
            output: {
              ...
              diagnostics: [
                {
                  code: 'agent.model.required-operation',
                  message: `The host requires ${required.operation} before this tool.`,
                  retryable: true,
                },
              ],
            },
      ...
      for (const [callIndex, call] of candidate.calls.entries()) {
        ...
        const fingerprint = capabilityCanonical({
          name: call.name,
          input: call.input,
          scope:
            expectedScope === undefined
              ? undefined
              : {
                  principalKey: expectedScope.principalKey,
                  current: expectedScope.current === undefined ? undefined : { ...expectedScope.current, results: [] },
                },
        });
        ...
        if (prior !== undefined) {
          messages.push({ role: 'tool', callId: call.id, output: prior.output });
          continue;
        }
        toolCalls++;
        const result = await boundary((child) =>
          endpoint.invoke(call.name, call.input, { requestId: `${requestId}-${toolCalls}`, signal: child }),
        );
        ...
        if (result.ok) {
          ...
          if (['partial', 'cancelled', 'failed', 'stale', 'denied'].includes(result.value.state))
            return finish(
              result.value.state === 'partial' || result.value.state === 'failed'
                ? 'failed'
                : (result.value.state as ToolModelStop),
            );
        }
        const afterTool = await admit(result.ok ? result.value : undefined);
        if (afterTool !== undefined) return finish(afterTool);
        if (result.ok && result.value.state === 'renderer-ready' && goal === 'experience')
          return finish('renderer-ready');
