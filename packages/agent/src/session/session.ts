...
function normalizeRecovery(value: AgentRecoveryReceipt): AgentRecoveryReceipt | undefined {
  if (
    value === null ||
    typeof value !== 'object' ||
    (value.state !== 'preserved-incumbent' && value.state !== 'cleared-revoked' && value.state !== 'manual-required') ||
    typeof value.reason !== 'string' ||
    value.reason.length === 0 ||
    value.reason.length > 4096 ||
    typeof value.safeToRetry !== 'boolean'
  )
    return undefined;
  if (
    value.regionRevision !== undefined &&
    (typeof value.regionRevision !== 'string' || value.regionRevision.length === 0 || value.regionRevision.length > 160)
  )
    return undefined;
  if (
    ...
    (!Array.isArray(value.results) ||
      ...
      value.results.some(
        (ref) =>
          ...
          typeof ref.outputId !== 'string' ||
          typeof ref.queryDigest !== 'string' ||
          typeof ref.scopeDigest !== 'string',
      ))
  )
    return undefined;
  return Object.freeze({
...
export function createAgentSession(options: AgentSessionOptions): AgentSession {
  if (
    ...
    throw new TypeError('A capability dispatcher is required.');
  ...
  if (!['direct', 'manual', 'mcp', 'webmcp', 'byok'].includes(transport))
    throw new TypeError('A valid trusted session transport is required.');

  const inspect = (): AgentSessionSnapshot =>
    Object.freeze({
      status,
      ...(currentRequestId === undefined ? {} : { requestId: currentRequestId }),
      attempts: Object.freeze([...snapshotAttempts]),
      ...(snapshotReceipt === undefined ? {} : { receipt: snapshotReceipt }),
    });
      ...
      if (reason !== 'complete' && reason !== 'needs-choice' && reason !== 'needs-meaning') {
        if (options.recover !== undefined) {
          ...
          const recovered = await awaitBoundary(
            (signal) => options.recover!({ receipt: last, incumbent: input.incumbent, signal }),
            controller.signal,
            remaining,
          );
          if (recovered.kind === 'value' && recovered.value.ok)
            recovery = normalizeRecovery(recovered.value.value) ?? defaultRecovery();
          else recovery = defaultRecovery();
        } else recovery = defaultRecovery();
      }
      const value: AgentSessionReceipt = Object.freeze({
        version: '1',
        requestId: request.requestId,
        targetRegionId: request.targetRegionId,
        goalEpoch: request.goalEpoch,
    ...
    try {
      while (true) {
        ...
        if (proposalBytes > budget.value.maxProposalBytes) {
          const attempt: AgentSessionAttempt = Object.freeze({
          ...
          snapshotAttempts = Object.freeze([...attempts]);
          lastDiagnostics = [
            {
        ...
        const dispatched = await awaitBoundary(
          (signal) => options.dispatcher.dispatch(dispatchInput, { signal, transport: request.transport ?? 'direct' }),
          controller.signal,
          remaining,
        );
        ...
        const progress =
          previousFingerprint === undefined
            ? 'new'
            : previousFingerprint !== candidateFingerprint || last.state !== attempts.at(-1)?.state
              ? 'gap-closed'
              : 'none';
        const attempt: AgentSessionAttempt = Object.freeze({
          turn: attempts.length + 1,
        ...
        repairs++;
        if (attempts.length >= budget.value.maxTurns) return finalize('turn-budget');
        const repairRequest: AgentSessionRepairRequest = Object.freeze({
          turn: attempts.length + 1,
          previous: Object.freeze(
            attempts.map(({ turn, state, fingerprint, proposalBytes, progress }) =>
              Object.freeze({ turn, state, fingerprint, proposalBytes, progress }),
            ),
          ),
          diagnostics: Object.freeze([...lastDiagnostics]),
          signal: controller.signal,
        });
        const next = await awaitBoundary(
          (signal) => input.propose!({ ...repairRequest, signal }),
          controller.signal,
          Math.max(1, budget.value.maxMilliseconds - elapsed(start, now)),
        );
...
): Promise<Outcome<AgentSessionReceipt>> => session.run(input);
