...
export function createStudioSession(
  ...
  options: StudioDocumentOptions & { readonly registry: FunctionRegistry },
): StudioSession {
  ...
  let state: StudioState = cloneState({
    document,
    area: 'data-meaning',
    previewState: 'ready',
  ...
  ): readonly { readonly meaning: MeaningDefinition; readonly source: MeaningSource }[] => [
    ...candidate.catalog.meanings.map((meaning) => ({
      meaning,
      source: { surface: 'code' as const, ownership: 'code' as const, readOnly: true },
    })),
  ...
  const checkMeaningLedger = (candidate: StudioDocument): Outcome<void> => {
    for (const { meaning, source } of meaningsIn(candidate)) {
      ...
      if (prior.canonical !== next.canonical)
        return fail<void>(
          'studio.meaning-conflict',
          'An immutable meaning ID and revision has different canonical contents from an earlier accepted document.',
          ['meanings', meaning.id, meaning.revision],
        );
      if (prior.ownership !== next.ownership)
        return fail<void>(
          'studio.meaning-source-conflict',
          'A meaning cannot silently change ownership or authoring surface.',
          ['meanings', meaning.id, meaning.revision, 'source'],
        );
    }
  ...
  const recordMeaningLedger = (candidate: StudioDocument): void => {
    for (const { meaning, source } of meaningsIn(candidate)) {
      meaningLedger.set(JSON.stringify([meaning.id, meaning.revision]), {
        canonical: canonical(meaning),
  ...
  const createAuthoring = () =>
    createMeaningAuthoring({
      ...
      definitions: [...document.catalog.meanings, ...document.meanings.map((entry) => entry.meaning)],
    });
  const defineMeaning = (input: unknown): Outcome<MeaningDraft> => {
    if (input === null || typeof input !== 'object' || Array.isArray(input))
      return fail('studio.meaning-input', 'Meaning authoring input must be an object.', ['input']);
    ...
    if (
      ...
      return fail(
        'studio.meaning-input',
        'Meaning authoring requires bounded id, label, description, entity and field text.',
        ['input'],
      );
    if (candidate.revision !== undefined && typeof candidate.revision !== 'string')
      return fail('studio.meaning-input', 'Meaning revision must be text when supplied.', ['input', 'revision']);
    ...
    if (candidate.source !== undefined) {
      if (candidate.source === null || typeof candidate.source !== 'object' || Array.isArray(candidate.source))
        return fail(
          'studio.meaning-source-untrusted',
          'Studio authoring source must be omitted; provenance is assigned by the host.',
          ['input', 'source'],
        );
      const source = candidate.source as Record<string, unknown>;
      if (source.surface !== 'studio' || source.ownership !== 'personal' || source.readOnly === true)
        return fail(
          'studio.meaning-source-untrusted',
          'Studio authoring cannot self-declare code, workspace or read-only provenance.',
          ['input', 'source'],
        );
    }
    ...
    const normalized = authoring.value.draft(draft.value.meaning, {
      source: { surface: 'studio', ownership: 'personal' },
      assumptions: [],
    });
    if (!normalized.ok) return normalized;
    const existingMeaning =
      document.meanings.find((entry) => sameRef(entry.meaning, normalized.value.meaning)) ??
      document.catalog.meanings.find((entry) => sameRef(entry, normalized.value.meaning));
    const ledgerEntry = meaningLedger.get(
      JSON.stringify([normalized.value.meaning.id, normalized.value.meaning.revision]),
    );
    if (
      ledgerEntry !== undefined &&
      (ledgerEntry.canonical !== canonical(normalized.value.meaning) || ledgerEntry.ownership === 'code')
    ) {
      return fail(
        'studio.meaning-conflict',
        'The meaning ID and revision is already owned by an immutable canonical definition; create a new revision.',
        ['meaning'],
      );
    }
    if (existingMeaning !== undefined && sourceOfMeaning(document, normalized.value.meaning)?.ownership === 'code') {
      return fail(
        'studio.meaning-read-only',
        'Code-owned meanings are read-only; create a new revision or propose a diff.',
        ['meaning'],
      );
    }
    ...
    withDocument(
      { ...document, revision: `${document.revision}-meaning`, meanings: [...existing, normalized.value] },
      true,
    );
    setState({
      area: 'data-meaning',
      selectedMeaning: { id: normalized.value.meaning.id, revision: normalized.value.meaning.revision },
      diagnostics: [],
    });
  ...
  const evaluateMeaning = (input: unknown): Outcome<QueryResult> => {
    if (input === null || typeof input !== 'object' || Array.isArray(input))
      return fail('studio.meaning-evaluation', 'Meaning evaluation input must be an object.', ['input']);
    const evaluator = createMeaningEvaluator({
      catalog: document.catalog,
      registry: options.registry,
      definitions: [...document.catalog.meanings, ...document.meanings.map((entry) => entry.meaning)],
    });
  ...
  const editExperience = (input: unknown): Outcome<StudioProfile> => {
    if (input === null || typeof input !== 'object' || Array.isArray(input))
      return fail('studio.experience-input', 'Experience edit input must be an object.', ['input']);
    const candidate = input as Record<string, unknown>;
    if (candidate.base === null || typeof candidate.base !== 'object' || Array.isArray(candidate.base))
      return fail('studio.experience-input', 'Experience edits require a base profile reference.', ['input', 'base']);
    const base = candidate.base as Record<string, unknown>;
    if (typeof base.id !== 'string' || typeof base.revision !== 'string')
      return fail('studio.experience-input', 'Experience base must be a version reference.', ['input', 'base']);
    const baseRef = { id: base.id, revision: base.revision };
    const prior = document.profiles.find((entry) => sameRef(entry.experience, baseRef));
    if (prior === undefined)
      return fail('studio.experience-unknown', 'The selected Experience profile is not in this document.', [
        'input',
        'base',
      ]);
    if (typeof candidate.label !== 'string' || candidate.label.length === 0 || candidate.label.length > 256)
      return fail('studio.experience-input', 'Experience label must be bounded text.', ['input', 'label']);
    ...
    if (!parsed.ok)
      return {
        ...
        diagnostics: parsed.diagnostics.map((diagnostic) => ({
          ...diagnostic,
          path: ['input', 'experience', ...(diagnostic.path ?? [])],
        })) as unknown as [Diagnostic, ...Diagnostic[]],
      };
    const experience = parsed.value as Experience;
    const sameIdentity = sameRef(experience, prior.experience);
    if (sameIdentity && prior.source.ownership === 'code')
      return fail(
        'studio.experience-read-only',
        'Code-owned Experience profiles are read-only; create a new revision for a Studio edit.',
        ['input', 'experience'],
      );
    if (sameIdentity && canonical(experience) === canonical(prior.experience) && candidate.label === prior.label)
      return { ok: true, value: prior };
    if (sameIdentity && canonical(experience) === canonical(prior.experience))
      return fail(
        'studio.experience-conflict',
        'An immutable Experience profile revision cannot change its label; create a new revision.',
        ['input', 'label'],
      );
    if (sameIdentity && canonical(experience) !== canonical(prior.experience))
      return fail(
        'studio.experience-conflict',
        'An immutable Experience ID and revision cannot change canonical contents; create a new revision.',
        ['input', 'experience'],
      );
    const collision = document.profiles.find(
      (entry) => sameRef(entry.experience, experience) && canonical(entry.experience) !== canonical(experience),
    );
    if (collision !== undefined)
      return fail(
        'studio.experience-conflict',
        'The proposed Experience revision conflicts with an existing profile.',
        ['input', 'experience'],
      );
    const nextProfile: StudioProfile = Object.freeze({
      label: candidate.label as string,
      source: { surface: 'studio' as const, ownership: 'personal' as const },
      experience,
    });
    ...
    const next = {
      ...document,
      revision: `${document.revision}-experience`,
      profiles: Object.freeze(profiles),
      activeProfile: sameRef(document.activeProfile, prior.experience)
        ? { id: experience.id, revision: experience.revision }
  ...
  const proposeMeaningDiff = (base: unknown, candidate: unknown): Outcome<MeaningDiff> => {
    if (!isVersionRef(base))
      return fail('studio.meaning-input', 'Meaning diff requires a canonical base reference.', ['base']);
    if (candidate === null || typeof candidate !== 'object' || Array.isArray(candidate))
      return fail('studio.meaning-input', 'Meaning diff candidate must be an object.', ['candidate']);
    const source = sourceOfMeaning(document, base);
    if (source?.ownership === 'code') {
      const authoring = createAuthoring();
      if (!authoring.ok) return authoring;
      const prior =
        document.meanings.find((entry) => sameRef(entry.meaning, base)) ??
        document.catalog.meanings.find((entry) => sameRef(entry, base));
      if (prior === undefined)
        return fail('studio.meaning-unknown', 'The selected meaning is not present in this document.', ['meaning']);
  ...
  const applyMeaningDiff = (diff: unknown): Outcome<MeaningDraft> => {
    if (diff === null || typeof diff !== 'object' || Array.isArray(diff))
      return fail('studio.meaning-diff', 'Meaning diff must be an object.', ['diff']);
    const candidate = diff as Record<string, unknown>;
    if (
      candidate.state !== 'proposed-diff' ||
      candidate.base === null ||
      ...
      return fail('studio.meaning-diff', 'Meaning diff must contain a proposed base and candidate.', ['diff']);
    const baseDraft = candidate.base as Record<string, unknown>;
    const baseMeaning = baseDraft.meaning;
    if (
      baseMeaning === null ||
      typeof baseMeaning !== 'object' ||
      Array.isArray(baseMeaning) ||
      !isVersionRef(baseMeaning)
    )
      return fail('studio.meaning-diff', 'Meaning diff base must contain a canonical definition.', ['diff', 'base']);
    const baseRef = {
      id: (baseMeaning as MeaningDefinition).id,
      revision: (baseMeaning as MeaningDefinition).revision,
    };
    ...
    if (current === undefined)
      return fail('studio.meaning-unknown', 'The proposed diff base is no longer present in this document.', [
        'diff',
        'base',
      ]);
    if (canonical(current) !== canonical(baseMeaning))
      return fail('studio.meaning-stale', 'The proposed meaning diff is stale because its base changed.', [
        'diff',
        'base',
      ]);
    const proposed = candidate.candidate as Record<string, unknown>;
    const proposedMeaning = proposed.meaning;
    if (proposedMeaning === null || typeof proposedMeaning !== 'object' || Array.isArray(proposedMeaning))
      return fail('studio.meaning-diff', 'Meaning diff candidate must contain a canonical definition.', [
        'diff',
        'candidate',
      ]);
    const candidateValue = proposedMeaning as MeaningDefinition;
    if (sameRef(candidateValue, baseRef))
      return fail(
        'studio.meaning-conflict',
        'Applying a diff requires a new meaning revision or identity; code-owned content remains immutable.',
        ['diff', 'candidate', 'meaning'],
      );
    const authoring = createAuthoring();
    if (!authoring.ok) return authoring;
    const next = authoring.value.draft(candidateValue, {
      source: { surface: 'studio', ownership: 'personal' },
      assumptions: [],
      base: baseRef,
    });
    ...
    if (existing !== undefined && canonical(existing.meaning) !== canonical(next.value.meaning))
      return fail(
        'studio.meaning-conflict',
        'The proposed meaning revision conflicts with an existing Studio definition.',
        ['diff', 'candidate', 'meaning'],
      );
    ...
    setState({
      area: 'data-meaning',
      selectedMeaning: { id: next.value.meaning.id, revision: next.value.meaning.revision },
      diagnostics: [],
    });
    return next;
  };
  const importDocument = (input: unknown): Outcome<StudioDocument> => {
    ...
    if (prior !== undefined && prior !== canonical(next.value)) {
      const conflict = fail<StudioDocument>(
        'studio.source-conflict',
        'A document with the same identity and revision has different canonical contents.',
        ['revision'],
      );
  ...
  return Object.freeze({
    ...
    inspect: () => {
      ...
      const codeOwnedCount = allMeanings.filter(
        (meaning) => sourceOfMeaning(document, meaning)?.ownership === 'code',
      ).length;
      return Object.freeze({
        sourceOfTruth: document.meanings.length > 0 ? ('studio-draft' as const) : ('application-bundle' as const),
        catalogRevision: document.catalog.revision,
        registryDigest: document.catalog.functionRegistryDigest,
        activeProfile: document.activeProfile,
        meaningCount: allMeanings.length,
        codeOwnedCount,
        studioOwnedCount: allMeanings.length - codeOwnedCount,
        diagnostics: state.diagnostics,
        currentView: { area: state.area, previewState: state.previewState, theme: document.tokens.theme },
      });
    },
    subscribe: (listener: (next: StudioState) => void) => {
      listeners.add(listener);
      return () => listeners.delete(listener);
    },
    selectArea: (area: unknown): Outcome<void> => {
      if (area !== 'data-meaning' && area !== 'experience' && area !== 'gallery' && area !== 'inspect')
        return reject('studio.area-invalid', 'Studio area is unsupported.', ['area']);
      setState({ area });
      return { ok: true, value: undefined };
    },
    ...
    setPreviewState: (previewState: unknown): Outcome<void> => {
      if (
        ...
        return reject('studio.preview-state-invalid', 'Studio preview state is unsupported.', ['previewState']);
    ...
    setTheme: (theme: unknown): Outcome<void> => {
      if (theme !== 'light' && theme !== 'dark')
        return reject('studio.theme-invalid', 'Studio theme is unsupported.', ['theme']);
      withDocument(
        { ...document, revision: `${document.revision}-theme`, tokens: { ...document.tokens, theme } },
        true,
      );
      return { ok: true, value: undefined };
    },
    setActiveProfile: (profile: unknown): Outcome<void> => {
      if (profile === null || typeof profile !== 'object' || Array.isArray(profile))
        return reject('studio.profile-invalid', 'The selected Experience profile reference is malformed.', [
          'activeProfile',
        ]);
      const candidate = profile as Record<string, unknown>;
      if (typeof candidate.id !== 'string' || typeof candidate.revision !== 'string')
        return reject('studio.profile-invalid', 'The selected Experience profile reference is malformed.', [
          'activeProfile',
        ]);
      const ref = { id: candidate.id, revision: candidate.revision };
      if (!document.profiles.some((entry) => sameRef(entry.experience, ref)))
        return reject('studio.profile-unknown', 'The selected Experience profile is not in this document.', [
          'activeProfile',
        ]);
