...
import { createMeaningDraft, meaningDigest, type MeaningDraft, type MeaningSource } from '@aeliqo/runtime/meaning';
...
function ref(value: unknown, name: string): Outcome<VersionRef> {
  if (value === null || typeof value !== 'object' || Array.isArray(value))
    return fail('studio.invalid-reference', `${name} must be a version reference.`, [name]);
  const candidate = value as Record<string, unknown>;
  const id = text(candidate.id, `${name}.id`);
  if (!id.ok) return id;
  const revision = text(candidate.revision, `${name}.revision`);
  if (!revision.ok) return revision;
  if (Object.keys(candidate).some((key) => key !== 'id' && key !== 'revision'))
    return fail('studio.invalid-reference', `${name} contains an unknown property.`, [name]);
...
function normalizeSource(value: unknown, path: readonly (string | number)[]): Outcome<MeaningSource> {
  if (value === null || typeof value !== 'object' || Array.isArray(value))
    return fail('studio.meaning-source', 'Meaning source must be a bounded object.', path);
  const candidate = value as Record<string, unknown>;
  if (Object.keys(candidate).some((key) => !['surface', 'ownership', 'readOnly', 'ownerId'].includes(key)))
    return fail('studio.meaning-source', 'Meaning source contains an unknown property.', path);
  if (
    !['code', 'studio', 'ai-assisted'].includes(String(candidate.surface)) ||
    !['code', 'session', 'personal', 'workspace', 'organization'].includes(String(candidate.ownership))
  )
  ...
  if (candidate.readOnly !== undefined && typeof candidate.readOnly !== 'boolean')
    return fail('studio.meaning-source', 'Meaning source readOnly must be boolean.', [...path, 'readOnly']);
...
function normalizeDraft(
  value: unknown,
  options: StudioDocumentOptions,
  catalog: Catalog,
  index: number,
  definitions: readonly MeaningDefinition[],
): Outcome<MeaningDraft> {
  if (value === null || typeof value !== 'object' || Array.isArray(value))
    return fail('studio.meaning-draft', 'Meaning entries must be versioned drafts.', ['meanings', index]);
  const candidate = value as Record<string, unknown>;
  if (candidate.version !== '1')
    return fail('studio.meaning-draft', 'Meaning draft version is unsupported.', ['meanings', index, 'version']);
  ...
  if (!Array.isArray(assumptions) || assumptions.some((entry) => typeof entry !== 'string' || entry.length > 512))
    return fail('studio.meaning-draft', 'Meaning assumptions must be bounded text.', [
      'meanings',
      index,
      'assumptions',
    ]);
  const authoring = createMeaningDraft(
    meaning as MeaningDefinition,
    { catalog, registry: options.registry, definitions },
    {
      source: source.value,
  ...
  if (!authoring.ok)
    return {
      ok: false,
      diagnostics: authoring.diagnostics.map((diagnostic) => ({
        ...diagnostic,
        path: ['meanings', index, ...(diagnostic.path ?? [])],
      })) as unknown as [Diagnostic, ...Diagnostic[]],
    };
  if (candidate.digest !== authoring.value.digest)
    return fail('studio.meaning-digest', 'Meaning draft digest does not match its canonical contents.', [
      'meanings',
      index,
      'digest',
    ]);
...
function normalizeProfile(value: unknown, index: number): Outcome<StudioProfile> {
  if (value === null || typeof value !== 'object' || Array.isArray(value))
    return fail('studio.profile', 'Experience profiles must be objects.', ['profiles', index]);
  const candidate = value as Record<string, unknown>;
  if (Object.keys(candidate).some((key) => !['label', 'source', 'experience'].includes(key)))
    return fail('studio.profile', 'Experience profile contains an unknown property.', ['profiles', index]);
  ...
  if (!experience.ok)
    return {
      ok: false,
      diagnostics: experience.diagnostics.map((diagnostic) => ({
        ...diagnostic,
        path: ['profiles', index, 'experience', ...(diagnostic.path ?? [])],
      })) as unknown as [Diagnostic, ...Diagnostic[]],
    };
...
function validateDocument(value: unknown, options: StudioDocumentOptions): Outcome<StudioDocument> {
  if (value === null || typeof value !== 'object' || Array.isArray(value))
    return fail('studio.document', 'Studio document must be a plain object.');
  const candidate = value as StudioDocumentInput;
  if (
    Object.keys(candidate as Record<string, unknown>).some(
      (key) =>
        !['version', 'id', 'revision', 'catalog', 'meanings', 'profiles', 'activeProfile', 'tokens'].includes(key),
    )
  )
    return fail('studio.document', 'Studio document contains an unknown property.');
  ...
  if (!catalogResult.ok)
    return {
      ok: false,
      diagnostics: catalogResult.diagnostics.map((diagnostic) => ({
        ...diagnostic,
        path: ['catalog', ...(diagnostic.path ?? [])],
      })) as unknown as [Diagnostic, ...Diagnostic[]],
    };
  const catalog = catalogResult.value;
  if (catalog.functionRegistryDigest !== options.registry.digest)
    return fail('studio.registry-stale', 'The function registry does not match the catalog pin.', [
      'catalog',
      'functionRegistryDigest',
    ]);
  if (!Array.isArray(candidate.meanings) || candidate.meanings.length > 512)
    return fail('studio.meanings', 'Studio meaning drafts must be a bounded array.', ['meanings']);
  ...
  for (let index = 0; index < candidate.meanings.length; index += 1) {
    ...
    if (draft.version !== '1')
      return fail('studio.meaning-draft', 'Meaning draft version is unsupported.', ['meanings', index, 'version']);
    if (draft.meaning === null || typeof draft.meaning !== 'object' || Array.isArray(draft.meaning))
      return fail('studio.meaning-draft', 'Meaning draft contents must be an object.', ['meanings', index, 'meaning']);
    ...
    if (typeof rawId === 'string' && typeof rawRevision === 'string') {
      ...
      if (prior !== undefined)
        return fail(
          prior === digest ? 'studio.meaning-duplicate' : 'studio.meaning-conflict',
          'A meaning ID and revision may occur only once in a Studio document.',
  ...
  const meaningBundle = validateMeaningBundle(
    { catalogRevision: catalog.revision, functionRegistryDigest: options.registry.digest, meanings: rawMeanings },
    { catalog, registry: options.registry, definitions: catalog.meanings },
  );
  ...
  for (let index = 0; index < candidate.meanings.length; index += 1) {
    ...
    if (codeMeaning !== undefined) {
      return fail(
        meaningDigest(draft.value.meaning) === meaningDigest(codeMeaning)
          ? 'studio.meaning-duplicate'
          : 'studio.meaning-conflict',
        'A document draft cannot shadow a catalog-owned meaning version.',
        ['meanings', index, 'meaning'],
      );
    }
    drafts.push(draft.value);
  }
  if (!Array.isArray(candidate.profiles) || candidate.profiles.length === 0 || candidate.profiles.length > 64)
    return fail('studio.profiles', 'Studio requires at least one bounded Experience profile.', ['profiles']);
  ...
  for (let index = 0; index < candidate.profiles.length; index += 1) {
    ...
    if (profileRefs.has(key))
      return fail('studio.profile-conflict', 'Experience profile IDs and revisions must be unique.', [
        'profiles',
        index,
        'experience',
      ]);
  ...
  if (!profiles.some((profile) => sameRef(profile.experience, activeProfile.value)))
    return fail('studio.active-profile', 'The active profile is not present in the document.', ['activeProfile']);
  if (candidate.tokens === null || typeof candidate.tokens !== 'object' || Array.isArray(candidate.tokens))
    return fail('studio.tokens', 'Studio tokens must be an object.', ['tokens']);
  const tokens = candidate.tokens as Record<string, unknown>;
  if (Object.keys(tokens).some((key) => !['profile', 'theme'].includes(key)))
    return fail('studio.tokens', 'Studio tokens contain an unknown property.', ['tokens']);
  const tokenProfile = ref(tokens.profile, 'tokens.profile');
  if (!tokenProfile.ok) return tokenProfile;
  const theme = tokens.theme;
  if (theme !== 'light' && theme !== 'dark')
    return fail('studio.tokens', 'Studio theme must be light or dark.', ['tokens', 'theme']);
  const document: StudioDocument = {
    version: '1',
    id: id.value,
    revision: revision.value,
    catalog,
    meanings: freeze(drafts),
...
export function parseStudioDocument(input: unknown, options: StudioDocumentOptions): Outcome<StudioDocument> {
  if (typeof input === 'string') {
    if (input.length > 2_000_000)
      return fail('studio.document-size', 'Studio document exceeds the local export limit.');
    try {
...
export function serializeStudioCode(document: StudioDocument, options: StudioDocumentOptions): Outcome<string> {
  ...
  return serialized.ok
    ? {
        ok: true,
        value: `/** Generated by Aeliqo Studio. Review before activation. */\nexport const studioDocument = ${serialized.value} as const;\n`,
      }
    : serialized;
}
...
export function activeStudioProfile(document: StudioDocument): StudioProfile {
  return (
    document.profiles.find((profile) => sameRef(profile.experience, document.activeProfile)) ?? document.profiles[0]!
  );
}
