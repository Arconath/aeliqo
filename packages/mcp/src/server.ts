...
export function createAeliqoServer(
  options: { port?: number; timeoutMs?: number; allowedOrigins?: readonly string[] } = {},
) {
  ...
  const invoke = async (method: BridgeRequest["method"], params: unknown) => {
    try {
  ...
  for (const contract of capabilityContracts) {
    server.registerTool(contract.id, {description:contract.description,inputSchema:contract.inputSchema}, (params: unknown) => invoke(contract.id,params));
  }
  return {
    server,
    bridge,
