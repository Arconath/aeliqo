...
for (let index = 0; index < args.length; index += 2) {
  ...
  if (!allowed.has(flag) || value === undefined || options.has(flag)) throw new Error('Use --report FILE --output FILE --corpus FILE --config FILE');
...
const [reportBytes, corpusBytes, configBytes] = await Promise.all([readFile(reportPath), readFile(corpusPath), readFile(configPath)]);
const corpus = JSON.parse(corpusBytes.toString('utf8'));
if (!Array.isArray(corpus)) throw new Error('The supplied corpus must be a JSON array');
const root = resolve(fileURLToPath(new URL('../..', import.meta.url)));
const currentSourceDigest = execFileSync('python3', ['scripts/gate.py', 'digest'], {cwd: root, encoding: 'utf8'}).trim();
const qualification = {
