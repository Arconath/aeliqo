...
if (!candidatePath || !publicationPath || !consumerPath || !rc || Number(rc[1]) < 2 || !/^[0-9a-f]{40}$/.test(sourceRevision ?? '')) {
  throw new Error('Approved RC verification requires candidate/publication/consumer files, 0.1.0-rc.N (N >= 2), and a full source SHA');
}
const [candidate, publication, consumer] = await Promise.all([
  readJson(resolve(root, candidatePath)), readJson(resolve(root, publicationPath)), readJson(resolve(root, consumerPath)),
]);
assertApprovedRc({candidate, publication, consumer, version, sourceRevision});
console.log(JSON.stringify({version, sourceRevision, packages: candidate.packages.length, provenanceVerified: true}, null, 2));
