...
import {
  NPM_ORG, NPM_REGISTRY, assertBootstrapAuthority, assertBootstrapPackageHistory, assertBootstrapRegistryReset, assertCandidateIdentity, assertCandidateTarball,
...
function publishTarball(path) {
  const publishArgs = ['publish', path, '--access', 'public', '--registry', NPM_REGISTRY];
...
function removeDistTag(name, distTag) {
  const result = spawnSync('npm', ['dist-tag', 'rm', name, distTag, '--registry', NPM_REGISTRY], {cwd: root, stdio: 'inherit', timeout: 300_000});
  if (result.error || result.status !== 0) throw new Error(`npm could not remove unexpected ${distTag} from ${name}`);
}
...
async function registryPackage(item) {
  ...
  const timer = setTimeout(() => controller.abort(), 20_000);
  try {
    const response = await fetch(`${NPM_REGISTRY}/${encodeURIComponent(item.name)}`, {redirect: 'error', signal: controller.signal, headers: {accept: 'application/json'}});
    if (response.status === 404) return {exists: false, selected: undefined, tags: {}, versions: [], deprecatedVersions: []};
    ...
    if (!tags || typeof tags !== 'object' || Array.isArray(tags)) throw new Error(`Registry returned malformed ${item.name} dist-tags`);
...
if (bootstrap) {
  ...
  assertBootstrapAuthority({
    ...
    tfa: commandJson('npm', ['profile', 'get', 'tfa', '--json', '--registry', NPM_REGISTRY]),
...
for (const item of candidate.packages) {
  if (basename(item.file) !== item.file || !/^aeliqo-[a-z]+-0\.1\.0(?:-rc\.[1-9]\d*)?\.tgz$/.test(item.file)) throw new Error(`Unsafe candidate tarball name for ${item.name}`);
  ...
  if (bootstrap) assertBootstrapPackageHistory({
    ...
    deprecatedVersions: registryPackageState.exists ? registryPackageState.deprecatedVersions : [],
  ...
  prepared.push({item, tarball, before, beforePackage: registryPackageState});
}
...
for (const {item, tarball, before, beforePackage} of prepared) {
  ...
  for (let attempt = 0; attempt < 24; attempt += 1) {
    ...
    try { after = await registryState(item); afterPackage = await registryPackage(item); } catch (error) { if (attempt === 23) throw error; }
  ...
  if (afterPackage?.selected !== candidate.version) throw new Error(`Registry dist-tag ${tag} does not select verified ${item.name}@${candidate.version}`);
  const reconciledTags = [];
  if (bootstrap) {
    const reconciliation = bootstrapTagReconciliation({
      name: item.name, desiredVersion: candidate.version,
      beforeTags: beforePackage.tags, afterTags: afterPackage.tags,
    });
    for (const distTag of reconciliation.removeTags) {
      removeDistTag(item.name, distTag);
      reconciledTags.push(distTag);
    }
    if (reconciledTags.length) {
      let verifiedTags;
      for (let attempt = 0; attempt < 12; attempt += 1) {
        if (attempt) await new Promise(resolvePromise => setTimeout(resolvePromise, 5_000));
        const current = await registryPackage(item);
        try {
          bootstrapTagReconciliation({
            name: item.name, desiredVersion: candidate.version,
            beforeTags: beforePackage.tags, afterTags: current.tags,
          });
          if (!reconciledTags.some(distTag => current.tags[distTag] === candidate.version)) { verifiedTags = current.tags; break; }
        } catch (error) { if (attempt === 11) throw error; }
      }
      if (!verifiedTags) throw new Error(`Registry did not remove unexpected prerelease dist-tags for ${item.name}`);
      afterPackage = {...afterPackage, tags: verifiedTags, selected: verifiedTags[tag]};
    }
  }
  report.packages.push({name: item.name, integrity: item.integrity, distTag: tag, action, reconciledTags});
