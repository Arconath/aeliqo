...
async function expectAbsent(name, version) {
  ...
  try {
    ...
    if (response.status !== 404) throw new Error(`Expected ${name}${version ? `@${version}` : ''} to be absent after the hold; registry returned HTTP ${response.status}`);
  } finally {
    clearTimeout(timer);
  }
}

const tracked = spawnSync('git', ['status', '--porcelain', '--untracked-files=no'], {cwd: root, encoding: 'utf8', timeout: 30_000});
if (tracked.error || tracked.status !== 0 || tracked.stdout.trim()) throw new Error('Run the post-hold preflight from a clean tracked worktree');

const preflight = JSON.parse(await readFile(preflightPath, 'utf8'));
const notBefore = Date.parse(preflight.conservativePublishNotBefore ?? '');
const now = Date.now();
if (!Number.isFinite(notBefore) || now < notBefore) throw new Error(`npm package-name hold remains active until ${preflight.conservativePublishNotBefore ?? 'an unknown time'}`);
...
if (visible === null || typeof visible !== 'object' || Array.isArray(visible) || Object.keys(visible).length !== 0) {
  throw new Error('The organization is not empty after the owner-directed unpublish; inspect registry history before bootstrap');
}
...
const refreshed = {
  ...
  packages: PUBLIC_PACKAGE_NAMES.map(name => ({
    name, localVersion: '0.1.0', registryStatus: 'public-404-post-hold', exactTarget: 'not-visible',
  })),
  postHoldVerifiedAt: observedAt,
  blockedReason: 'owner-bootstrap-not-yet-run',
  availabilityCaveat: 'Authenticated owner access and public registry reads found the exact direct six-package RC identities absent after the hold. Successful exact first publication remains the registry acceptance proof.',
};
await writeFile(preflightPath, JSON.stringify(refreshed, null, 2) + '\n');
console.log(JSON.stringify({observedAt, actor: NPM_OWNER, packages: PUBLIC_PACKAGE_NAMES.length, preflight: preflightPath}, null, 2));
