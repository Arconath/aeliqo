...
const refKey = (r) => JSON.stringify([r.id, r.revision, r.outputId, r.queryDigest, r.scopeDigest]);
...
function callbackOutcome(raw, failureCode, failureMessage) {
    if (isThenable(raw))
...
function parseRendererCapabilities(input) {
    ...
    if (!parsed.success)
        return fail('renderer', 'Renderer capability references are malformed or exceed their limit.');
...
export function preparePresentationContext(context) {
    ...
    try {
        for (const input of context.results) {
            ...
            if (seen.has(key))
                return fail('results', 'The descriptor list repeats a result reference.');
...
function patternIsAllowed(pattern, constraints) {
    return constraints.allowedPatterns.includes(pattern.ref.id);
}
function matchesPattern(plan, prepared, registry, requiredPattern) {
    ...
    for (const pattern of patterns) {
        try {
            ...
            if (isThenable(matched) || typeof matched !== 'boolean')
                continue;
            if (matched)
                return true;
        }
...
export function validatePresentationPlan(input, context, registry, options = {}) {
...
function validatePreparedPresentationPlanOriginal(input, context, registry, preparedContext, options = {}, nodeMemo) {
    ...
    if (plan.nodes.length === 0 || plan.nodes.length > c.maxNodes)
    ...
    if (!rendererCapabilities.ok)
        return rendererCapabilities;
    const readSet = validateCommitReadSet(plan.preconditions, current, [...prepared.value.taskStructure.resultReferences, ...plan.nodes.flatMap(n => n.result === undefined ? [] : [n.result])]);
    if (!readSet.ok)
        return readSet;
    const nodes = new Map(plan.nodes.map(node => [node.id, node]));
    if (nodes.size !== plan.nodes.length || !nodes.has(plan.rootId))
        return fail('tree', 'Node IDs must be unique and the root must exist.');
    const parents = new Map();
    for (const node of plan.nodes) {
        for (const child of node.children) {
            if (!nodes.has(child) || child === plan.rootId || parents.has(child))
    ...
    for (let i = 0; i < visit.length; i++) {
        const id = visit[i];
        if (visited.has(id))
            return fail('tree', 'Containment cycles are not allowed.');
        visited.add(id);
        visit.push(...nodes.get(id).children);
    }
    if (visited.size !== nodes.size)
        return fail('tree', 'All nodes must be reachable from the declared root.');
    ...
    for (const node of plan.nodes) {
        ...
        if (m === undefined || !renderer.has(key))
            return fail('renderer', 'The requested representation is unavailable on this renderer.');
        if (!c.allowedRepresentations.includes(m.ref.id) || (m.extension && !extensions.has(key)))
            return fail('restricted', 'The experience does not allow this representation.');
        if (!m.roles.includes(node.role) || versionKey(node.config.schema) !== versionKey(m.configSchema))
            return fail('configuration', 'The representation role or configuration schema does not match its registration.');
        if (node.children.length < m.children.min || node.children.length > m.children.max)
        ...
        let output;
        try {
            output = m.resolveConfig(node.config.values, result, node);
        }
        catch {
        ...
        if (!config.success || new Set(config.data.fields).size !== config.data.fields.length)
        ...
        if (config.data.fields.some(field => result === undefined || !resultFieldIds.get(result).has(field)))
        ...
        if (!portGraph.ok)
            return portGraph;
        const values = freezePresentation(config.data.values);
        ...
        if (m.assess !== undefined) {
            let assessed;
            try {
                assessed = m.assess(resolvedConfig, result, prepared.value.environment);
            }
            catch {
        ...
        if (memoKey !== undefined)
            nodeMemo?.set(memoKey, resolvedNode);
    }
    ...
    for (const entry of plan.coverage) {
        ...
        if (need === undefined || !entry.operations.some(op => versionKey(op) === versionKey(need.operation)))
            return fail('coverage', 'Coverage must identify a declared task operation.');
        const fields = new Set();
        for (const id of entry.nodeIds) {
            const n = byId.get(id);
            if (n === undefined)
                return fail('coverage', 'Coverage points to a missing node.');
            if (need.outputId !== undefined && n.result?.ref.outputId !== need.outputId)
                return fail('coverage', 'The operation is bound to a different task output.');
            if (!entry.operations.every(op => n.config.operations.some(supported => versionKey(supported) === versionKey(op))))
                return fail('coverage', 'The representation configuration does not support the claimed operation.');
            if (c.allowedOperations !== undefined && !entry.operations.every(op => c.allowedOperations.some(allowed => versionKey(allowed) === versionKey(op))))
        ...
        coverage.set(need.id, entry);
        entry.operations.forEach(op => operations.add(op.id));
    }
        ...
        for (const id of group) {
            ...
            for (let parent = parents.get(child); parent !== undefined; parent = parents.get(child)) {
                ...
                if (manifests.get(versionKey(ancestor.manifest)).visibility === 'exclusive') {
                    if (branches.has(parent) && branches.get(parent) !== child)
    ...
    if (context.incumbent !== undefined) {
        ...
        if (!old.ok)
            return old;
        const oldReadSet = validateCommitReadSet(old.value.preconditions, current, old.value.nodes.flatMap(n => n.result === undefined ? [] : [n.result]));
        if (!oldReadSet.ok)
            return oldReadSet;
        const stateCapabilities = parseRendererCapabilities(context.stateMappingCapabilities ?? []);
        if (!stateCapabilities.ok)
            return stateCapabilities;
        const topology = (p) => JSON.stringify(p.nodes.map(n => [n.id, n.role, n.children]).sort((a, b) => (String(a[0]) < String(b[0]) ? -1 : String(a[0]) > String(b[0]) ? 1 : 0)));
        const variants = (p) => JSON.stringify(p.nodes.map(n => [n.id, versionKey(n.representation)]).sort());
        const structural = topology(old.value) !== topology(plan);
        const replacing = variants(old.value) !== variants(plan);
        const configurations = (p) => JSON.stringify(p.nodes.map(n => [n.id, n.config]).sort());
        const configurationChanged = configurations(old.value) !== configurations(plan);
        const semantics = (p) => JSON.stringify([p.links, p.coverage, p.nodes.map(n => [n.id, n.result]).sort()]);
        const semanticChanged = semantics(old.value) !== semantics(plan);
        if ((!c.compositionChangeAllowed && structural) || (!c.representationReplacementAllowed && replacing))
        ...
        for (const transfer of plan.stateTransfer) {
            ...
            const from = old.value.nodes.find(n => n.id === transfer.fromNode);
            const to = nodes.get(transfer.toNode);
    ...
    else if (plan.stateTransfer.length)
...
let validatePreparedPresentationPlanDepth=0;
export function validatePreparedPresentationPlan(...args) {const outer=validatePreparedPresentationPlanDepth++===0;const start=outer?performance.now():0;try{return validatePreparedPresentationPlanOriginal(...args);}finally{validatePreparedPresentationPlanDepth--;if(outer){const stats=globalThis.__functionDiagnostic??=Object.create(null);const entry=stats['validatePreparedPresentationPlan']??={calls:0,ms:0};entry.calls++;entry.ms+=performance.now()-start;stats['validatePreparedPresentationPlan']=entry;}}}
