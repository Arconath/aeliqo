...
sdk_version="$(jq -er '.version | select(test("^[0-9]+\\.[0-9]+\\.[0-9]+$"))' release-metadata.json)"

image="ghcr.io/arconath/aeliqo-relay"
tag="${SOURCE_SHA}-${GITHUB_RUN_ID}-${GITHUB_RUN_ATTEMPT}"
reference="$image:$tag"
output="apps/relay/artifacts/production-image"
mkdir -p "$output"

docker_config="$(mktemp -d "$RUNNER_TEMP/aeliqo-relay-registry.XXXXXX")"
trivy_dir="$(mktemp -d "$RUNNER_TEMP/aeliqo-relay-trivy.XXXXXX")"
...
builder_name="aeliqo-relay-$GITHUB_RUN_ID-$GITHUB_RUN_ATTEMPT"
...
python3 apps/site/scripts/ci/verify-buildkit-provenance.py \
	--repository "$image" \
	--index-digest "$digest" \
	--source "$SOURCE_SHA" \
...
export TRIVY_CACHE_DIR="$RUNNER_TEMP/aeliqo-relay-trivy-cache"
