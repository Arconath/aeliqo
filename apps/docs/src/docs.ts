...
async function updateSearch(): Promise<void> {
  ...
  status.textContent = query
    ? `${ranked.length} matching pages${ranked.length > matches.length ? `; first ${matches.length} shown` : ''}.`
    : 'Browse documentation or type to narrow the results.';
...
async function openSearch(): Promise<void> {
  ...
  try {
    ...
    status.textContent = 'Search is unavailable. Use the documentation navigation or the no-JavaScript search page.';
  }
}
...
if (componentMount) {
  void Promise.resolve()
    .then(() => {
      const full = componentMount.dataset.componentPreview ?? '';
      const candidate = full.slice(full.indexOf('.') + 1);
      const id = CATALOG_EXAMPLE_IDS.find((item) => item === candidate);
      if (!id) throw Error('The component example is unavailable.');
      ...
      const cleanup = catalogExample(id, previewMount);
      window.addEventListener('pagehide', cleanup, { once: true });
      void syncComponentTheme(previewMount);
    ...
    .catch(() => {
      ...
      if (previewStatus) {
        ...
        previewStatus.textContent = 'The interactive example could not load. Use the public API below.';
      }
    });
}
