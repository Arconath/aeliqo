...
const SIZING_PROPERTIES = [
 'min-inline-size','max-inline-size','inline-size','min-block-size','max-block-size','block-size',
 'width','height','overflow','overflow-x','overflow-y','grid-template-columns','grid-auto-flow',
 'flex-wrap','aspect-ratio',
];
const WORKLOAD_PROPERTY = /^(?:max[A-Z]|pageSize$|virtualized$|virtualStart$|virtualCount$|overscan$|windowSize$|capacity$|budget$)/u;
const BOUND_CONSTANT = /(?:^|_)(?:MAX|LIMIT|CAPACITY|BUDGET)(?:_|$)/u;
const compact=value=>value.replace(/\s+/g,' ').trim();
const identifierPattern=name=>new RegExp(`\\b${name.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}\\b`,'u');

function visit(node,callback){callback(node);node.forEachChild(child=>visit(child,callback));}
...
function classCss(node,source){
 const styles=[];
 visit(node,current=>{if(ts.isTaggedTemplateExpression(current)&&current.tag.getText(source)==='css')styles.push(templateText(current.template));});
 return styles.join('\n');
}

function importedNames(statement){
 const names=[];const clause=statement.importClause;
...
function numericValue(node,constants,seen=new Set()){
 ...
 if(ts.isIdentifier(node)){
  ...
  const declaration=constants.get(node.text);if(!declaration)return undefined;
 ...
 if(ts.isBinaryExpression(node)){
  const left=numericValue(node.left,constants,seen);const right=numericValue(node.right,constants,seen);
...
function boundSentences(node,source){
 const result=[];const text=node.getText(source);const constants=sourceConstants(source);
 for(const [name,initializer]of constants){
  ...
  if(value!==undefined&&Number.isFinite(value))result.push(`The source-defined \`${name}\` limit is ${value.toLocaleString('en-US')}.`);
 }
 return result;
}
...
export async function loadComponentSources(root){
 const classes=new Map();const api=new API({cwd:root});const repositoryRoot=resolve(root,'../../..');
 ...
 for(const path of project.program.getSourceFileNames()){if(!path.startsWith(root+'/')||path.endsWith('.d.ts'))continue;const source=project.program.getSourceFile(path);if(!source)continue;for(const node of source.statements)if(ts.isClassDeclaration(node)&&node.name)classes.set(node.name.text,{node,source,path,repositoryRoot});}
 return classes;
 }finally{api.close();}
}
...
export function componentApi(classes,name){
 ...
 for(const {node,source,repositoryRoot}of lineage){
  ...
  const text=node.getText(source);const cssText=classCss(node,source);
  for(const statement of source.statements){
   if(!ts.isImportDeclaration(statement)||!ts.isStringLiteral(statement.moduleSpecifier))continue;
   const names=importedNames(statement);
   if(names.some(imported=>identifierPattern(imported).test(text)))dependencies.add(statement.moduleSpecifier.text);
  }
  ...
  for(const match of text.matchAll(/\bpart="([a-z][a-z0-9 -]*)"/g))for(const part of match[1].split(' '))parts.add(part);
  ...
  const versionMatch=text.match(/static\s+readonly\s+aeliqoVersion\s*=\s*["']([^"']+)["']/);if(versionMatch)version=versionMatch[1];
 }
 const workload=[...properties.values()].filter(property=>WORKLOAD_PROPERTY.test(property.name)).map(property=>property.default==='undefined'?`\`${property.name}\``:`\`${property.name}\` (default ${compact(property.default)})`);
