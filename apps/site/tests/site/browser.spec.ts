...
test('native WebMCP registers exactly the standard tools and renders through the current Region', async ({page}) => {
  ...
  const result = await page.evaluate(async () => {
    const tools = (globalThis as typeof globalThis & {__aeliqoWebMcpTools: Map<string, {execute(input: unknown): Promise<unknown>}>}).__aeliqoWebMcpTools;
    const names = [...tools.keys()].sort();
    const context = await tools.get('aeliqo_context')?.execute({});
    const render = await tools.get('aeliqo_render')?.execute({version: '1', id: 'webmcp-engineering', kind: 'browse',
      resource: 'people', fields: ['name', 'team', 'location'], filter: {op: 'compare', field: 'team', comparison: 'eq', value: 'Engineering'}});
...
test('home proof uses the public adaptive facade and remains legible on narrow forced-color surfaces', async ({page}) => {
  ...
  expect((await new AxeBuilder({page}).include('main').analyze()).violations.filter(({id}) => id === 'color-contrast')).toEqual([]);
...
test('legal and support pages state the offline, reporting and commercial boundaries without default egress', async ({page}) => {
  const requests: string[] = [];
  page.on('request', request => requests.push(request.url()));
  await page.goto('/');
  ...
  await expect(page.getByRole('link', {name: 'GitHub private vulnerability reporting'})).toHaveAttribute('href', 'https://github.com/Arconath/aeliqo/security/advisories/new');
  await expect(page.getByText(/does not publish a monitored security email address or response-time SLA/)).toBeVisible();
  ...
  expect(requests.some(url => url.includes('/otel/v1/') || url.includes('googletagmanager.com') || url.includes('google-analytics.com'))).toBe(false);
  expect(requests.every(url => new URL(url).hostname === '127.0.0.1')).toBe(true);
});
