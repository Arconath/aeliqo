...
const COMPONENT_ROUTES = componentCatalog.map(({id}) => `/docs/components/${id}/`);
...
test("enabled analytics consent is announced in flow before documentation", async ({page}) => {
  ...
  const headingBox = await page.getByRole("heading", {level: 1, name: "Documentation", exact: true}).boundingBox();
