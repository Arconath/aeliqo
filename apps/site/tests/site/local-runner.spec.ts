...
test('the local runner pairs one browser Region with the three real MCP tools', async ({page, baseURL}, testInfo) => {
  ...
  try {
    ...
    await expect(endpoint.invoke('aeliqo_context', {}, {requestId: 'local-context'})).resolves.toMatchObject({ok: true, value: {state: 'accepted'}});
    await expect(endpoint.invoke('aeliqo_render', {
      version: '1', id: 'local-products', kind: 'browse', resource: 'products', fields: ['name', 'category', 'price'], preferredView: 'cards',
    }, {requestId: 'local-render'})).resolves.toMatchObject({ok: true, value: {state: 'renderer-ready'}});
  ...
  try {
    ...
    expect(discovered.ok && discovered.value.map(({name}) => name).sort()).toEqual(['aeliqo_act', 'aeliqo_context', 'aeliqo_render']);
    await expect(stdio.invoke('aeliqo_context', {}, {requestId: 'stdio-context'})).resolves.toMatchObject({ok: true, value: {state: 'accepted'}});
  } finally {
    stdio.close();
  }
});
...
test('the local runner rejects untrusted origins and invalid MCP credentials', async ({request, baseURL}) => {
  const rejectedOrigin = await request.get(`${baseURL}/api/aeliqo/session`, {headers: {origin: 'https://attacker.example'}});
