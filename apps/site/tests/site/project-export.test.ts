...
const releaseVersion = (JSON.parse(readFileSync(new URL('../../../../release-metadata.json', import.meta.url), 'utf8')) as {version: string}).version;

describe('playground project export', () => {
  it.each(['people', 'products', 'support', 'knowledge'] as const)('builds a bounded installable %s project without credentials', async scenario => {
    const files = projectFiles(scenario, releaseVersion);
    expect(files.map(file => file.path)).toEqual(['package.json', 'tsconfig.json', 'index.html', 'src/main.ts', 'src/style.css', 'README.md']);
    expect(new Set(files.map(file => file.path)).size).toBe(files.length);
    const manifest = JSON.parse(files[0]!.content) as {dependencies: Record<string, string>; scripts: Record<string, string>};
    expect(manifest.dependencies).toMatchObject({'@aeliqo/core': releaseVersion, '@aeliqo/runtime': releaseVersion, '@aeliqo/web': releaseVersion});
