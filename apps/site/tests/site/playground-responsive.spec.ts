...
test('task chooser, inspector summaries, and actionable recovery form a complete people journey', async ({page}) => {
  ...
  const initialSourceWidth = Number(await sourceHandle.getAttribute('aria-valuenow'));
  ...
  await page.mouse.up();
  expect(Number(await sourceHandle.getAttribute('aria-valuenow'))).toBeGreaterThan(initialSourceWidth);
...
test('the complete toolbar reflows without page overflow at 320 pixels', async ({page}) => {
  ...
  await page.locator('#dataset').selectOption('employees');
  const [firstAction, secondAction, thirdAction] = await Promise.all(
    ['#browse-people', '#define-meaning', '#freeze'].map((selector) => page.locator(selector).boundingBox()),
  );
  ...
  expect(thirdAction!.y).toBeGreaterThan(firstAction!.y + firstAction!.height);
});

test('narrow exact tables disclose horizontal navigation without hiding comparison columns', async ({page}) => {
  await page.setViewportSize({width: 390, height: 844});
  await page.goto('/playground/');
  await expect(page.locator('.result-scroll-hint')).toBeVisible();
  await expect(page.locator('.result-scroll-hint')).toContainText('view every column');
  const geometry = await page.locator('aeliqo-table').evaluate(element => {
    const viewport = element.shadowRoot?.querySelector<HTMLElement>('[part="scroll"]');
    return {clientWidth: viewport?.clientWidth ?? 0, scrollWidth: viewport?.scrollWidth ?? 0};
  });
  expect(geometry.scrollWidth).toBeGreaterThan(geometry.clientWidth);
  await page.locator('aeliqo-table').evaluate(element => {
    element.shadowRoot?.querySelector<HTMLElement>('[part="scroll"]')?.focus();
  });
  await page.keyboard.press('ArrowRight');
  expect(await page.locator('aeliqo-table').evaluate(element => element.shadowRoot?.querySelector<HTMLElement>('[part="scroll"]')?.scrollLeft ?? 0)).toBeGreaterThan(0);
  await page.keyboard.press('End');
  const end = await page.locator('aeliqo-table').evaluate(element => element.shadowRoot?.querySelector<HTMLElement>('[part="scroll"]')?.scrollLeft ?? 0);
  expect(end).toBeGreaterThan(0);
  await page.keyboard.press('Home');
  expect(await page.locator('aeliqo-table').evaluate(element => element.shadowRoot?.querySelector<HTMLElement>('[part="scroll"]')?.scrollLeft ?? -1)).toBe(0);
  await page.locator('aeliqo-table').evaluate(element => {
  ...
  expect(await page.locator('aeliqo-table').evaluate(element => element.shadowRoot?.querySelector<HTMLElement>('[part="scroll"]')?.scrollLeft ?? 0)).toBeLessThan(0);
  await page.keyboard.press('End');
  const rtlEnd = await page.locator('aeliqo-table').evaluate(element => {
    const viewport = element.shadowRoot?.querySelector<HTMLElement>('[part="scroll"]');
    return viewport ? {actual: viewport.scrollLeft, expected: viewport.clientWidth - viewport.scrollWidth} : undefined;
  });
  expect(Math.abs((rtlEnd?.actual ?? 0) - (rtlEnd?.expected ?? 0))).toBeLessThanOrEqual(1);
  await page.keyboard.press('ArrowRight');
  expect(await page.locator('aeliqo-table').evaluate(element => element.shadowRoot?.querySelector<HTMLElement>('[part="scroll"]')?.scrollLeft ?? 0)).toBeGreaterThan(rtlEnd?.actual ?? 0);
  await page.keyboard.press('Home');
  expect(await page.locator('aeliqo-table').evaluate(element => element.shadowRoot?.querySelector<HTMLElement>('[part="scroll"]')?.scrollLeft ?? -1)).toBe(0);
  await page.locator('aeliqo-table').evaluate(element => { (element as HTMLElement).dir = 'ltr'; });
  await page.getByRole('button', {name: 'Define absence days', exact: true}).click();
  await page.locator('#task-choice').selectOption('people-rank');
  await page.getByRole('button', {name: 'Run task'}).click();
  await expect(page.locator('#play-status')).toContainText('4 result rows');
  ...
  await expect(page.locator('.result-scroll-hint')).toBeHidden();
});
