...
async function setupHomeDemo(demoRoot: HTMLElement): Promise<void> {
  ...
  async function renderIntent(): Promise<void> {
    ...
    try {
      ...
      setFlow('view');
      if (status) status.textContent = `${selectedTeam === 'all' ? '4' : '2'} of 4 synthetic people in an exact Result · ${receipt.presentation.plan.nodes[0]?.representation.id ?? 'registered view'} · 0 model calls.`;
  ...
  manualButton?.addEventListener('click', () => { if (teamControl) teamControl.value = 'all'; void renderIntent(); });
  void renderIntent();
