import {createQueryFunctionRegistry, defineResource, type Intent} from '@aeliqo/core';
...
const people = defineResource({
  id: 'people', revision: 'home-people-1', label: 'People', identity: ['id'],
...
export function mountPeopleExample(target: HTMLElement): HomePeopleExample {
  const data = createLocalDataService({
    ...
    authorize: ({context}) => context.principal === 'home-public'
      ? {ok: true, value: {scopeDigest: 'home-synthetic', policyRevision: 'home-policy-1'}}
      : {ok: false, diagnostics: [{code: 'home.denied', message: 'The public demo principal is unavailable.', retryable: false}]},
  });
  const app = createAeliqoApp({
    resources: [{resource: people, data}],
    authority: {read: () => ({ok: true, value: {
      principalKey: 'home-public', scopeDigest: 'home-synthetic', policyRevision: 'home-policy-1', experienceRevision: 'home-web-1',
      grants: ['task.evaluate', 'result.inspect', 'experience.commit'], readContext: {principal: 'home-public'},
    }})},
  });
  ...
  return Object.freeze({
    render(team = 'all') {
      const intent: Intent = {
        version: '1', id: `home-browse-${++request}`, kind: 'browse', resource: 'people', fields: ['name', 'team', 'location'],
