...
const baseCatalog = {
  version: '1',
  revision: 'studio-demo-catalog-1',
...
const profile: Experience = {
  version: '1',
  id: 'employee-inspection',
  revision: '1',
  mode: 'adaptive',
  agentAllowed: false,
  allowedRepresentations: ['table', 'metric'],
  allowedPatterns: ['record-inspection'],
  composition: { allowWithoutPreset: true, maxNodes: 16, maxExpansions: 16 },
  requiredOperations: [],
  tokenProfile: { id: 'tokens.aeliqo', revision: '1' },
  extensionAllowlist: [],
      ...
      : [
          {
            ...
            digest: meaningDigest(codeMeaning),
            assumptions: [],
          },
        ],
  profiles: [
    {
      label: 'Employee inspection',
      source: { surface: 'code' as const, ownership: 'code' as const, readOnly: true },
      experience: profile,
    },
  ],
  activeProfile: { id: profile.id, revision: profile.revision },
...
const localSource: QuerySource = {
  revision: 'studio-local-source-1',
  catalogRevision: baseCatalog.revision,
  scopeDigest: 'studio-local',
  policyRevision: 'studio-local-1',
  relations: {
    employees: {
      entity: 'employees',
      complete: true,
      ...
      if (document.activeElement === control) {
        ...
        focusedSelection = {
          start:
            ...
            control instanceof HTMLInputElement || control instanceof HTMLTextAreaElement ? control.selectionEnd : null,
        };
...
function restoreControls(): void {
  root
    ...
    .forEach((control) => {
      ...
      if (
        ...
        if (control.id !== 'profile-select' && [...control.options].some((option) => option.value === saved.value))
  ...
  ].find((candidate) => controlKey(candidate) === focusedControl);
  ...
  if (
    ...
    focusedSelection.start !== null &&
    focusedSelection.end !== null
  ) {
...
root.addEventListener('focusin', (event) => {
  ...
  lastEditableSelection = {
    start: target instanceof HTMLInputElement || target instanceof HTMLTextAreaElement ? target.selectionStart : null,
    end: target instanceof HTMLInputElement || target instanceof HTMLTextAreaElement ? target.selectionEnd : null,
  };
});
...
function diagnosticText(sessionValue: ReturnType<StudioSession['getState']>): string {
  return sessionValue.diagnostics.length === 0
    ? ''
    : sessionValue.diagnostics.map((diagnostic) => `${diagnostic.code}: ${diagnostic.message}`).join(' ');
}
...
function renderDataMeaning(state: ReturnType<StudioSession['getState']>): string {
  ...
  const evaluation =
    firstMeaning === undefined || entity === undefined
      ? undefined
      : session.evaluateMeaning({
          meaning: { id: firstMeaning.id, revision: firstMeaning.revision },
          entity: entity.id,
          source: localSource,
        });
  const evaluationCopy = evaluation?.ok ? JSON.stringify(evaluation.value.rows) : undefined;
  const evaluationError =
    evaluation !== undefined && !evaluation.ok
      ? evaluation.diagnostics.map((diagnostic) => `${diagnostic.code}: ${diagnostic.message}`).join(' ')
      : undefined;
...
function renderExperience(state: ReturnType<StudioSession['getState']>): string {
  const active =
    state.document.profiles.find(
      (entry) =>
        entry.experience.id === state.document.activeProfile.id &&
        entry.experience.revision === state.document.activeProfile.revision,
    ) ?? state.document.profiles[0]!;
...
function bindExperiencePreview(state: StudioState): void {
  ...
  const evaluation =
    entity === undefined || meaning === undefined
      ? undefined
      : session.evaluateMeaning({
  ...
  const value =
    state.previewState === 'loading' || state.previewState === 'empty' || state.previewState === 'error'
      ? undefined
      : evaluatedValue;
  const scope = {
    kind: 'filtered',
    loaded: localSource.relations.employees?.rows.length ?? 0,
    filteredTotal: localSource.relations.employees?.rows.length ?? 0,
    populationDigest: localSource.scopeDigest,
    label: 'Authorized employees',
  };
  root.querySelectorAll<HTMLElement>('[data-preview-metric]').forEach((metric) =>
    Object.assign(metric, {
      label: previewLongLabel
        ? `${meaning?.label ?? 'Local result'} from the authorized employee population`
        : (meaning?.label ?? 'Local result'),
      value,
...
function bindEvents(): void {
  root
    ...
    .forEach((button) => button.addEventListener('click', () => session.selectArea(button.dataset.area as StudioArea)));
  root
    .querySelectorAll<HTMLButtonElement>('[data-preview]')
    .forEach((button) =>
      button.addEventListener('click', () =>
        session.setPreviewState(button.dataset.preview as ReturnType<StudioSession['getState']>['previewState']),
      ),
    );
  root
    .querySelectorAll<HTMLButtonElement>('[data-theme]')
    .forEach((button) =>
      button.addEventListener('click', () => session.setTheme(button.dataset.theme as 'light' | 'dark')),
    );
  root.querySelector<HTMLSelectElement>('#preview-direction')?.addEventListener('change', (event) => {
    const select = event.currentTarget;
    if (!(select instanceof HTMLSelectElement)) return;
    previewDirection = select.value === 'rtl' ? 'rtl' : 'ltr';
    render(session.getState());
  });
  root.querySelector<HTMLInputElement>('#preview-long-label')?.addEventListener('change', (event) => {
    const checkbox = event.currentTarget;
    if (!(checkbox instanceof HTMLInputElement)) return;
    previewLongLabel = checkbox.checked;
    render(session.getState());
  });
  root.querySelector<HTMLSelectElement>('#gallery-component-select')?.addEventListener('change', (event) => {
    const select = event.currentTarget;
    if (!(select instanceof HTMLSelectElement)) return;
    session.selectComponent(select.value);
  });
  root.querySelectorAll<HTMLButtonElement>('[data-meaning]').forEach((button) =>
    button.addEventListener('click', () => {
      const [id, revision] = button.dataset.meaning!.split('@');
      session.selectMeaning({ id: id!, revision: revision! });
      session.selectArea('inspect');
    }),
  );
  const profileSelect = root.querySelector<HTMLSelectElement>('#profile-select');
  profileSelect?.addEventListener('change', () => {
    const [id, revision] = profileSelect.value.split('@');
    session.setActiveProfile({ id: id!, revision: revision! });
  });
  root.querySelector<HTMLFormElement>('#experience-edit-form')?.addEventListener('submit', (event) => {
    ...
    const active = session
      ...
      .document.profiles.find(
        (entry) =>
          entry.experience.id === session.getState().document.activeProfile.id &&
          entry.experience.revision === session.getState().document.activeProfile.revision,
      );
    if (active === undefined) return;
    const data = new FormData(form);
    const candidateExperience = parseExperience({
      ...active.experience,
      id: active.experience.id,
      revision: String(data.get('profile-revision')),
      mode: String(data.get('profile-mode')),
    });
    if (!candidateExperience.ok) {
      session.showDiagnostics(candidateExperience.diagnostics);
      return;
    }
    const result = session.editExperience({
      base: { id: active.experience.id, revision: active.experience.revision },
      label: String(data.get('profile-label')),
      experience: candidateExperience.value,
    });
    if (!result.ok) session.showDiagnostics(result.diagnostics);
  });
  root.querySelector<HTMLFormElement>('#meaning-form')?.addEventListener('submit', (event) => {
    event.preventDefault();
    const form = event.currentTarget;
    if (!(form instanceof HTMLFormElement)) return;
    const data = new FormData(form);
    const result = session.defineMeaning({
  ...
  root.querySelector<HTMLButtonElement>('#export-button')?.addEventListener('click', () => {
    const exported = session.exportDocument();
    if (!exported.ok) return;
    const blob = new Blob([exported.value], { type: 'application/json' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = `${session.getState().document.id}.aeliqo.json`;
    link.click();
    URL.revokeObjectURL(link.href);
  });
  root.querySelector<HTMLButtonElement>('#export-code-button')?.addEventListener('click', () => {
    const exported = session.exportCode();
    if (!exported.ok) return;
    const blob = new Blob([exported.value], { type: 'text/typescript' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = `${session.getState().document.id}.aeliqo.ts`;
    link.click();
    URL.revokeObjectURL(link.href);
  });
  root
    .querySelector<HTMLButtonElement>('#import-button')
    ?.addEventListener('click', () => root.querySelector<HTMLInputElement>('#import-file')?.click());
  root.querySelector<HTMLInputElement>('#import-file')?.addEventListener('change', async (event) => {
    const input = event.currentTarget;
    if (!(input instanceof HTMLInputElement) || input.files?.[0] === undefined) return;
    session.importDocument(await input.files[0].text());
  });
}

session.subscribe((state) => {
  document.documentElement.dataset.theme = state.document.tokens.theme;
  render(state);
});
