...
function modelConfiguration() {
  ...
  if (apiKey === undefined || modelName === undefined || baseURL === undefined)
...
const server = createServer(async (request, response) => {
  try {
    ...
    if (url.pathname === '/api/aeliqo/events' && request.method === 'GET') {
      ...
      request.once('close', () => {
        clearInterval(heartbeat);
        if (session === current) current.broker.detach(attachment);
      });
      return;
    }
        ...
        body.operation === 'discover'
          ...
          : body.operation === 'invoke' && typeof body.name === 'string' && typeof body.requestId === 'string'
            ...
            : {
                ok: false,
                diagnostics: [
                  { code: 'playground.local-bridge', message: 'The bridge operation is invalid.', retryable: false },
                ],
              };
    ...
    if (url.pathname === '/mcp') {
      await writeFetchResponse(await mcpHandler.fetch(await nodeRequest(request)), response);
      return;
    }
...
server.listen(port, host, () => {
  ...
  process.stderr.write(
    `MCP stdio: AELIQO_LOCAL_URL=${origin} AELIQO_MCP_TOKEN=${mcpToken} pnpm --dir apps/playground mcp:stdio\n`,
  );
});
