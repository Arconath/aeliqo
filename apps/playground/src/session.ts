...
export function createPlaygroundSession(
  ...
  const syncAgentEgress = (): void => {
    agentEgress = webMcp !== undefined || agentEndpoints.size > 0;
  };
    ...
    services.set(
      ...
      createLocalDataService({
        snapshot: {
          ...
          sourceRevision: `demo-${sourceRevision}`,
      ...
      dispatch: ({ input }) => {
        ...
        if (rows === undefined)
          return {
            state: 'rejected',
            diagnostics: [
              { code: 'playground.resource', message: 'The local resource is unavailable.', retryable: false },
            ],
          };
        ...
        if (mode === 'create' && index >= 0)
          return {
            state: 'rejected',
            diagnostics: [{ code: 'playground.duplicate', message: 'That ID already exists.', retryable: false }],
          };
        if (mode === 'update' && index < 0)
          return {
            state: 'rejected',
            diagnostics: [{ code: 'playground.missing', message: 'The record no longer exists.', retryable: false }],
          };
    ...
    host: {
      readContext: () => ({
        ...
        value: {
          ...
          domainRevision: `demo-${sourceRevision}`,
  ...
  const app = createAeliqoApp({
    resources: resources.map((resource) => ({ resource, data: services.get(resource.id)! })),
    authority,
    ...
    onActionEvent,
    formState: {
      read: ({ resource, intent }) => {
        if (intent.kind === 'create') return { ok: true, value: { values: {}, entityRevision: 'new' } };
        const rows = records[resource.id] ?? [];
        const selected = rows.find((row) =>
          Object.entries(intent.identity).every(([field, value]) => row[field] === value),
        );
        if (selected === undefined)
          return failure('playground.form-missing', 'The selected record is no longer available.');
        return {
          ok: true,
  ...
  const connectAgent = async (
    ...
    if (target === undefined)
      return failure('playground.agent-region', 'Render a scenario before connecting an agent.');
    agentEgress = true;
      ...
      render: {
        async render(input) {
          ...
          if (!parsed.ok)
            return {
              status: 'failed',
              requestId: 'playground-agent-intent',
              regionId: REGION_ID,
  ...
  return Object.freeze({
    ...
    async connectWebMcp(): Promise<Outcome<{ readonly registrations: number; readonly evidence: WebMcpEvidence }>> {
      const target = mountedTarget;
      if (target === undefined)
        return failure<{ readonly registrations: number; readonly evidence: WebMcpEvidence }>(
          'playground.webmcp-region',
          'Render a scenario before connecting WebMCP.',
        );
      webMcp?.close();
      webMcp = undefined;
      syncAgentEgress();
        ...
        render: {
          async render(input) {
            ...
            if (!parsed.ok)
              return {
                status: 'failed',
                requestId: 'playground-webmcp-intent',
                regionId: REGION_ID,
                diagnostics: parsed.diagnostics,
              };
            const receipt = await render(target, parsed.value, input.signal);
    ...
    dispose() {
      webMcp?.close();
      webMcp = undefined;
      for (const endpoint of [...agentEndpoints]) endpoint.close();
