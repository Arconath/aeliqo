...
export function evidenceFor(
  ...
  if (section === 'intent')
    return {
      summary: 'The exact bounded request sent by guided controls, application code, or an agent tool.',
      value: evidence.intent ?? {},
    };
  if (section === 'diagnostics')
    return {
      summary: 'Actionable runtime diagnostics. Empty means no diagnostic was emitted for the last request.',
      value: receipt?.diagnostics ?? [],
    };
  ...
  if (section === 'result') {
    ...
    return {
      ...
      value: outputs.map(
        (output) => output.handle.snapshot().descriptor ?? { status: output.handle.snapshot().status },
      ),
    };
  }
