...
function Page({ id }: { id: PageId }) {
  if (id === 'start') return <>
    ...
    <h3>Install the release candidate</h3><p>The packages are being verified for public distribution. Until registry publication is authorized, use the checked tarballs produced by this repository.</p>
    <pre><code>{'pnpm check:packages'}</code></pre>
    <p>The release check builds five allowlisted packages from a clean directory, packs them, installs the tarballs into consumers outside the workspace, checks declarations and server rendering, and examines isolated imports. Find the artifacts in <code>artifacts/tarballs/</code> and the measured results in <code>artifacts/package-evidence.json</code>.</p>
  ...
  return <>
    ...
    <div className="docs-table-wrap"><table><thead><tr><th>Surface</th><th>Current support</th></tr></thead><tbody>
      ...
      <tr><td>License / publication</td><td>Apache-2.0 approved; registry publication and post-publish install smoke remain release actions.</td></tr>
