...
function bridgeCall(value: unknown): BridgeCall | undefined {
  ...
  const call = value;
  if (Object.keys(call).some((key) => !['id', 'transport', 'operation', 'name', 'input', 'requestId'].includes(key))) return undefined;
  ...
  if (call.operation === 'invoke' && (typeof call.name !== 'string' || typeof call.requestId !== 'string')) return undefined;
  return {
    id: call.id,
...
async function invoke(
  ...
  if (call.operation === 'authorize') {
    if (call.transport !== 'byok') {
      return {ok: false, diagnostics: [{code: 'playground.local-operation', message: 'MCP does not expose model authorization.', retryable: false}]};
    }
...
export async function connectLocalHost(session: PlaygroundSession): Promise<LocalHostConnection> {
  ...
  await new Promise<void>((resolve, reject) => {
    const timeout = window.setTimeout(() => reject(new Error('The local host event stream did not become ready.')), 5_000);
    events.addEventListener('open', () => { window.clearTimeout(timeout); resolve(); }, {once: true});
    events.addEventListener('error', () => { window.clearTimeout(timeout); reject(new Error('The local host event stream is unavailable.')); }, {once: true});
  }).catch((cause: unknown) => {
    events.close();
  ...
  events.addEventListener('call', (event) => {
    ...
    void invoke(call, endpoints, controller.signal)
      ...
      .catch(() => acknowledge(call.id, {ok: false, diagnostics: [{code: 'playground.local-call', message: 'The browser rejected the local host call.', retryable: false}]}))
