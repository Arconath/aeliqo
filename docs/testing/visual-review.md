...
Run `pnpm test:visual`. This builds the production packages used by the catalog fixtures, typechecks the review fixtures, and runs Chromium, Firefox, and WebKit. For a focused run after a fresh `pnpm build:platform`, use `pnpm exec playwright test --config tests/visual/playwright.config.mjs field-states`.
