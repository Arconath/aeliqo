...
<p>The fixture-agent comparison is deterministic parity coverage for the same typed intent; it is not a provider evaluation or a claim of user research. The host-only baseline notes and the exact automated outcomes are kept with the source so the release record does not invent commercial or usability results.</p>
