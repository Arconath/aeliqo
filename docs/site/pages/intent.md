...
<h2>Example envelope</h2><p>This browse request asks for the registered <code>people</code> resource, one equality filter on a declared field, and a preferred registered view:</p>
...
<p>Application code passes the same object to <code>app.render({ intent })</code>; an agent sends it through <code>aeliqo_render</code>. Both paths run the identical validation.</p>
