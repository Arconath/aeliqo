...
<h2>Minimal definition</h2>${code('people.ts', `import {defineResource} from '@aeliqo/core';
import {z} from 'zod';
...
<h2>Technical type is not business meaning</h2><p>A numeric field only says how a value is represented. Aggregation, unit, temporal grain, and definitions such as “absence rate” belong in a versioned Catalog meaning. Aeliqo never assumes every number is summable or every date forms a valid trend.</p>
...
${next([{href: '/concepts/semantics/', title: 'Semantic meaning', description: 'Model units, measures, grain, and temporal policy.'}, {href: '/guides/data/', title: 'Connect data', description: 'Bind the resource to a bounded source.'}])}`,
  }),
...
<h2>Choose the correct boundary</h2>${cards([{title: 'Local snapshot', body: 'Use when rows are already authorized and bounded in the browser or worker.', href: '/guides/local-data/', label: 'Local guide'}, {title: 'Application server', body: 'Use the Aeliqo HTTP contract when records or credentials must stay on the server.', href: '/guides/http-data/', label: 'HTTP guide'}, {title: 'Custom source', body: 'Implement the small DataService interface and map filtering, sort, pagination, results, errors, and cancellation explicitly.'}])}
...
${next([{href: '/guides/permissions/', title: 'Permissions', description: 'Supply trusted context to every evaluation and commit.'}, {href: '/concepts/safety/', title: 'Safety model', description: 'Understand why browser validation does not replace server authorization.'}])}`,
  }),
...
<h2>Create the service</h2>${code('data.ts', `const data = createLocalDataService({
  snapshot: {
...
${next([{href: '/start/', title: 'Complete quickstart', description: 'See this adapter in the compiled People example.'}, {href: '/guides/http-data/', title: 'Move execution server-side', description: 'Keep private source records out of the browser.'}])}`,
  }),
...
${next([{href: '/guides/permissions/', title: 'Authority adapter', description: 'Unify evaluator, Region, action, and agent context.'}, {href: '/ship/', title: 'Production checks', description: 'Verify source isolation and failure handling.'}])}`,
  }),
...
<h2>Adapter</h2>${code('authority.ts', `const authority = {
...
${next([{href: '/concepts/safety/', title: 'Safety infrastructure', description: 'See the checks that make invalid proposals non-executable.'}, {href: '/guides/actions/', title: 'Business actions', description: 'Apply the same authority model to writes.'}])}`,
  }),
...
${next([{href: '/guides/forms/', title: 'Forms', description: 'Bind create and edit recipes to registered actions.'}, {href: '/agents/quickstart/', title: 'Agent actions', description: 'Expose preview and execution through the bounded tool endpoint.'}])}`,
  }),
...
<h2>Register form bindings</h2>${code('resource.ts', `const people = defineResource({
...
${next([{href: '/guides/actions/', title: 'Action boundary', description: 'Implement confirmation and idempotency correctly.'}, {href: '/concepts/state-ownership/', title: 'Draft ownership', description: 'Prevent state loss during adaptation.'}])}`,
  }),
...
<h2>Recommended mapping</h2>${code('navigation.ts', `const navigation = {
...
${next([{href: '/guides/responsive-behavior/', title: 'Responsive behavior', description: 'See the default cross-container policy.'}, {href: '/concepts/state-ownership/', title: 'Selection ownership', description: 'Understand which state transfers between views.'}])}`,
  }),
...
${next([{href: '/guides/adaptive-region/', title: 'Adaptive Region', description: 'Use the lifecycle and inspect selected recipes.'}, {href: '/guides/custom-views/', title: 'Custom views', description: 'Add a domain-specific renderer without changing core.'}])}`,
  }),
...
<h2>Lifecycle API</h2>${code('region.ts', `const mounted = app.mount({target, regionId: 'main', resourceId: 'people'});
...
${next([{href: '/reference/app-api/', title: 'App API reference', description: 'Review exact methods, ownership, and lifecycle.'}, {href: '/concepts/safety/', title: 'Commit safety', description: 'See revision and read-set validation.'}])}`,
  }),
...
<h2>Define and register</h2>${code('kanban.ts', `const kanban = defineView({
...
${next([{href: '/examples/knowledge/', title: 'Knowledge example', description: 'See content use a custom presentation outside dashboard UI.'}, {href: '/reference/app-api/', title: 'Definition reference', description: 'Inspect <code>defineView</code> and <code>defineRecipe</code>.'}])}`,
  }),
];
