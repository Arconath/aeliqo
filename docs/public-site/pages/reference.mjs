...
${next([{href: '/reference/packages/', title: 'Package map', description: 'Choose the smallest entry point and understand dependencies.'}, {href: '/reference/app-api/', title: 'App API', description: 'Read method contracts and receipts.'}, {href: '/components/', title: 'Components', description: 'Browse generated public declarations and live examples.'}])}`,
  }),

  definePage('packages', {
    title: 'Packages and entry points',
    description: 'Six version-aligned public packages with explicit browser, server, provider, and framework boundaries.',
...
${next([{href: '/ship/migration-0.1/', title: 'Migrate from 0.1', description: 'Move low-level wiring to the 0.3 facade.'}, {href: '/ship/ssr/', title: 'SSR entries', description: 'Keep DOM code outside server module evaluation.'}])}`,
  }),
...
<h2>subscribe and snapshot</h2>${code('Signatures', `app.subscribe(regionId, listener): () => void
app.snapshot(regionId): RuntimeRegionState | undefined`)}<p>Subscribers receive sanitized lifecycle state, Result references, and diagnostics—not materialized rows. Subscription is safe before the Region child lifecycle completes.</p>
<h2>unmount and dispose</h2>${code('Signatures', `app.unmount(regionId): boolean
app.dispose(): void`)}<p>Unmount cancels and releases one Region. Dispose is idempotent and releases every Region, listener, pending render, Result handle, and owned observer. Calls after disposal fail closed.</p>
...
${next([{href: '/reference/intent-schema/', title: 'Intent schema', description: 'Build valid render inputs.'}, {href: '/reference/diagnostics/', title: 'Diagnostics', description: 'Handle failure and recovery consistently.'}])}`,
  }),
...
${next([{href: '/concepts/intent/', title: 'Intent concepts', description: 'Understand the application and agent boundary.'}, {href: '/agents/quickstart/', title: 'Send from an agent', description: 'Use the exact same schema through <code>aeliqo_render</code>.'}])}`,
  }),
...
<h2>Diagnostic shape</h2>${code('Type', `interface Diagnostic {
  code: string;
  message: string;
...
${next([{href: '/agents/recovery/', title: 'Agent recovery', description: 'Apply outcomes to language-driven UI.'}, {href: '/ship/', title: 'Production acceptance', description: 'Exercise every applicable state before release.'}])}`,
  }),
];
