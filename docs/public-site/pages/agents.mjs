...
export const agentPages = [
  definePage('agents', {
    ...
    description:
      'Pair MCP, experimental WebMCP, or a host-supplied model with one authorized application session and Region.',
...
<h2>Choose a transport</h2>${cards([
      {
        title: 'MCP',
        body: 'Connect an external agent over a local stdio or HTTP host.',
        href: '/agents/mcp/',
        label: 'MCP setup',
      },
      {
        title: 'BYOK',
        body: 'Run a bounded model loop in your trusted host process and keep the provider key local.',
        href: '/agents/byok/',
        label: 'BYOK setup',
      },
      {
        title: 'WebMCP',
        body: 'Use native browser capability detection when available, with manual UI as the fallback.',
        href: '/agents/webmcp/',
...
${next([
  {
    ...
    description: 'Create one paired endpoint and inspect receipts.',
  },
  { href: '/concepts/safety/', title: 'Safety model', description: 'Understand validation and proof boundaries.' },
])}`,
  }),
  ...
  definePage('agents-quickstart', {
    title: 'Agent quickstart',
    description:
      'Expose the three standard tools for one expiring Region session while the same app remains usable manually.',
...
<h2>Create the endpoint</h2>${code(
      'agent.ts',
...
${next([
  { href: '/agents/mcp/', title: 'MCP transport', description: 'Connect stdio or local HTTP.' },
  { href: '/agents/byok/', title: 'BYOK model loop', description: 'Keep keys and egress policy in the host.' },
])}`,
  }),
...
<h2>Run the local playground</h2>${code(
      'terminal',
...
${next([
  {
    ...
    title: 'Recovery',
    description: 'Handle expiry, disconnect, stale proposals, and uncertain actions.',
  },
  {
    href: '/playground/',
    title: 'Playground',
    description: 'Use the same Region manually while no MCP host is connected.',
  },
])}`,
  }),
  ...
  definePage('webmcp', {
    ...
    description:
      'Detect native browser support and register bounded tools only when the current browser exposes the required capability.',
...
<h2>Progressive enhancement</h2>${code(
      'capability.ts',
...
${next([
  { href: '/agents/', title: 'Agent overview', description: 'Compare MCP, BYOK, and WebMCP.' },
  {
    href: '/guides/adaptive-region/',
    title: 'Manual Region path',
    description: 'Keep the full interface functional without an agent.',
  },
])}`,
  }),
  ...
  definePage('byok', {
    ...
    description:
      'Run the provider SDK and bounded tool loop in a trusted host process without sending keys to the public playground.',
...
<h2>Configure the local process</h2>${code(
      'terminal',
...
${next([
  {
    href: '/agents/recovery/',
    title: 'Recovery and limits',
    description: 'Handle provider failure, ambiguity, and disconnect safely.',
  },
  {
    href: '/concepts/safety/',
    title: 'Data egress',
    description: 'Separate metadata discovery from record disclosure.',
  },
])}`,
  }),
  ...
  definePage('agent-recovery', {
    ...
    description:
      'Keep the application understandable when language is ambiguous, the provider fails, the session expires, or an action outcome is uncertain.',
...
${next([
  { href: '/reference/diagnostics/', title: 'Diagnostics', description: 'Map outcomes to actionable recovery.' },
  { href: '/ship/', title: 'Ship checklist', description: 'Verify manual and connected journeys together.' },
])}`,
  }),
];
