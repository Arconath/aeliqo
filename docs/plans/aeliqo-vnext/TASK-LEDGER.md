...
| T03  | review-fixes-verified    | Initial implementation `b3f17a1`; round-1 fixes are verified but uncommitted (fix SHA pending): authority-fenced proposals, typed denial/capability intent, observer isolation, failed-proposal retirement, bounded generation, 31/31 vNext, and installed evidence at `artifacts/runtime-consumers/run-9Gv4NH/report.json`; re-review and publication remain. |
