...
| T03  | rereview-fixes-verified  | Initial and round-1 fixes are committed through reviewed base `bffdec2`; uncommitted round-2 fixes on that exact base publish denial once and make capability initial intent address-independent (next fix SHA pending, not invented): 33/33 vNext and installed evidence at `artifacts/runtime-consumers/run-edxqqJ/report.json`; acceptance review and publication remain. |
