...
Enam nama `@aeliqo/sdk-*` harus dibuat sekali sebelum npm trusted publishing dapat dikonfigurasi. Owner npm `arconath`, dari terminal lokal interaktif dengan mode 2FA `auth-and-writes`, membangun exact `0.1.0-rc.1` memakai `pnpm release:bootstrap:build`, meninjau manifest/tarball, lalu menjalankan `pnpm release:bootstrap:publish`. Jalur ini hanya menerima kandidat bersih pada HEAD saat ini, keenam identity dalam dependency order, tag `next`, byte/integrity yang sama, dan state registry yang absent atau cocok untuk resume. Jalur ini mewariskan stdio agar prompt OTP tetap lokal dan sengaja tidak mengklaim provenance cloud. Ia menolak CI, actor/org non-owner, tag rollback, dan semua versi lain.
