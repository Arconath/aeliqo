...
Each compound has a recipe helper ending in `PresentationRecipe`. Supply `parts` as configured shared primitive nodes, exact task `coverage`, any registered interaction `links`, preconditions, and `validation: {context, registry}`. The helper returns an `Outcome` after the real canonical presentation validator accepts the expanded `layout.stack` tree. It does not invent task needs, materializations, binding revisions, permissions, or interaction mappings. `AELIQO_COMPOUND_REFS` identifies authoring macros; those names are not renderer representations. The returned plan uses existing registered primitives, and the host follows the ordinary evaluate/present/commit path. A rejected macro preserves the current presentation.
