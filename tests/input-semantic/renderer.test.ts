...
function manifests(): ReturnType<typeof createInputPresentationManifests> {
  const bindings: AeliqoInputBindings = {revision: "inputs-7", inputs: [
    ...
    {id: "slider", ref: {id: "input.slider", revision: "1"}, config: {label: "Level", min: 0, max: 10, step: 2}, draft: {...draft, field: "level", type: {value: "integer" as const, nullable: false}}},
...
function resolved(id: string, values: Record<string, unknown>, ports: readonly Record<string, unknown>[], operations: readonly Record<string, unknown>[] = []) {
...
describe("semantic input renderer boundary", () => {
  ...
  it("does not emit an invalid number draft and canonicalizes valid integer text", () => {
    const target = resolved("input.number-field", {entity: "records", key: "record-1", field: "count", entityRevision: "7", type: {value: "integer", nullable: false}, min: "0", max: "20", step: "2", bindingRevision: "inputs-7"}, [{id: "draft", payload: "draft", type: {value: "integer", nullable: false}}]);
    ...
    expect(resolveInputInteraction(target, new AeliqoInputCommitEvent({source: "user", value: {text: "13", value: "13", valid: true}}))).toEqual([]);
    expect(resolveInputInteraction(target, new AeliqoInputCommitEvent({source: "user", value: {text: "22", value: "22", valid: true}}))).toEqual([]);
    ...
    const forgedType = resolved("input.number-field", {entity: "records", key: "record-1", field: "count", entityRevision: "7", type: {value: "integer", nullable: false, grain: ["forged"]}, bindingRevision: "inputs-7"}, [{id: "draft", payload: "draft", type: {value: "integer", nullable: false}}]);
    expect(resolveInputInteraction(forgedType, new AeliqoInputCommitEvent({source: "user", value: {text: "12", value: "12", valid: true}}))).toEqual([]);
  });
  ...
  it("revalidates slider bounds and exact step increments", () => {
    ...
    const target = node(manifest, {entity: "records", key: "record-1", field: "level", entityRevision: "7", type: {value: "integer", nullable: false}, min: 0, max: 10, step: 2, bindingRevision: "inputs-7"}, [{id: "draft", payload: "draft", type: {value: "integer", nullable: false}}]);
    expect(resolveInputInteraction(target, new AeliqoInputCommitEvent({source: "user", value: {value: 3, unit: "",}} as never))).toEqual([]);
    expect(resolveInputInteraction(target, new AeliqoInputCommitEvent({source: "user", value: {value: 12, unit: "",}} as never))).toEqual([]);
    expect(resolveInputInteraction(target, new AeliqoInputCommitEvent({source: "user", value: {value: 4, unit: "",}} as never))).toEqual([{portId: "draft", payload: {kind: "draft", entity: "records", key: "record-1", field: "level", entityRevision: "7", value: 4}}]);
  });
