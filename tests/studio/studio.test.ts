...
describe('local Studio document and session', () => {
  ...
  it('supports manual meaning authoring and keeps code-owned entries inspectable', () => {
    const session = createStudioSession(document(), { registry });
    const draft = session.defineMeaning({
      id: 'employees.total',
      label: 'Total amount',
  ...
  it('edits a validated Experience into a new personal revision', () => {
    ...
    const edited = session.editExperience({
      base,
      label: 'Compact employee inspection',
      experience: { ...base, revision: '2', mode: 'fixed' },
    });
    ...
    expect(
      session.getState().document.profiles.map((entry) => `${entry.experience.id}@${entry.experience.revision}`),
    ).toEqual([`${base.id}@${base.revision}`, `${base.id}@2`]);
    expect(session.getState().document.profiles[0]!.source).toEqual({
      surface: 'code',
      ownership: 'code',
      readOnly: true,
    });
    const codeSession = createStudioSession(document(), { registry });
    const readOnly = codeSession.editExperience({
      base,
      label: 'Code mutation',
      experience: { ...base, mode: 'fixed' },
    });
  ...
  it('applies an explicit meaning diff only as a new revision and rejects a stale base', () => {
    ...
    expect(session.getState().document.meanings.filter((entry) => entry.meaning.id === 'employees.total')).toHaveLength(
      2,
    );
  });

  it('rejects conflicting immutable definitions and invalid active profiles', () => {
    const conflict = createStudioDocument(
      {
        ...input,
        meanings: [
          {
            ...
            meaning: {
              ...
              implementation: { kind: 'host-capability', capability: { id: 'total', revision: '1' } },
              dependencies: [],
  ...
  it('does not accept caller-declared code provenance and handles malformed inputs as diagnostics', () => {
    const session = createStudioSession(document(), { registry });
    const forged = session.defineMeaning({
      id: 'employees.forged',
      label: 'Forged',
      description: 'Forged provenance.',
      entity: 'employees',
      field: 'amount',
      source: { surface: 'code', ownership: 'code', readOnly: true },
    });
  ...
  it('rebuilds meaning validation when an import switches catalogs', () => {
    ...
    const draft = session.defineMeaning({
      id: 'projects.total',
      label: 'Total amount',
      description: 'Sum of project amounts.',
      entity: 'projects',
      field: 'amount',
    });
    expect(draft.ok).toBe(true);
  ...
  it('keeps an immutable meaning ledger across accepted document revisions', () => {
    ...
    const codeDraft = authoring.value.draft(defined.value.meaning, {
      source: { surface: 'code', ownership: 'code', readOnly: true },
      assumptions: [],
    });
    expect(codeDraft.ok).toBe(true);
    if (!codeDraft.ok) return;
    const codeDocumentResult = createStudioDocument(
      { ...input, id: 'studio-code', revision: 'studio-code-1', meanings: [codeDraft.value] },
      { registry },
    );
    ...
    const alteredDocument = {
      ...codeDocument,
      revision: 'studio-code-1-altered',
      meanings: [{ ...codeDraft.value, meaning: alteredMeaning, digest: meaningDigest(alteredMeaning) }],
    };
  ...
  it('validates a dependency DAG independently of draft array order', () => {
    ...
    const base = authoring.value.defineMeaning({
      id: 'employees.base',
      label: 'Base',
    ...
    const dependent = withBase.value.draft(
      {
        ...
        implementation: {
          kind: 'expression',
          expression: { kind: 'definition', ref: { id: 'employees.base', revision: '1' } },
        },
        dependencies: [{ id: 'employees.base', revision: '1' }],
      },
      { source: { surface: 'studio', ownership: 'personal' }, assumptions: [] },
    );
    const baseDraft = withBase.value.draft(base.value.meaning, {
      source: { surface: 'studio', ownership: 'personal' },
      assumptions: [],
    });
