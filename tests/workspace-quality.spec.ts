...
test("focused filter draft survives resize and IME; keyboard history and motion remain usable", async ({ page, browserName }, testInfo) => {
  const bridgePort = await availablePort();
  const companionPort = await availablePort();
  await page.emulateMedia({ reducedMotion: "reduce" });
  await page.goto(`/playground/?aeliqoBridgePort=${bridgePort}&aeliqoCompanionPort=${companionPort}`);
  ...
  const filterExample = page.getByRole("region", { name: "Controlled Filter example", exact: true });
  const input = filterExample.getByLabel("Value", { exact: true });
  ...
  await expect(filterExample.getByRole("list", { name: "Committed filters" })).toBeEmpty();
  await input.dispatchEvent("compositionend", { data: "Small cohort" });
  await page.keyboard.press("Enter");
  await expect(filterExample.getByRole("list", { name: "Committed filters" })).toContainText("Small cohort");
  await expect(input).toBeFocused();
  expect(await page.evaluate(() => matchMedia("(prefers-reduced-motion: reduce)").matches)).toBe(true);
  await page.screenshot({ path: testInfo.outputPath("filter-mobile-ime.png"), fullPage: true });
