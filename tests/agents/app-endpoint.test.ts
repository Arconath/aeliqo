...
function setup() {
  ...
  const data = createLocalDataService({snapshot: {catalog: resource.catalog, sourceRevision: '1', records: {people: [
    {id: 'p-1', name: 'Ada', team: 'Platform'}, {id: 'p-2', name: 'Grace', team: 'Research'},
  ]}}, functionRegistry: registry.value, authorize: () => ({ok: true, value: {scopeDigest: 'scope-1', policyRevision: 'policy-1'}})});
  const actions = new ActionRegistry();
  let executions = 0;
  const registered = actions.register({
    descriptor: {ref: {id: 'people.create', revision: '1'}, input: {id: 'people.create.input', revision: '1'},
      output: {id: 'people.create.output', revision: '1'}, sideEffect: 'domain-write', confirmation: 'required',
      idempotency: 'required', entityRevision: 'none'},
    inputSchema: {ref: {id: 'people.create.input', revision: '1'}, parse: input => resource.parseRecord(input)},
    outputSchema: {ref: {id: 'people.create.output', revision: '1'}, parse(input) {
    ...
    dispatch: () => { executions += 1; return {state: 'completed', output: {saved: true}}; },
  });
  if (!registered.ok) throw new Error('action registry unavailable');
  const actionPort = createActionPort({registry: actions, host: {
    readContext: () => ({ok: true, value: {principalKey: 'person-1', actorKey: 'person-1', scopeDigest: 'scope-1',
      policyRevision: 'policy-1', domainRevision: 'domain-1', confirmationEpoch: 'confirmation-1',
      grants: ['action.propose', 'action.execute']}}),
    issueConfirmation: () => ({ok: true, value: undefined}),
  }});
  const runtime = createAeliqoRuntime({resources: [{resource, data}], actionPort, authority: {read: () => ({ok: true, value: {
    principalKey: 'person-1', scopeDigest: 'scope-1', policyRevision: 'policy-1', experienceRevision: 'experience-1',
    grants: ['catalog.read', 'task.propose', 'task.evaluate', 'experience.commit', 'result.inspect', 'action.propose', 'action.execute', 'model.egress'], readContext: {principal: 'person-1'},
  }})}});
...
describe('standard app tool endpoint', () => {
  it('exposes exactly context, render, and act for one paired Region', async () => {
    const {runtime} = setup();
    const endpoint = createAppToolEndpoint({runtime, regionId: 'main', goalEpoch: 'goal-1', transport: 'manual', expiresAt: Date.now() + 60_000});
  ...
  it('uses the runtime pipeline and does not misreport a runtime commit as browser-ready', async () => {
    ...
    const rendered = await endpoint.value.invoke('aeliqo_render', {
      version: '1', id: 'browse-people', kind: 'browse', resource: 'people', fields: ['name', 'team'], search: {text: 'platform', fields: ['team']},
    }, {requestId: 'render-1'});
    if (!rendered.ok || rendered.value.state !== 'plan-committed') throw new Error(JSON.stringify({rendered, context: runtime.context('main'), snapshot: runtime.snapshot('main')}));
    expect(rendered).toMatchObject({ok: true, value: {state: 'plan-committed', status: 'plan-committed', value: {status: 'committed'}}});
  ...
  it('keeps confirmation host-owned, executes once, and cancels previews when the pairing closes', async () => {
    ...
    const endpoint = createAppToolEndpoint({runtime, regionId: 'main', goalEpoch: 'goal-actions', transport: 'manual', expiresAt: Date.now() + 60_000});
    if (!endpoint.ok) throw new Error(endpoint.diagnostics[0].message);
    const preview = await endpoint.value.invoke('aeliqo_act', {
      mode: 'preview', action: {id: 'people.create', revision: '1'}, input: {id: 'p-3', name: 'Lin', team: 'Platform'},
    ...
    await expect(endpoint.value.invoke('aeliqo_act', {mode: 'execute', previewId}, {requestId: 'action-execute-early'}))
    ...
    await expect(endpoint.value.invoke('aeliqo_act', {mode: 'execute', previewId}, {requestId: 'action-execute-1'}))
      .resolves.toMatchObject({ok: true, value: {state: 'accepted', value: {state: 'executed'}}});
    expect(executions()).toBe(1);

    const abandoned = await endpoint.value.invoke('aeliqo_act', {
      mode: 'preview', action: {id: 'people.create', revision: '1'}, input: {id: 'p-4', name: 'Rin', team: 'Research'},
