...
const output = join(root, "artifacts/plot-consumers");
await mkdir(output, {recursive: true});
const runDirectory = await mkdtemp(join(output, "run-"));
const consumer = await mkdtemp(join(tmpdir(), "aeliqo-plot-consumer-"));
...
const result={version:'1',ref,taskId:'t',fields:[{id:'id',label:'ID',type:{value:'text',nullable:false},role:'identity'},{id:'x',label:'X',type:{value:'integer',nullable:false},role:'dimension'},{id:'y',label:'Y',type:{value:'decimal',nullable:true},role:'measure'}],identity:['id'],rowGrain:['id'],counts:{loaded:3,population:{kind:'unknown'}},precision:{kind:'exact'},coverage:{kind:'unknown',reason:'Host supplied rows'},consistency:{kind:'unknown',reason:'Host snapshot'},evidence:{kind:'computed',queryDigest:'q',definitions:[]},filters:[],warnings:[],lineage:[]};
const unit={kind:'unit',result:ref,mark:'line',missing:'gap',encoding:{x:{field:'x',scale:'linear'},y:{field:'y',scale:'linear'}}};
const rows=[{id:'a',x:1,y:{decimal:'9007199254740993.001'}},{id:'b',x:2,y:null},{id:'c',x:3,y:{decimal:'9007199254740993.003'}}];
...
const datasets:readonly PlotDataset[]=[{result:result.ref,rows:${JSON.stringify(rows)}}];
const element=new AeliqoPlotElement();element.spec=spec;element.results=[result];element.datasets=datasets;
const component=<Aeliqo spec={spec} results={[result]} datasets={datasets} onSelectionChange={e=>{const digest:string=e.detail.result.scopeDigest;void digest;}}/>;
void [component,compilePlotComposition(spec,[result],datasets,{width:400,height:240,maxRows:100,maxMarks:100})];
...
try{
 browser=await chromium.launch();browserVersion=browser.version();const page=await browser.newPage({viewport:{width:1280,height:900}});page.on('pageerror',e=>failures.push(e.message));await page.goto(`http://127.0.0.1:${server.address().port}`);await page.locator('aeliqo-plot table').waitFor();
 assert.equal(await page.locator('aeliqo-plot circle').count(),2);assert.equal(await page.getByRole('cell',{name:'9007199254740993.001',exact:true}).count(),1);
 const points=await page.locator('circle').evaluateAll(nodes=>nodes.map(n=>[Number(n.getAttribute('cx')),Number(n.getAttribute('cy'))]));
 await page.evaluate(async()=>{window.plot.renderer='canvas';await window.plot.updateComplete;});
 const samples=await page.locator('canvas').evaluate((c,points)=>{const ctx=c.getContext('2d');const ratio=c.width/400;return points.map(([x,y])=>ctx.getImageData(Math.round(x*ratio),Math.round(y*ratio),1,1).data[3]);},points);assert(samples.every(a=>a>0));
 await page.evaluate(async()=>{const p=window.plot;p.renderer='svg';p.spec=window.spec;p.results=[window.result];p.datasets=[{result:window.result.ref,rows:window.rows}];p.addEventListener('aeliqo-plot-select',e=>{p.selectedIdentity=e.detail.identity;p.selectedResult=e.detail.result;});await p.updateComplete;});
 assert.equal(await page.locator('table').count(),3);assert.equal(await page.locator('tbody tr').count(),3);
 await page.getByRole('button',{name:'Select a',exact:true}).focus();await page.keyboard.press('Enter');assert.equal(await page.getByRole('button',{name:'Select a',exact:true}).getAttribute('aria-pressed'),'true');
 await page.screenshot({path:join(runDirectory,'installed-plot.png')});assert.deepEqual(failures,[]);
 await page.evaluate(async()=>{window.plot.results=[];await window.plot.updateComplete;});assert.equal(await page.locator('table').count(),0);
}finally{await browser?.close();await new Promise(resolve=>server.close(resolve));}
assert.equal(sourceDigest(),before,'Source changed during installed plot proof');
await writeFile(join(runDirectory,'report.json'),JSON.stringify({sourceDigest:before,sourceChangedDuringRun:false,scope:'Installed PlotSpec/geometry/web/React declarations, actual React SSR composition and request isolation, Chromium exact values/SVG/Canvas/facets/selection/revocation',artifacts:artifacts.map(({bytes,entries,...a})=>a),consumerDirectory:consumer,consumerLock:{sha256:hash(lockBytes)},ssr,failures,environment:{node:process.version,chromium:browserVersion},passed:true},null,2)+'\n');
