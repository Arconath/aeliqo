...
assert.equal(run(['node','scripts/source-digest.mjs'],root).trim(),sourceDigest,'Source changed during consumer test');
await writeFile(join(runDirectory,'report.json'),JSON.stringify({sourceDigest,
 scope:'Installed web, React, styles, input, form, focus, theme, and all thirteen foundation types; Lit SSR, React client rendering/action events and standalone Button bundle. Full React/Vue hydration and manual assistive-technology matrix are separate.',
