...
await writeFile(join(consumer,'browser.mjs'),`import {createOpaqueModelSecret} from '@aeliqo/sdk-agent/model';import {probe} from './probe.mjs';let browserCredentialCreationBlocked=false;try{createOpaqueModelSecret('synthetic-browser-value');}catch{browserCredentialCreationBlocked=true;}window.agentReport={...await probe(),browserCredentialCreationBlocked};document.querySelector('#status').textContent='Passed';`);
