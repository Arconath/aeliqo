...
it('registers all shared component constructors at the actual package version',()=>{
 const version=(JSON.parse(readFileSync(new URL('../../packages/web/package.json',import.meta.url),'utf8')) as {version:string}).version;
 ...
 for(const [name,constructor]of constructors)expect((constructor as CustomElementConstructor&{aeliqoVersion?:string}).aeliqoVersion,name).toBe(version);
 registerAeliqoElements(registry);
 expect(()=>registerAeliqoElements({get:()=>class {static aeliqoVersion='0.1.0-m0';}} as unknown as CustomElementRegistry)).toThrow('incompatible');
});
