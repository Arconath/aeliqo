...
describe('runtime named-output and cohort evaluation', () => {
  ...
  it('accepts the original fixed population through direct LocalDataService and HTTP ADC', async () => {
    ...
    const directPlan = await targetService.plan(planRequest);
    ...
    const directEvents = await collect(targetService.execute(directPlan.value));
    ...
    const handler = createDataHttpHandler({service: targetService});
