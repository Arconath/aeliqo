...
describe('Studio independent boundary review', () => {
  ...
  it('rejects a changed canonical definition for an accepted immutable ID/revision', () => {
    ...
    expect(field.ok).toBe(true);
    if (!field.ok) return;
    const defined = hostAuthoring.value.defineMeaning({
      id: 'employees.total',
      label: 'Total amount',
    ...
    const codeDraft = hostAuthoring.value.draft(defined.value.meaning, {
      source: { surface: 'code', ownership: 'code', readOnly: true },
      assumptions: [],
    });
    expect(codeDraft.ok).toBe(true);
    if (!codeDraft.ok) return;
    const codeDocument = createStudioDocument(
      { ...input, revision: 'studio-demo-code-1', meanings: [codeDraft.value] },
      { registry },
    );
    ...
    const changed = {
      ...first,
      revision: 'studio-imported-altered',
      meanings: [{ ...codeDraft.value, meaning: altered, digest: meaningDigest(altered) }],
    };
  ...
  it('fails closed for forged source provenance and malformed public calls', () => {
    ...
    const forged = session.defineMeaning({
      id: 'employees.forged',
      label: 'Forged',
      description: 'Untrusted source claim.',
      entity: 'employees',
      field: 'amount',
      source: { surface: 'code', ownership: 'code', readOnly: true },
    });
