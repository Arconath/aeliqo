...
describe('fixed-source follow-up review', () => {
  it('evaluates a Studio meaning against the bounded local source', () => {
    ...
    if (!draft.ok) return;
    const result = session.evaluateMeaning({
      meaning: { id: draft.value.meaning.id, revision: draft.value.meaning.revision },
      entity: 'employees',
      source: localSource,
    });
    ...
    expect(
      session
        ...
        .document.profiles.some(
          (entry) =>
            entry.experience.id === base.experience.id &&
            entry.experience.revision === base.experience.revision &&
            entry.source.ownership === 'code',
        ),
    ).toBe(true);
    expect(
      session
        ...
        .document.profiles.some(
          (entry) =>
            entry.experience.id === base.experience.id &&
            entry.experience.revision === '2' &&
            entry.source.ownership === 'personal',
        ),
    ).toBe(true);
  });
  ...
  it('proposes and applies a meaning diff as a new immutable revision', () => {
    ...
    const diff = session.proposeMeaningDiff(
      { id: draft.value.meaning.id, revision: draft.value.meaning.revision },
      candidate,
    );
