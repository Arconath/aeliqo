...
test.describe('Studio browser boundary review', () => {
  test('shows malformed import diagnostics in the visible workspace', async ({ page }) => {
    await page.goto('/');
    await page
      .locator('#import-file')
      .setInputFiles({ name: 'malformed.json', mimeType: 'application/json', buffer: Buffer.from('{ malformed') });
  ...
  test('binds evaluated data to real preview components at each declared width', async ({ page }) => {
    ...
    await expect(page.locator('[data-preview-metric="320"]')).toContainText(
      'Total amount from the authorized employee population',
    );
  ...
  test('defers visible rerender until an IME composition ends', async ({ page }) => {
    ...
    await page
      .locator('#import-file')
      .setInputFiles({ name: 'ime-malformed.json', mimeType: 'application/json', buffer: Buffer.from('{ malformed') });
...
test('dark previews inherit theme and narrow frames support keyboard inspection in both directions', async ({
  page,
}) => {
