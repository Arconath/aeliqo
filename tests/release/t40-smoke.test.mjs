...
import {
  qualifyT40Smoke,
  T40_MCP_TOOL_SCHEMA_SHA256,
  T40_OBSOLETE_SCOPE_BLOCK,
} from '../../scripts/release/t40-smoke-lib.mjs';
...
const makeCase = (id) => {
  ...
  return {
    id,
    partition: 'heldout',
    independentAuthor: 'independent fixture author',
    exposure: 'sealed',
...
const weak = {
  ...
  priceSource: 'verified test rate',
};
const strong = {
  ...weak,
  label: 'strong',
  model: 'deepseek-v4-pro',
  expectedReportedModel: 'deepseek-v4-pro',
...
const config = {
  version: '1',
  authorized: true,
  authorizationReference: 'Owner-authorized bounded release smoke',
  authorizedCorpusSha256: corpusSha256,
  maxUSD: 1,
  trials: 1,
  models: [weak, strong],
  budget,
...
const outputFor = (testCase, resultId = '1'.repeat(64)) => ({
  descriptor: {
    version: '1',
    ref: {
      id: `result-${resultId}`,
      revision: testCase.fixture.sourceRevision,
      outputId: 'rows',
      queryDigest: `query-${'2'.repeat(64)}`,
      scopeDigest: testCase.fixture.scopeDigest,
    },
    taskId: testCase.explicitTask.id,
    fields: [
    ...
    identity: ['id'],
    rowGrain: ['id'],
    counts: { loaded: 1, population: { kind: 'exact', value: 1, populationDigest: `population-${'3'.repeat(64)}` } },
    precision: { kind: 'exact' },
    coverage: { kind: 'complete', populationDigest: `population-${'3'.repeat(64)}` },
    consistency: {
      kind: 'snapshot',
      snapshotId: testCase.fixture.sourceRevision,
      sourceRevisions: { source: testCase.fixture.sourceRevision },
    },
    evidence: { kind: 'observed', source: { id: 'local-source', revision: testCase.fixture.sourceRevision } },
    filters: [],
    warnings: [],
    lineage: [],
  },
          ...
          observations: [
            {
              stage: 'evaluate',
              elapsedMs: 1,
              task: testCase.explicitTask,
              outputs: [outputFor(testCase, String(index + 1).repeat(64))],
            },
          ],
        }
      : {
          ...
          observation: {
            ...
            server: { name: 'aeliqo-evaluation-baseline', version: '0.1.0' },
            node: 'v24.20.0',
            discoveredTools: ['read_catalog', 'evaluate_task'],
            toolSchemaSha256: T40_MCP_TOOL_SCHEMA_SHA256,
            ...
            stderrBytes: 0,
            modelExecution: 'No model adapter is configured.',
            fixtureProjection: 'Application fixture fields only.',
            limits: ['Explicit baseline only.'],
          },
...
const baseLive = (caseId, trial = 1) => {
  ...
  return {
    ...
    model: weak.model,
    modelLabel: 'weak',
    qualifiedModelSnapshot: true,
    connection: {
      protocol: weak.protocol,
      origin: 'https://api.deepseek.com',
      authScheme: 'bearer',
      capabilities: weak.capabilities,
    },
    snapshots: [
      {
        reportedModel: 'deepseek-flash',
        reportedModelSha256: sha('deepseek-flash'),
        matchesExpected: true,
        responseIdSha256: sha(`${caseId}:${trial}`),
      },
    ],
    trial,
    elapsedMs: 1000,
    reservedUSD: reservation,
    firstAttempt: { dataCorrect: false },
...
live[1] = {
  ...baseLive(cases[1]),
  result: {
    ...baseLive(cases[1]).result,
    toolCalls: 3,
    receiptStates: [
      { operation: 'catalog.read', state: 'data-ready' },
      { operation: 'task.evaluate', state: 'invalid' },
      { operation: 'task.evaluate', state: 'data-ready' },
    ],
  },
  observations: [
...
const groups = [
  {
    ...
    trials: 3,
    dataCorrect: 0,
    interval: { lower: 0, upper: 0.5614970317550454, confidence: 0.95, trials: 3 },
    uiTaskCompletion: null,
    narrativeGrounding: null,
  },
  {
...
const report = {
  ...
  mcpExplicit: true,
  sourceDigest,
  blocks: [T40_OBSOLETE_SCOPE_BLOCK],
  authorization: { reference: config.authorizationReference, maximumUSD: 1, authorizedCorpusSha256: corpusSha256 },
  corpus: {
    path: corpusPath,
    sha256: corpusSha256,
    cases: corpusCases.map(({ id, partition, independentAuthor, exposure }) => ({
      id,
      partition,
      independentAuthor,
      exposure,
  ...
  groups,
  rows: [...deterministic('explicit-task'), ...deterministic('explicit-mcp'), ...live],
};
...
test('fails closed on forged source, corpus, authorization, blockers, or report fields', () => {
  ...
  assert.throws(
    () => qualifyT40Smoke(report, { ...context, config: { ...config, authorized: false } }),
    /owner-authorized/,
  );
  assert.throws(
    () => qualifyT40Smoke({ ...report, blocks: [...report.blocks, 'injected'] }, context),
    /blocker|scope claim/,
  );
  assert.throws(() => qualifyT40Smoke({ ...report, injected: true }, context), /closed evidence schema/);
  assert.throws(
    () => qualifyT40Smoke(report, { ...context, config: { ...config, injected: true } }),
    /closed evidence schema/,
  );
  assert.throws(
    () =>
      qualifyT40Smoke(report, {
        ...context,
        config: {
          ...config,
          budget: { ...budget, maxInputBytes: 999999999, maxOutputBytes: 999999999, maxRepeatedCalls: 999999999 },
        },
      }),
    /budget is invalid/,
  );
  assert.throws(
    () =>
      qualifyT40Smoke(report, { ...context, config: { ...config, models: [weak, { ...strong, model: weak.model }] } }),
    /distinct weak and strong/,
  );
});
...
test('rejects simulated MCP evidence and uncorroborated invalid-repair claims', () => {
  ...
  for (const row of incompleteText.rows.filter((item) => item.mode === 'governed-model-data'))
    row.result.incompleteRequiredOperations = ['task.evaluate'];
