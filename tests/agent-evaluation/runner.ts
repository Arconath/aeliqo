...
export async function runEvaluation(args:readonly string[]):Promise<number>{
 const options=new Map<string,string>();const selectedCaseIds:string[]=[];let selectedModelLabel:'weak'|'strong'|undefined;let live=false;let mcpExplicit=false;
 for(let index=0;index<args.length;index++){const argument=args[index]!;if(argument==='--live'){live=true;continue;}if(argument==='--mcp-explicit'){mcpExplicit=true;continue;}if(argument==='--case'){const id=args[++index];if(!id)throw Error('--case requires an exact corpus case ID.');selectedCaseIds.push(id);continue;}if(argument==='--model-label'){const label=args[++index];if(label!=='weak'&&label!=='strong')throw Error('--model-label must be weak or strong.');selectedModelLabel=label;continue;}if(!['--config','--corpus','--output'].includes(argument)||!args[index+1])throw Error('Use --config FILE --corpus FILE --output DIRECTORY [--live] [--mcp-explicit] [--case ID ...] [--model-label weak|strong].');options.set(argument,args[++index]!);}
 ...
 if(new Set(selectedCaseIds).size!==selectedCaseIds.length)throw Error('Selected case IDs must be unique.');const selectedCaseSet=new Set(selectedCaseIds);if(selectedCaseIds.some(id=>!validCases.some(item=>item.id===id)))throw Error('Every selected case ID must exist in the authorized corpus.');const runCases=selectedCaseIds.length===0?validCases:validCases.filter(item=>selectedCaseSet.has(item.id));
 const root=resolve(import.meta.dirname,'../..');const sourceDigest=()=>execFileSync('node',['scripts/source-digest.mjs'],{cwd:root,encoding:'utf8'}).trim();const before=sourceDigest();const output=resolve(outputPath);await mkdir(output,{recursive:true});
 ...
 else if(config.models.filter(model=>selectedModelLabel===undefined||model.label===selectedModelLabel).some(model=>!process.env[model.credentialEnvironment]))blocks.push('One or more selected opaque model credentials are unavailable. No provider request was made.');
 else{
  let reservedUSD=0;
  for(const model of config.models.filter(item=>selectedModelLabel===undefined||item.label===selectedModelLabel))for(const testCase of runCases)for(let trial=1;trial<=config.trials;trial++){
 ...
 blocks.push('This data-only runner does not establish full release qualification: UI completion, narrative review, fixed-template ablation, external MCP-host reasoning trials, and independently accepted held-out coverage remain separate requirements.');
 const report={schemaVersion:1,status:'blocked',mcpExplicit,sourceDigest:before,sourceChangedDuringRun:after!==before,corpus:{path:resolve(corpusPath),sha256:hash(corpusBytes),cases:validCases.map(item=>({id:item.id,partition:item.partition,independentAuthor:item.independentAuthor,exposure:item.exposure})),selection:{caseIds:runCases.map(item=>item.id),modelLabels:selectedModelLabel===undefined?['weak','strong']:[selectedModelLabel]}},authorization:config?{reference:config.authorizationReference,maximumUSD:config.maxUSD,authorizedCorpusSha256:config.authorizedCorpusSha256}:null,groups,blocks,rows};await writeFile(join(output,'report.json'),JSON.stringify(report,null,2)+'\n');console.log(`Evaluation evidence written to ${join(output,'report.json')}. Overall release qualification: BLOCKED.`);return rows.some(row=>(row.mode==='explicit-task'||row.mode==='explicit-mcp')&&object(row.score)&&row.score.dataCorrect===false)?1:2;
}
