    ...
    const plan = {
      ...
      nodes: original.nodes.map((node) =>
        node.id === 'tabs-root'
          ? {
              ...node,
              children: node.children.map((id) => (id === 'popover' ? 'tooltip' : id === 'tooltip' ? 'popover' : id)),
              config: { ...node.config, values: { ...node.config.values, value: 'tooltip' } },
            }
          : node,
      ),
    };
