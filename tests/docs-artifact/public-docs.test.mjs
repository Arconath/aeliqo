...
test('public docs artifact binds exact source, packages, API metadata, and runnable examples', async () => {
  ...
  assert.match(artifact.pages.find(page => page.path === '/docs/components/foundation.button/').body, /export declare class AeliqoButtonElement/u);
  assert.match(artifact.pages.find(page => page.path === '/docs/components/foundation.button/').body, /data-example-code="button"/u);
...
test('producer can bind the docs artifact to one exact release-candidate package set', async () => {
  ...
  await execFileAsync(process.execPath, [
    'scripts/docs/build-public-docs.mjs', '--output', scratch,
    '--source-revision', revision, '--version', '0.1.0-rc.2',
  ], {cwd: root});
