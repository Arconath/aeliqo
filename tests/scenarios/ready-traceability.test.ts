...
describe('ready-stage scenario traceability', () => {
  it('indexes exactly the approved deterministic scenario subset', () => {
    ...
    for (const excluded of ['S24', 'S28', 'S36', 'S52', 'S61', 'S62']) {
      expect(ids).not.toContain(excluded);
    }
  });

  it.each(readyScenarioTraceability)(
    '$id retains its exact production assertion and immutable evidence link',
    (trace) => {
...
describe('S63 split-era evidence scope', () => {
  it('keeps the frozen site proof explicitly historical instead of binding it to current public source', () => {
    const scenario = scenarioById.get('S63');
    const task = taskById.get('T27');
    expect(scenario).toMatchObject({
      status: 'done',
      evidenceScope: 'historical-pre-split-site-acceptance',
    });
    ...
    for (const reference of [...(scenario?.evidence ?? []), ...(task?.evidence ?? [])]) {
      const path = safePath(reference.path);
      expect(existsSync(path), reference.path).toBe(true);
      expect(sha256(path), reference.path).toBe(reference.sha256);
    }
  });
});
...
const decisionMappings = [
  {
    ids: ['D01', 'D02', 'D03'],
    docs: ['MASTER-SOT.md', 'docs/09-experience-dx.md', 'docs/29-component-contracts.md'],
    tasks: ['T13', 'T16', 'T18', 'T21'],
  },
  { ids: ['D04', 'D05', 'D06'], docs: ['docs/02-data-contract.md'], tasks: ['T06', 'T08'] },
  { ids: ['D07', 'D08'], docs: ['docs/03-semantics-derived.md'], tasks: ['T04', 'T23'] },
  {
    ids: ['D09', 'D10', 'D11', 'D12', 'D13'],
    docs: ['docs/14-agents-protocols.md', 'docs/37-model-failure-containment.md'],
    tasks: ['T22', 'T24', 'T41'],
  },
  {
    ids: ['D14', 'D15', 'D16', 'D17', 'D18', 'D19', 'D20'],
    docs: ['docs/06-presentation-compiler.md', 'docs/07-ui-grammar.md', 'docs/10-interaction-a11y.md'],
    tasks: ['T11', 'T19', 'T20', 'T39'],
  },
  {
    ids: ['D21', 'D22', 'D23'],
    docs: ['docs/02-data-contract.md', 'docs/04-intent-query.md', 'docs/31-contract-closure.md'],
    tasks: ['T05', 'T07', 'T09'],
  },
  {
    ids: ['D24', 'D25', 'D26', 'D27'],
    docs: ['docs/08-rendering-stack.md', 'docs/09-experience-dx.md'],
    tasks: ['T02', 'T12', 'T25'],
  },
  {
    ids: ['D28', 'D29', 'D30'],
    docs: ['docs/12-performance.md', 'docs/16-quality-gates.md', 'docs/29-component-contracts.md'],
    tasks: ['T29', 'T30'],
  },
  ...
  {
    ids: ['D37', 'D38'],
    docs: ['docs/32-evaluation-plan.md', 'docs/37-model-failure-containment.md'],
    tasks: ['T40', 'T41'],
  },
] as const;
...
describe('S64 master decision and extension-boundary audit', () => {
  ...
  it('keeps no-preset composition generic and the pure core free of held-out domain branches', () => {
    ...
    const core = sourceFiles(join(root, 'packages/core/src'))
    ...
    const corpus = readJson<{ readonly cases: readonly { readonly domain: string }[] }>(
      'fixtures/evaluation/t40-data-query-heldout.manifest.json',
    );
