...
it('rejects evicted root results and exposes no retained rows after disposal',async()=>{const engine=createDemoEngine();try{const firstOutput=first(value(await engine.evaluate(engine.peopleTask())));for(let index=0;index<40;index++)value(await engine.evaluate(engine.peopleTask()));expect(firstOutput.handle.snapshot().status).toBe('disposed');expect(firstOutput.rows).toEqual([]);expect(engine.present(firstOutput)).toMatchObject({ok:false,diagnostics:[{code:'demo.stale-result'}]});}finally{engine.dispose();}});
