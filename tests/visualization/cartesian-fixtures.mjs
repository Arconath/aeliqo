...
const plot = (mark, encoding) => ({
  version: '1',
  root: { kind: 'unit', mark, result: ref, missing: 'gap', encoding },
});
const specs = {
  trend: {
    version: '1',
...
const contextFor = (tag) => ({
  results: [result],
  catalog,
  histograms: tag === 'histogram' ? [{ result: ref, bins: specs.histogram.bins }] : [],
});
const rowsFor = (tag) =>
  tag === 'area'
    ? rows.map((row, index) => ({
...
export { ref, result, rows, specs, catalog, contextFor, rowsFor };
