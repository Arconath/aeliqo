...
export const unknownEnvironment: PresentationContext['environment'] = {
...
export async function createHrViewSession() {
  ...
  try {
              ...
              diagnostics: [
                {
                  ...
                  message: checked.diagnostics.map((d) => d.message).join('; '),
                  retryable: false,
                },
              ],
            };
    ...
    if (composed.presentation === undefined) {
      evaluated.release();
      regions.dispose();
      data.dispose();
      throw new Error(JSON.stringify(composed.rejected));
    }
    ...
    const token = unwrap(
      await region.stage({
        ...
        state: { task, presentation: presentation.plan },
    ...
    return {
      ...
      selectionKey(employeeId: string) {
        const row = rankedRows().find((row) => row.employee_id === employeeId);
        return row === undefined ? undefined : rowKey(row);
      },
      selectionLabel(key: string) {
        const id = rankedRows().find((row) => rowKey(row) === key)?.employee_id;
      ...
      get interaction() {
        return region.snapshot().state?.interaction;
      },
      ...
      async reorder() {
        ...
        const checked = unwrap(
          validatePresentationPlan(
            proposal.candidate,
            context(proposal.nextTask, pins(proposal.expected), proposal.previous),
            registry,
          ),
        );
        const staged = unwrap(
          await region.stage({
            requestId: `reorder-${serial}`,
            expected: proposal.expected,
            state: { task: proposal.nextTask, presentation: checked.plan },
          }),
        );
        unwrap(await region.commit(staged));
        presentation = { ...checked, plan: region.snapshot().state!.presentation! };
        sync();
      },
      async refuseStaleProposal() {
        const proposal = reorderCandidate();
        unwrap(
          await region.publishData({
            results: refs(),
            reason: 'Fixture data epoch advanced before a delayed proposal.',
          }),
        );
        const staged = await region.stage({
          requestId: `stale-${serial}`,
          expected: proposal.expected,
      ...
      revoke() {
        interactions.revoke('Fixture access revoked.');
        data.revoke();
        region.revoke('Fixture access revoked.');
        sync();
      },
      dispose() {
