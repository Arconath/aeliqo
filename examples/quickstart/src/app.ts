...
export function mountPeople(target: HTMLElement, records: readonly DataRecord[]) {
  ...
  const data = createLocalDataService({
    ...
    authorize: ({context}) => context.principal === 'current-user'
      ? {ok: true, value: {scopeDigest: 'permitted-people', policyRevision: 'policy-1'}}
      : {ok: false, diagnostics: [{code: 'people.denied', message: 'People access is denied.', retryable: false}]},
  });
  const app = createAeliqoApp({
    resources: [{resource: people, data}],
    authority: {read: () => ({ok: true, value: {
      principalKey: 'current-user', scopeDigest: 'permitted-people', policyRevision: 'policy-1',
  ...
  return {
    render: () => app.render({regionId: 'people-main', intent: {
