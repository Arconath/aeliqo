...
function renderLocale() {
  ...
  document.querySelector('#intro')!.textContent = arabic
    ? 'بيانات تجريبية لعرض أعضاء الفريق ونشاطهم الأسبوعي.'
    : 'A small working view of people and their weekly activity.';
  ...
  people.columns = [
    { key: 'name', label: arabic ? 'الاسم' : 'Name' },
    { key: 'role', label: arabic ? 'الدور' : 'Role' },
    { key: 'location', label: arabic ? 'الموقع' : 'Location' },
  ];
  people.rows = [
    { name: 'Ada', role: arabic ? 'مهندسة' : 'Engineer', location: 'Jakarta' },
    { name: 'Grace', role: arabic ? 'باحثة' : 'Researcher', location: 'London' },
    { name: 'Katherine', role: arabic ? 'محللة' : 'Analyst', location: 'New York' },
  ];
