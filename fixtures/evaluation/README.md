...
The independent oracle provenance is checked in beside the sealed corpus. `t40-quality-oracle.py` authors the expected quality metadata without importing the SDK; `t40-independent-oracle.py` recomputes every expected row with Python records and validates fields, grain, population, precision, source revisions, evidence kind and definitions. Their raw reports are `t40-quality-oracle-report.json` and `t40-independent-oracle-report.json`; both reports record the corpus and script digests, and the independent row report records `21/21` output matches across `18` cases.
...
python3 fixtures/evaluation/t40-quality-oracle.py
python3 fixtures/evaluation/t40-independent-oracle.py
```
